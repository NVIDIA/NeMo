...
for request_batch_size in ${request_batch_sizes[@]}; do
    ...
    python3 /lustre/fsw/joc/donghyukc/bignlp-scripts/bignlp/inference_scripts/identity_test.py --vocab_size ${vocab_size} -r -s ${input_length} -o ${output_length} --input_len_name ${input_len_name} --output_len_name ${output_len_name} -b ${request_batch_size} --name ${model_train_name} 2>&1 | tee ${tmp_log}
