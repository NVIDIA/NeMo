...
ALL_LANGS = ['af', 'am', 'ar', 'az', 'be', 'bg', 'bn', 'ca', 'co', 'cs', 'cy', 'da', 'de', 'el', 'en', 'eo', 'es', 'et',
             'eu', 'fa', 'fi', 'fr', 'fy', 'ga', 'gd', 'gl', 'gu', 'ha', 'hi', 'ht', 'hu', 'hy', 'id', 'ig', 'is', 'it',
             'iw', 'ja', 'jv', 'ka', 'kk', 'km', 'kn', 'ko', 'ku', 'ky', 'la', 'lb', 'lo', 'lt', 'lv', 'mg', 'mi', 'mk',
             'ml', 'mn', 'mr', 'ms', 'mt', 'my', 'ne', 'nl', 'no', 'ny', 'pa', 'pl', 'ps', 'pt', 'ro', 'ru', 'sd', 'si',
             'sk', 'sl', 'sm', 'sn', 'so', 'sq', 'sr', 'st', 'su', 'sv', 'sw', 'ta', 'te', 'tg', 'th', 'tr', 'uk', 'ur',
    ...
    for lang in langs:
        ...
        if lang in LANG_SPLIT:
            for split, pattern in LANG_SPLIT[lang]:
                num_files = subprocess.check_output(f"cd {c4_path} && "
                                                    f"git lfs ls-files -I '{pattern}' | wc -l", shell=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--languages", help="Specify the language list e.g. `en,es,zh,de,...` or "
                                            "use `all` to download all languages", required=True)
