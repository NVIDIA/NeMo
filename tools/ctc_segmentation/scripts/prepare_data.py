...
parser = argparse.ArgumentParser(
    description="Prepares text and audio files for segmentation"
)
parser.add_argument(
    "--in_text",
    type=str,
    default=None,
    help="Path to a text file or a directory with .txt files",
)
parser.add_argument(
    "--output_dir", type=str, required=True, help="Path to output directory"
)
parser.add_argument(
    "--audio_dir", type=str, help="Path to folder with .mp3 or .wav audio files"
)
parser.add_argument(
    "--sample_rate",
    type=int,
    default=16000,
    help="Sampling rate used during ASR model training, Hz",
)
parser.add_argument(
    "--bit_depth",
    type=int,
    default=16,
    help="Bit depth to use for processed audio files",
)
parser.add_argument(
    "--n_jobs",
    default=-2,
...
parser.add_argument(
    ...
    type=int,
    default=40,
    help="Max number of words of the text segment for alignment.",
)
parser.add_argument(
...
def split_text(
    ...
    if language == "en" and use_nemo_normalization:
        ...
        normalizer = Normalizer(
            input_case="cased",
            cache_dir=os.path.join(os.path.dirname(out_file), "en_grammars"),
        )
    ...
    with open(
        os.path.join(out_dir, out_file_name[:-4] + "_with_punct_normalized.txt"), "w"
    ) as f:
        f.write(sentences)
...
if __name__ == "__main__":
    ...
    if args.in_text:
        if args.model is None:
            ...
            model_cfg = ASRModel.restore_from(
                restore_path=args.model, return_config=True
            )
    ...
    if args.audio_dir:
        if not os.path.exists(args.audio_dir):
            raise ValueError(
                f"{args.audio_dir} not found. '--audio_dir' should contain .mp3 or .wav files."
            )
        ...
        Parallel(n_jobs=args.n_jobs)(
            delayed(process_audio)(
                ...
                os.path.join(
                    args.output_dir,
                    os.path.splitext(os.path.basename(audio_paths[i]))[0] + ".wav",
                ),
                args.cut_prefix,
                args.sample_rate,
                args.bit_depth,
