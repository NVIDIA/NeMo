...
def make_ass_files(
    ...
    utt_obj = make_word_level_ass_file(
        utt_obj, output_dir_root, ass_file_config, audio_dur
    )
    utt_obj = make_token_level_ass_file(
        utt_obj, output_dir_root, ass_file_config, audio_dur
    )
    ...
    while all_words_and_tokens_pointer < len(all_words_and_tokens):
        ...
        if type(word_or_token) is Word:
            ...
            if not new_segments_and_tokens[-1].words_and_tokens:
                ...
                segment_so_far_n_chars = _get_segment_n_chars(
                    new_segments_and_tokens[-1]
                )
...
def make_word_level_ass_file(utt_obj, output_dir_root, ass_file_config, audio_dur):
    ...
    if ass_file_config.vertical_alignment == "top":
        default_style_dict["Alignment"] = (
            "8"  # text will be 'center-justified' and in the top of the screen
        )
    elif ass_file_config.vertical_alignment == "center":
        default_style_dict["Alignment"] = (
            "5"  # text will be 'center-justified' and in the middle of the screen
        )
    elif ass_file_config.vertical_alignment == "bottom":
        default_style_dict["Alignment"] = (
            "2"  # text will be 'center-justified' and in the bottom of the screen
        )
    else:
        raise ValueError(
            f"got an unexpected value for ass_file_config.vertical_alignment"
        )
    ...
    already_spoken_color_code = (
        r"{\c&H" + rgb_list_to_hex_bgr(ass_file_config.text_already_spoken_rgb) + r"&}"
    )
    being_spoken_color_code = (
        r"{\c&H" + rgb_list_to_hex_bgr(ass_file_config.text_being_spoken_rgb) + r"&}"
    )
    not_yet_spoken_color_code = (
        r"{\c&H" + rgb_list_to_hex_bgr(ass_file_config.text_not_yet_spoken_rgb) + r"&}"
    )
    ...
    with open(output_file, "w") as f:
        ...
        text_before_speech = (
            not_yet_spoken_color_code
            + " ".join([x.text for x in words_in_first_segment])
            + r"{\r}"
        )
        subtitle_text = (
            ...
            if type(segment_or_token) is Segment:
                ...
                for word_i, word in enumerate(words_in_segment):
                    ...
                    if word_i < len(words_in_segment) - 1:
                        text_after = " " + " ".join(
                            [x.text for x in words_in_segment[word_i + 1 :]]
                        )
                    else:
                        text_after = ""
        ...
        text_after_speech = (
            already_spoken_color_code
            + " ".join([x.text for x in words_in_final_segment])
            + r"{\r}"
        )
...
def make_token_level_ass_file(utt_obj, output_dir_root, ass_file_config, audio_dur):
    ...
    with open(output_file, "w") as f:
        ...
        for token in tokens_in_first_segment:
            ...
            token.text_cased = token.text_cased.replace(
                SPACE_TOKEN, " "
            )  # space token with actual space

        text_before_speech = (
            not_yet_spoken_color_code
            + "".join([x.text_cased for x in tokens_in_first_segment])
            + r"{\r}"
        )
        subtitle_text = (
        ...
        for segment_or_token in utt_obj.segments_and_tokens:
            if type(segment_or_token) is Segment:
                ...
                for token_i, token in enumerate(tokens_in_segment):
                    ...
                    text_before = "".join(
                        [x.text_cased for x in tokens_in_segment[:token_i]]
                    )
                    text_before = already_spoken_color_code + text_before + r"{\r}"

                    if token_i < len(tokens_in_segment) - 1:
                        text_after = "".join(
                            [x.text_cased for x in tokens_in_segment[token_i + 1 :]]
                        )
                    else:
                        text_after = ""
                    text_after = not_yet_spoken_color_code + text_after + r"{\r}"
                    ...
                    aligned_text_off = (
                        already_spoken_color_code + token.text_cased + r"{\r}"
                    )
        ...
        text_after_speech = (
            already_spoken_color_code
            + "".join([x.text_cased for x in tokens_in_final_segment])
            + r"{\r}"
        )
        # note: for now doing some extra padding with math.ceil(audio_dur)+1) to account for the fact that the video with subtitles can become
        # longer than the original audio during the MP4 creation stage.
