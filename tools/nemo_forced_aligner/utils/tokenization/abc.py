...
from utils.units import Alignment, Segment, Word, Token
...
class BaseAligner(ABC):
    ...
    def text_to_segments(text, separator: str = None):
        ...
        segmented_text = [seg for seg in [seg.strip() 
                                          for seg in segmented_text] if len(seg) > 0]
    ...
    def is_alignable(self, text, T: int):
        if text is None or len(text) == 0:
            logging.info(
                f"utterance is empty - we will not generate"
    ...
    def align_unit(self, unit_text: str, unit_type: Type[Union[Segment, Word, Token]], unit_s_pointer: int = 0):
        if unit_type == Segment or unit_type == Word:
            ...
            unit = unit_type(text = unit_text,
                             ...
                             s_end = unit_s_pointer + len(unit_tokens) * 2 - 2)
