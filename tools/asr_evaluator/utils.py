...
def run_asr_inference(cfg: DictConfig) -> DictConfig:
    ...
    """
    if (cfg.model_path and cfg.pretrained_name) or (
        not cfg.model_path and not cfg.pretrained_name
    ):
    ...
    if cfg.inference.mode == "offline":
        ...
        if (
            ...
            raise ValueError(
                f"Please specify both total_buffer_in_secs and chunk_len_in_secs for chunked inference"
            )
        ...
        if (
            "chunk_len_in_secs" not in cfg.inference
            or not cfg.inference.chunk_len_in_secs
        ):
            with open_dict(cfg):
        ...
        raise ValueError(
            f"inference could only be offline or chunked, but got {cfg.inference.mode}"
        )
...
def run_chunked_inference(cfg: DictConfig) -> DictConfig:
    ...
    if "output_filename" not in cfg or not cfg.output_filename:
        ...
        with open_dict(cfg):
            cfg.output_filename = (
                model_name + "-" + dataset_name + "-" + mode_name + ".json"
            )
...
def run_offline_inference(cfg: DictConfig) -> DictConfig:
    ...
    with tempfile.NamedTemporaryFile(mode="w", encoding="utf-8") as f:
        ...
        script_path = (
            Path(__file__).parents[2] / "examples" / "asr" / "transcribe_speech.py"
        )
        ...
        hydra_overrides = get_hydra_override_from_config(
            cfg.get("transcribe_params", None), exclude_keys=exclude_keys
        )
        # If need to change other config such as decoding strategy, could either:
...
def cal_target_metadata_wer(
    ...
    if len(wer_per_class) > 0:
        ...
        for target_class in wer_per_class:
            ...
            res_wer_per_class[target_class]["samples"] = wer_per_class[target_class][
                "samples"
            ]
            res_wer_per_class[target_class][eval_metric] = (
                wer_per_class[target_class]["errors"]
                / wer_per_class[target_class]["tokens"]
            )
            res_wer_per_class[target_class]["tokens"] = wer_per_class[target_class][
                "tokens"
            ]
            res_wer_per_class[target_class]["ins_rate"] = (
                wer_per_class[target_class]["inss"]
                / wer_per_class[target_class]["tokens"]
            )
            res_wer_per_class[target_class]["del_rate"] = (
                wer_per_class[target_class]["dels"]
                / wer_per_class[target_class]["tokens"]
            )
            res_wer_per_class[target_class]["sub_rate"] = (
                wer_per_class[target_class]["subs"]
                / wer_per_class[target_class]["tokens"]
            )
    else:
        ...
        for target_class in wer_per_class:
            for s in meta_cfg.slot:
                if isinstance(s[0], float) or isinstance(s[0], int):
                    ...
                    raise ValueError(
                        "Current only support target metadata belongs to numeric or string "
                    )

        for slot_key in slot_wer:
            slot_wer[slot_key][eval_metric] = (
                slot_wer[slot_key]["errors"] / slot_wer[slot_key]["tokens"]
            )
            slot_wer[slot_key]["ins_rate"] = (
                slot_wer[slot_key]["inss"] / slot_wer[slot_key]["tokens"]
            )
            slot_wer[slot_key]["del_rate"] = (
                slot_wer[slot_key]["dels"] / slot_wer[slot_key]["tokens"]
            )
            slot_wer[slot_key]["sub_rate"] = (
                slot_wer[slot_key]["subs"] / slot_wer[slot_key]["tokens"]
            )
