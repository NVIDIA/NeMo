import pytest
import pandas as pd
from customization_dataset_preparation import recommend_hyperparameters, warn_completion_is_not_empty, \
                                warn_imbalanced_completion, get_common_suffix, parse_template, \
                                validate_template, warn_duplicated_rows, drop_duplicated_rows, \
                                template_mapper, drop_unrequired_fields, convert_into_template, \
                                convert_into_prompt_completion_only, warn_and_drop_long_samples, warn_low_n_samples, \
                                warn_low_n_samples, show_first_example_in_df, get_prepared_filename, \
                                warn_missing_suffix, split_into_train_validation
...
def test_warn_completion_is_not_empty():
    ...
    msg_all_empty = "TODO: Note all completion fields are empty. This is possibly expected for inference but not for training"

    assert warn_completion_is_not_empty(df_all_empty) == msg_all_empty

    df_some_empty = pd.DataFrame({
        'prompt': ['prompt']*2,
...
def test_warn_imbalanced_completion():
    ...
    df_classification_balanced = pd.DataFrame({
        'prompt': [f'prompt{i}' for i in range(100)],
        'completion': [f'completion{i}' for i in range(5)] * 20
        })

    msg_classification_balanced = f"There are {5} unique completions over {100} samples.\nThe five most common completions are:"
    for i in range(5):
    ...
    msg_classification_imbalanced = f"There are {5} unique completions over {100} samples.\nThe five most common completions are:"
...
def test_warn_duplicated_rows():
    ...
    df_only_prompt_duplicated = pd.DataFrame({
        'prompt': ['prompt'] *2,
        'completion': ['completion', 'completion1']
    })
...
def test_template_mapper():
    ...
    assert template_mapper(df_qa.iloc[0], field_names_qa, template_qa) == "Context: context sample Question: question sample Answer:"
...
def test_drop_unrequired_fields():
    ...
    df_dropped_unnecessary_fields = pd.DataFrame({
        'prompt':['prompt'],
        'completion': ['completion']
    })
...
def test_convert_into_template():
    df_non_existant_field_name = pd.DataFrame({
        'question': ['question']
    })
...
def test_convert_into_prompt_completion_only():
    ...
    df_prompt = pd.DataFrame({
        'prompt': ["Context: context sample Question: question sample Answer:"],
        'completion': ["answer sample"]
    })

    prompt_template = "Context: {context} Question: {question} Answer:"
    completion_template = "{answer}"

    assert df_prompt.equals(convert_into_prompt_completion_only(df, prompt_template=prompt_template, completion_template=completion_template))
...
def test_warn_and_drop_long_samples():
    df = pd.DataFrame({
        'prompt':['a'*12000, 'a'*9000, 'a'],
        'completion': ['b'*12000, 'b'*2000,  'b']
    })
...
def test_warn_low_n_samples():
    ...
    message = "TODO: We would recommend having more samples (>64) if possible but current_file only contains 10 samples. "
...
def test_get_prepared_filename():
    ...
    assert get_prepared_filename(filename, split_train_validation=True) == [prepared_train_filename, prepared_val_filename]
