...
def test_recommend_hyperparameters():
    df_100 = pd.DataFrame(
        {"prompt": ["prompt"] * 100, "completion": ["completion"] * 100}
    )
    ...
    df_1000 = pd.DataFrame(
        {"prompt": ["prompt"] * 1000, "completion": ["completion"] * 1000}
    )
    ...
    df_10000 = pd.DataFrame(
        {"prompt": ["prompt"] * 10000, "completion": ["completion"] * 10000}
    )
    ...
    df_100000 = pd.DataFrame(
        {"prompt": ["prompt"] * 100000, "completion": ["completion"] * 100000}
    )
...
def test_warn_completion_is_not_empty():
    ...
    df_some_empty = pd.DataFrame(
        {"prompt": ["prompt"] * 2, "completion": ["", "completion"]}
    )
...
def test_warn_imbalanced_completion():
    ...
    assert (
        warn_imbalanced_completion(df_classification_balanced)
        == msg_classification_balanced
    )
    ...
    assert (
        warn_imbalanced_completion(df_classification_imbalanced)
        == msg_classification_imbalanced
    )
...
def test_warn_duplicated_rows():
    ...
    message_duplicated += (
        "Please check the original file to make sure that is expected\n"
    )
    ...
    df_unique = pd.DataFrame(
        {"prompt": ["prompt", "prompt1"], "completion": ["completion", "completion1"]}
    )
    assert warn_duplicated_rows(df_unique) is None

    df_only_prompt_duplicated = pd.DataFrame(
        {"prompt": ["prompt"] * 2, "completion": ["completion", "completion1"]}
    )
...
def test_drop_duplicated_rows():
    ...
    assert drop_duplicated_rows(df_only_prompt_duplicated) == (
        df_only_prompt_duplicated,
        None,
    )
...
def test_template_mapper():
    ...
    df_qa = pd.DataFrame(
        {"question": ["question sample"], "context": ["context sample"]}
    )
