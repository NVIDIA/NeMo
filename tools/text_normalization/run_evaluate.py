...
from utils import known_types, load_files, evaluate, training_data_to_sentences, training_data_to_tokens
...
def parse_args():
    ...
    parser.add_argument("--cat", dest="category", help="focus on class only (" + ", ".join(known_types) + ")", type=str, default=None)
...
if __name__ == "__main__":
    ...
    token_weighted_accuracy = [token_count_per_type[token_type] * accuracy for token_type, accuracy in token_accuracy.items()]
    ...
    if args.category is None: 
        print('numbers\t' + str(len(sentences_un_normalized)) + '\t' + '\t'.join([str(token_count_per_type[known_type]) for known_type in known_types]))
        print(args.normalizer + '\t' + str(sentences_accuracy) + '\t' + '\t'.join([str(token_accuracy[known_type]) for known_type in known_types]))
    else:
