...
@click.option(
    ...
    type=int,
    default=100,
    help="Number of conversations per shard.",
)
...
def export(manifest: str, output_dir: str, shard_size: int, shuffle: bool, seed: int):
    with NeMoMultimodalConversationTarWriter(
        output_dir, shard_size=shard_size
    ) as writer:
        source = NeMoMultimodalConversationJsonlAdapter(
            manifest, audio_locator_tag="<dummy>"
        )
        if shuffle:
