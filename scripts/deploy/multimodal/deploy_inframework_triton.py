...
megatron_neva_supported = True
try:
    from nemo.deploy.multimodal import MegatronNevaDeployable
except Exception as e:
    LOGGER.warning(f"Cannot import MegatronNevaDeployable, it will not be available. {type(e).__name__}: {e}")
    megatron_neva_supported = False
...
def get_nemo_deployable(args):
    ...
    return MegatronNevaDeployable(args.nemo_checkpoint, args.num_gpus)
...
def nemo_deploy(argv):
    ...
    if not megatron_neva_supported:
        raise ValueError("MegatronNevaDeployable is not supported in this environment.")
