...
def main(args: argparse.Namespace):
    ...
    try:
        ...
        server = DeployPyTriton(
            model=exporter, triton_model_name=args.triton_model_name, streaming=args.enable_streaming
        )
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '-nc', '--nemo_checkpoint', required=True, help='Path to the .nemo file with the model checkpoint'
    )
    parser.add_argument(
        '-md',
        '--model_dir',
    ...
    parser.add_argument(
        '-mt', '--model_type', required=True, help='Model architecture, such as llama, mistral, mixtral.'
    )
    parser.add_argument(
        '-tmn',
        '--triton_model_name',
    ...
    parser.add_argument(
        '-ng', '--num_gpus', type=int, default=1, help='Number of GPUs to use in tensor-parallel mode.'
    )
