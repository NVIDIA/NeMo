...
def get_args(argv):
    ...
    parser.add_argument("-hfp", "--hf_model_path", type=str, help="Huggingface model path")
    parser.add_argument("-hfid", "--hf_model_id", type=str, help="Huggingface model id")
    parser.add_argument(
        "-ptnc",
...
def get_trtllm_deployable(args):
    ...
    if args.hf_model_path is None and args.hf_model_id is not None:
        ...
        try:
            ...
            args.hf_model_path = os.path.join(hf_model_cache_dir, args.hf_model_id)
            Path(args.hf_model_path).mkdir(parents=True, exist_ok=True)

            # Save tokenizer files and config to model directory
            tokenizer.save_pretrained(args.hf_model_path)
            config.save_pretrained(args.hf_model_path)
    ...
    trt_llm_exporter = TensorRTLLM(
        ...
        load_model=(args.nemo_checkpoint is None and args.hf_model_path is None),
