...
def run_pretraining(language_model_from_pretrained=None, checkpoint_path=None, output_dir=None, freeze_modules=None):
    # pylint: disable=C0115,C0116
    recipe = configure_recipe(pretrain=True, language_model_from_pretrained=language_model_from_pretrained, checkpoint_path=checkpoint_path, output_dir=output_dir, freeze_modules=freeze_modules)
...
def run_finetuning(checkpoint_path=None, output_dir=None, freeze_modules=None):
    # pylint: disable=C0115,C0116
    recipe = configure_recipe(pretrain=False, checkpoint_path=checkpoint_path, output_dir=output_dir, freeze_modules=freeze_modules)
    executor = local_executor_torchrun(nodes=recipe.trainer.num_nodes, devices=recipe.trainer.devices)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="./outputs/checkpoints/avlm",
        help="Path to store checkpoints (optional)."
    )
    ...
    parser.add_argument("--unfreeze_vision_projection", action="store_true", help="Unfreeze vision projection (optional).")
    parser.add_argument("--unfreeze_audio_projection", action="store_true", help="Unfreeze audio projection (optional).")
