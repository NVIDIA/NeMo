...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Path to Flux transformer checkpoint(s)",
    )
    parser.add_argument(
        "--controlnet_ckpt",
        type=str,
        default="",
        help="Path to controlnet checkpoint(s)",
    )
    parser.add_argument(
        "--control_image",
        type=str,
    ...
    parser.add_argument(
        "--vae_ckpt",
        type=str,
        default="/ckpts/ae.safetensors",
        help="Path to 'ae.safetensors'",
    )
    parser.add_argument(
        "--clip_version",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Number of joint transformer layers in controlnet.",
    )
    parser.add_argument(
        "--num_single_layers",
        type=int,
        default=1,
        help="Number of single transformer layers in controlnet.",
    )
    parser.add_argument(
        "--flux_num_joint_layers",
        type=int,
        default=1,
        help="Number of joint transformer layers in controlnet.",
    )
    parser.add_argument(
        "--flux_num_single_layers",
        type=int,
        default=1,
        help="Number of single transformer layers in controlnet.",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Offload modules to cpu after being called.",
    )
    parser.add_argument(
        "--prompts",
        type=str,
    ...
    parser.add_argument(
        "--output_path",
        type=str,
        default="/tmp/flux_controlnet_output",
...
if __name__ == "__main__":
    ...
    params.clip_params.version = (
        args.clip_version
        if os.path.exists(args.clip_version)
        else "openai/clip-vit-large-patch14"
    )
    params.t5_params.version = (
        args.t5_version if os.path.exists(args.t5_version) else "google/t5-v1_1-xxl"
    )
