...
class EvalContextBiasingConfig:
    ...
    device: str = (
        "cuda"  # The device to load the model onto to calculate log probabilities
    )
    use_amp: bool = (
        False  # Whether to use AMP if available to calculate log probabilities
    )
    ...
    context_file: str = (
        MISSING  # text file with context biasing words and their spellings
    )
    spelling_separator: str = (
        "_"  # separator between word and its spellings in context biasing file
    )
    beam_threshold: list[float] = field(
        default_factory=lambda: [5.0]
    )  # beam pruning threshold for ctc-ws decoding
    context_score: list[float] = field(
        default_factory=lambda: [3.0]
    )  # per token weight for context biasing words
    ...
    sort_logits: bool = (
        True  # do logits sorting before decoding - it reduces computation on puddings
    )
...
def decoding_step(
    ...
    if cfg.apply_context_biasing:
        ...
        for idx, logits in tqdm(
            enumerate(ctc_logprobs),
            desc=f"Eval CTC-based Word Spotter...",
            ncols=120,
            total=len(ctc_logprobs),
        ):
        ...
        for batch_idx, probs in enumerate(ctc_logprobs):
            ...
            if cfg.apply_context_biasing and ws_results[audio_file_paths[batch_idx]]:
                ...
                if isinstance(asr_model, EncDecHybridRNNTCTCModel):
                    hyp = asr_model.ctc_decoding.ctc_decoder_predictions_tensor(
                        preds_tensor
                    )[0]
                else:
                    hyp = asr_model.wer.decoding.ctc_decoder_predictions_tensor(
                        preds_tensor
                    )[0]
                pred_text = hyp.text
    ...
    # rnnt part for EncDecHybridRNNTCTCModel
        ...
        if progress_bar:
            ...
            it = tqdm(
                range(int(np.ceil(len(encoder_outputs) / beam_batch_size))),
                desc=description,
                ncols=120,
            )
        else:
        ...
        for batch_idx in it:
            ...
            with torch.no_grad():
                packed_batch = torch.zeros(
                    len(probs_batch),
                    probs_batch[0].shape[0],
                    max(probs_lens),
                    device="cpu",
                )

                for prob_index in range(len(probs_batch)):
                    packed_batch[prob_index, :, : probs_lens[prob_index]] = (
                        torch.tensor(
            ...
            for beams_idx, beams in enumerate(beams_batch):
                ...
                for candidate_idx, candidate in enumerate(beams):
                    if (
                        cfg.apply_context_biasing
                        and ws_results[audio_file_paths[sample_idx + beams_idx]]
                    ):
                        # make new text by mearging alignment with ctc-ws predictions:
                        if cfg.print_cb_stats:
                            logging.info("\n" + "********" * 10)
                            logging.info(f"File name: {audio_file_paths[batch_idx]}")
                        pred_text, raw_text = (
                            context_biasing.merge_alignment_with_ws_hyps(
                                candidate,
                                asr_model,
                        ...
                        if cfg.print_cb_stats:
                            ...
                            logging.info(f"hyp text: {pred_text}")
                            logging.info(
                                f"ref text: {target_transcripts[sample_idx + beams_idx]}"
                            )
                    else:
...
@hydra_runner(
    config_path=None,
    config_name="EvalContextBiasingConfig",
    schema=EvalContextBiasingConfig,
)
def main(cfg: EvalContextBiasingConfig):
    ...
    ), f"preds_output_folder {cfg.preds_output_folder} does not exist"
            ...
            if isinstance(asr_model, EncDecCTCModelBPE):
                ...
                with tempfile.TemporaryDirectory() as tmpdir:
                    ...
                    for test_batch in tqdm(
                        temporary_datalayer,
                        desc="Getting encoder and CTC decoder outputs...",
                        disable=False,
                    ):
    ...
    if cfg.decoder_type == "rnnt" and cfg.sort_logits:
        encoder_outputs_with_indeces = sorted(
            enumerate(encoder_outputs), key=lambda x: x[1].size()[1], reverse=True
        )
    ...
    for hp in hp_grid:
        ...
        # compute fscore
        fscore_stats = context_biasing.compute_fscore(
            preds_output_manifest, context_words
        )
        ...
        logging.info(
            f"Precision/Recall/Fscore = {fscore_stats[0]:.4f}/{fscore_stats[1]:.4f}/{fscore_stats[2]:.4f}"
        )
        logging.info(
