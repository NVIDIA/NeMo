...
def process_atis(infold, outfold, modes=["train", "test"], do_lower_case=False):
    ...
    for mode in modes:
        outfiles[mode] = open(
            os.path.join(outfold, mode + ".tsv"), "w", encoding="utf-8"
        )
        outfiles[mode].write("sentence\tlabel\n")
        outfiles[mode + "_slots"] = open(
            f"{outfold}/{mode}_slots.tsv", "w", encoding="utf-8"
        )

        queries = open(
            f"{infold}/atis.{mode}.query.csv", "r", encoding="utf-8"
        ).readlines()
        intents = open(
            f"{infold}/atis.{mode}.intent.csv", "r", encoding="utf-8"
        ).readlines()
        slots = open(
            f"{infold}/atis.{mode}.slots.csv", "r", encoding="utf-8"
        ).readlines()
...
def process_snips(
    infold, outfold, do_lower_case, modes=["train", "test"], dev_split=0.1
):
    """
...
def process_jarvis_datasets(
    ...
    outfiles["dict_intents"] = open(
        f"{outfold}/dict.intents.csv", "w", encoding="utf-8"
    )
    ...
    for mode in modes:
        ...
        if not if_exist(infold, [f"{mode}.tsv"]):
            logging.info(
                f"{mode} mode of {dataset_name}" f" is skipped as it was not found."
            )
            continue
        ...
        for i, query in enumerate(queries):
            ...
            outfiles[mode].write(
                f"{sentence_cld}\t" f"{str(intents_list[intent_str])}\n"
            )
            ...
            for tag_start, tag_end, tag_str in slot_tags_list:
                ...
                slots.extend(
                    [str(slots_list_all[f"I-{tag_str}"])] * (len(words_list) - 1)
                )
...
if __name__ == "__main__":
    ...
    if dataset_name == "atis":
        process_atis(
            infold=source_dir, outfold=target_dir, do_lower_case=args.do_lower_case
        )
