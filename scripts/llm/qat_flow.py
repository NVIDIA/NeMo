...
def export_most_recent_ckpt(exp_dir: str, output_path: str):
    ...
    print("RECENT EXP: ", most_recent_exp)
    most_recent_ckpt = get_most_recent_subdir(f"{most_recent_exp}/checkpoints/")
    # TODO check that -unfinished doesn't exist?
...
if __name__ == "__main__":
    ...
    # 3. Export
    export = run.Partial(export_most_recent_ckpt, exp_dir=train.log.log_dir, output_path=f"{model_name}_hf")
    ...
    with run.Experiment(args.experiment, executor=executor, log_level="INFO") as exp:
        ...
        s3 = exp.add(export, tail_logs=True, name="export_hf", executor=run.LocalExecutor(), dependencies=[s2])
