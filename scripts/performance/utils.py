...
def slurm_executor(
    ...
    srun_args.extend(
        [
            "--mpi=pmix",
            "numactl --cpunodebind=$((SLURM_LOCALID/4)) --membind=$((SLURM_LOCALID/4))",  # numactl command should be always at the end of srun_args # pylint: disable=line-too-long
        ]
    )
...
def run_performance_experiment(task: str, model: str, model_size: str, override_recipe_configs: Callable, **kwargs):
    ...
    # set experiment name
    num_nodes, mbs, gbs, tp_size, pp_size, cp_size, vp_size, ep_size, etp_size = config_args[:9]
    ...
    plugins = [
        PerfEnvPlugin(
            enable_vboost=True,
            nccl_pp_comm_chunksize=2097152 if args.pp_size > 1 else None,
