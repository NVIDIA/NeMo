...
def main():
    ...
    if not use_ctm:
        ...
        ctm_directory = base_alignment_path
    ...
    while src_i < len(manifest):
        ...
        # append last end
        if file['duration'] > prev_end:
            words.append("")
            end_times.append(file['duration'])

        # build target manifest entry
        target_manifest.append({})
        target_manifest[tgt_i]['audio_filepath'] = file['audio_filepath']
        target_manifest[tgt_i]['duration'] = file['duration']
        target_manifest[tgt_i]['text'] = file['text']
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--base_alignment_path", help="path to alignments (LibriSpeech or CTM)", type=str, required=True
    )
