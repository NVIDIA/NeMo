...
def split_into_pairwise_rttm(audio_rttm_map, input_manifest_path, output_dir):
    ...
    for uniq_id, line in tqdm(
        input_manifest_dict.items(), total=len(input_manifest_dict)
    ):
...
def main(
    ...
    if output_manifest_path and ".json" not in output_manifest_path:
        ...
        output_manifest_path = rreplace(
            input_manifest_path, ".json", f".{step_count}seg.json"
        )
    ...
    segments_manifest_file = write_rttm2manifest(
        AUDIO_RTTM_MAP, segment_manifest_path, decimals
    )
    ...
    subsegments_dict = get_subsegment_dict(
        subsegments_manifest_file, window, shift, decimals
    )
    write_truncated_subsegments(
        input_manifest_dict,
        subsegments_dict,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input_manifest_path", help="input json file name", type=str, required=True
    )
    parser.add_argument(
        "--output_manifest_path",
        help="output manifest_file name",
        type=str,
        default=None,
        required=False,
    )
    ...
    parser.add_argument(
        "--window", help="Window length for segmentation", type=float, required=True
    )
    parser.add_argument(
        "--shift", help="Shift length for segmentation", type=float, required=True
    )
    parser.add_argument(
        "--decimals", help="Rounding decimals", type=int, default=3, required=False
    )
    parser.add_argument(
        "--step_count",
