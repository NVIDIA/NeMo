...
if [[ $# -ne 3 ]]; then
  echo "Require max sample number | data directory | resample data directory as arguments to the script"
  exit 2
fi
...
while((FILECOUNT <= MAX_SAMPLE_NUM))
do
  for category in "${categories[@]}"
  do
    python freesound_download.py --data_dir "${DATADIR}" --category "${category}" --max_samples 200 --min_filesize 0 --max_filesize 100
  done
...
python freesound_resample.py --data_dir={data_dir} --resampled_dir={resampled_dir}
