...
Required to set: `export PYTHONPATH=/path/to/google/gemma/jax:$PYTHONPATH`
"""
...
def create_rename_keys(num_hidden_layers):
    ...
    for i in range(num_hidden_layers):
        ...
        rename_keys.extend([
            ...
            (f"transformer.layer_{i}.attn.qkv_einsum.w", f"model.decoder.layers.{i}.self_attention.linear_qkv.weight"),
            (f"transformer.layer_{i}.attn.kv_einsum.w", f"model.decoder.layers.{i}.self_attention.linear_kv.weight"),
            ...
            (f"transformer.layer_{i}.pre_ffw_norm.scale", f"model.decoder.layers.{i}.mlp.linear_fc1.layer_norm_weight"),
        ])
