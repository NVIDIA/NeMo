...
Example script to preprocess the JSON file for music gpt training dataset

```python
python scripts/nlp_language_modeling/preprocess_music_data_for_megatron.py \
       --input=PATH_TP_MUSIC_TXT_data \
       --output-prefix=PATH_TO_PREPROCESSED_BIN_IDX \
       --json-keys text \
       --vocab-file=PATH_TO_MUSIC_VOCAB_JSON_FILE \
       --dataset-impl mmap \
       --tokenizer-library=music \
       --tokenizer-type=MusicTokenizer \
       --workers=1 \
       --append-eod
```
"""
...
def main():
    ...
    print(f"Vocab size: {music_tokenizer.vocab_size}")
