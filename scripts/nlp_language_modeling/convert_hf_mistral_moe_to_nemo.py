...
r"""
Conversion script to convert Huggingface Mistral-MoE checkpoints into NeMo checkpoint.
  Example to run this conversion script:
    python convert_hf_mistral_moe_to_nemo.py \
...
def get_args():
    ...
    parser.add_argument(
        "--in-file", type=str, default=None, required=True, help="Path to Huggingface Mistral-MoE checkpoints",
    )
...
def load_config(mistral_config, tokenizer_path):
    ...
    nemo_config.num_moe_experts = int(mistral_config['num_local_experts'])
    assert nemo_config.num_moe_experts > 0, "num_experts must be greater than zero."
    nemo_config.num_experts_per_token = int(mistral_config['num_experts_per_tok'])
...
def convert(args):
    ...
    for l in range(int(num_layers)):
        ...
        if mcore_gpt:
            moe_gate_name = f'model.decoder.layers.{l}.mlp.router.gate.weight'
        else:
