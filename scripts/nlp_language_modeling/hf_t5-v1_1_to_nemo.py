...
try:
    ...
    raise ImportError(
        "Please install accelerate package via `pip install accelerate` to use this script."
    )
...
def convert_weights(hf_model, nemo_state_dict_path):
    ...
    hf_model = T5ForConditionalGeneration.from_pretrained(
        hf_model, low_cpu_mem_usage=True, torch_dtype=torch_dtype
    )
    ...
    for k, v in hf_weights.items():
        ...
        # Decoder embeddings
            ...
            print(
                f"Mapped {k} to enc_dec_model.encoder_embedding.word_embeddings.weight"
            )
        ...
        #################################################
            ...
            k == "encoder.block.0.layer.0.SelfAttention.relative_attention_bias.weight"
        ):
            nemo_weights[
            ...
            k == "decoder.block.0.layer.0.SelfAttention.relative_attention_bias.weight"
        ):
            nemo_weights[
        ...
        #################################################
            ...
            if "final" in k:
                ...
                nemo_weights[
                    f"enc_dec_model.enc_dec_model.{model_type}.model.final_layernorm.weight"
                ] = v
                print(
                    f"Mapped {k} to enc_dec_model.enc_dec_model.{model_type}.model.final_layernorm.weight"
                )
            else:
                model_type, block_number, layer_number = _get_model_type_block_layer(k)
...
def package_into_nemo_file(
    ...
    """
    trainer = Trainer(
        devices=1, strategy=NLPDDPStrategy(), accelerator="cpu", precision=32
    )
    ...
    with open_dict(base_cfg):
        ...
        base_cfg.encoder.relative_attention_num_buckets = (
            hf_model_config.relative_attention_num_buckets
        )
        ...
        base_cfg.decoder.relative_attention_num_buckets = (
            hf_model_config.relative_attention_num_buckets
        )
...
if __name__ == "__main__":
    ...
    if not os.path.exists(args.base_yaml_config):
        raise FileNotFoundError(
            f"Base yaml config file {args.base_yaml_config} does not exist."
        )
