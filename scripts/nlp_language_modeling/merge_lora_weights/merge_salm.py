...
def main(cfg) -> None:
    ...
    merged_weights = merge(
        lora_weights[0], lora_weights, num_layers=lora_model_cfg.num_layers, mcore=lora_model_cfg.mcore_gpt
    )
    del lora_model_cfg.peft
    imported_cls = ModularAudioGPTModel
    config_file = f"/lustre/fsw/portfolios/llmservice/users/zhehuaic/works/mod_speech_llm/code/NeMo_s2s_duplex3/examples/multimodal/speech_llm/conf/salm/modular_audio_gpt_config_llama_lhotse.yaml"
    model_config = OmegaConf.load(config_file)
    model_config.model = lora_model_cfg
    model_config.model.data.train_ds.global_batch_size = 8
    model_config.model.data.train_ds.micro_batch_size = 4
    trainer = MegatronLMPPTrainerBuilder(model_config).create_trainer()
    exp_manager(trainer, model_config.exp_manager)

    model = imported_cls.restore_from_pretrained_models(model_config, trainer=trainer)
    ...
    merged_weights = {k: v for k, v in merged_weights.items() if 'lora_' not in k}
    model.load_state_dict(merged_weights)  # remove lora weights
