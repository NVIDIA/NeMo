...
def get_args():
    ...
    parser.add_argument(
        "--mamba_ssm_ngroups", type=int, default=8, help="ngroups for Mamba model"
    )
    parser.add_argument(
        "--precision",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to the tokenizer.model, required for 8b models",
    )
...
def convert(args):
    ...
    if "backbone" in list(checkpoint_weights.keys())[0]:
        ...
        layer_keys = [
            key
            for key in checkpoint_weights.keys()
            if re.match(r"backbone\.layers\.\d+\.", key)
        ]
        layer_numbers = set(
            int(re.search(r"backbone\.layers\.(\d+)\.", key).group(1))
        ...
        for i in range(num_layers):
            for attr in layer_attributes:
                if attr == "norm.weight":
                    new_key = (
                        f"model.decoder.layers.{i}.mixer.in_proj.layer_norm_weight"
                    )
        ...
        layer_keys = [
            key
            for key in checkpoint_weights.keys()
            if re.match(r"decoder\.layers\.\d+\.", key)
        ]
        layer_numbers = set(
            int(re.search(r"decoder\.layers\.(\d+)\.", key).group(1))
            for key in layer_keys
        )
        num_layers = max(layer_numbers) + 1
    ...
    for i in range(max(map(int, layers.keys())) + 1):
        ...
        if "mixer" in layer_types:
            ...
            raise AssertionError(
                "Layer not found. Each layer must be eiher MLP, Mamba, or Attention"
            )
