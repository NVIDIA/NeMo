...
def get_args():
    ...
    parser.add_argument(
        ...
        default=os.path.join(
            ...
            "../../examples/nlp/language_modeling/conf/megatron_llama_config.yaml",
        ),
        required=False,
...
def convert(args):
    ...
    if f"model.layers.0.self_attn.rotary_emb.inv_freq" in model.state_dict():
        rotary_embed_weight = model.state_dict()[
            f"model.layers.0.self_attn.rotary_emb.inv_freq"
        ]
        if mcore_gpt:
            rotary_embed_weight_base_name = f"model.rotary_pos_emb.inv_freq"
        else:
            rotary_embed_weight_base_name = (
                f"model.language_model.rotary_pos_emb.inv_freq"
            )
    ...
    for l in range(int(num_layers)):
        ...
        q = model.state_dict()[f"model.layers.{l}.self_attn.q_proj.weight"].view(
            *new_q_tensor_shape
        )
        k = model.state_dict()[f"model.layers.{l}.self_attn.k_proj.weight"].view(
            *new_kv_tensor_shape
        )
        v = model.state_dict()[f"model.layers.{l}.self_attn.v_proj.weight"].view(
            *new_kv_tensor_shape
        )
    ...
    if "tokenizer_model" not in hf_config:
        if args.llama31:
            if hf_config["num_hidden_layers"] == 32:
                ...
                model.cfg.tokenizer.update(type="meta-llama/Llama-3.1-70B")
            elif hf_config["num_hidden_layers"] == 126:
                model.cfg.tokenizer.update(
                    type="meta-llama/Llama-3.1-8B"
                )  # 405B tokenizer is the same as 8B
            else:
                logging.warning(
                    "Unexpected model config for Llama3. Tokenizer config has not been modified."
                )
        else:
            if hf_config["num_hidden_layers"] == 32:
