...
def get_args():
    ...
    parser.add_argument(
        ...
        default=None,
        required=True,
        help="Path to NeMo Starcoder2 checkpoint",
    )
    parser.add_argument(
        "--output_path",
        type=str,
...
def load_config(hf_model_name, nemo_config):
    ...
    hf_config.sliding_window = (
        nemo_config.window_size[0] if "window_size" in nemo_config else None
    )
    ...
    hf_config.rope_theta = (
        nemo_config["rotary_base"] if "rotary_base" in nemo_config else 10000
    )
    return hf_config
...
def convert(in_file, precision=None, cpu_only=True) -> None:
    ...
    for l in range(int(num_layers)):
        ...
        state_dict[hf_mlp_cfc_weight_name] = param_to_weights(
            ckpt[mlp_down_base_name_weight]
        )
        if has_bias:
            state_dict[hf_mlp_cfc_bias_name] = param_to_weights(
                ckpt[mlp_down_base_name_bias]
            )
        ...
        if mcore_gpt:
            ...
            raise Exception("not implemented")
        state_dict[hf_mlp_up_base_name_weight] = param_to_weights(
            ckpt[mlp_up_base_name_weight]
        )
        if has_bias:
            state_dict[hf_mlp_up_base_name_bias] = param_to_weights(
                ckpt[mlp_up_base_name_bias]
            )
        ...
        if mcore_gpt:
            ...
            input_ln_base_name_bias = (
                f"model.decoder.layers.{l}.self_attention.linear_qkv.layer_norm_bias"
            )
        else:
            input_ln_base_name = (
        ...
        if has_bias:
            state_dict[hf_input_ln_bias_name] = param_to_weights(
                ckpt[input_ln_base_name_bias]
            )
        ...
        if mcore_gpt:
            ...
            post_attn_ln_base_name_bias = (
                f"model.decoder.layers.{l}.mlp.linear_fc1.layer_norm_bias"
            )
        else:
        ...
        if has_bias:
            state_dict[hf_post_attn_ln_bias_name] = param_to_weights(
                ckpt[post_attn_ln_base_name_bias]
            )
    ...
    if has_bias:
        state_dict[hf_final_ln_bias_name] = param_to_weights(
            ckpt[final_ln_base_name_bias]
        )
...
if __name__ == "__main__":
    args = get_args()
    hf_state_dict, nemo_config = convert(
        args.input_name_or_path, args.precision, args.cpu_only
    )
