...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--cuda", action="store_true", help="put Nemo model onto GPU prior to savedown"
    )
    ...
    if not os.path.exists(args.input_name_or_path):
        logging.critical(
            f"Input directory [ {args.input_name_or_path} ] does not exist or cannot be found. Aborting."
        )
        exit(255)

    if not os.path.exists(args.hparams_file):
        logging.critical(
            f"Path to config file [ {args.hparams_file} ] does not exist or cannot be found. Aborting."
        )
        exit(255)
    ...
    mpt_1 = torch.load(
        os.path.join(args.input_name_or_path, "pytorch_model-00001-of-00002.bin"),
        map_location="cpu",
    )
    mpt_2 = torch.load(
        os.path.join(args.input_name_or_path, "pytorch_model-00002-of-00002.bin"),
        map_location="cpu",
    )
    ...
    def convert_state_dict(state_dict, amp=False):
        def get_new_key(old_key):
            if old_key == "transformer.wte.weight":
                ...
                p1 = old_key.replace(
                    "transformer.blocks.", "language_model.encoder.layers."
                )
                p2 = p1.replace("norm_1.weight", "input_layernorm.weight")
                p3 = p2.replace(
                    "attn.Wqkv.weight", "self_attention.query_key_value.weight"
                )
    ...
    if model_dict["megatron_amp_O2"]:
        missing_keys, unexpected_keys = model.model.load_state_dict(
            convert_dict, strict=True
        )
    else:
        missing_keys, unexpected_keys = super(GPTModel, model.model).load_state_dict(
            convert_dict, strict=True
        )
    ...
    if len(unexpected_keys) > 0:
        logging.warning(
            "Unexpected keys were detected which should not happen. Please investigate."
        )
