...
def convert(args):
    ...
    new_state_dict["embed_tokens.weight"] = nemo_model.state_dict()[
        "model.embedding.word_embeddings.weight"
    ]
    new_state_dict["final_norm.weight"] = nemo_model.state_dict()[
    ...
    for l in range(nemo_config.model.num_layers):
        ...
        (
            ...
            f"model.decoder.layers.{l}.mlp.linear_fc1.bias"
        ].chunk(
            2
        )
        new_state_dict[f"layers.{l}.mlp_block.down_proj.weight"] = (
            nemo_model.state_dict()[f"model.decoder.layers.{l}.mlp.linear_fc2.weight"]
        )
        new_state_dict[f"layers.{l}.mlp_block.down_proj.bias"] = (
            nemo_model.state_dict()[f"model.decoder.layers.{l}.mlp.linear_fc2.bias"]
        )
