...
def get_args():
    ...
    parser.add_argument(
        "--input_name_or_path", type=str, default="google/recurrentgemma-2b"
    )
    parser.add_argument(
        "--precision",
        type=str,
...
def convert(args):
    ...
    hf_model = AutoModelForCausalLM.from_pretrained(
        args.input_name_or_path, device_map="auto"
    )
    ...
    new_state_dict["model.embedding.word_embeddings.weight"] = hf_model.state_dict()[
        "model.embed_tokens.weight"
    ]
    new_state_dict["model.decoder.final_layernorm.weight"] = hf_model.state_dict()[
    ...
    for l in range(nemo_config.model.num_layers):
        ...
        new_state_dict[f"model.decoder.layers.{l}.mlp.linear_fc2.weight"] = (
            hf_model.state_dict()[f"model.layers.{l}.mlp_block.down_proj.weight"]
        )
        new_state_dict[f"model.decoder.layers.{l}.mlp.linear_fc2.bias"] = (
        ...
        new_state_dict[f"model.decoder.layers.{l}.mlp.linear_fc1.layer_norm_weight"] = (
            hf_model.state_dict()[f"model.layers.{l}.channel_pre_norm.weight"]
        )

        if l % 3 == 2:
            new_state_dict[
                f"model.decoder.layers.{l}.self_attention.linear_proj.weight"
            ] = hf_model.state_dict()[f"model.layers.{l}.temporal_block.o_proj.weight"]
            new_state_dict[
                f"model.decoder.layers.{l}.self_attention.linear_proj.bias"
            ] = hf_model.state_dict()[f"model.layers.{l}.temporal_block.o_proj.bias"]
            new_state_dict[
            ...
            new_state_dict[
                ...
                [
                    hf_model.state_dict()[
                        f"model.layers.{l}.temporal_block.q_proj.weight"
                    ],
                    hf_model.state_dict()[
                        f"model.layers.{l}.temporal_block.k_proj.weight"
                    ],
                    hf_model.state_dict()[
                        f"model.layers.{l}.temporal_block.v_proj.weight"
                    ],
                ]
            )
            new_state_dict[
            ...
            new_state_dict[
                f"model.decoder.layers.{l}.recurrent_layer.linear_in.weight"
            ] = torch.cat(
                [
                    hf_model.state_dict()[
            ...
            new_state_dict[
                f"model.decoder.layers.{l}.recurrent_layer.linear_in.bias"
            ] = torch.cat(
                [
                    hf_model.state_dict()[
            ...
            ] = hf_model.state_dict()[f"model.layers.{l}.temporal_block.conv_1d.weight"]
            new_state_dict[
                f"model.decoder.layers.{l}.recurrent_layer.conv_1d.conv_1d.bias"
            ] = hf_model.state_dict()[f"model.layers.{l}.temporal_block.conv_1d.bias"]
