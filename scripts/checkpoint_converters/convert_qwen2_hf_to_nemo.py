...
def get_args():
    ...
    parser.add_argument(
        ...
        default=os.path.join(
            ...
            "../../examples/nlp/language_modeling/conf/megatron_qwen2_config.yaml",
        ),
        required=False,
...
def convert(args):
    ...
    for l in range(int(num_layers)):
        ...
        q = model.state_dict()[f"model.layers.{l}.self_attn.q_proj.bias"].view(
            *new_q_tensor_shape
        )
        k = model.state_dict()[f"model.layers.{l}.self_attn.k_proj.bias"].view(
            *new_kv_tensor_shape
        )
        v = model.state_dict()[f"model.layers.{l}.self_attn.v_proj.bias"].view(
            *new_kv_tensor_shape
        )
