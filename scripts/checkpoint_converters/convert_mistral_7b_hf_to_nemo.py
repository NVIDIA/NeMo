        ...
        if "cfg" in kwargs:
            ...
            for name, module in model.named_parameters():
                if name in checkpoint["state_dict"]:
                    ...
                    module.data = checkpoint["state_dict"][name].to(
                        dtype=module.data.dtype
                    )
...
def load_config(mistral_config, tokenizer, config_path):
    ...
    if getattr(tokenizer, "vocab_file", None) is not None:
        ...
        assert os.path.exists(
            vocab_output_path
        ), f"Expected {vocab_output_path} to exist"
        assert (
            os.path.getsize(vocab_output_path) > 0
        ), f"Expected {vocab_output_path} to be non-empty"
...
class LazyStateDict:
    ...
    def __getitem__(self, key):
        ...
        assert os.path.exists(
            ckpt_part_path
        ), f"Expected ckpt-part to exist {ckpt_part_path}"
...
def make_trainer(args, nemo_config):
    """creates PTL trainer"""
    model_args, ckpt, tokenizer = load_mistral_ckpt(
        args.input_name_or_path, load_model=False
    )
...
def save_to_nemo(args, checkpoint):
    ...
    checkpoint[MegatronGPTModel.CHECKPOINT_HYPER_PARAMS_KEY] = nemo_config
    checkpoint[MegatronGPTModel.CHECKPOINT_HYPER_PARAMS_KEY].use_cpu_initialization = (
        True
    )
    checkpoint[MegatronGPTModel.CHECKPOINT_HYPER_PARAMS_KEY].perform_initialization = (
        False
    )
    ...
    if (
        ...
        or md5_checksum(getattr(tokenizer, "vocab_file", None))
        == "2bbc01eba250283314fdbd53d05de94b"
    ):
        ...
        if len(tokenizer.added_tokens_decoder) > 0:
            with open_dict(model.cfg.tokenizer):
                ...
                model.cfg.tokenizer.special_tokens["bos_token"] = (
                    tokenizer.bos_token or "<s>"
                )
                model.cfg.tokenizer.special_tokens["eos_token"] = (
                    tokenizer.eos_token or "</s>"
                )
                model.cfg.tokenizer.special_tokens["pad_token"] = (
                    tokenizer.pad_token or "<pad>"
                )
