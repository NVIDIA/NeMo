...
def load_config(args, chatglm_config):
    ...
    if "multi_query_attention" in chatglm_config:
        if (
            chatglm_config["multi_query_attention"]
            and "multi_query_group_num" in chatglm_config
        ):
...
def convert(args):
    ...
    for l in range(int(num_layers)):
        ...
        hf_qkv_weights = model.state_dict()[
        ...
        # MLP
        mlp_down_weight = model.state_dict()[
        ...
        mlp_up_weight = model.state_dict()[
        ...
        input_ln_weight = model.state_dict()[
            f"transformer.encoder.layers.{l}.input_layernorm.weight"
        ]
        if mcore_gpt:
            input_ln_base_name = (
