...
def verify_forward(model_path, tokenizer_path, model_string):
    ...
    if model_string == "hf":
        ...
        dummy_trainer = Trainer(
            devices=1, accelerator="auto", strategy=NLPDDPStrategy()
        )
        model_config = MegatronGPTModel.restore_from(
            model_path, trainer=dummy_trainer, return_config=True
        )
        ...
        masks_and_position_ids = [
            get_ltor_masks_and_position_ids(
                id_tensor, tokenizer.eos_token, False, False, False
            )
    ...
    logging.info(
        f"{model_string} predicted next token is: '{tokenizer.convert_ids_to_tokens([next_token])}'."
    )
    logging.info(f"=" * 100)
