...
def load_config(sc2_config, tokenizer_path):
    nemo_config = OmegaConf.load(
        os.path.join(
            ...
            "../../examples/nlp/language_modeling/conf/megatron_starcoder2_config.yaml",
        )
    ).model

    nemo_config.encoder_seq_length = (
        sc2_config["sliding_window"]
        if sc2_config.get("sliding_window")
        else sc2_config["max_position_embeddings"]
    )
...
def convert(args):
    ...
    nemo_config = load_config(
        model_args, os.path.join(args.input_name_or_path, "tokenizer.model")
    )
    ...
    for l in range(int(num_layers)):
        ...
        q_bias = ckpt[f"model.layers.{l}.self_attn.q_proj.bias"].view(
            *new_q_bias_tensor_shape
        )
        k_bias = ckpt[f"model.layers.{l}.self_attn.k_proj.bias"].view(
            *new_kv_bias_tensor_shape
        )
        v_bias = ckpt[f"model.layers.{l}.self_attn.v_proj.bias"].view(
            *new_kv_bias_tensor_shape
        )
        ...
        checkpoint["state_dict"][mlp_down_base_name_weight] = param_to_weights(
            mlp_cfc_weight
        )
        checkpoint["state_dict"][mlp_down_base_name_bias] = param_to_weights(
            mlp_cfc_bias
        )
        ...
        if mcore_gpt:
            ...
            raise Exception("not implemented")
        checkpoint["state_dict"][mlp_up_base_name_weight] = param_to_weights(
            mlp_up_weight
        )
        ...
        checkpoint["state_dict"][post_attn_ln_base_name] = param_to_weights(
            post_attn_ln_weight
        )
        checkpoint["state_dict"][post_attn_ln_bias_name] = param_to_weights(
            post_attn_ln_bias
        )
    ...
    if nemo_config.get("megatron_amp_O2", False):
        ...
        for key in keys:
            checkpoint["state_dict"][key.replace("model.", "model.module.", 1)] = (
