    ...
    for old_key, val in state_dict.items():
        ...
        if 'linear_qkv.weight' in new_key:
            new_dict[new_key] = (
                val.view(
                    3,
                    override_model_dict['num_attention_heads'],
                    val.shape[0] // 3 // override_model_dict['num_attention_heads'],
                    val.shape[1],
                )
                .transpose(0, 1)
                .reshape(val.shape[0], val.shape[1])
            )
        else:
...
def get_args():
    ...
    parser.add_argument("--name_or_path", type=str, default="mosaicml/mpt-7b")
    parser.add_argument(
        "--hparams_file",
        type=str,
    ...
    parser.add_argument(
        "--save_path", type=str, default='mpt_7b_mcore.nemo', required=False, help="Path to output .nemo file."
    )
    parser.add_argument(
...
def main():
    ...
    hf_model = AutoModelForCausalLM.from_pretrained(
        args.name_or_path, torch_dtype=torch.bfloat16, trust_remote_code=True
    )
