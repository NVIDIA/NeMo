...
def get_args():
    ...
    parser.add_argument("--output_path", type=str, default=None,
                        required=True, help="Path to HF .bin file")
    parser.add_argument(
        "--hf_input_path",
        type=str,
        default=None,
...
def convert(input_nemo_file, output_hf_file, precision=None, cpu_only=False) -> None:
    ...
    for l in range(int(num_layers)):
        ...
        qkv_weights = qkv_weights.reshape(
            [qkv_total_dim, head_size, hidden_size])

        q_slice = torch.cat(
            [
                torch.arange((heads_per_group + 2) * i,
        ...
        v_slice = torch.arange(heads_per_group + 1,
                               qkv_total_dim, (heads_per_group + 2))
        # Example of slices
        ...
        checkpoint[mlp_down_proj_base_name] = param_to_weights(
            mlp_down_proj_weight)
        checkpoint[mlp_gate_proj_base_name] = param_to_weights(
        ...
        checkpoint[mlp_up_proj_base_name] = param_to_weights(
            mlp_up_proj_weight)
        ...
        checkpoint[post_attn_ln_base_name] = param_to_weights(
            post_attn_ln_weight)

        print(f"done layer {l}")

    final_ln_weight = model.state_dict(
    )[f'model.decoder.final_layernorm.weight']
...
if __name__ == '__main__':
    ...
    dtype = convert(args.input_name_or_path, args.output_path,
                    precision=args.precision, cpu_only=args.cpu_only)
