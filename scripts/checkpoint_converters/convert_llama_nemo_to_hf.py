...
from transformers import AutoModelForCausalLM, AutoTokenizer, LlamaTokenizer, LlamaTokenizerFast, convert_slow_tokenizer
...
def replace_hf_weights_and_tokenizer(
    ...
    if tokenizer_path:
        if os.path.exists(f'{tokenizer_path}/tokenizer.model'):
            ...
            fast_tokenizer = AutoTokenizer.from_pretrained(tokenizer_path, local_files_only=True,)
            tokenizer_length = len(fast_tokenizer)
            model.resize_token_embeddings(tokenizer_length)
