...
def get_args():
    ...
    parser.add_argument(
        ...
        default=False,
        required=False,
        help="Whether to have the postprocessing modules",
    )
    parser.add_argument(
        "--precision",
        type=str,
...
def convert(args):
    ...
    nemo_config.model = adjust_nemo_config(
        nemo_config.model, hf_model.config.to_dict(), mcore_bert=args.mcore
    )
    ...
    for l in range(num_layers):
        ...
        old_tensor_shape = hf_model.state_dict()[
            f"encoder.layer.{l}.attention.self.query.weight"
        ].size()
        new_q_tensor_shape = (head_num, head_size) + old_tensor_shape[1:]
        new_q_tensor_shape_bias = (head_num, head_size)

        q = hf_model.state_dict()[
            f"encoder.layer.{l}.attention.self.query.weight"
        ].view(*new_q_tensor_shape)
        k = hf_model.state_dict()[f"encoder.layer.{l}.attention.self.key.weight"].view(
            *new_q_tensor_shape
        )
        v = hf_model.state_dict()[
            f"encoder.layer.{l}.attention.self.value.weight"
        ].view(*new_q_tensor_shape)
        bias_q = hf_model.state_dict()[
            f"encoder.layer.{l}.attention.self.query.bias"
        ].view(*new_q_tensor_shape_bias)
        bias_k = hf_model.state_dict()[
            f"encoder.layer.{l}.attention.self.key.bias"
        ].view(*new_q_tensor_shape_bias)
        bias_v = hf_model.state_dict()[
            f"encoder.layer.{l}.attention.self.value.bias"
        ].view(*new_q_tensor_shape_bias)
        ...
        if args.mcore:
            qkv_weights_base_name = (
                f"model.encoder.layers.{l}.self_attention.linear_qkv.weight"
            )
            qkv_biases_base_name = (
                f"model.encoder.layers.{l}.self_attention.linear_qkv.bias"
            )
        else:
        ...
        # attention dense
        dense_weight = hf_model.state_dict()[
            f"encoder.layer.{l}.attention.output.dense.weight"
        ]
        dense_bias = hf_model.state_dict()[
            f"encoder.layer.{l}.attention.output.dense.bias"
        ]
        if args.mcore:
            dense_weight_base_name = (
                f"model.encoder.layers.{l}.self_attention.linear_proj.weight"
            )
            dense_bias_base_name = (
                f"model.encoder.layers.{l}.self_attention.linear_proj.bias"
            )
        else:
            dense_weight_base_name = (
        ...
        # LayerNorm1
        LayerNorm1_weight = hf_model.state_dict()[
            f"encoder.layer.{l}.attention.output.LayerNorm.weight"
        ]
        LayerNorm1_bias = hf_model.state_dict()[
            f"encoder.layer.{l}.attention.output.LayerNorm.bias"
        ]
        if args.mcore:
            LayerNorm1_weight_base_name = (
                f"model.encoder.layers.{l}.post_att_layernorm.weight"
            )
            LayerNorm1_bias_base_name = (
                f"model.encoder.layers.{l}.post_att_layernorm.bias"
            )
        else:
            LayerNorm1_weight_base_name = (
        ...
        nemo_state_dict[LayerNorm1_weight_base_name] = param_to_weights(
            LayerNorm1_weight
        )
        nemo_state_dict[LayerNorm1_bias_base_name] = param_to_weights(LayerNorm1_bias)

        # MLP 1
        MLP1_weight = hf_model.state_dict()[
            f"encoder.layer.{l}.intermediate.dense.weight"
        ]
        ...
        # LayerNorm2
        LayerNorm2_weight = hf_model.state_dict()[
            f"encoder.layer.{l}.output.LayerNorm.weight"
        ]
        LayerNorm2_bias = hf_model.state_dict()[
            f"encoder.layer.{l}.output.LayerNorm.bias"
        ]
        if args.mcore:
            LayerNorm2_weight_base_name = (
                f"model.encoder.layers.{l}.post_mlp_layernorm.weight"
            )
            LayerNorm2_bias_base_name = (
                f"model.encoder.layers.{l}.post_mlp_layernorm.bias"
            )
        else:
        ...
        nemo_state_dict[LayerNorm2_weight_base_name] = param_to_weights(
            LayerNorm2_weight
        )
        ...
        nemo_state_dict[f"model.encoder.layers.{l}.mlp.linear_fc1._extra_state"] = (
            model.state_dict()[f"model.encoder.layers.{l}.mlp.linear_fc1._extra_state"]
        )
        nemo_state_dict[f"model.encoder.layers.{l}.mlp.linear_fc2._extra_state"] = (
            model.state_dict()[f"model.encoder.layers.{l}.mlp.linear_fc2._extra_state"]
        )

    # Non-layer dependent keys
    word_embeddings_weight = hf_model.state_dict()["embeddings.word_embeddings.weight"]
    position_embeddings_weight = hf_model.state_dict()[
        "embeddings.position_embeddings.weight"
    ]
    token_type_embeddings_weight = hf_model.state_dict()[
    ...
    if args.mcore:
        ...
        position_embeddings_weight_base_name = (
            "model.embedding.position_embeddings.weight"
        )
        token_type_embeddings_weight_base_name = (
            "model.embedding.tokentype_embeddings.weight"
        )
        ...
        word_embeddings_weight_base_name = (
    ...
    nemo_state_dict[word_embeddings_weight_base_name] = param_to_weights(
        word_embeddings_weight
    )
    nemo_state_dict[position_embeddings_weight_base_name] = param_to_weights(
        position_embeddings_weight
    )
    nemo_state_dict[token_type_embeddings_weight_base_name] = param_to_weights(
    ...
    if args.mcore:
        ...
        original_embedding = nemo_state_dict[
            "model.language_model.embedding.word_embeddings.weight"
        ]
    ...
    if model.padded_vocab_size > vocab_size:
        ...
        if args.mcore:
            ...
            nemo_state_dict["model.language_model.embedding.word_embeddings.weight"] = (
                padded_embedding
            )
