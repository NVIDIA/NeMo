...
class ASRTarredDatasetBuilder:
    ...
    def create_new_dataset(
        ...
        entries_duration = (
            f"{total_duration:.2f} / {total_duration + filtered_duration:.2f} s"
        )
        ...
        print(
            "\n"
            + tabulate(
                data, headers=header, tablefmt="grid", colalign=["center"] * len(header)
            )
        )
        if dry_run:
            return
...
def main(args):
    if args.buckets_num > 1:
        bucket_length = (args.max_duration - args.min_duration) / float(
            args.buckets_num
        )
        for i_bucket in range(args.buckets_num):
            ...
            bucket_config.target_dir = os.path.join(
                args.target_dir, f"bucket{i_bucket+1}"
            )
            print(
...
def create_tar_datasets(
    ...
    if not dry_run and (
        DALI_INDEX_SCRIPT_AVAILABLE and dali_index.INDEX_CREATOR_AVAILABLE
    ):
