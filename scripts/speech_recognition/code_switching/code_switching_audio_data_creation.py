...
parser = argparse.ArgumentParser(
    description="Create synthetic code-switching data audio data from monolingual data"
)
parser.add_argument(
    "--manifest_path",
    default=None,
    type=str,
    help="Path to CS indermediate manifest",
    required=True,
)
parser.add_argument(
...
parser.add_argument(
    ...
    default=None,
    type=str,
    help="Path to save the created manifest",
    required=True,
)
parser.add_argument(
    "--audio_normalized_amplitude",
    default=15000,
    type=int,
    help="Normalized amplitdue of audio samples",
)
parser.add_argument(
...
def create_cs_data(
    ...
    with open(manfest_save_path, "w") as outfile:
        for data in tqdm(intermediate_cs_manifest_list):
            ...
            # saving audio
            wavfile.write(
                audio_file_path, fs, np.array(combined_audio).astype(np.int16)
            )
...
def main():
    ...
    # Sanity Checks
    if (cs_intermediate_manifest_path is None) or (
        not os.path.exists(cs_intermediate_manifest_path)
    ):
        logging.error(
            "Please provide correct CS manifest (obtained from code_switching_manifest_creation.py)"
        )
        exit
    ...
    print(
        "Total number of samples in the generated dataset :", str(num_samples_combined)
    )
