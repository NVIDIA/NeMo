...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--onset_range",
        help="range of onset in list 'START,END,STEP' to be tuned on",
        type=str,
    )
    parser.add_argument(
        "--offset_range",
        help="range of offset in list 'START,END,STEP' to be tuned on",
        type=str,
    )
    parser.add_argument(
        "--pad_onset_range",
    ...
    parser.add_argument(
        "--min_duration_on_range",
        help="range of min_duration_on in list 'START,END,STEP' to be tuned on",
        type=str,
    )
    parser.add_argument(
        "--min_duration_off_range",
        help="range of min_duration_off in list 'START,END,STEP' to be tuned on",
        type=str,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--vad_pred",
        help="Directory of vad predictions or a file contains the paths of them.",
        required=True,
    )
    parser.add_argument(
        "--groundtruth_RTTM",
    ...
    try:
        ...
        if args.min_duration_on_range:
            start, stop, step = [
                float(i) for i in args.min_duration_on_range.split(",")
            ]
        ...
        if args.min_duration_off_range:
            start, stop, step = [
                float(i) for i in args.min_duration_off_range.split(",")
            ]
