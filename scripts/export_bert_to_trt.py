...
# pooled_output = self.dense(first_token_tensor), nn.Linear(config.hidden_size, config.hidden_size)
...
def bert_pooler(prefix, init_dict, network, input_tensor):
    ...
    first_token_tensor = network.add_slice(shuf.get_output(0), start=(0,0,0,0), shape=(1,1,1,hidden_size), stride=(1,1,1,1))
...
def sequence_class_output(prefix, init_dict, network, input_tensor, softmax=True):
    ...
    out_shape_tensor = network.add_gather(in_shape_tensor, network.add_constant((5,), trt.Weights(np.array([0,1,2,2,4]).astype(np.int32))).get_output(0), 0).get_output(0)
    
    first_token_tensor = network.add_slice(shuf.get_output(0), start=(0,0,0,0,0), shape=(-1,1,1,1,hidden_size), stride=(1,1,1,1,1))
    first_token_tensor.set_input(1, network.add_constant((5,), trt.Weights(np.array([0,0,0,0,0]).astype(np.int32))).get_output(0))
...
def main(bert_weight_path, class_weight_path, B, S, config_path, outputbase, min_batch=None, max_batch=None, seq_class_prefix=None, tok_class_prefix=None):
...
#    import pdb;pdb.set_trace()
    with trt.Builder(TRT_LOGGER) as builder:
        ...
        with builder.create_network(explicit_batch_flag) as network, builder.create_builder_config() as builder_config:
            ...
            # Specify only a single profile for now, even though this is less optimal
...
if __name__ == "__main__":
    ...
    parser.add_argument('-b', '--batch-size', type=int, required=False, default=1, help='Preferred batch size (default = 1)')
    parser.add_argument('--max-batch-size', type=int, required=False, default=None, help='Maximum batch size (default = same as batch-size)')
    parser.add_argument('--min-batch-size', type=int, required=False, default=None, help='Minimum batch size (default = same as batch-size)')

    parser.add_argument('-l', '--seq-length', type=int, required=False, default=128, help='Sequence length of the BERT model (default=128)')
    ...
    main(opt.bert_weight, opt.class_weight, opt.batch_size, opt.seq_length, config_path, 
         outputbase, min_batch=opt.min_batch_size, max_batch=opt.max_batch_size,
         tok_class_prefix=opt.token_classifier, seq_class_prefix=opt.seq_classifier)
