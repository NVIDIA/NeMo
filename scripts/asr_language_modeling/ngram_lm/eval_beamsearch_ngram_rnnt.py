...
class EvalBeamSearchNGramConfig:
    ...
    maes_expansion_gamma: List[float] = field(default_factory=lambda: [0.3])  # The maes_expansion_gamma or list of the maes_expansion_gamma for the maes decoding
    ...
    decoding_strategy: str = "maes"  # Supports only beam for now
...
def beam_search_eval(
    ...
    cfg.decoding.maes_prefix_alpha = cfg.maes_prefix_alpha[0]
    ...
    for batch_idx in it:
        ...
        #logging.warning(f"****[DEBUG]: probs_batch.shape is: {probs_batch[0].shape}") # [D, T]
        ...
        with torch.no_grad():
            ...
            #logging.warning(f"****[DEBUG]: packed_batch.shape is: {packed_batch.shape}") # [B, D, T]
            ...
            # )
            _, beams_batch = model.decoding.rnnt_decoder_predictions_tensor(
...
def main(cfg: EvalBeamSearchNGramConfig):
    ...
    valid_decoding_modes = ["greedy", "greedy_batch", "maes"]
    ...
    for batch_idx, probs in tqdm(enumerate(all_probs), desc=f"Greed search decoding...", total=len(all_probs), ncols=120):
        #preds = np.argmax(probs, axis=1)
        preds_tensor = probs.unsqueeze(0)
        preds_len = torch.tensor([preds_tensor.shape[-1]], device=preds_tensor.device)
        #logging.warning(f"****[DEBUG]: preds_tensor.shape is: {preds_tensor.shape}")
        #pred_text = asr_model._wer.decoding.ctc_decoder_predictions_tensor(preds_tensor)[0][0]
        #logging.warning(f"****[DEBUG]: preds_tensor.device is: {preds_tensor.device}")
        #logging.warning(f"****[DEBUG]: preds_len.device is: {preds_len.device}")
        #asr_model = asr_model.eval()
        #asr_model.change_decoding_strategy(None)

        pred_text = asr_model.decoding.rnnt_decoder_predictions_tensor(preds_tensor, preds_len)[0][0]
    ...
    if cfg.decoding_mode in ["greedy_batch", "beamsearch", "maes"]:
