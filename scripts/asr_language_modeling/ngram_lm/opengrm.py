...
def ngrammerge(arpa_a, alpha, arpa_b, beta, arpa_c):
    ...
    if os.path.isfile(mod_c):
        ...
        sh_args = [os.path.join(NGRAM_BIN_PATH,"ngrammerge"),
                "--alpha="+str(alpha),
                "--beta="+str(beta),
                "--normalize",
                # "--use_smoothing",
...
def arpa2mod(arpa_path, force):
    ...
    if os.path.isfile(mod_path) and not force:
        ...
        sh_args = [os.path.join(NGRAM_BIN_PATH,"ngramread"),
            "--ARPA",
            arpa_path,
            mod_path]
...
def farcompile(symbols, text_file, test_far, nemo_model_file, clean_text, do_lowercase, rm_punctuation):
    ...
    if os.path.isfile(test_far):
        ...
        sh_args = [os.path.join(NGRAM_BIN_PATH, "farcompilestrings"),
            "--generate_keys=10",
            "--fst_type=compact",
            "--symbols="+symbols,
            "--keep_symbols",
...
def perplexity(ngram_mod, test_far):
    sh_args = [os.path.join(NGRAM_BIN_PATH, "ngramperplexity"),
            "--v=1",
            ngram_mod,
            test_far,
            "--OOV_symbol=d"]
...
def make_arpa(ngram_mod, ngram_arpa, force):
    if os.path.isfile(ngram_arpa) and not force:
        ...
        sh_args = [os.path.join(NGRAM_BIN_PATH, "ngramprint"),
            "--ARPA",
            ngram_mod,
            ngram_arpa]
        return subprocess.run(sh_args, capture_output=False, text=True, stdout=sys.stdout, stderr=sys.stderr)
...
def _parse_args():
    ...
    parser.add_argument("--test_file", required=False, type=str, default=None, help="Path to test file. Count perplexity if provided.")
    parser.add_argument("--symbols", required=False, type=str, default=None, help="Path symbols file. Count perplexity if provided. Used as: symbols=earnest.syms")
