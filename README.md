...
- [5. Quick Start Guide](#5-quick-start-guide)
  ...
  * [5.12. Model Export](#512-model-export)
    ...
    + [5.12.2. T5/mT5 Export](#5122-t5-export)
...
#### 5.12.2. T5/mT5 Export
<a id="markdown-t5-export" name="t5-export"></a>

T5/mT5 models are evaluated with `lambada` task which results can be compared with results from evaluation stage.

The configuration used for the export needs to be specified in the
`conf/config.yaml` file, specifying the `export` parameter, which specifies the
file to use for export purposes. The `export` parameter must be inclueded in `stages`
to run the training pipeline export stage. The value can be set to
`t5/export_t5` or `mt5/export_mt5`, which are found in `conf/export/t5/export_t5.yaml` and `conf/export/mt5/export_mt5.yaml`, respectively. The parameters can be modified to adapt different export and set of tests run on prepared Triton Model Repository.
For Base Command Platform, all these parameters should be overridden from the command line.
