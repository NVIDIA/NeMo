...
For each category of tasks, there exists several Data Processors to convert raw data from various sources into a common format as well as Dialogue Models that approach the task in various ways.
...
Unfortunately other datasets are currently not publicly available.
