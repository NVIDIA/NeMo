...
NeMo toolkit supports a fast context-biasing method for CTC and Transducer (RNN-T) ASR models with CTC-based Word Spotter (paper link).
...
More details about CTC-WS context-biasing can be found in the `tutorial <tutorial link>`__.

To use CTC-WS context-biasing in Nemo, you need to create a context-biasing text file which contains words/phrases to be boosted, with its transcriptions (spellings) separated by undetscore.
Multiple transcriptions can be useful for abbreviations ("gpu" -> "g p u"), compound words ("nvlink" -> "nv link"), 
or words with common mistakes in case of our ASR model ("nvidia" -> "n video").
