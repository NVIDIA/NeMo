...
You can selectively freeze any modules inside a Nemo model by specifying a freezing schedule in the config. Freezing stops any gradient updates
...
when finetuning a large pretrained model on a very small dataset.
...
for example, from step 20 - 100, or even activate freezing from some N until the end of training. Dynamic freezing is specified in training steps, not epochs.
