...
RUN _=$(/bin/bash /tmp/torchaudio_build/scripts/installers/install_torchaudio_latest.sh); INSTALL_TORCHAUDIO_CODE=$?; \
...
RUN _=$(/bin/bash /tmp/nemo/scripts/speech_recognition/k2/setup.sh); INSTALL_K2_CODE=$?; \
