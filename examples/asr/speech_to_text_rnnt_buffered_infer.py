...
Example:
    python speech_to_text_rnnt_buffered_infer.py \
        --asr_model "/home/smajumdar/PycharmProjects/nemo-eval/nemo_beta_eval/mls/pretrained/ContextNet-SPE/ContextNet-SPE-1024/ContextNet-RNNT-1024-SPE-1024-MLS.nemo" \
        --test_manifest="/home/smajumdar/PycharmProjects/nemo-eval/tmp/notebook/concat_manifest.json" \
        --output_path="." \
        --model_stride=8 \
        --stateful_decoding \
        --batch_size=32 \
        --total_buffer_in_secs=22.0 \
...
from nemo.collections.asr.parts.utils.streaming_utils import TransducerFrameBatchASR
from nemo.utils import logging

can_gpu = torch.cuda.is_available()


def get_wer_feat(mfst, asr, frame_len, delay, mid_delay, preprocessor_cfg, model_stride_in_secs, device):
    ...
    with open(mfst, "r") as mfst_f:
        for l in mfst_f:
            ...
            asr.read_audio_file(row['audio_filepath'], mid_delay, model_stride_in_secs)
            hyp = asr.transcribe(None, delay)
            hyps.append(hyp)
...
def main():
    ...
    parser.add_argument("--chunk_len_in_ms", type=int, default=5000, help="Chunk length in milliseconds")
    ...
    parser.add_argument(
        '--stateful_decoding', action='store_true', help='Whether to perform stateful decoding or not.'
    )
    parser.add_argument(
        '--decode_steps_per_timestep', type=int, default=3, help='Number of RNNT decode steps per time step'
    )
    ...
    delay = math.floor((total_buffer - chunk_len) / model_stride_in_secs)
    mid_delay = math.ceil((chunk_len + (total_buffer - chunk_len)) / model_stride_in_secs)
    print(delay, mid_delay)

    frame_asr = TransducerFrameBatchASR(
        asr_model=asr_model,
        frame_len=chunk_len,
        total_buffer=args.total_buffer_in_secs,
        batch_size=args.batch_size,
        stateful_decoding=args.stateful_decoding,
        max_steps_per_timestep=args.decode_steps_per_timestep,
    )
        ...
        with open(hyp_json, "w") as out_f:
            for i, hyp in enumerate(hyps):
                record = {
                    "pred_text": hyp,
                    "text": refs[i],
                    "wer": round(word_error_rate(hypotheses=[hyp], references=[refs[i]]) * 100, 4),
                }
