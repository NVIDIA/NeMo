...
# trtexec --onnx=encoder-temp_rnnt.onnx --saveEngine=encoder.trt \
#         --verbose --profilingVerbosity=detailed \
#         --avgRuns=100 \
#         --minShapes=audio_signal:1x80x96,length:1 \
#         --optShapes=audio_signal:16x80x1024,length:16 \
#         --maxShapes=audio_signal:32x80x4096,length:32  2>&1 > encoder_fp32.perf
...
trtexec --onnx=encoder-temp_rnnt.onnx --saveEngine=encoder_fp16.trt \
        ...
        --optShapes=audio_signal:16x80x1024,length:16 \
        --maxShapes=audio_signal:32x80x4096,length:32 2>&1 > encoder_fp16.perf

# decoder
# trtexec --onnx=decoder_joint-temp_rnnt.onnx --saveEngine=decoder.trt \
#         --verbose --profilingVerbosity=detailed \
#         --avgRuns=100 \
...
#         --maxShapes=encoder_outputs:32x512x1,targets:32x1,target_length:32,input_states_1:1x32x640,input_states_2:1x32x640 2>&1 > decoder_fp32.perf
...
trtexec --onnx=decoder_joint-temp_rnnt.onnx --saveEngine=decoder_fp16.trt \
        ...
        --minShapes=encoder_outputs:1x512x1,targets:1x1,target_length:1,input_states_1:1x1x640,input_states_2:1x1x640 \
        --optShapes=encoder_outputs:16x512x1,targets:16x1,target_length:16,input_states_1:1x16x640,input_states_2:1x16x640 \
        --maxShapes=encoder_outputs:32x512x1,targets:32x1,target_length:32,input_states_1:1x32x640,input_states_2:1x32x640 2>&1 > decoder_fp16.perf
