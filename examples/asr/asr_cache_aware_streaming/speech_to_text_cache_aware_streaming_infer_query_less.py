...
def create_cut(audio_filepath, rttm_filepath):
    wav_dur = soundfile.info(audio_filepath).duration
    wav_samples = soundfile.info(audio_filepath).frames
    custom = {
        'rttm_filepath': rttm_filepath,
    }
    cut = MonoCut(
        id=audio_filepath.split('/')[-1].replace('.wav', ''),
        start=0,
        duration=wav_dur,
        channel=0,
        supervisions=[],
...
def perform_streaming(
    asr_model, streaming_buffer, compare_vs_offline=False, debug_mode=False, pad_and_drop_preencoded=False, spk_targets=None
):
    ...
    if args.audio_file is not None:
        ...
        with open(args.manifest_file, 'r') as f:
            for line in f:
                ...
                uniq_ids.append(item['audio_filepath'].split('/')[-1].replace('.wav', ''))
                samples.append(item)
