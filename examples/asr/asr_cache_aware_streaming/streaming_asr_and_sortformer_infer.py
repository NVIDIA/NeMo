...
def get_frame_and_words(asr_model, step_num, chunk_audio, chunk_lengths, pred_out_stream, previous_hypotheses, word_and_ts_seq):
    ...
    for token_group, word in zip(new_token_group, new_words):
        ...
        word_and_ts_seq["word_ts"].append([word, [frame_stt, frame_end]])
...
def perform_streaming(
    ...
    word_and_ts_seq = {"word_ts": [], "token_frame_index": [], "offset_count": 0}
    for step_num, (chunk_audio, chunk_lengths) in enumerate(streaming_buffer_iter):
        with torch.inference_mode():
            with autocast:
                ...
                word_and_ts_seq = get_frame_and_words(asr_model, step_num, chunk_audio, chunk_lengths, pred_out_stream, previous_hypotheses, word_and_ts_seq)
        if debug_mode:
