...
def load_ssl_encoder(model, cfg):
    if cfg.get("init_from_ptl_ckpt", None) is not None:
        state_dict = torch.load(cfg.init_from_ptl_ckpt, map_location="cpu")[
            "state_dict"
        ]
        logging.info(
            f"Loading encoder from PyTorch Lightning checkpoint: {cfg.init_from_ptl_ckpt}"
        )
    elif cfg.get("init_from_nemo_model", None) is not None:
        ssl_model = SpeechEncDecSelfSupervisedModel.restore_from(
            cfg.init_from_nemo_model, map_location="cpu"
        )
        ...
        ssl_model = SpeechEncDecSelfSupervisedModel.from_pretrained(
            cfg.init_from_pretrained_model, map_location="cpu"
        )
        state_dict = ssl_model.state_dict()
        logging.info(
            f"Loading encoder from pretrained model: {cfg.init_from_pretrained_model}"
        )
    else:
        logging.info(
            "No model checkpoint or pretrained model specified for encoder initialization."
        )
        return model
...
@hydra_runner(
    config_path="../conf/ssl/nest/multi_layer_feat", config_name="nest_ecapa_tdnn_small"
)
def main(cfg):
