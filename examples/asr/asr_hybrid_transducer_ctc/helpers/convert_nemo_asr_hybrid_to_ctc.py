...
def extract_model_ctc(args, hybrid_model):
    ...
    ctc_model = ctc_class.restore_from(
        ...
        override_config_path=new_cfg_oc,
        strict=False,
    )
    ...
    assert all(
        [
            torch.allclose(
                hybrid_model.ctc_decoder.state_dict()[x],
                ctc_model.decoder.state_dict()[x],
            )
    ...
    assert isinstance(
        ctc_model, ctc_class
    ), "Extracted CTC model is of the wrong expected class!"
...
def extract_model_rnnt(args, hybrid_model):
    ...
    assert isinstance(
        rnnt_model, rnnt_class
    ), "Extracted RNNT model is of the wrong expected class!"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        required=True,
        type=str,
        help="path to Nemo Hybrid model .nemo file",
    )
    parser.add_argument(
        "-o",
        "--output",
        required=True,
        type=str,
        help="path and name of output .nemo file",
    )
    parser.add_argument(
        "-t",
        "--model_type",
    ...
    if not os.path.exists(args.input):
        logging.critical(
            f"Input file [ {args.input} ] does not exist or cannot be found. Aborting."
        )
        exit(255)
    ...
    logging.info(
        f"Converted {args.model_type.upper()} model was successfully saved to {args.output}"
    )
