            ...
            for name, module in model.named_parameters():
                if name in checkpoint["state_dict"]:
                    if module.data.shape != checkpoint["state_dict"][name].shape:
                        ...
                        module.data[
                            : checkpoint["state_dict"][name].size(0),
                            : checkpoint["state_dict"][name].size(1),
...
def load_config(args, llava_config):
    nemo_config = OmegaConf.load(
        os.path.join(os.path.dirname(__file__), "conf", args.config_file)
    ).model
    nemo_config.mm_cfg.mm_mlp_adapter_type = llava_config.get(
    ...
    if args.mm_vision_tower is not None:
        ...
        if os.path.exists(config_file):
            with open(config_file, "r") as f:
                ...
                nemo_config.mm_cfg.vision_encoder["model_type"] = (
                    vision_model_config.get("model_type", "clip")
                )
    ...
    nemo_config.data.image_aspect_ratio = llava_config.get(
    ...
    if llava_config["rope_scaling"] is not None:
        if llava_config["rope_scaling"]["type"] == "linear":
            nemo_config["seq_len_interpolation_factor"] = llava_config["rope_scaling"][
                "factor"
            ]
        else:
...
def convert(args):
    ...
    if mcore_gpt:
        mm_projection_layer_base_name = f"model.embedding.word_embeddings.adapter_layer.mm_projector_adapter.mm_projector"
    else:
        mm_projection_layer_base_name = f"model.language_model.embedding.word_embeddings.adapter_layer.mm_projector_adapter.mm_projector"
        ...
        if os.path.exists(args.mm_projector_ckpt_dir):
            ...
            if os.path.exists(ckpt_path):
                ...
                ckpt_path = os.path.join(
                    args.mm_projector_ckpt_dir, "model.safetensors"
                )
                proj_ckpt = {}
            ...
            raise FileNotFoundError(
                f"mm_projector_ckpt_dir {args.mm_projector_ckpt_dir} does not exist."
            )
