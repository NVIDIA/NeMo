...
Usage:
    ...
    a. run a training run for a base gpt nemo file:
        python megatron_gpt_adapter_tuning.py \
            model.data.train_ds.manifest_filepath=[PATH TO TRAINING JSONL FILE], \
            model.data.validation_ds.manifest_filepath=[PATH TO VALIDATION JSONL FILE]",\
...
@hydra_runner(config_path="conf", config_name="modularized_speech_gpt_config")
def main(cfg) -> None:
