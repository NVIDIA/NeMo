...
class StableDiffusionXLTRTPipeline(Serialization):
    def __init__(self, cfg):
        ...
        self.denoiser = StableDiffusionXLTRTPipeline.from_config_dict(
            cfg.denoiser_config
        ).to(self.device)
    ...
    def loadResources(
        self, image_height, image_width, batch_size, adm_in_channels, seed
    ):
        ...
        for model_name in self.modules:
            self.engine[model_name].allocate_buffers(
                shape_dict=self.get_shape_dict(
                    batch_size, image_height, image_width, adm_in_channels, model_name
                ),
                device=self.device,
            )

    def get_shape_dict(
        self, batch_size, image_height, image_width, adm_in_channels, model_name
    ):
    ...
    def run(
        ...
        prompt = (
            np.repeat(prompt, repeats=math.prod([num_samples]))
            .reshape([num_samples])
            .tolist()
        )
        negative_prompt = (
            np.repeat(negative_prompt, repeats=math.prod([num_samples]))
            .reshape([num_samples])
            .tolist()
        )
        ...
        with torch.inference_mode(), torch.autocast("cuda"), trt.Runtime(TRT_LOGGER):
            ...
            for _, v in additional_emb_dict.items():
                emb_out = additional_emb_model(torch.Tensor([v])).repeat(
                    (int(self.cfg.num_samples), 1)
                )
            ...
            for k in c:
                if not k == "crossattn":
                    c[k], uc[k] = map(
                        lambda y: y[k][: math.prod([num_samples])].to(self.device),
                        (c, uc),
                    )
            ...
            def denoiser(input, sigma, c):
                ...
                return self.denoiser(
                    self.run_denoiser_engine, input, sigma, c, **additional_model_inputs
                )
...
def main(cfg):
    ...
    base.loadResources(
        cfg.height, cfg.width, cfg.num_samples, cfg.adm_in_channels, cfg.seed
    )
    ...
    if cfg.get("orig_width", None) and cfg.get("orig_height", None):
        additional_emb_dict["original_size_as_tuple"] = (
            cfg.orig_width,
            cfg.orig_height,
        )

    if (
        cfg.get("crop_coords_top", None) is not None
        and cfg.get("crop_coords_left", None) is not None
    ):
        additional_emb_dict["crop_coords_top_left"] = (
            cfg.crop_coords_top,
            cfg.crop_coords_left,
        )
    ...
    for prompt in cfg.prompts:
        base.run(
            ...
            cfg.height,
            cfg.num_samples,
            additional_emb_dict=additional_emb_dict,
        )
