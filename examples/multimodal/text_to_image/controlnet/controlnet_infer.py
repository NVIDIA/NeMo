...
def get_control_input(
    image_path, batch_size, hint_image_size, control_image_preprocess=None
):
    ...
    with (
        ...
        for control, prompt in zip(control, prompts):
            ...
            control = get_control_input(
                control, batch_size, hint_image_size, control_image_preprocess
            ).to(torch.cuda.current_device(), dtype=autocast_dtype)
        ...
        if save_to_file:
            ...
            control_image = Image.fromarray(
                (control_image * 255).round().astype("uint8")
            )
            ...
            if output_type == "pil":
                for text_prompt, pils in zip(prompts, output):
                    for idx, image in enumerate(pils):
                        image.save(
                            os.path.join(out_path, f"{text_prompt[:50]}_{idx}.png")
                        )
            else:
