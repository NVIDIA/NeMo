...
from niva.utils.common_utils import read_config
from niva.core.factory.buffered_streaming_pipeline_builder  import BufferedStreamingPipelineBuilder
from hydra.core.global_hydra import GlobalHydra

from nemo.collections.asr.parts.utils.multispk_transcribe_utils import SpeakerTaggedASR, write_seglst

seed_everything(42)
torch.backends.cudnn.deterministic = True


def run_niva_buffered_streaming_asr_pipeline(asr_model_path, audio_filepaths, frame_len_sec=0.08):
    GlobalHydra.instance().clear()
    # Load the streaming pipeline configuration from a YAML file
    cfg = read_config(config_path='/home/taejinp/projects/NIVA_chunked_asr/niva/conf', config_name='buffered_streaming_ctc.yaml')

    cfg.asr.model_name=asr_model_path
    cfg.streaming.chunk_size=0.16
    cfg.streaming.left_padding_size=1.92
    cfg.streaming.right_padding_size=1.92
    cfg.streaming.batch_size=1024 
    
    # Build the streaming CTC pipeline
    print(f"Loading the streaming pipeline with the following configuration...")
    buffered_streaming_pipeline = BufferedStreamingPipelineBuilder.build_pipeline(cfg)

    # Run the buffered streaming pipeline to obtain transcriptions
    transcriptions = buffered_streaming_pipeline.run(audio_filepaths)

    asr_hypotheses = []
    for idx, fpath in enumerate(audio_filepaths):
        uniq_id = get_uniqname_from_filepath(fpath)
        # word_ts_list_dict[uniq_id] = []
        word_ts_list = transcriptions.words[idx]
        hyp_word_dict_list = []
        for word_ts in word_ts_list:
            word_dict = {
                "word": word_ts.text,
                "start_offset": int(word_ts.start/frame_len_sec),
                "end_offset": int(word_ts.end/frame_len_sec),
            }
            hyp_word_dict_list.append(word_dict)
        asr_hypotheses.append(hyp_word_dict_list)

    return asr_hypotheses


def export_total_seglst(cfg, word_and_ts_seq):
    """
    Export the total segmentation list (seglst) to a file.
    """
    seglst_total_out_list = []
    for session_dict in word_and_ts_seq:
        session_id = session_dict["uniq_id"]
        sentence_list = session_dict["sentences"]
        seglst_total_out_list.extend(sentence_list)
    
    write_seglst(cfg.seglst_output_path, seglst_total_out_list)
    logging.info(f"Successfully exported the segmentation list to {cfg.seglst_output_path}.")
...
class DiarizationConfig:
    ...
    include_tensor_outputs: bool = True  # If True, include tensor outputs in the model's output
...
def main(cfg: DiarizationConfig) -> Union[DiarizationConfig]:
    ...
    max_len = 0 
    for pred_tensor in diar_model_preds_total_list:
        max_len = max(max_len, pred_tensor.shape[1])
    
    new_diar_model_preds_total_list = []
    for pred_tensor in diar_model_preds_total_list:
        pad_pred_tensor = torch.nn.functional.pad(pred_tensor, (0, 0, 0, max_len - pred_tensor.shape[1]), "constant", 0)
        if not (pad_pred_tensor.shape[1] == max_len):
            import ipdb; ipdb.set_trace()
        new_diar_model_preds_total_list.append(pad_pred_tensor)
    
    uniq_id_list = [uniq_id for uniq_id, _ in infer_audio_rttm_dict.items()] 
    diar_model.include_tensor_outputs = True
    outputs = torch.vstack(new_diar_model_preds_total_list)
    diar_model._diarize_audio_rttm_map = infer_audio_rttm_dict
    cfg.postprocessing_params = postprocessing_cfg
    spk_timestamps, pred_tensors = diar_model._diarize_output_processing(outputs=outputs, uniq_ids=uniq_id_list, diarcfg=cfg)
    
    # Get the audio file paths from the `infer_audio_rttm_dict`
    audio_filepaths = [ infer_audio_rttm_dict[uniq_id]["audio_filepath"] for uniq_id in infer_audio_rttm_dict ]
    asr_hypotheses = run_niva_buffered_streaming_asr_pipeline(asr_model_path=cfg.asr_model_path, audio_filepaths=audio_filepaths)
    cfg.manifest_file = cfg.dataset_manifest
    multispk_asr_streamer = SpeakerTaggedASR(cfg=cfg, asr_model=None, diar_model=diar_model)
   
    speaker_transcriptions, word_and_ts_seq = multispk_asr_streamer.merge_transcript_and_speakers(
                            test_manifest_dict=diar_model._diarize_audio_rttm_map, 
                            asr_hypotheses=asr_hypotheses,
                            diar_pred_out=pred_tensors
                        )
    export_total_seglst(cfg, word_and_ts_seq)
    # break 
