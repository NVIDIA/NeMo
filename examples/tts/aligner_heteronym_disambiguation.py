...
def get_args():
    ...
    parser = argparse.ArgumentParser(
        "G2P disambiguation using Aligner input embedding distances."
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="Confidence threshold to keep a disambiguation.",
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
...
def load_and_prepare_audio(aligner, audio_path, target_sr, device):
    ...
    if orig_sr != target_sr:
        audio_data = librosa.core.resample(
...
def disambiguate_candidates(
    aligner, text, spec, spec_len, confidence, device, heteronyms, log_file=None
):
    ...
    for word in words:
        ...
        if word in heteronyms:
            ...
            for pron in aligner_g2p.phoneme_dict[word]:
                ...
                candidate = (
                    base_g2p[:word_start_idx]
                    + pron
                    + base_g2p[word_start_idx + g2p_default_len :]
                )
            ...
            with torch.no_grad():
                soft_attn, _ = aligner(
                    spec=spec_in,
                    spec_len=spec_len_in,
                    text=text_in,
                    text_len=text_len_in,
                )
            ...
            l2_dists = aligner.alignment_encoder.get_dist(
                keys=text_embeddings, queries=spec_in
            ).sqrt()

            durations = aligner.alignment_encoder.get_durations(
                soft_attn, text_len_in, spec_len_in
            ).int()
            ...
            for i in range(num_candidates):
                candidate_mean_dist = (
                    aligner.alignment_encoder.get_mean_distance_for_word(
                        l2_dists=l2_dists[i],
                        durs=durations[i],
                        start_token=word_start_idx
                        + (1 if aligner.tokenizer.pad_with_space else 0),
                        num_tokens=candidate_prons_and_lengths[i][1],
                    )
                )
                if log_file:
                    log_file.write(
                        f"{candidate_prons_and_lengths[i][0]} -- {candidate_mean_dist}\n"
                    )
            ...
            if disamb_conf < confidence:
                if log_file:
                    log_file.write(
                        f"Below confidence threshold: {best_candidate} ({disamb_conf})\n"
                    )
            ...
            if log_file:
                log_file.write(
                    f"best candidate: {best_candidate} (confidence: {disamb_conf})\n"
                )
    ...
    with open(out_path, "w") as f_out:
        with open(manifest_path, "r") as f_in:
            ...
            for line in f_in:
                ...
                text = aligner.normalizer.normalize(
                    entry["text"], punct_post_process=True
                )
