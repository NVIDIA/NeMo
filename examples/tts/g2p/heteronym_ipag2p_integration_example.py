...
def process_batch(args, batch: List[str]):
    ...
    if heteronym_model:
        ipa_g2p.setup_heteronym_model(
            heteronym_model=heteronym_model, wordid_to_phonemes_file=args.wordid_to_phonemes_file
        )
    ...
    with Pool(processes=args.num_workers) as pool:
        results = pool.starmap(process_text, [(ipa_g2p, text) for text in batch])
...
def main():
    ...
    parser.add_argument("--phoneme_dict", required=True, help="Path to the IPA pronunciation dictionary file")
    parser.add_argument(
        "--heteronym_model_path", default=None, help="Path to the pretrained HeteronymClassificationModel .nemo file"
    )
    parser.add_argument(
    ...
    parser.add_argument("--heteronyms_file", default=None, help="Path to a file containing list of heteronyms")
    parser.add_argument(
        "--input_file", default=None, help="Path to a text file with sentences to process (one per line)"
    )
    parser.add_argument("--num_workers", type=int, default=2, help="Number of worker processes")
    parser.add_argument("--use_ambiguous_words", action="store_true", help="Don't ignore ambiguous words in G2P")
