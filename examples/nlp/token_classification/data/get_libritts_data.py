...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="dev_clean",
        type=str,
        help="List of subsets separated by comma",
    )
    parser.add_argument(
        "--data_dir",
        required=True,
        type=str,
        help="Path to dir where data will be stored",
    )
    parser.add_argument(
        "--clean",
        "-c",
    ...
    for split in splits:
        ...
        with (
            open(f"{data_dir}/{split}.txt", "w") as text_data,
            open(f"{data_dir}/audio_{split}.txt", "w") as audio_data,
        ):
            for file in tqdm(
                glob.glob(f"{data_dir}/LibriTTS/{split}*/*/*/*.wav"),
                ...
                shutil.copy(
                    file.strip(),
                    (f"{data_dir}/audio/{split}/" + file.split("/")[-1]).strip(),
                )
