...
\end{aligned}
$ --> <img style="transform: translateY(0.1em); width:900px; background: transparent;" src="https://render.githubusercontent.com/render/math?math=\color{gray}%5Cbegin%7Baligned%7D%0D%0A%5Cmathcal%7BL%7D_%7B%5Ctext%7BNLL%7D%7D(%5Ctheta)%26%3D-%5Csum_%7Bn%3D1%7D%5EN%20%5Clog%20p(%5Cbf%20y%5E%7B(n)%7D%7C%5Cbf%20x%5E%7B(n)%7D)%5C%5C%26%3D-%5Csum_%7Bn%3D1%7D%5EN%5Csum_%7Bt%3D1%7D%5ET%5Clog%20p(%5Cbf%20y%5E%7B(n)%7D_t%20%7C%20%5Cbf%20y_%7B%3Ct%7D%5E%7B(n)%7D%2Ch_%7Bt-1%7D%5E%7B(n)%7D%2C%5Ctext%7BAtt%7D(%5Ctext%7BEnc%7D(%5Cbf%20x%5E%7B(n)%7D)%2Cy%5E%7B(n)%7D_%7B%3Ct%7D%2Ch_%7Bt-1%7D%5E%7B(n)%7D)%3B%5Ctheta)%0D%0A%5Cend%7Baligned%7D">
...
# Hinton-style Word-Level Distillation for NMT [Hinton et al., 2015; Kim & Rush, 2016]
...
Suppose you have a pretrained model, say in `/nemo_models/teacher_24_6_de_en/AAYNBase.nemo`. You may train a 3x3 layer encoder-decoder DE->EN student NMT model with word-level distillation using <!-- $\alpha_{\text{NLL}}=1.0$ --> <img style="transform: translateY(0.1em); background: transparent;" src="https://render.githubusercontent.com/render/math?math=\color{gray}%5Calpha_%7B%5Ctext%7BNLL%7D%7D%3D1.0"> and <!-- $\alpha_{\text{WORD-KD}}=1.0$ --> <img style="transform: translateY(0.1em); background: transparent;" src="https://render.githubusercontent.com/render/math?math=\color{gray}%5Calpha_%7B%5Ctext%7BWORD-KD%7D%7D%3D1.0"> on WMT21 and validated on WMT13, WMT14, WMT18, WMT19, WMT20 as follows (note, in your own training using tarred training datasets is highly recommended). Label-smoothing is not recommended for training the teacher or student when performing knowledge distillation [Muller et al. 2019]. Important: your student tokenizer must exactly match your teacher tokenizer, otherwise you will get errors so it is safest to specify it directly:
...
where <!-- $\mathcal{L}_{\cos}(\bf h_s, \bf h_t)=1-\frac{\bf h_s\cdot \bf h_t}{||\bf h_s||||\bf h_t||}$ --> <img style="transform: translateY(0.1em); background: transparent;width:160px;" src="https://render.githubusercontent.com/render/math?math=\color{gray}%5Cmathcal%7BL%7D_%7B%5Ccos%7D(%5Cbf%20h_s%2C%20%5Cbf%20h_t)%3D1-%5Cfrac%7B%5Cbf%20h_s%5Ccdot%20%5Cbf%20h_t%7D%7B%7C%7C%5Cbf%20h_s%7C%7C%7C%7C%5Cbf%20h_t%7C%7C%7D"> is the cosine embedding loss between the hidden states vectors of student and teacher models. You can activate this DistilBERT-style distillation by merely making `COSINE_LOSS_WEIGHT` non-zero and setting `model.distillation.distilbert_initialization=True` if you want DistilBERT initialization. When we ran ablation studies investigating the effect of each of these losses, it was found that while the word-level KD and NLL substantially improved performance, the cosine embedding loss did not substantively improve performance. We do not recommend DistilBERT initialization for NMT, but for other applications such as masked language modeling, its effect could be different.
...
Finding the mode iscommensurately expensive so we approximate it with beam search:
...
We can use the pre-trained teacher to run beam search over the transfer dataset to get a teacher-generated "soft translation" dataset via `translate_dpp.py` on NGC. You should comment out the line `model.replace_beam_with_sampling(topk=args.topk)` if you wish to use beam search, otherwise it will default to top-k sampling. Note, while you may want use greedy-sampling or top-k sampling because it is more efficient, in our experiments, this did not perform as well as the beam search method. 

Below, I am specifying a path to the teacher-tokenized WMT21 dataset since we will use the German monolingual data as our transfer dataset (but, the transfer and training sets need not be the same):
...
Next, you need to perform tokenization and punctuation normalization with moses: 
...
Then use `create_tarred_parallel_dataset.py` to create the tarred data set of teacher-generated translations `beam_teacher_predictions_de_en_8k_tokens`:
...
Like sequence-level distillation, we need the teacher-generated dataset for (noisy) pseudo-labels. Then we augment the training dataset with this teacher-generated data by sampling examples from datasets with different probabilities (which is the same as example weighting in the limit that we train for a long time). This added noise from the pseudo-labels helps SGD optimization. We can also view this as a form of regularization and, in the case of NMT, there are many ways to translate a source sentence so this added noise gives us a signal that there are many valid translations, thereby reducing the likelihood of the model overfitting. We run sequence-level interpolation on the WMT21 dataset and teacher-generated translations of WMT21 as follows: 
...
2. Then we combine the teacher-generated translations with the (labeled) parallel dataset using a mixed sampling, as before.
...
# Low Data Regime
Hinton et. al proposed that "soft targets allow the student to generalize well from only 3% of the training set". Using only 5% of ground labels and pseudo labels (with a 1:2 mixture) at a temperature of 1 with hybrid distillation, we saw similar performance to training on the entire transfer dataset and teacher-generated dataset. This leads us to propose the heuristic illustrated below, whereby you can use a large (unlabeled) monolingual corpus to get pseudo-labels from the teacher. Then you can concatenate this with a small (labeled) parallel dataset to obtain a "noisy" parallel dataset. We can train the student on this augmented dataset with word-level (or sequence-level NLL, since they are equivalent) as well as word-level distillation using the teacher model.

![low_data](https://user-images.githubusercontent.com/35356586/131950223-a224f112-50d7-44df-87a6-47615801a70b.png)

# Future Work

We plan to continue exploring semi-supervised distillation with little available labeled data and a large amount of unlabeled data.​ Many ideas from Well-Read Students Learn Better [Chang et al., 2019], such as pre-training students, and Big Self-Supervised Models are Strong Semi-Supervised Learners [Chen et al., 2020]​ are promising in using unlabeled data to train the student model. In particular, the teacher model generates the soft-label (alt. pseudo-label) target distribution on the unlabeled data, which can drastically increase generalizability of the student model. Other research like Self-training with Noisy Student [Xie et al., 2019] points to a potentially promising avenue whereby noise is added to the student, through e.g. RandAugment or stochastic depth, which makes it more difficult to learn from the pseudo-labels.

# References

1. Distilling the Knowledge in a Neural Network [Hinton et al. 2015]​

2. Sequence-Level Knowledge Distillation [Kim et al., 2016]​

3. DistilBERT, a distilled version of BERT [Chaumond et al., 2019]​

4. Self-training with Noisy Student improves ImageNet Classification [Xie et al., 2019] ​

5. Well-Read Students Learn Better [Chang et al., 2020]​

6. Achieving Human Parity on Automatic Chinese to English News Translation [Hassan et al., 2018]​

7. Big Self-Supervised Models are Strong Semi-Supervised Learners [Chen et al., 2020]​

8. When Does Label Smoothing Help? [Muller et al., 2019]​
