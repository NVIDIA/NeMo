...
def main():
    ...
    for model_path in args.model.split(','):
        ...
        model = nemo_nlp_models.machine_translation.MTEncDecModel.restore_from(restore_path=model_path).eval()
        models.append(model)
    ...
    if args.lm_model is not None:
        lm_model = nemo_nlp_models.language_modeling.TransformerLMModel.restore_from(restore_path=args.lm_model).eval()
    else:
        lm_model = None
...
#                 max_delta_length=args.max_delta_length,
#             )
            model.beam_search = GreedySequenceGenerator(
                embedding=model.decoder.embedding,
                decoder=model.decoder.decoder,
                log_softmax=model.log_softmax,
                bos=model.decoder_tokenizer.bos_id,
