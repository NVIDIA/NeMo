...
@hydra_runner(config_path="conf", config_name="aayn_base_megatron_retrieval")
def main(cfg) -> None:
    ...
    # TODO : Does it even make sense to use pretrained model for retrieval training? Maybe it does make sense but need to modify
    # ! Maybe need to modify the config for retrieval model to have a pretrained model
    if hasattr(cfg.model, 'pretrained_model_path') and cfg.model.pretrained_model_path is not None:
        ...
        with open_dict(pretrained_cfg):
            ...
            # ? What even is this? Do I need to change?
        ...
        model = MegatronNMTRetrievalModel.restore_from(
        ...
        model = MegatronNMTRetrievalModel(cfg.model, trainer)
