...
def main(cfg) -> None:
    ...
    if hasattr(cfg.model, 'pretrained_model_path') and cfg.model.pretrained_model_path is not None:
        ...
        with open_dict(pretrained_cfg):
            ...
            ## TODO: quick fix for now -> using tokenizer paths from the cfg
            # Set tokenizer paths:
            # pretrained_cfg.encoder_tokenizer = pretrained_cfg.tokenizer
            pretrained_cfg.encoder_tokenizer = cfg.model.encoder_tokenizer
            pretrained_cfg.decoder_tokenizer = cfg.model.decoder_tokenizer
        ...
        logging.error('Adapter tuning needs a pre-trained model!')
    ...
    print("Decoder adapter(s) :", model.enc_dec_model.decoder.get_enabled_adapters())
    print("Encoder adapter(s) :", model.enc_dec_model.encoder.get_enabled_adapters())

    # freeze the model and unfreeze the adapters
    model.freeze()
