...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--tar_file_prefix", type=str, default="parallel", help="Prefix for tar files"
    )
    parser.add_argument(
        "--src_fname", type=str, required=True, help="Path to the source file"
    )
    parser.add_argument(
        "--tgt_fname", type=str, required=True, help="Path to the target file"
    )
    parser.add_argument(
        "--out_dir",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="For use with pretrained encoders",
    )
    parser.add_argument(
        "--decoder_model_name",
        type=str,
        default=None,
        help="For use with pretrained decoders (not yet supported)",
    )
    parser.add_argument(
        "--encoder_tokenizer_model",
        type=str,
        default="None",
        help="Path to pre-trained encoder tokenizer model",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=32000,
        help="Encoder Vocab size after BPE",
    )
    parser.add_argument(
        "--encoder_tokenizer_coverage",
        type=float,
        default=0.999,
        help="Encoder Character coverage for BPE",
    )
    parser.add_argument(
        "--encoder_tokenizer_bpe_dropout",
        type=float,
        default=0.1,
        help="Encoder BPE dropout prob",
    )
    parser.add_argument(
        "--encoder_tokenizer_r2l",
        action="store_true",
        help="Whether to return encoded sequence from right to left",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        default="None",
        help="Path to pre-trained decoder tokenizer model",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--coverage", type=float, default=0.999, help="BPE character coverage [0-1]"
    )
    parser.add_argument(
    ...
    if (
        ...
        if args.shared_tokenizer:
            ...
            raise ValueError(
                "Both encoder and decoder pre-trained tokenizer models must be specified"
            )
    ...
    if (
        ...
        encoder_tokenizer_model, decoder_tokenizer_model = (
        ...
        encoder_tokenizer_model, decoder_tokenizer_model = (
            args.encoder_tokenizer_model,
            args.decoder_tokenizer_model,
        )
