...
def compute_tp_splits(
    ...
    fast_swiglu_activation = 'fast-swiglu' in str(model_cfg.get('activation', '')).lower()
    ...
    if param.shape == partitions[0][idx].shape:
        ...
        if 'query_key_value.weight' in param_name and megatron_legacy:
            ...
            for i in range(tp_size):
                ...
                split.append(tp_qkv)
        elif 'dense_h_to_4h.weight' in param_name and fast_swiglu_activation:
            # For Megatron GPT model with Fast Swiglu activation
...
def compute_tp_merge(idx, name, param, partitions_pp, model_cfg):
    ...
    # Logic for Fast Swiglu activation
    if 'dense_h_to_4h.weight' in name and fast_swiglu_activation:
        # concat all the first halves ('W's) and all the second halves ('V's)
        wk_splits = []
