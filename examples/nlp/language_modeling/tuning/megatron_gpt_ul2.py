...
def _modify_config(gpt_cfg, cfg, add_cfg_to_tree=False):
    ...
    with open_dict(gpt_cfg):
        # Setup megatron AMP 02
        amp_O2 = cfg.model.get('megatron_amp_O2', None)
        if amp_O2 is None:
            amp_O2 = gpt_cfg.get('megatron_amp_O2', False)

        if 'megatron_amp_O2' not in gpt_cfg:
            gpt_cfg.megatron_amp_O2 = amp_O2

        if gpt_cfg.megatron_amp_O2 != amp_O2:
            raise ValueError(
                f"Base model megatron_amp_o2 ({gpt_cfg.megatron_amp_O2}) is not equal to finetuning config "
        ...
        gpt_cfg.tokenizer.expand_tokens_dataset_type = "ul2"
        # Update Dataset Config
        gpt_cfg.data = cfg.model.data
        # Update Optimizer + Scheduler Config
        ...
        # Model Dropout
        if cfg.model.get('hidden_dropout', None) is None:
            cfg.model.hidden_dropout = gpt_cfg.get('hidden_dropout', 0.0)
        gpt_cfg.hidden_dropout = cfg.model.get('hidden_dropout', 0.0)

        if cfg.model.get('attention_dropout', None) is None:
            cfg.model.attention_dropout = gpt_cfg.get('attention_dropout', 0.0)
        gpt_cfg.attention_dropout = cfg.model.get('attention_dropout', 0.0)

        if cfg.model.get('ffn_dropout', None) is None:
            cfg.model.ffn_dropout = gpt_cfg.get('ffn_dropout', 0.0)
...
@hydra_runner(config_path="conf/", config_name="megatron_gpt_ul2")
def main(cfg) -> None:
    ...
    if cfg.model.restore_from_path:
        ...
        model = load_from_checkpoint_dir(MegatronUGPTModel, cfg, trainer, modify_confg_fn=_modify_config)
