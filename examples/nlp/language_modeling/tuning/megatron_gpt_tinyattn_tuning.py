...
from nemo.collections.nlp.models.language_modeling.megatron_gpt_adapter_model import MegatronGPTTinyAttnAdapterModel
...
@hydra_runner(config_path="conf", config_name="megatron_gpt_tinyattn_tuning_config")
def main(cfg) -> None:
    ...
    if cfg.model.get("restore_path", None):
        model = MegatronGPTTinyAttnAdapterModel.restore_from(
            cfg.model.restore_path, cfg.model, trainer=trainer, save_restore_connector=NLPSaveRestoreConnector()
        )
    else:
        model = MegatronGPTTinyAttnAdapterModel(cfg.model, trainer=trainer)
