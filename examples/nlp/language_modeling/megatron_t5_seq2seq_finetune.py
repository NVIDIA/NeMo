        ...
        if hasattr(t5_cfg, "encoder") and hasattr(t5_cfg, "decoder"):
            ...
            if hasattr(cfg.model, "encoder"):
                if hasattr(cfg.model.encoder, "position_embedding_type"):
                    t5_cfg.encoder.position_embedding_type = (
                        cfg.model.encoder.position_embedding_type
                    )
                if hasattr(cfg.model.encoder, "use_flash_attention"):
                    t5_cfg.encoder.use_flash_attention = (
                        cfg.model.encoder.use_flash_attention
                    )
                if hasattr(cfg.model.encoder, "attention_dropout"):
                    t5_cfg.encoder.attention_dropout = (
                        cfg.model.encoder.attention_dropout
                    )
            if hasattr(cfg.model, "decoder"):
                if hasattr(cfg.model.decoder, "position_embedding_type"):
                    t5_cfg.decoder.position_embedding_type = (
                        cfg.model.decoder.position_embedding_type
                    )
                if hasattr(cfg.model.decoder, "use_flash_attention"):
                    t5_cfg.decoder.use_flash_attention = (
                        cfg.model.decoder.use_flash_attention
                    )
                if hasattr(cfg.model.decoder, "attention_dropout"):
                    t5_cfg.decoder.attention_dropout = (
                        cfg.model.decoder.attention_dropout
                    )
        else:
...
def load_from_checkpoint_dir(cls, cfg, trainer, modify_confg_fn):
    ...
    if (
        ...
        or cfg.model.pipeline_model_parallel_size > 1
    ):
...
def validate_checkpoint_loading_args(cfg):
    if cfg.checkpoint_dir is None or not os.path.isdir(cfg.checkpoint_dir):
        raise ValueError(
            f"Checkpoint directory {cfg.checkpoint_dir} does not exist or is not a directory."
        )
    ...
    if cfg.hparams_file is None or not os.path.isfile(cfg.hparams_file):
        raise ValueError(
            f"Hparams file {cfg.hparams_file} does not exist or is not a file."
        )
    ...
    if hasattr(cfg.model.data.train_ds, "task_name"):
        if cfg.model.restore_from_path:
            ...
            model = load_from_checkpoint_dir(
                MegatronT5GLUEModel, cfg, trainer, modify_confg_fn=_modify_config
            )
        ...
        if cfg.model.restore_from_path:
            ...
            model = load_from_nemo(
                MegatronT0Model, cfg, trainer, t5_cfg, modify_confg_fn=_modify_config
            )
        else:
            validate_checkpoint_loading_args(cfg.model.pretrained_checkpoint)
            model = load_from_checkpoint_dir(
                MegatronT0Model, cfg, trainer, modify_confg_fn=_modify_config
            )
    else:
        if cfg.model.restore_from_path:
            ...
            model = load_from_nemo(
                MegatronT5SFTModel, cfg, trainer, t5_cfg, modify_confg_fn=_modify_config
            )
        else:
            validate_checkpoint_loading_args(cfg.model.pretrained_checkpoint)
            model = load_from_checkpoint_dir(
                MegatronT5SFTModel, cfg, trainer, modify_confg_fn=_modify_config
            )
