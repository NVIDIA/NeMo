...
def main():
    ...
    parser.add_argument('--out_dir', type=str, default='/datap/misc/speechllm_codecdatasets/')
    parser.add_argument('--dataset_name', type=str, default='libritts_val')
    parser.add_argument('--uniaudio_codec_resume_path', type=str, default='/datap/misc/Checkpoints/uniaudio_codec/universal_model/model.pth')
    parser.add_argument('--dac_codec_model_path', type=str, default='/datap/misc/Checkpoints/dac/weights_44khz_8kbps_0.0.1.pth')
    parser.add_argument('--codec_bw', type=float, default=1.5) # 6 for 8 codebooks, 1.5 for 3 codebooks
    parser.add_argument('--codec_model', type=str, default='encodec') # encodec, uniaudio_codec or dac
    ...
    if args.codec_model == 'encodec':
        ...
        codec_config_path = os.path.join(os.path.dirname(args.uniaudio_codec_resume_path), 'config.yaml')
        codec_config = OmegaConf.load(codec_config_path)
        codec_model = eval(codec_config.generator.name)(**codec_config.generator.config)
        codec_parameter_dict = torch.load(args.uniaudio_codec_resume_path)
        ...
        model_path = args.dac_codec_model_path
