...
def _modify_config(t5_cfg, cfg, add_cfg_to_tree=False):
    ...
    with open_dict(t5_cfg):
        ...
        if cfg.model.data.validation_ds.get("src_file_name", None) is not None:
            ...
            t5_cfg.data.validation_ds.src_file_name = (
                cfg.model.data.validation_ds.src_file_name
            )
        if cfg.model.data.validation_ds.get("tgt_file_name", None) is not None:
            ...
            t5_cfg.data.validation_ds.tgt_file_name = (
                cfg.model.data.validation_ds.tgt_file_name
            )
        ...
        if "output_file_path_prefix" in cfg.model.data.validation_ds:
            t5_cfg.data.validation_ds.output_file_path_prefix = (
                cfg.model.data.validation_ds.output_file_path_prefix
            )

        t5_cfg.data.validation_ds.micro_batch_size = (
            cfg.model.data.validation_ds.micro_batch_size
        )
        t5_cfg.data.validation_ds.global_batch_size = (
