...
parser.set_defaults(eval_datasets=["dev.small"], work_dir="outputs/test2", batch_size=8)
...
if args.data_for_eval == "passages":
    data_layer_params["passages"] = f"{args.data_dir}/collection.tsv"
elif args.data_for_eval == "queries":
    data_layer_params["queries"] = f"{args.data_dir}/queries.{args.eval_datasets[0]}.tsv"
...
if args.local_rank == 0:
    vectors = np.stack([tensor[:, 0].detach().numpy() for tensor in evaluated_tensors[0]])
    indices = np.stack([tensor.detach().numpy() for tensor in evaluated_tensors[1]])
    np.savez(f"{args.data_for_eval}.npz",
             vectors=vectors.reshape(-1, args.d_model),
