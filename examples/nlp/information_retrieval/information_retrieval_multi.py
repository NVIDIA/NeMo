...
def create_pipeline(documents, queries, triples, batch_size):
    data_layer = nemo_nlp.nm.data_layers.information_retrieval_datalayer.BertInformationRetrievalDataLayerMulti(
        tokenizer=tokenizer,
        documents=documents,
        queries=queries,
        triples=triples,
    ...
    input_ids, input_mask, input_type_ids = data_layer()
...
train_loss = create_pipeline(documents, train_queries, train_triples, args.batch_size)

eval_queries = f"{args.data_dir}/queries.{args.eval_datasets[0]}.tsv"
#eval_triples = f"{args.data_dir}/triples.{args.eval_datasets[0]}.tsv"
eval_triples = f"{args.data_dir}/triples.dev.tiny.tsv"
eval_loss = create_pipeline(documents, eval_queries, eval_triples, args.eval_batch_size)
