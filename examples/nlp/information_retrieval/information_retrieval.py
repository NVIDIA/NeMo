...
parser.add_argument("--save_step_freq", default=10000, type=int)
...
loss_fn = nemo_nlp.nm.losses.HingeLoss(epsilon=0.2)


def create_pipeline(documents, queries, qrels, batch_size):
    data_layer = nemo_nlp.nm.data_layers.information_retrieval_datalayer.BertInformationRetrievalDataLayer(
        tokenizer=tokenizer,
        documents=documents,
        queries=queries,
        qrels=qrels,
    ...
    rel_ids, rel_mask, rel_type_ids, irrel_ids, irrel_mask, irrel_type_ids = data_layer()
    
    rel_hiddens = encoder(input_ids=rel_ids, token_type_ids=rel_type_ids, attention_mask=rel_mask)
    rel_scores = classifier(hidden_states=rel_hiddens)
    irrel_hiddens = encoder(input_ids=irrel_ids, token_type_ids=irrel_type_ids, attention_mask=irrel_mask)
    irrel_scores = classifier(hidden_states=irrel_hiddens)

    loss = loss_fn(positive=rel_scores, negative=irrel_scores)
...
train_qrels = f"{args.data_dir}/triples.{args.train_dataset}.tsv"
train_loss = create_pipeline(documents, train_queries, train_qrels, args.batch_size)

eval_queries = f"{args.data_dir}/queries.{args.eval_datasets[0]}.tsv"
#eval_qrels = f"{args.data_dir}/triples.{args.eval_datasets[0]}.tsv"
eval_qrels = f"{args.data_dir}/triples.dev.tiny.tsv"
eval_loss = create_pipeline(documents, eval_queries, eval_qrels, args.eval_batch_size)
