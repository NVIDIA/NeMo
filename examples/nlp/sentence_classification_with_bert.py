...
from nemo_nlp.text_data_utils import process_imdb, process_nlu, process_nvidia_car

# Parsing arguments
parser = argparse.ArgumentParser(description='Sentiment analysis_with_pretrained_BERT')
...
parser.add_argument("--data_dir", default='/home/chipn/data/nvidia/car', type=str)
parser.add_argument("--dataset_name", default='nvidia-car', type=str)
...
sentence_classification_loss = neural_factory.get_module(
    name="SentenceClassificationLoss",
...
vocab_positional_map = pretrained_bert_model.local_parameters[
...
train_loss, train_logits = \
    sentence_classification_loss(hidden_states=hidden_states,
                                                        labels=labels,
...
input_ids, input_type_ids, eval_input_mask, eval_labels = eval_data_layer()
...
callback_eval = nemo.core.EvaluatorCallback(
    ...
    user_iter_callback=
    lambda x, y: eval_iter_callback(x, y, eval_data_layer),
    user_epochs_done_callback=
    lambda x: eval_epochs_done_callback(x, f'{work_dir}/graphs'),
    tensorboard_writer=tb_writer,
