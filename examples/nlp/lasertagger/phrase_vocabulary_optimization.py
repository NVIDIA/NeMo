...
flags.DEFINE_integer(
    'max_input_examples',
    50000,
    'At most this many examples from the `input_file` are used for optimizing ' 'the vocabulary.',
)
flags.DEFINE_string(
    'output_file',
    None,
...
flags.DEFINE_integer('vocabulary_size', 500, 'Number of phrases to include in the vocabulary.')
flags.DEFINE_integer(
  ...
  """
    ...
    for sources, target in data_iterator:
        if num_examples >= max_input_examples:
            break
        logging.log_every_n(logging.INFO, f'{num_examples} examples processed.', 1000)
    ...
    with open(FLAGS.output_file, 'w') as writer:
        with open(statistics_file, 'w') as stats_writer:
            ...
            for i, (phrase, count) in enumerate(
                phrase_counter.most_common(FLAGS.vocabulary_size + FLAGS.num_extra_statistics)
            ):
                # Write tags.
