    ...
    stage('L2: Segmentation Tool') {
     ...
     stages {
        stage('G2P models') {
          failFast true
          parallel {
            stage('L2: G2P Conformer training, evaluation and inference') {
              steps {
            ...
            stage('L2: ByT5G2P training, evaluation and inference') {
              steps {
                sh 'cd examples/text_processing/g2p && \
           ...
           stage('L2: HeteronymClassificationModel training, evaluation and inference') {
              steps {
                sh 'cd examples/text_processing/g2p && \
                    ...
                    python heteronym_classification_inference.py \
                        manifest=eval.json \
                        pretrained_model=${OUTPUT_DIR}/checkpoints/HeteronymClassification.nemo \
