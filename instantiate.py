...
def instantiate(config: Any, *args: Any, **kwargs: Any) -> Any:
    ...
    if OmegaConf.is_dict(config):
        ...
        return instantiate_node(config, *args, partial=_partial_)
...
def instantiate_node(
    ...
    *args: Any,
    ...
    if OmegaConf.is_list(node):
        items = [instantiate_node(item) for item in node._iter_ex(resolve=True)]

        return items
    elif OmegaConf.is_dict(node):
        exclude_keys = set(item.value for item in _Keys)
        if _is_target(node):
