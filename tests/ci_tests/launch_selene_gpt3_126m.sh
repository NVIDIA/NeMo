...
srun --nodes=1 --ntasks-per-node=1 --account=${SLURM_ACCOUNT} --partition=${SLURM_PARTITION} --job-name="${SLURM_ACC}-bignlp_ci:pytest_gpt3_126m" --time-limit=20:00 --dependency=singleton --exclusive --mem=0 --overcommit --container-image=${BUILD_IMAGE_NAME_SRUN} --container-mounts=.:/workspace/bignlp-scripts sh -c "cd /workspace/bignlp-scripts; pytest tests/ci_tests"
