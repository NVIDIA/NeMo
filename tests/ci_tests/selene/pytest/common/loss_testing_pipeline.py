    ...
    def _test_loss_helper(self, loss_type, test_type):
        ...
        for i, step in enumerate(range(expected["start_step"], expected["end_step"], expected["step_interval"])):
            if test_type == TypeOfTest.APPROX:
                assert actual_loss_list[step] == pytest.approx(expected=expected_loss_list[i], rel=self.margin_loss), f"{self.job_name} : The loss at step {step} should be approximately {expected_loss_list[i]} but it is {actual_loss_list[step]}."
            else:
