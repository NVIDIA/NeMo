...
HYDRA_FULL_ERROR=1 BIGNLP_CI=1 python3 main.py \
    ...
    export.run.model_train_name=mt5_${RUN_MODEL_SIZE}_tp${TP_SIZE}_pp${PP_SIZE} \
    export.run.results_dir=${BASE_RESULTS_DIR}/${RUN_NAME} \
    export.model.checkpoint_path=${BASE_RESULTS_DIR}/convert_mt5_${RUN_MODEL_SIZE}_tp${TP_SIZE}_pp${PP_SIZE}/results/megatron_mt5.nemo \
