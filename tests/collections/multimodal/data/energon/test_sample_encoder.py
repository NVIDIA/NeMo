...
class TestBaseSampleEncoder(unittest.TestCase):
    ...
    def setUp(self):
        self.config = MultiModalSampleConfig(
            ...
            ignore_place_holder=self.ignore_place_holder,
        )
        self.encoder = BaseSampleEncoder(
    ...
    def test_compute_loss_mask(self):
        labels = torch.tensor(
            [1, self.ignore_place_holder, 2, self.ignore_place_holder]
        )
...
class TestVQASampleEncoder(unittest.TestCase):
    ...
    def test_apply_prompt_template(self):
        ...
        self.assertTrue(
            prompt.endswith("</s>"),
            msg="Prompt does not end with the expected '</s>' sequence.",
        )
    ...
    def test_encode(self):
        ...
        self.assertIsNotNone(
            encoded_sample.images, "The encoded sample should have images."
        )
        self.assertIsNotNone(
            encoded_sample.tokens, "The encoded sample should have tokens."
        )
        self.assertIsNotNone(
            encoded_sample.labels, "The encoded sample should have labels."
        )
        self.assertIsNotNone(
            encoded_sample.loss_mask, "The encoded sample should have a loss mask."
        )
    ...
    class TestInterleavedSampleEncoder(unittest.TestCase):
        ...
        def setUp(self):
            ...
            self.encoder = InterleavedSampleEncoder(
                self.tokenizer, self.image_processor, self.config
            )
        ...
        def test_tokenize_with_text_and_images(self):
            ...
            input_sample = InterleavedSample(
                __key__="sample_key", sequence=sample_sequence
            )
            tokens, images = self.encoder.tokenize(input_sample)
            text_tokens_part1 = self.tokenizer(
                sample_sequence[0], add_special_tokens=False
            ).input_ids
            text_tokens_part2 = self.tokenizer(
                sample_sequence[2], add_special_tokens=False
            ).input_ids
            expected_tokens = (
                text_tokens_part1
            ...
            self.assertEqual(
                images.shape,
                (1, 2, 3, 336, 336),
                "The concatenated image tensor shape is incorrect.",
            )

        def test_compute_labels(self):
            tokens = torch.tensor(
                [101, 200, 102, -200, 103, 104, -200, 105], dtype=torch.long
            )
            labels = self.encoder.compute_labels(tokens)
            expected_labels = torch.tensor(
                [200, 102, -200, 103, 104, -200, 105], dtype=torch.long
            )
            expected_labels[expected_labels == self.config.image_token.token_id] = (
                self.config.ignore_place_holder
            )
            self.assertTrue(
                torch.equal(labels, expected_labels),
                msg=f"Expected labels {expected_labels.tolist()}, but got {labels.tolist()}",
        ...
        def test_encode_function(self):
            ...
            text_tokens_part1 = self.tokenizer(
                "This is a test.", add_special_tokens=False
            ).input_ids
            text_tokens_part2 = self.tokenizer(
                "Here is another test.", add_special_tokens=False
            ).input_ids
...
class TestSimilarityInterleavedEncoder(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.encoder = SimilarityInterleavedEncoder(
            self.tokenizer, self.image_processor, self.config
        )
    ...
    def test_tokenize_with_interleaving_images_after_text(self):
        ...
        sample.matched_text_indices = [
            0,
            2,
        ]  # Images should be interleaved after texts at indices 0 and 2
        tokens, images = self.encoder.tokenize(sample)

        text_tokens_1 = self.tokenizer(
            "This is the first sentence.", add_special_tokens=False
        ).input_ids
        text_tokens_2 = self.tokenizer(
            "This is the second sentence.", add_special_tokens=False
        ).input_ids
        text_tokens_3 = self.tokenizer(
            "This is the third sentence.", add_special_tokens=False
        ).input_ids

        expected_tokens = (
            text_tokens_1 + [-200] + text_tokens_2 + text_tokens_3 + [-200]
        )
        self.assertTrue(
            torch.equal(tokens, torch.tensor(expected_tokens, dtype=torch.long)),
            msg=f"Expected tokens {expected_tokens}, but got {tokens.tolist()}",
    ...
    def test_tokenize_with_interleaving_images_before_text(self):
        ...
        sample.matched_text_indices = [
            0,
            2,
        ]  # Images should be interleaved before texts at indices 0 and 2
        tokens, images = self.encoder.tokenize(sample)
        text_tokens_1 = self.tokenizer(
            "This is the first sentence.", add_special_tokens=False
        ).input_ids
        ...
        expected_tokens = (
            [-200] + text_tokens_1 + text_tokens_2 + [-200] + text_tokens_3
        )
