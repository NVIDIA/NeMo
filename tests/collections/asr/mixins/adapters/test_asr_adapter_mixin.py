...
def multitask_model(test_data_dir):
    ...
    decoding = {
        "strategy": "beam",
        "beam": {"beam_size": 1, "len_pen": 0.0, "max_generation_delta": 50},
    }
...
class TestASRAdapterMixin:
    ...
    def test_conformer_constructor_mha_adapter(self, conformer_ctc_adapter):
        ...
        conformer_ctc_adapter.add_adapter(
            name="adapter_0", cfg=get_adapter_cfg(atype="relmha")
        )
    ...
    def test_squeezeformer_constructor_mha_adapter(self, squeezeformer_ctc_adapter):
        ...
        squeezeformer_ctc_adapter.add_adapter(
            name="adapter_0", cfg=get_adapter_cfg(atype="relmha")
        )
    ...
    @pytest.mark.parametrize(
        "name", ["adapter_0", "encoder:adapter_0", "decoder:adapter_0"]
    )
    ...
    def test_conformer_forward_mha(self, conformer_ctc_adapter, name):
        ...
        origial_output = conformer_ctc_adapter(
            input_signal=input_signal, input_signal_length=input_signal_length
        )[0]

        conformer_ctc_adapter.add_adapter(
            name=name, cfg=get_adapter_cfg(in_features=128, atype="mha")
        )
        new_output = conformer_ctc_adapter(
            input_signal=input_signal, input_signal_length=input_signal_length
        )[0]
    ...
    def test_squeezeformer_forward_mha(self, squeezeformer_ctc_adapter, name):
        ...
        origial_output = squeezeformer_ctc_adapter(
            input_signal=input_signal, input_signal_length=input_signal_length
        )[0]
        ...
        new_output = squeezeformer_ctc_adapter(
            input_signal=input_signal, input_signal_length=input_signal_length
        )[0]
    ...
    def test_canary_forward_mha(self, multitask_model, name, adapter_type):
        ...
        transcript = torch.randint(
            0, multitask_model.tokenizer.vocab_size, size=(2, 10)
        )
        ...
        multitask_model.add_adapter(
            name=name,
            cfg=get_adapter_cfg(in_features=128, atype=adapter_type, proj_dim=4),
        )
        ...
        with pytest.raises(ValueError):
            multitask_model.add_adapter(
                name="transf_encoder:adapter_1",
                cfg=get_adapter_cfg(in_features=128, atype="mha"),
            )

    @pytest.mark.unit
    @pytest.mark.parametrize("name", ["transf_decoder:adapter_0"])
    def test_canary_forward_mha_decoder_fails_without_support(
        self, multitask_model, name
    ):
        multitask_model.eval()
        torch.random.manual_seed(0)
        ...
        multitask_model.transf_decoder.__class__ = get_registered_adapter(
            adapter_class
        ).base_class
        ...
        with pytest.raises(AttributeError):
            adapter_type = "transf_mha" if "transf" in name else "mha"
            multitask_model.add_adapter(
                name=name, cfg=get_adapter_cfg(in_features=128, atype=adapter_type)
            )

    @pytest.mark.unit
    @pytest.mark.parametrize(
        "name1", ["adapter_0", "encoder:adapter_0", "decoder:adapter_0"]
    )
    @pytest.mark.parametrize(
        "name2", ["adapter_1", "encoder:adapter_1", "decoder:adapter_1"]
    )
    ...
    def test_constructor_pretrained(self):
        ...
        cfg = ASRModel.from_pretrained(
            "stt_en_citrinet_256", map_location="cpu", return_config=True
        )
        ...
        model = ASRModel.from_pretrained(
            "stt_en_citrinet_256", override_config_path=cfg
        )
        ...
        model.add_adapter(
            "adapter_0",
            cfg=get_adapter_cfg(in_features=cfg.encoder.jasper[0].filters, dim=5),
        )
    ...
    def test_constructor_pretrained_rnnt(self):
        # Check to/from config_dict:
        cfg = ASRModel.from_pretrained(
            "stt_en_contextnet_256", map_location="cpu", return_config=True
        )
        adapter_metadata = get_registered_adapter(cfg.encoder._target_)
        if adapter_metadata is not None:
            cfg.encoder._target_ = adapter_metadata.adapter_class_path
        model = ASRModel.from_pretrained(
            "stt_en_contextnet_256", override_config_path=cfg
        )
        ...
        model.add_adapter(
            "decoder:adapter_1",
            cfg=get_adapter_cfg(in_features=cfg.decoder.prednet.pred_hidden, dim=5),
        )
        model.add_adapter(
            "joint:adapter_2",
            cfg=get_adapter_cfg(in_features=cfg.joint.jointnet.joint_hidden, dim=5),
        )
        assert model.is_adapter_available()
    ...
    def test_asr_model_adapter_loss(self, model):
        ...
        with torch.no_grad():
            ...
            AccessMixin.update_access_cfg(
                {"save_encoder_tensors": False}, model.model_guid
            )
