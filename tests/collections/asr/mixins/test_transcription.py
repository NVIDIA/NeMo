...
def audio_files(test_data_dir):
    ...
    audio_file2 = os.path.join(
        test_data_dir, "asr", "train", "an4", "wav", "an104-mrcb-b.wav"
    )
...
class TestTranscriptionMixin:
    ...
    def test_transcribe_generator(self, dummy_model):
        ...
        generator = dummy_model.transcribe_generator(
            audio, override_config=transribe_config
        )
    ...
    @pytest.mark.unit
    def test_transcribe_return_hypothesis(
        self, test_data_dir, fast_conformer_ctc_model
    ):
        ...
        # Audio file test
        outputs = fast_conformer_ctc_model.transcribe(
            audio_file, batch_size=1, return_hypotheses=True
        )
        assert len(outputs) == 1
    ...
    @pytest.mark.unit
    def test_transcribe_return_nbest_rnnt(
        self, audio_files, fast_conformer_transducer_model
    ):
        fast_conformer_transducer_model.eval()
        audio1, audio2 = audio_files

        orig_decoding_config = copy.deepcopy(
            fast_conformer_transducer_model.cfg.decoding
        )
        ...
        outputs = fast_conformer_transducer_model.transcribe(
            [audio1, audio2], batch_size=1, timestamps=False
        )
    ...
    def test_transcribe_return_nbest_canary(self, audio_files, canary_1b_flash):
        ...
        outputs = canary_1b_flash.transcribe(
            [audio1, audio2], batch_size=1, timestamps=False
        )
    ...
    @pytest.mark.with_downloads()
    @pytest.mark.unit
    def test_timestamps_with_transcribe_hybrid(
        self, audio_files, fast_conformer_hybrid_model
    ):
        audio1, audio2 = audio_files

        output = fast_conformer_hybrid_model.transcribe(
            [audio1, audio2], timestamps=True
        )
    ...
    @pytest.mark.with_downloads()
    @pytest.mark.unit
    def test_timestamps_with_transcribe_hybrid_ctc_head(
        self, audio_files, fast_conformer_hybrid_model
    ):
        audio1, audio2 = audio_files
    ...
    @pytest.mark.with_downloads()
    @pytest.mark.unit
    def test_timestamps_with_transcribe_canary_flash(
        self, audio_files, canary_1b_flash
    ):
        audio1, audio2 = audio_files
