...
from nemo.collections.asr.models import (EncDecClassificationModel,
                                         EncDecCTCModel, EncDecRNNTModel,
...
NUMBA_RNNT_LOSS_AVAILABLE = numba_utils.numba_cuda_is_supported(
...
class TestExportable:
    ...
    @pytest.mark.run_only_on("GPU")
    @pytest.mark.unit
    def test_EncDecClassificationModel_export_to_onnx(
        self, speech_classification_model
    ):
    ...
    def test_ConformerModel_export_to_onnx(self, conformer_model):
        ...
        with tempfile.TemporaryDirectory() as tmpdir, torch.cuda.amp.autocast():
            ...
            input_example_length = torch.full(
                size=(input_example.shape[0],), fill_value=777, device=device
            )
            model.export(
                output=filename,
    ...
    def test_EncDecCitrinetModel_limited_SE_export_to_onnx(self, citrinet_model):
        ...
        asr_module_utils.change_conv_asr_se_context_window(
            model, context_window=24, update_config=False
        )
    ...
    def test_EncDecRNNTModel_export_to_onnx(self, citrinet_rnnt_model):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert len(onnx_model.graph.input) == (
                1 + (len(input_examples) - 1) + num_states
            )
            ...
            assert (
                len(onnx_model.graph.output) == (len(input_examples) - 1) + num_states
            )
            assert onnx_model.graph.output[0].name == "outputs"
    ...
    def test_EncDecRNNTModel_export_to_ts(self, citrinet_rnnt_model):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            files, descr = model.export(
                output=filename, verbose=False, check_trace=True
            )
            ...
            ts_decoder_joint_args = ts_decoder_joint.forward.schema.arguments[
                1:
            ]  # First value is self
            ...
            # enc_logits + (all decoder inputs - state tuple) + flattened state list
            assert len(ts_decoder_joint_args) == (
                1 + (len(input_examples) - 1) + num_states
            )
    ...
    def test_EncDecCTCModel_adapted_export_to_onnx(self):
        ...
        model_config.encoder.cls = (
            model_config.encoder.cls + "Adapter"
        ...
        adapter_cfg = OmegaConf.structured(
            LinearAdapterConfig(
                in_features=model_config.encoder.params.jasper[0].filters, dim=32
            )
        )
