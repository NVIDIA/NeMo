...
def n_gpu_lm(test_data_dir):
    ...
    vocab_size = 1024
    return NGramGPULanguageModel.from_arpa(
        kenlm_model_path, vocab_size=vocab_size, normalize_unk=False
    )
...
def kenlm_wrapper(test_data_dir):
    ...
    vocab_size = 1024
    return KenLMBatchedWrapper.from_file(
        lm_path=kenlm_model_path, vocab_size=vocab_size
    )
...
class TestNGramGPULanguageModel:
    ...
    def test_initial_states(
        ...
        init_states_kenlm = kenlm_wrapper.get_init_states(
            batch_size=batch_size, bos=bos
        )
    ...
    def test_triton_vs_pytorch_random_states(
        self, n_gpu_lm: NGramGPULanguageModel, batch_size=2, num_iterations=100
    ):
    ...
    def test_sentences(
        ...
        # non-batched
        for sentence in sentences:
            scores_ref = kenlm_wrapper.score_sentences([sentence], bos=bos, eos=eos).to(
                device
            )
            scores_lm = n_gpu_lm(
            ...
            assert torch.allclose(
                scores_ref, scores_lm
            ), "Non-batched scores do not match"

        # batched
        scores_ref = kenlm_wrapper.score_sentences(sentences, bos=bos, eos=eos).to(
            device
        )
        scores_lm = n_gpu_lm(
            labels=pad_sequence(
                [torch.LongTensor(sentence) for sentence in sentences], batch_first=True
            ).to(device),
            labels_lengths=torch.LongTensor(
    ...
    def test_save_load_nemo(self, tmp_path, test_data_dir):
        ...
        n_gpu_lm = NGramGPULanguageModel.from_arpa(
            kenlm_model_path, vocab_size=vocab_size, normalize_unk=False
        )
        nemo_path = tmp_path / "ngram_lm.nemo"
        n_gpu_lm.save_to(f"{nemo_path}")
        n_gpu_lm_loaded = NGramGPULanguageModel.from_nemo(
            f"{nemo_path}", vocab_size=vocab_size
        )
    ...
    def test_save_load_from_file(self, tmp_path, test_data_dir):
        ...
        n_gpu_lm = NGramGPULanguageModel.from_file(
            kenlm_model_path, vocab_size=vocab_size, normalize_unk=False
        )
        nemo_path = tmp_path / "ngram_lm.nemo"
        n_gpu_lm.save_to(f"{nemo_path}")
        n_gpu_lm_loaded = NGramGPULanguageModel.from_file(
            f"{nemo_path}", vocab_size=vocab_size
        )
