...
class TestASRConfidenceBenchmark:
    ...
    def test_run_confidence_benchmark(
        ...
        model = (
            conformer_ctc_bpe_model if model_name == "ctc" else conformer_rnnt_bpe_model
        )
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            assert np.allclose(
                np.array(
                    run_confidence_benchmark(
                        model, target_level, filepaths, reference_texts, plot_dir=tmpdir
                    )[target_level]
                ),
                np.array(
                    [
                        ...
                        0.0,
                        -math.inf,
                        ECE_VALUES[(target_level, model_name)],
                        0.0,
                        0.0,
                        0.0,
                    ]
    ...
    @pytest.mark.with_downloads
    @pytest.mark.parametrize("model_name", ("ctc", "rnnt"))
    def test_deprecated_config_args(
        self, model_name, conformer_ctc_bpe_model, conformer_rnnt_bpe_model
    ):
        assert (
            ConfidenceConfig().method_cfg.alpha == 0.33
        ), "default `alpha` is supposed to be 0.33"
        model = (
            conformer_ctc_bpe_model if model_name == "ctc" else conformer_rnnt_bpe_model
        )
        assert isinstance(model, ASRModel)
        ...
        confidence_cfg = ConfidenceConfig(
            preserve_word_confidence=True, **test_args_main
        )
        model.change_decoding_strategy(
            RNNTDecodingConfig(
                fused_batch_size=-1, strategy="greedy", confidence_cfg=confidence_cfg
            )
            if model_name == "rnnt"
            else CTCDecodingConfig(confidence_cfg=confidence_cfg)
        )
        ...
        model.change_decoding_strategy(
            RNNTDecodingConfig(
                fused_batch_size=-1,
                strategy="greedy",
                greedy=GreedyBatchedRNNTInferConfig(
                    preserve_frame_confidence=True, **test_args_greedy
                ),
            )
            if model_name == "rnnt"
            else CTCDecodingConfig(
                greedy=GreedyCTCInferConfig(
                    preserve_frame_confidence=True, **test_args_greedy
                )
            )
        )
