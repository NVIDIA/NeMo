...
name2metric = {
    f.__name__: (f, ans)
    for f, ans in zip(
        (auc_roc, auc_pr, auc_nt, auc_yc, ece, nce),
        (0.833, 0.917, 0.833, 0.421, 0.232, 0.403),
    )
}
...
name2metric_all_correct = {
    f.__name__: (f, ans)
    for f, ans in zip(
        (auc_roc, auc_pr, auc_nt, auc_yc, nce), (0.5, 1.0, 0.0, 0.0, -math.inf)
    )
}
name2metric_all_incorrect = {
    f.__name__: (f, ans)
    for f, ans in zip(
        (auc_roc, auc_pr, auc_nt, auc_yc, nce), (0.5, 0.0, 1.0, 0.0, -math.inf)
    )
}
...
Y_SCORE_RANDOM = uniform.rvs(size=1000, loc=0.5, scale=0.5) - 0.5 * np.random.choice(
    2, 1000, p=[0.8, 0.2]
)
...
class TestConfidenceMetrics:
    ...
    def test_metric_auc_yc_aux(self):
        ...
        assert np.allclose(
            np.array(thresholds),
            np.array([i / n_bins for i in range(0, n_bins + 1)]),
            atol=TOL,
        )
        assert np.allclose(
            np.array(yc_curve),
            np.array([0.0, 0.5, 0.5, 0.5, 0.5, 0.5, 0.167, 0.667, 0.667, 0.333, 0.0]),
            atol=TOL,
        )
...
class TestSaveConfidencePlot:
    ...
    @pytest.mark.parametrize(
        "plot_func", (save_roc_curve, save_pr_curve, save_nt_curve)
    )
