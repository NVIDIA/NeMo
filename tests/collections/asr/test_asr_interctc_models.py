...
class TestInterCTCLoss:
    ...
    @pytest.mark.parametrize(
        "encoder_config",
        [
            jasper_encoder_config(num_layers=8),
    ...
    def test_forward(self, model_class, encoder_config, apply_at_layers, loss_weights):
        ...
        if model_class is EncDecCTCModel:
            ...
            decoding_config = {
                ...
                "greedy": {"max_symbols": 30},
            }
            loss_config = {
                "loss_name": "default",
                "warprnnt_numba_kwargs": {"fastemit_lambda": 0.001},
            }
            ...
            # loss use-case.
            if (
                encoder_config["_target_"]
                == "nemo.collections.asr.modules.SqueezeformerEncoder"
            ):
        ...
        model_config.update(
            {
                "interctc": {
                    ...
                    "apply_at_layers": apply_at_layers,
                },
                "optim": {"name": "adamw"},
            }
        )
        ...
        if len(apply_at_layers) != len(loss_weights):
            ...
            with pytest.raises(
                ValueError,
                match="Length of interctc.apply_at_layers has to match interctc.loss_weights",
            ):
                asr_model = model_class(cfg=model_config)
                asr_model.train()
                logprobs, _, _ = asr_model.forward(
                    input_signal=input_signal, input_signal_length=input_length
                )
        else:
            asr_model = model_class(cfg=model_config)
            asr_model.train()
            AccessMixin.set_access_enabled(
                access_enabled=True, guid=asr_model.model_guid
            )
            ...
            for output in captured_tensors:
                ...
                assert output[0].shape != logprobs.shape or not torch.allclose(
                    output[0], logprobs
                )
            ...
            required_metrics += [f"{prefix}wer"] + [
                f"{prefix}inter_wer_l{idx}" for idx in apply_at_layers
            ]
