...
class TestBatchedBeamHyps:
    ...
    def test_instantiate_incorrect_init_length(self, init_length: Literal[-1] | Literal[0]):
        with pytest.raises(ValueError):
...
class TestConvertToHypotheses:
    ...
    @pytest.mark.unit
    @pytest.mark.parametrize("device", DEVICES)
    def test_to_nbest_hyps_list(self, device: torch.device):
        hyps = BatchedBeamHyps(batch_size=2, beam_size=3, init_length=1, device=device, blank_index=1024)
...
class TestBatchedBeamHypsTDT:
    @pytest.mark.unit
    @pytest.mark.parametrize("device", DEVICES)
    def test_instantiate(self, device: torch.device):
        _ = BatchedBeamHypsTDT(batch_size=2, beam_size=3, init_length=4, device=device, blank_index=1024)
    ...
    def test_instantiate_incorrect_beam_size(self, beam_size: Literal[-1] | Literal[0]):
        with pytest.raises(ValueError):
            _ = BatchedBeamHypsTDT(batch_size=2, beam_size=beam_size, init_length=3, blank_index=1024)

    @pytest.mark.unit
    @pytest.mark.parametrize("init_length", [-1, 0])
    def test_instantiate_incorrect_init_length(self, init_length: Literal[-1] | Literal[0]):
        with pytest.raises(ValueError):
            _ = BatchedBeamHypsTDT(batch_size=1, beam_size=4, init_length=init_length, blank_index=1024)
