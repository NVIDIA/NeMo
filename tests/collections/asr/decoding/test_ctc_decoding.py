...
class TestCTCDecoding:
    ...
    def test_vectorized_decoding_logprobs(self, tmp_tokenizer, alignments, timestamps, preserve_frame_confidence):
        cfg = CTCBPEDecodingConfig(
            strategy='greedy',
        ...
        unvectorized_decoding = CTCBPEDecoding(decoding_cfg=cfg, tokenizer=tmp_tokenizer)

        cfg.strategy = 'greedy_vectorized'
        vectorized_decoding = CTCBPEDecoding(decoding_cfg=cfg, tokenizer=tmp_tokenizer)

        torch.manual_seed(1)
        B, T = 4, 20
        V = unvectorized_decoding.tokenizer.tokenizer.vocab_size + 1
        input_signal = torch.randn(size=(B, T, V))
        # Set the blank index to a very high probability to make sure
        # that we always handle at least a few blanks.
        input_signal[:, 0, unvectorized_decoding.tokenizer.tokenizer.vocab_size] = 1000
        input_signal[:, 1, unvectorized_decoding.tokenizer.tokenizer.vocab_size] = 1000
        length = torch.randint(low=1, high=T, size=[B])

        with torch.inference_mode():
            hyps, _ = unvectorized_decoding.ctc_decoder_predictions_tensor(
                input_signal, length, fold_consecutive=True, return_hypotheses=True
            )

            vectorized_hyps, _ = vectorized_decoding.ctc_decoder_predictions_tensor(
                input_signal, length, fold_consecutive=True, return_hypotheses=True
            )

            assert len(hyps) == len(vectorized_hyps) == B
            for hyp, vectorized_hyp in zip(hyps, vectorized_hyps):
                assert torch.abs(hyp.score - vectorized_hyp.score) <= 1e-5
                assert torch.all(hyp.y_sequence == vectorized_hyp.y_sequence)
                if timestamps:
                    assert hyp.timestep == vectorized_hyp.timestep
                if alignments:
                    assert torch.all(hyp.alignments[0] == vectorized_hyp.alignments[0])
                    assert torch.all(hyp.alignments[1] == vectorized_hyp.alignments[1])

    @pytest.mark.unit
    @pytest.mark.parametrize('timestamps', [False, True])
    def test_vectorized_decoding_labels(self, tmp_tokenizer, timestamps):
        ...
        input_labels[:, 0] = unvectorized_decoding.tokenizer.tokenizer.vocab_size
        input_labels[:, 1] = unvectorized_decoding.tokenizer.tokenizer.vocab_size
        length = torch.randint(low=1, high=T, size=[B])
        ...
        with torch.inference_mode():
            ...
            assert len(hyps) == len(vectorized_hyps) == B
            for hyp, vectorized_hyp in zip(hyps, vectorized_hyps):
                assert abs(hyp.score - vectorized_hyp.score) <= 1e-5
                assert torch.all(hyp.y_sequence == vectorized_hyp.y_sequence)
                if timestamps:
                    assert hyp.timestep == vectorized_hyp.timestep
