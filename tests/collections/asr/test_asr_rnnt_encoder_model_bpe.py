    ...
    def __init__(self, cfg: DictConfig, trainer: "Trainer" = None):
        ...
        if "inner_model" in self.cfg:
            self.register_nemo_submodule(
                ...
                model=EncDecRNNTBPEModel(self.cfg.inner_model),
            )
        else:
            ...
            self.register_nemo_submodule(
                ...
                model=ASRModel.from_pretrained(
                    "stt_en_conformer_transducer_small", map_location="cpu"
                ),
            )

        self.linear = torch.nn.Linear(
            self.inner_model.tokenizer.vocab_size + 1,
            self.inner_model.tokenizer.vocab_size + 1,
        )
...
class TestEncDecRNNTBPEModel:
    ...
    def test_predict_step(self, asr_model):
        ...
        dataset = LhotseSpeechToTextBpeDataset(
            tokenizer=asr_model.tokenizer, return_cuts=True
        )
        batch = dataset[cuts]
    ...
    def test_save_restore_artifact_spe(self, asr_model, test_data_dir):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            tokenizer_dir = os.path.join(
                test_data_dir, "asr", "tokenizers", "an4_spe_128"
            )
            asr_model.change_vocabulary(
                new_tokenizer_dir=tokenizer_dir, new_tokenizer_type="bpe"
            )
    ...
    def test_save_restore_artifact_agg(self, asr_model, test_data_dir):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            asr_model.change_vocabulary(
                new_tokenizer_dir=tcfg, new_tokenizer_type="agg"
            )
    ...
    def test_vocab_change(self, test_data_dir, asr_model):
        with tempfile.TemporaryDirectory() as tmpdir:
            old_tokenizer_dir = os.path.join(
    ...
    def test_save_restore_nested_model(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            assert os.path.getsize(fp_weights) > 50 * (
                2**20
            )  # Assert the weights are more than 50 MB
