...
def msdd_model():
    ...
    preprocessor = {'cls': 'nemo.collections.asr.modules.AudioToMelSpectrogramPreprocessor', 
                'params': {
                    "features": 80,
                    "window_size": 0.025,
                    "window_stride": 0.01,
...
class TestEncDecDiarLabelModel:
    ...
    def test_constructor(self, msdd_model):
        ...
        assert diar_model.cfg.scale_n == len(diar_model.cfg.diarizer.speaker_embeddings.parameters.window_length_in_sec)
    ...
    def test_forward_infer(self, msdd_model):
        ...
        with torch.no_grad():
            ...
            for i in range(input_signal.size(0)):
                preds, scale_weights = diar_model.forward_infer(input_signal[i:i+1], 
                                                                input_signal_length[i:i+1], 
                                                                emb_vectors[i:i+1], 
                                                                targets[i:i+1])
            ...
            # batch size 4
            preds_batch, scale_weights_batch = diar_model.forward_infer(input_signal, input_signal_length, emb_vectors, targets)
