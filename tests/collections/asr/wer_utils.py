...
class AbstractWEREncoderDecoder(ABC):
    ...
    def batch_of_text_batches_to_tensor(
        ...
        if not all(is_batch_size_equal_to_size_of_first_batch):
            raise ValueError(
                f"All batches have to have equal sizes. For example theses batches with indices "
                f"{np.non_zero(is_batch_size_equal_to_size_of_first_batch)[:3]} have sizes which are not equal to the "
                f"size of the first batch."
            )
