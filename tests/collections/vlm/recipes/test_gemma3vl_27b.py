...
from nemo.collections.vlm import Gemma3VLConfig27B, Gemma3VLModel, LoRA
from nemo.collections.vlm.recipes import gemma3vl_27b
from nemo.lightning import Trainer


class TestGemma3VL27B:
    ...
    def test_model(self, recipe_module):
        ...
        # Verify the inner configuration is a run.Config for Gemma3VLConfig27B
        assert isinstance(model_config.config, run.Config)
        assert model_config.config.__fn_or_cls__ == Gemma3VLConfig27B
    ...
    def test_finetune_recipe_default(self, recipe_module):
        ...
        # Verify strategy settings (note: for 'none' peft, tensor_model_parallel_size is updated to 8)
        strat = recipe.trainer.strategy
        assert isinstance(strat, run.Config)
        assert strat.tensor_model_parallel_size == 8
        assert strat.pipeline_model_parallel_size == 1
    ...
    def test_finetune_recipe_peft_lora(self, recipe_module):
        ...
        # For 27B model, tensor parallelism should be reduced to 4 for LoRA
