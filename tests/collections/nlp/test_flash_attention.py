...
class TestFlashAttention:
    ...
    def test_flash_self_attention(self, cfg):
        ...
        attention_mask_2d = ~(
            torch.arange(sl, device=device).unsqueeze(0) < torch.randint(1, sl, (bz,), device=device).unsqueeze(1)
        )
    ...
    def test_flash_cross_attention(self, cfg):
        ...
        attention_mask_2d_q = ~(
            torch.arange(sq, device=device).unsqueeze(0) < torch.randint(1, sq, (bz,), device=device).unsqueeze(1)
        )

        attention_mask_2d_k = ~(
            torch.arange(sk, device=device).unsqueeze(0) < torch.randint(1, sk, (bz,), device=device).unsqueeze(1)
        )
