...
class TestGPTContrastiveSFTChatDataset:
    @classmethod
    def setup_class(cls):
        pass
    ...
    def test_43B_tokenizer_mask_user(self):
        ...
        try:
            ...
            d = GPTContrastiveSFTChatDataset(temp_file, tokenizer, 4096, 1, index_mapping_dir='/tmp/', hf_dataset=True)
            for i in range(len(d)):
                result = d[i]
                input_ids = result['input_ids']
                mask = result['mask']
                text = tokenizer.ids_to_text(input_ids[(mask >= 0).bool()].tolist())
                full_expected_text = ''
    ...
    def test_mpt_tokenizer_mask_user(self):
        ...
        try:
            ...
            for i in range(len(d)):
                ...
                input_ids = result['input_ids']
                mask = result['mask']
                text = ids_to_text(tokenizer, input_ids[(mask >= 0).bool()].tolist())
                full_expected_text = ''
                for j in range(1, turn_num, 2):
                    expected_text = data_points[i]['conversations'][j]['value'] + '\n' + '<extra_id_1>'
