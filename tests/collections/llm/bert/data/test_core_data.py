...
def test_create_sft_dataset(temp_jsonl_file):
    ...
    dataset = create_sft_dataset(
        path=temp_jsonl_file, tokenizer=tokenizer, seq_length=128, data_type="train"
    )
...
def test_truncation_methods(temp_jsonl_file):
    ...
    dataset_left = BertEmbeddingDataset(
        file_path=str(temp_jsonl_file), tokenizer=tokenizer, truncation_method="left"
    )
...
def test_collate_fn_all_data_types(temp_jsonl_file):
    ...
    expected_train_items = len(batch_train) * (
        1 + 1 + 2
    )  # query + pos_doc + 2 neg_docs
