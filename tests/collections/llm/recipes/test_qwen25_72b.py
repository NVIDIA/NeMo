...
from nemo.collections.llm.recipes import qwen25_72b
from nemo.lightning import Trainer


class TestQwen25_72B:
    ...
    def test_pretrain_recipe(self, recipe_module):
        ...
        assert recipe.trainer.strategy.pipeline_model_parallel_size == 4  # Different from 32B model
        assert recipe.trainer.strategy.pipeline_dtype == torch.bfloat16
        assert recipe.trainer.num_nodes == 4  # Different from 32B model
    ...
    def test_finetune_recipe(self, recipe_module):
        recipe = recipe_module.finetune_recipe(num_nodes=4)  # Minimum nodes required
        assert isinstance(recipe, run.Partial)
        ...
        # Check PEFT configuration and tensor parallelism
    ...
    def test_finetune_recipe_with_dora(self, recipe_module):
        recipe = recipe_module.finetune_recipe(num_nodes=4, peft_scheme='dora')
        assert isinstance(recipe.peft, run.Config)
    ...
    def test_finetune_recipe_without_peft(self, recipe_module):
        recipe = recipe_module.finetune_recipe(num_nodes=4, peft_scheme=None)
    ...
    def test_finetune_recipe_insufficient_nodes(self, recipe_module):
        with pytest.raises(AssertionError):
            recipe_module.finetune_recipe(num_nodes=2, peft_scheme=None)
    ...
    def test_finetune_recipe_with_invalid_peft(self, recipe_module):
        with pytest.raises(ValueError, match="Unrecognized peft scheme: invalid_scheme"):
            recipe_module.finetune_recipe(num_nodes=4, peft_scheme="invalid_scheme")
