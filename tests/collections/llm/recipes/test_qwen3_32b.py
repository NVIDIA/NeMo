...
class TestQwen3_32B:
    ...
    def test_pretrain_recipe(self, recipe_module):
        ...
        # Check strategy configuration - different from smaller models
        ...
        assert recipe.data.tokenizer.pretrained_model_name == "Qwen/Qwen3-32B"

        # Check recompute configuration
        assert recipe.model.config.recompute_granularity == "full"
        assert recipe.model.config.recompute_method == "uniform"
        assert recipe.model.config.recompute_num_layers == 1
