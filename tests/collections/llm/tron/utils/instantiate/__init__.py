...
from tests.collections.llm.tron.utils.instantiate.module_shadowed_by_function import a_function
...
class UntypedPassthroughConf:
    _target_: str = "tests.collections.llm.tron.utils.instantiate.UntypedPassthroughClass"
    a: Any = MISSING
...
@dataclass
class AdamConf:
    _target_: str = "tests.collections.llm.tron.utils.instantiate.Adam"
    lr: float = 0.001
    betas: Tuple[float, ...] = (0.9, 0.999)
    eps: float = 1e-08
    weight_decay: int = 0
...
class CenterCropConf(TransformConf):
    _target_: str = "tests.collections.llm.tron.utils.instantiate.CenterCrop"
...
class RotationConf(TransformConf):
    _target_: str = "tests.collections.llm.tron.utils.instantiate.Rotation"
    degrees: int = MISSING


@dataclass
class ComposeConf:
    _target_: str = "tests.collections.llm.tron.utils.instantiate.Compose"
    _partial_: bool = False
...
@dataclass
class TreeConf:
    _target_: str = "tests.collections.llm.tron.utils.instantiate.Tree"
    _partial_: bool = False
...
@dataclass
class MappingConf:
    _target_: str = "tests.collections.llm.tron.utils.instantiate.Mapping"
    _partial_: bool = False
...
class SimpleClassPrimitiveConf:
    _target_: str = "tests.collections.llm.tron.utils.instantiate.SimpleClass"
    str = "partial"
    a: Any = None
    b: Any = None
...
class TargetWithInstantiateInInit:
    def __init__(self, user_config: Optional[DictConfig], user: Optional[User] = None) -> None:
        if user:
            ...
            user_config["_target_"] = "tests.collections.llm.tron.utils.instantiate.User"
