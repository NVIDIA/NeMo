...
# Run Mcore
PYTHONPATH=/opt/megatron-lm:/opt/NeMo:/mnt/4tb/nemo_ux_bitexact_ci_test/NeMo-Run/src \
...
--num-layers 2 --hidden-size 32 --num-attention-heads 4 --seq-length 4096 --max-position-embeddings 4096 \
...
--lr 1e-2 --min-lr 1e-4 --lr-decay-style constant --weight-decay 0 --adam-beta1 0.9 --adam-beta2 0.9 \
...
python3 pretrain_mini_mixtral.py \
 --devices=1 \
 --data-path="$DATA_PATH" \
...
# Compare outputs
python3 compare_ckpts.py "$NEMO_OUTPUT_PATH/checkpoints/--None=0.0000-epoch=0/" "$MCORE_OUTPUT_PATH/iter_0000010/"
