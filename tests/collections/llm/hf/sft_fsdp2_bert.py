...
if __name__ == '__main__':
    if PkgVersion(get_torch_version_str()) >= PkgVersion("2.4"):
        ...
        model = llm.HFAutoModelForSequenceClassification(model_name=args.model, model_accelerator=model_accelerator)
        tokenizer = model.tokenizer

        llm.api.finetune(
            model=model,
            data=llm.bert.data.IMDBHFDataModule(tokenizer=tokenizer),
            trainer=nl.Trainer(
