...
def get_latest_checkpoint(base_dir):
    ...
    pattern = re.compile(
        r"[a-z0-9_]+--reduced_train_loss=([\d\.]+)-epoch=(\d+)-step=(\d+)-last"
    )
...
def verify_sft_checkpoint_structure(path, has_io_bytes=False):
    ...
    for file in hf_weights.glob("*"):
        if file.name.startswith(model_shard_prefix) and file.name.endswith(
            ".safetensors"
        ):
    ...
    if has_sharded_files:
        ...
        expected_shards = [
            f"model-{i:05d}-of-{shard_numbers[-1]:05d}.safetensors"
            for i in range(1, shard_numbers[-1] + 1)
        ]
    ...
    assert expected_files.issubset(
        found_files
    ), f"Missing files: {expected_files - found_files}"
...
class ValidateCheckpointRestoreCallback(pl.Callback):
    ...
    def has_loaded_checkpoint(self):
        return (
            self.loaded_model_state is not None
            and self.loaded_optimizer_states is not None
        )

    def on_train_batch_start(
        self, trainer: "pl.Trainer", lightning_module, batch, batch_idx
    ) -> None:
        """
        ...
        if self.check_weights and self.loaded_model_state is not None:
            for name, current_param in lightning_module.model.named_parameters():
                ...
                if name not in self.loaded_model_state:
                    raise ValueError(
                        f"Parameter '{name}' not found in checkpoint state dict."
                    )

                # Compare current parameter to the checkpointed parameter
                checkpoint_param = self.loaded_model_state[name]
                current_param, checkpoint_param = to_cpu(current_param), to_cpu(
                    checkpoint_param
                )
                if not torch.allclose(
                    current_param, checkpoint_param, atol=self.tolerance
                ):
                    rank = os.environ.get("LOCAL_RANK", "0")
                    diff = (
                        (current_param - checkpoint_param)
                        .view(-1)
                        .float()
                        .abs()
                        .cpu()
        ...
        if self.check_optimizer and self.loaded_optimizer_states is not None:
            ...
            for optimizer, loaded_opt_state in zip(
                optimizers, self.loaded_optimizer_states
            ):
                ...
                for param_id, param_state in current_opt_state["state"].items():
                    ...
                    for state_key, current_tensor_or_val in param_state.items():
                        ...
                        if isinstance(current_tensor_or_val, torch.Tensor):
                            if not torch.allclose(
                                current_tensor_or_val.cpu(),
                                loaded_tensor_or_val.cpu(),
                                atol=1e-5,
                            ):
                                raise ValueError(
...
def main():
    ...
    model_cls = (
        ZeroInitHFAutoModelForCausalLM
        if args.auto_resume
        else llm.HFAutoModelForCausalLM
    )
    ...
    llm.api.finetune(
        ...
        data=make_squad_hf_dataset(
            llm.HFAutoModelForCausalLM.configure_tokenizer(args.model)
        ),
        trainer=trainer,
    ...
    ans = AutoModelForCausalLM.from_pretrained(
        path / "hf_weights", output_loading_info=True
    )
