...
class TestSpectrogramToMultichannelFeatures:
    ...
    def test_magnitude(
        ...
        for n in range(num_examples):
            ...
            spec, spec_len = audio2spec(
                ...
                input_length=torch.Tensor([num_samples] * batch_size),
            )
            ...
            if mag_reduction is None:
                ...
                feat_golden = np.sqrt(
                    np.mean(np.abs(spec_np) ** 2, axis=1, keepdims=True)
                )
                ...
                raise NotImplementedError(
                    f"Magnitude reduction {mag_reduction} not implemented"
                )
            ...
            if mag_normalization == "mean":
                feat_golden = feat_golden - np.mean(
                    feat_golden, axis=(1, 3), keepdims=True
                )
            elif mag_normalization == "mean_var":
                feat_golden = feat_golden - np.mean(
                    feat_golden, axis=(1, 3), keepdims=True
                )
                feat_golden = feat_golden / np.sqrt(
                    np.mean(feat_golden**2, axis=(1, 3), keepdims=True)
                )

            # Compare shape
            assert (
                feat_np.shape == feat_golden.shape
            ), f"Feature shape not matching for example {n}"

            # Compare values
            assert np.allclose(
                feat_np, feat_golden, atol=atol
            ), f"Features not matching for example {n}"
    ...
    def test_ipd(
        ...
        fft_length: int,
        num_channels: int,
        ipd_normalization: Optional[str],
        ...
        for n in range(num_examples):
            ...
            # UUT output
            feat, _ = spec2feat(
                input=spec, input_length=spec_len if use_input_length else None
            )
            feat_np = feat.cpu().detach().numpy()
            ...
            if ipd_normalization == "mean":
                ipd_golden = ipd_golden - np.mean(
                    ipd_golden, axis=(1, 3), keepdims=True
                )
                ...
                ipd_golden = ipd_golden / np.sqrt(
                    np.maximum(
                        np.mean(ipd_golden**2, axis=(1, 3), keepdims=True),
                        spec2feat.eps,
                    )
                )
            ...
            # Compare shape
            assert (
                ipd.shape == ipd_golden.shape
            ), f"Feature shape not matching for example {n}"

            # Compare values
            assert np.allclose(
                ipd, ipd_golden, atol=atol
            ), f"Features not matching for example {n}"
    ...
    def test_num_channels(self, use_ipd: bool):
        ...
        for num_channels in [1, 2, 3, 4]:
            ...
            uut = SpectrogramToMultichannelFeatures(
                num_subbands=32, num_input_channels=num_channels, use_ipd=use_ipd
            )
    ...
    def test_num_features(self, use_ipd: bool):
        ...
        for num_subbands in [5, 10]:
            uut = SpectrogramToMultichannelFeatures(
                num_subbands=num_subbands, use_ipd=use_ipd
            )
...
class TestMaskBasedProcessor:
    ...
    def test_mask_reference_channel(
        self, fft_length: int, num_channels: int, num_masks: int
    ):
            ...
            for n in range(num_examples):
                ...
                mask = _rng.uniform(
                    ...
                    size=(
                        batch_size,
                        num_masks,
                        audio2spec.num_subbands,
                        spec.shape[-1],
                    ),
                )

                # UUT output
                out, _ = mask_processor(
                    input=spec, input_length=spec_len, mask=torch.tensor(mask)
                )
                ...
                for m in range(num_masks):
                    out_golden[:, m, ...] = (
                        spec_np[:, ref_channel, ...] * mask[:, m, ...]
                    )

                # Compare shape
                assert (
                    out_np.shape == out_golden.shape
                ), f"Output shape not matching for example {n}"

                # Compare values
                assert np.allclose(
                    out_np, out_golden, atol=atol
                ), f"Output not matching for example {n}"
...
class TestMaskBasedDereverb:
    ...
    def test_wpe_convtensor(self, num_channels: int, filter_length: int, delay: int):
        ...
        for n in range(num_examples):
            ...
            # UUT
            tilde_X_uut = WPEFilter.convtensor(
                torch.tensor(X), filter_length=filter_length, delay=delay
            )
            ...
            assert np.allclose(
                tilde_X_uut, tilde_X_ref, atol=atol
            ), f"Example {n}: comparison failed"
    ...
    def test_wpe_filter(self, num_channels: int, filter_length: int, delay: int):
        ...
        wpe_filter = WPEFilter(
            filter_length=filter_length, prediction_delay=delay, diag_reg=None
        )
        ...
        for n in range(num_examples):
            X = torch.tensor(
                _rng.normal(size=input_size) + 1j * _rng.normal(size=input_size)
            )
            weight = torch.tensor(
                _rng.uniform(size=(batch_size, num_subbands, num_frames))
            )
            ...
            Q_golden = torch.matmul(
                tilde_X_golden.transpose(-1, -2).conj(),
                weight[..., None] * tilde_X_golden,
            )
            # (B, F, C * filter_length, C)
            R_golden = torch.matmul(
                tilde_X_golden.transpose(-1, -2).conj(), weight[..., None] * X_golden
            )

            # UUT
            Q_uut, R_uut = wpe_filter.estimate_correlations(
                input=X, weight=weight, tilde_input=tilde_X
            )
            # Flatten (B, F, C, filter_length, C, filter_length) into (B, F, C*filter_length, C*filter_length)
            Q_uut_flattened = Q_uut.flatten(start_dim=-2, end_dim=-1).flatten(
            ...
            assert torch.allclose(
                Q_uut_flattened, Q_golden, atol=atol
            ), f"Example {n}: comparison failed for Q"
            assert torch.allclose(
                R_uut_flattened, R_golden, atol=atol
            ), f"Example {n}: comparison failed for R"
            ...
            G_uut_flattened = G_uut.reshape(
                batch_size, num_channels, num_subbands, -1
            ).permute(0, 2, 3, 1)

            assert torch.allclose(
                G_uut_flattened, G_golden, atol=atol
            ), f"Example {n}: comparison failed for G"
    ...
    def test_mask_based_dereverb_init(
        self, num_channels: int, filter_length: int, delay: int
    ):
...
class TestMaskEstimator:
    ...
    @pytest.mark.parametrize(
        "channel_block_type",
        ["transform_average_concatenate", "transform_attend_concatenate"],
    )
    def test_flex_channels(
        self,
        channel_reduction_position: int,
        channel_reduction_type: str,
        channel_block_type: str,
    ):
    ...
    def test_gss(
        self, num_channels: int, num_subbands: int, num_outputs: int, batch_size: int
    ):
...
class TestSSLPretrainMaskingWithPatch:
    ...
    def test_masking(self, patch_size: int, mask_fraction: float, training: bool):
        ...
        # Instantiate
        uut = SSLPretrainWithMaskedPatch(
            patch_size=patch_size, mask_fraction=mask_fraction
        )
        ...
        input_length = torch.randint(
            num_frames // 2, num_frames, (batch_size,), generator=rng
        )
        for b in range(batch_size):
        ...
        for b in range(batch_size):
            ...
            est_mask_fraction = torch.sum(
                masked_spec[b, :, :, : input_length[b]].abs() == 0.0
