...
class TestDataSimulationUtils:
    ...
    def test_check_angle(self):
        ...
        assert (
            check_angle(
                "azimuth", random.uniform(low=-180, high=180, size=num_examples)
            )
            == True
        )
        assert (
            check_angle(
                "elevation", random.uniform(low=-90, high=90, size=num_examples)
            )
            == True
        )
        assert (
            check_angle("yaw", random.uniform(low=-180, high=180, size=num_examples))
            == True
        )
        assert (
            check_angle("pitch", random.uniform(low=-90, high=90, size=num_examples))
            == True
        )
        assert (
            check_angle("roll", random.uniform(low=-180, high=180, size=num_examples))
            == True
        )
    ...
    def test_convert_rir_to_mc(self, num_mics: int, num_sources: int):
        ...
        for n_source in range(num_sources):
            for n_mic in range(num_mics):
                ...
                assert np.all(
                    diff == 0.0
                ), f"Original RIR not matching: source={n_source}, channel={n_mic}"
...
class TestArrayGeometry:
    ...
    def test_array_geometry(self, mic_spacing: float, num_mics: int, axis: int):
        ...
        assert (
            np.max(np.abs(uut.centered_positions - mic_positions_centered))
            < max_abs_tol
        )
        ...
        test_cases.append(
            {
                ...
                "new_positions": np.vstack(
                    (
                        ...
                        centered_positions[:, 0],
                        centered_positions[:, 2],
                    )
                ).T,
            }
        )
        ...
        for test_case in test_cases:
            ...
            assert (
                np.max(
                    np.abs(new_array.centered_positions - test_case["new_positions"])
                )
                < max_abs_tol
            )
        ...
        # test radius
        assert (
            np.max(np.abs(uut.radius - (num_mics - 1) / 2 * mic_spacing)) < max_abs_tol
        )
        ...
        test_cases = []
        test_cases.append(
            {"center": 0, "dist": np.linalg.norm(point - 0), "azim": 0, "elev": 0}
        )
...
class TestRoomSimulation:
    ...
    def test_simulate_room_mix(self, test_data_dir):
        ...
        audio_metadata = {
            "target": [
                {"audio_filepath": "target.wav", "duration": 1.5, "offset": 0.8}
            ],
            "target_dir": data_dir,
        ...
        with tempfile.TemporaryDirectory() as output_dir:
            ...
            for suffix in [
                ...
                mix_from_parts += AudioSegment.from_file(
                    base_output_filepath + suffix
                ).samples
