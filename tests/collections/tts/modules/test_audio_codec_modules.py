...
class TestResidualVectorQuantizer:
    ...
    def test_rvq_eval(self, num_codebooks: int):
        ...
        rvq = ResidualVectorQuantizer(
            num_codebooks=num_codebooks, codebook_dim=self.codebook_dim
        )
        rvq.eval()
        ...
        for i in range(self.num_examples):
            ...
            input_len = torch.tensor(
                [self.max_len] * self.batch_size, dtype=torch.int32
            )

            # apply forward
            dequantized_fw, indices_fw, commit_loss = rvq(
                inputs=inputs, input_len=input_len
            )

            # make sure the commit loss is zero
            assert (
                commit_loss == 0.0
            ), f"example {i}: commit_loss is {commit_loss}, expected 0.0"
            ...
            torch.testing.assert_close(
                indices_enc, indices_fw, msg=f"example {i}: indices mismatch"
            )
            torch.testing.assert_close(
                dequantized_dec,
                dequantized_fw,
                msg=f"example {i}: dequantized mismatch",
            )
    ...
    def test_group_rvq_eval(self, num_groups: int, num_codebooks: int):
        ...
        if num_groups > num_codebooks:
            ...
            for i in range(self.num_examples):
                ...
                # apply forward
                dequantized_fw, indices_fw, commit_loss = grvq(
                    inputs=inputs, input_len=input_len
                )
                ...
                for g in range(num_groups):
                    dequantized, indices, _ = grvq.rvqs[g](
                        inputs=inputs_grouped[g], input_len=input_len
                    )
                    torch.testing.assert_close(
                        dequantized,
                        dequantized_fw_grouped[g],
                        msg=f"example {i}: dequantized mismatch for group {g}",
                    )
                    torch.testing.assert_close(
                        indices,
                        indices_fw_grouped[g],
                        msg=f"example {i}: indices mismatch for group {g}",
                    )
...
class TestFiniteScalarQuantizer:
    ...
    def test_fsq_eval(self, num_levels: list):
        ...
        for i in range(self.num_examples):
            ...
            assert (
                dequantized_fw.max() <= 1.0
            ), f"example {i}: dequantized_fw.max() is {dequantized_fw.max()}"
            assert (
                dequantized_fw.min() >= -1.0
            ), f"example {i}: dequantized_fw.min() is {dequantized_fw.min()}"
    ...
    def test_fsq_output(self):
        ...
        dequantized, indices = fsq(inputs=inputs, input_len=input_len)
        torch.testing.assert_close(
            dequantized, dequantized_expected, msg=f"dequantized mismatch"
        )
    ...
    def test_group_fsq_eval(self, num_groups: int, num_levels_per_group: int):
        ...
        gfsq = GroupFiniteScalarQuantizer(
            num_groups=num_groups, num_levels_per_group=num_levels_per_group
        )
        ...
        for i in range(self.num_examples):
            ...
            for g in range(num_groups):
                dequantized, indices = gfsq.fsqs[g](
                    inputs=inputs_grouped[g], input_len=input_len
                )
                torch.testing.assert_close(
                    dequantized,
