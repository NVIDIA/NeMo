...
class TestTTSFeatures:
    ...
    def test_compute_mel_spectrogram(self):
        ...
        with self._create_test_dir() as test_dir:
            spec = mel_featurizer.compute_mel_spec(
    ...
    def test_save_and_load_mel_spectrogram(self):
        mel_name = "mel_test"
        mel_featurizer = MelSpectrogramFeaturizer(
            feature_name=mel_name,
            mel_dim=self.spec_dim,
    ...
    def test_compute_pitch(self):
        pitch_featurizer = PitchFeaturizer(
            hop_length=self.hop_len, sample_rate=self.sample_rate
        )
    ...
    def test_compute_pitch_batched(self, test_data_dir):
        audio_filepath = (
            Path(test_data_dir) / "tts" / "mini_ljspeech" / "wavs" / "LJ003-0182.wav"
        )
        manifest_entry = {"audio_filepath": audio_filepath}

        # Compute pitch with batching disabled
        pitch_featurizer = PitchFeaturizer(
            hop_length=self.hop_len, sample_rate=self.sample_rate, batch_seconds=None
        )
        pitch, voiced, voiced_prob = pitch_featurizer.compute_pitch(
        ...
        pitch_batch, voiced_batch, voiced_prob_batch = (
            pitch_featurizer_batch.compute_pitch(
                manifest_entry=manifest_entry, audio_dir=test_data_dir
            )
        )
    ...
    def test_save_and_load_pitch_segments(self, test_data_dir):
        ...
        offset2 = 2.5
        duration2 = 1.2
        start1, end1 = self._compute_start_end_frames(
            offset=offset1, duration=duration1
        )
        start2, end2 = self._compute_start_end_frames(
            offset=offset2, duration=duration2
        )
        manifest_entry_segment1 = {
            "audio_filepath": audio_filepath,
            "offset": offset1,
            "duration": duration1,
        }
        manifest_entry_segment2 = {
            "audio_filepath": audio_filepath,
            "offset": offset2,
            "duration": duration2,
        }
        ...
        with self._create_test_dir() as test_dir:
            ...
            pitch_dict_segment1 = pitch_featurizer.load(
                manifest_entry=manifest_entry_segment1,
                audio_dir=test_data_dir,
                feature_dir=feature_dir,
            )
            pitch_dict_segment2 = pitch_featurizer.load(
                manifest_entry=manifest_entry_segment2,
                audio_dir=test_data_dir,
                feature_dir=feature_dir,
            )
    ...
    def test_compute_energy(self):
        ...
        with self._create_test_dir() as test_dir:
            energy = energy_featurizer.compute_energy(
                manifest_entry=self.manifest_entry, audio_dir=test_dir
            )
    ...
    def test_save_and_load_energy(self):
        ...
        energy_featurizer = EnergyFeaturizer(
            feature_name=energy_name, spec_featurizer=mel_featurizer
        )

        with self._create_test_dir() as test_dir:
            feature_dir = test_dir / "feature"
            energy_featurizer.save(
                manifest_entry=self.manifest_entry,
                audio_dir=test_dir,
                feature_dir=feature_dir,
            )
    ...
    def test_save_and_load_energy_segments(self, test_data_dir):
        ...
        manifest_entry_segment1 = {
            "audio_filepath": self.audio_filename,
            "offset": offset1,
            "duration": duration1,
        }
        manifest_entry_segment2 = {
