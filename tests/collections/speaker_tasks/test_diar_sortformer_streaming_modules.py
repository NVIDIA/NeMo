...
class TestSortformerStreamingModules_ParameterCheck:
    ...
    def test_rule1_valid_non_negative_parameters(self, spkcache_len, fifo_len, chunk_len, chunk_left_context, chunk_right_context, spkcache_refresh_rate):
    ...
    def test_rule1_invalid_negative_parameters(self, param_name, param_value):
        ...
        with pytest.raises(ValueError, match=f"All streaming parameters should not be negative. {param_name}: {param_value}"):
    ...
    def test_rule1_invalid_float_parameters(self, param_name, param_value):
        ...
        with pytest.raises(ValueError, match=f"All streaming parameters should be integers. {param_name}: {param_value}"):
            SortformerModules(**params)
    ...
    def test_rule4_invalid_spkcache_refresh_rate_out_of_range(self, fifo_len, chunk_len, spkcache_refresh_rate):
        ...
        with pytest.raises(ValueError, match=f"The effective range of self.spkcache_refresh_rate is: \\[ chunk_len: {chunk_len}, fifo_len: {fifo_len} \\], but got {spkcache_refresh_rate}"):
            SortformerModules(
                num_spks=4,
                spkcache_len=188,
                fifo_len=fifo_len,
...
class TestSortformerStreamingModules_StreamingUtils:
    ...
    def test_length_to_mask(self, batch_size, max_length, lengths):
        ...
        # Call the method
        mask = sortformer_modules.length_to_mask(lengths, max_length)
    ...
    def test_streaming_feat_loader(self, batch_size, feat_dim, feat_len, chunk_len, subsampling_factor, chunk_left_context, chunk_right_context):
        ...
        total_processed = sum(chunk['chunk_feat_seq_t'].shape[1] - chunk['left_offset'] - chunk['right_offset'] 
                            for chunk in chunks)
    ...
    def test_concat_embs(self, tensor_shapes, dim, return_lengths, device):
        ...
        # Call the method
        if return_lengths:
            embs, lengths = sortformer_modules.concat_embs(
                list_of_tensors, return_lengths=return_lengths, dim=dim, device=device
            )
        else:
            embs = sortformer_modules.concat_embs(
                list_of_tensors, return_lengths=return_lengths, dim=dim, device=device
            )
    ...
    def test_init_streaming_state(self, batch_size, async_streaming, device):
        ...
        if async_streaming:
            ...
            assert streaming_state.spkcache.shape == (batch_size, sortformer_modules.spkcache_len, sortformer_modules.fc_d_model)
            ...
            assert streaming_state.spkcache_preds.shape == (batch_size, sortformer_modules.spkcache_len, sortformer_modules.n_spk)
            ...
            assert streaming_state.fifo.shape == (batch_size, sortformer_modules.fifo_len, sortformer_modules.fc_d_model)
    ...
    def test_apply_mask_to_preds(self, batch_size, n_frames, n_spk, encoder_lengths):
        ...
        # Call the method
        masked_preds = sortformer_modules.apply_mask_to_preds(
        ...
        for i in range(batch_size):
            ...
            if valid_length > 0:
                assert torch.allclose(
                    masked_preds[i, :valid_length], 
                    original_preds[i, :valid_length]
                )
        ...
        zero_masked_preds = sortformer_modules.apply_mask_to_preds(
            spkcache_fifo_chunk_preds, 
            zero_lengths
        )
        ...
        full_masked_preds = sortformer_modules.apply_mask_to_preds(
            spkcache_fifo_chunk_preds, 
            full_lengths
        )
...
#         device_state = sortformer_modules.init_streaming_state(batch_size=batch_size, async_streaming=False, device=device)
...
class TestSortformerStreamingModules_StreamingScoreComputations:
    ...
    def test_boost_topk_scores(self, batch_size, n_frames, n_spk, n_boost_per_spk, scale_factor, offset):
        ...
        # Call the method
        boosted_scores = sortformer_modules._boost_topk_scores(
            scores, n_boost_per_spk, scale_factor, offset
        )
        ...
        boosted_no_boost = sortformer_modules._boost_topk_scores(
            scores_no_boost, 0, scale_factor, offset
        )
        ...
        boosted_all_boost = sortformer_modules._boost_topk_scores(
            scores_all_boost, n_frames, scale_factor, offset
        )
    ...
    def test_get_log_pred_scores(self, batch_size, n_frames, n_spk, pred_score_threshold):
        ...
        expected_threshold_score = (math.log(pred_score_threshold) - 
    ...
    def test_get_topk_indices(self, batch_size, n_frames, n_spk, spkcache_len, spkcache_sil_frames_per_spk, custom_minimum_spkcache_len):
    ...
    def test_gather_spkcache_and_preds(self, batch_size, n_frames, n_spk, spkcache_len):
        ...
        edge_emb, edge_preds = sortformer_modules._gather_spkcache_and_preds(
            emb_seq, preds, edge_indices, is_disabled
        )
    ...
    def test_disable_low_scores(self, batch_size, n_frames, n_spk, min_pos_scores_per_spk):
        ...
        all_speech_modified = sortformer_modules._disable_low_scores(all_speech_preds, all_speech_scores, min_pos_scores_per_spk)
        ...
        all_nonspeech_modified = sortformer_modules._disable_low_scores(all_nonspeech_preds, all_nonspeech_scores, min_pos_scores_per_spk)
    ...
    def test_compress_spkcache(self, batch_size, n_frames, n_spk, spkcache_len):
        ...
        spkcache_perm, spkcache_preds_perm, spk_perm_perm = sortformer_modules._compress_spkcache(emb_seq, preds, permute_spk=True)
        ...
        if n_frames == spkcache_len:
            ...
            edge_spkcache, edge_spkcache_preds, edge_spk_perm = sortformer_modules._compress_spkcache(edge_emb_seq, edge_preds, permute_spk=False)
