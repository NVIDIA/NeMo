...
class TestOrdinal:
    ...
    @parameterized.expand(parse_test_case_file('data_inverse_text_normalization/en/test_cases_ordinal.txt'))
    @pytest.mark.skipif(
    ...
    @parameterized.expand(parse_test_case_file('data_inverse_text_normalization/es/test_cases_ordinal.txt'))
    @pytest.mark.skipif(
        not PYNINI_AVAILABLE, reason="`pynini` not installed, please install via nemo_text_processing/setup.sh"
    )
    @pytest.mark.run_only_on('CPU')
