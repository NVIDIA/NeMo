...
class TestPreemptionCallback:
    ...
    def test_on_train_batch_start_distributed_init(self, callback, mock_trainer, initially_supported, becomes_supported):
        with patch.object(PreemptionCallback, '_check_preemption_support') as mock_check, \
    ...
    @pytest.mark.parametrize("is_supported,interrupted,expected", [
        (True, True, True),
        (True, False, False),
        (False, True, False),
        (False, False, False),
    ...
    def test_interrupted_property(self, callback, is_supported, interrupted, expected):
        with patch.object(PreemptionCallback, '_check_preemption_support', return_value=is_supported), \
    ...
    def test_on_train_start(self, callback, mock_trainer):
        with patch.object(PreemptionCallback, 'preemption_supported', new_callable=PropertyMock) as mock_supported, \
            patch.object(callback, '_preemption_handler') as mock_handler:
