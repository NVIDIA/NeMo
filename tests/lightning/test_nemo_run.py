...
@pytest.mark.parametrize("module, recipe, name", [
...
def test_recipes_with_nemo_run(module, recipe, name, tmpdir, monkeypatch):
    ...
    recipe_config = getattr(getattr(llm, module), recipe)(name=name, dir=BASE_CHECKPOINT_DIR, num_nodes=1, num_gpus_per_node=8)
    run_plugins = [plugins.PreemptionPlugin(), plugins.WandbPlugin(name=name, logger_fn=partial(wandb_logger, entity="dummy", project="dummy"))]
    validation_plugin = plugins.ConfigValidationPlugin(
        validate_wandb=True
    )
