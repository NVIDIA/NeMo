...
coverage run -a --data-file=/workspace/.coverage --source=/workspace/nemo -m torch.distributed.launch --nproc_per_node=2 --use-env tests/collections/llm/gpt/model/test_nemotron5.py \
    --num-nodes=1 \
    --devices=1 \
    --max-steps=10 \
