...
set -x
# 1D buckets [SSL, CTC]
coverage run -a --data-file=/workspace/.coverage --source=/workspace/nemo -m torch.distributed.launch --nproc_per_node=2 scripts/speech_recognition/estimate_duration_bins.py \
  /home/TestData/an4_dataset/an4_train.json \
  --buckets 5
# 2D buckets [CTC, RNNT, TDT] / with tokenizer
coverage run -a --data-file=/workspace/.coverage --source=/workspace/nemo -m torch.distributed.launch --nproc_per_node=2 scripts/speech_recognition/estimate_duration_bins_2d.py \
