...
set -x
# 1D buckets [SSL, CTC]
coverage run --concurrency=multiprocessing --branch -a --data-file=/workspace/.coverage --source=/workspace/nemo scripts/speech_recognition/estimate_duration_bins.py \
  /home/TestData/an4_dataset/an4_train.json \
  --buckets 5
# 2D buckets [CTC, RNNT, TDT] / with tokenizer
coverage run --concurrency=multiprocessing --branch -a --data-file=/workspace/.coverage --source=/workspace/nemo scripts/speech_recognition/estimate_duration_bins_2d.py \
