...
coverage run -a --data-file=/workspace/.coverage --source=/workspace/nemo -m torch.distributed.launch --nproc_per_node=1 --use-env examples/tts/g2p/g2p_heteronym_classification_train_and_evaluate.py \
...
export CUDA_VISIBLE_DEVICES=0
coverage run -a --data-file=/workspace/.coverage --source=/workspace/nemo -m torch.distributed.launch --nproc_per_node=1 --use-env examples/tts/g2p/g2p_heteronym_classification_inference.py \
