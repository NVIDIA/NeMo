...
class TestExpManager:
    ...
    def test_trainer_loggers(self, tmp_path):
        ...
        test_trainer = pl.Trainer(
            accelerator="cpu"
        )  # Should create logger and modelcheckpoint
        ...
        log_dir = exp_manager(
            test_trainer,
            {"create_tensorboard_logger": False, "create_checkpoint_callback": False},
        )
    ...
    def test_trainer_neptune_logger(self, tmp_path):
        pytest.importorskip(
            "neptune",
            reason="could not import `neptune`, use `pip install neptune` to run this test",
        )
    ...
    def test_checkpoint_configurations(self):
        ...
        ):  # Fails because both try to create modelcheckpoint
            exp_manager(test_trainer, disable_tb_logger)

        # Should succeed without error
        exp_manager(
            test_trainer,
            {"create_checkpoint_callback": False, "create_tensorboard_logger": False},
        )
    ...
    def test_default_log_dir(self):
        ...
        assert (log_dir / "..").resolve() == Path(
            "./nemo_experiments/default/"
        ).resolve()
        assert Path("./nemo_experiments").exists()
        assert Path("./nemo_experiments/default/").exists()
        sub_dirs = [
            x for x in Path("./nemo_experiments/default/").iterdir() if x.is_dir()
        ]
        assert len(sub_dirs) == 1
        assert re.match(
            r"[0-9]{4}-[0-9]{2}-[0-9]{2}_[0-9]{2}-[0-9]{2}-[0-9]{2}", sub_dirs[0].name
        )
    ...
    def test_log_dir_overrides(self, monkeypatch, tmp_path):
        ...
        log_dir = exp_manager(
            test_trainer, {"explicit_log_dir": str(tmp_path / "test_log_dir_overrides")}
        )
        ...
        log_dir = exp_manager(
            test_trainer, {"exp_dir": str(tmp_path / "test_no_name"), "version": 957}
        )
        assert (
            log_dir.resolve()
            == (tmp_path / "test_no_name" / "default" / "957").resolve()
        )
        assert Path(tmp_path).exists()
        ...
        log_dir = exp_manager(
            test_trainer,
            {"exp_dir": str(tmp_path / "test_no_name"), "use_datetime_version": False},
        )
        assert (
            log_dir.resolve()
            == (tmp_path / "test_no_name" / "default" / "version_0").resolve()
        )
        assert Path(tmp_path).exists()
        ...
        assert (
            log_dir.resolve()
            == (tmp_path / "test_no_name" / "default" / "version_1").resolve()
        )
        assert Path(tmp_path).exists()
    ...
    def test_resume(self, tmp_path):
        ...
        with pytest.raises(NotFoundError):
            exp_manager(
                test_trainer,
                {"resume_if_exists": True, "exp_dir": str(tmp_path / "test_resume")},
            )
        ...
        Path(tmp_path / "test_resume" / "default" / "version_0" / "checkpoints").mkdir(
            parents=True
        )
            ...
            exp_manager(
                test_trainer,
                {
                    ...
                    "explicit_log_dir": str(
                        tmp_path / "test_resume" / "default" / "version_0"
                    ),
                },
            )
        ...
        exp_manager(
            test_trainer, {"resume_if_exists": True, "explicit_log_dir": str(log_dir)}
        )
        checkpoint = Path(
            tmp_path
            / "test_resume"
        ...
        assert Path(test_trainer.ckpt_path).resolve() == checkpoint.resolve()
        prev_run_dir = Path(
            tmp_path / "test_resume" / "default" / "version_0" / "run_0"
        )
        ...
        dirpath_checkpoint_dir = Path(
            tmp_path / "test_resume" / "dirpath_test" / "ckpts"
        )
    ...
    def test_nemo_checkpoint_save_best_model_1(self, tmp_path):
        test_trainer = pl.Trainer(
            accelerator="cpu", enable_checkpointing=False, logger=False, max_epochs=4
        )
        exp_manager(
            test_trainer,
            {
        ...
        model = ExampleModel.restore_from(
            str(tmp_path / "test" / "checkpoints" / "default.nemo")
        )
    ...
    def test_nemo_checkpoint_save_best_model_2(self, tmp_path):
        ...
        assert (
            math.fabs(
                float(model(torch.tensor([1.0, 1.0], device=model.device))) - 0.03
            )
            < 1e-5
        )
    ...
    def test_nemo_checkpoint_restore_model(self, tmp_path):
        ...
        test_trainer = pl.Trainer(
            accelerator="cpu", enable_checkpointing=False, logger=False, max_epochs=5
        )
        exp_manager(
            test_trainer,
            {
        ...
        assert (
            math.fabs(float(model(torch.tensor([1.0, 1.0], device=model.device))) - 1.0)
            < 1e-5
        )
    ...
    def test_base_checkpoints_are_not_overwritten(self, tmp_path, test_dist_ckpt):
        ...
        all_checkpoints = [
            p.name for p in Path(str(tmp_path / "test" / "checkpoints")).glob("*")
        ]
        ...
        assert not _get_versioned_name(
            ckpt_2
        ).exists(), all_checkpoints  # ckpt2 didn't exist before
    ...
    def test_last_checkpoint_saved(self, tmp_path):
        ...
        exp_manager(
            trainer,
            {
                ...
                "checkpoint_callback_params": {
                    "filename": f"{{val_loss:.4f}}-{{epoch}}-{{step}}"
                },
            },
        )
        model = TestModel()
    ...
    def test_skipped_unfinished_checkpoints_when_restoring(self, tmp_path):
        ...
        self._write_fake_checkpoint(
            checkpoints_dir
            / "megatron_gpt--val_loss=5.01-step=900-consumed_samples=1000.0.ckpt",
            isdir=False,
            add_unfinished_marker=False,
        )  # not last
        self._write_fake_checkpoint(
            checkpoints_dir
            / "megatron_gpt--val_loss=5.01-step=900-consumed_samples=1000.0-last.ckpt",
            isdir=False,
    ...
    def test_skipped_unfinished_dist_checkpoints_when_restoring(self, tmp_path):
        ...
        self._write_fake_checkpoint(
            checkpoints_dir
            / "megatron_gpt--val_loss=5.01-step=1000-consumed_samples=16000.0",
            isdir=True,
            add_unfinished_marker=False,
        )
        self._write_fake_checkpoint(
        ...
        self._write_fake_checkpoint(
            checkpoints_dir
            / "megatron_gpt--val_loss=5.01-step=1100-consumed_samples=17600.0",
            isdir=True,
            add_unfinished_marker=False,
        )
        self._write_fake_checkpoint(
            checkpoints_dir
            / "megatron_gpt--val_loss=5.01-step=1100-consumed_samples=17600.0-last",
            isdir=True,
            add_unfinished_marker=True,
        )
    ...
    def test_incomplete_checkpoints_cleanup(self, tmp_path):
        ...
        for ckpt_filepath in complete_ckpts:
            self._write_fake_checkpoint(
                ckpt_filepath, isdir=False, add_unfinished_marker=False
            )
        ...
        for ckpt_filepath in incomplete_ckpts:
            self._write_fake_checkpoint(
                ckpt_filepath, isdir=False, add_unfinished_marker=True
            )

        # sanity check
        remaining_ckpts = {
            f for f in (test_dir / "checkpoints").rglob("*.ckpt") if f.is_file()
        }
        ...
        (
            checkpoints_dir
            / f"orphan-marker001-{NeMoModelCheckpoint.UNFINISHED_CHECKPOINT_SUFFIX}"
        ).touch()
        ...
        self._write_fake_checkpoint(
            checkpoints_dir / "incomplete01.ckpt",
            isdir=False,
            add_unfinished_marker=True,
        )
        ...
        (
            checkpoints_dir
            / f"incomplete02{NeMoModelCheckpoint.UNFINISHED_CHECKPOINT_SUFFIX}"
        ).touch()

        test_trainer = pl.Trainer(
            accelerator="cpu", enable_checkpointing=False, logger=False, max_epochs=1
        )
        ...
        remaining_markers = list(
            checkpoints_dir.rglob(
                f"*{NeMoModelCheckpoint.UNFINISHED_CHECKPOINT_SUFFIX}"
            )
        )
    ...
    def test_incomplete_dist_checkpoints_cleanup(self, tmp_path):
        ...
        for ckpt_dirpath in complete_dist_ckpts:
            self._write_fake_checkpoint(
                ckpt_dirpath, isdir=True, add_unfinished_marker=False
            )
        ...
        for ckpt_dirpath in incomplete_dist_ckpts:
            self._write_fake_checkpoint(
                ckpt_dirpath, isdir=True, add_unfinished_marker=True
            )
    ...
    _chkpt_path_and_marker_path_pairs = [
        (
            "a=1_b=1.c.d.e",
            f"a=1_b=1.c.d.e{NeMoModelCheckpoint.UNFINISHED_CHECKPOINT_SUFFIX}",
        ),
        (
            "a=1_b=1.c.d.e-last",
            f"a=1_b=1.c.d.e-last{NeMoModelCheckpoint.UNFINISHED_CHECKPOINT_SUFFIX}",
        ),
        (
            ".ckpt/a=1_b=1.c.d.e.ckpt",
            f".ckpt/a=1_b=1.c.d.e{NeMoModelCheckpoint.UNFINISHED_CHECKPOINT_SUFFIX}",
        ),
        (
            ".ckpt/a=1_b=1.c.d.e-EMA.ckpt",
            f".ckpt/a=1_b=1.c.d.e{NeMoModelCheckpoint.UNFINISHED_CHECKPOINT_SUFFIX}",
        ),
        (
        ...
        (
            "nemo/a=1_b=1.c.d.e.nemo",
            f"nemo/a=1_b=1.c.d.e{NeMoModelCheckpoint.UNFINISHED_CHECKPOINT_SUFFIX}",
        ),
        (
            "nemo/a=1_b=1.c.d.e-last.nemo",
            f"nemo/a=1_b=1.c.d.e-last{NeMoModelCheckpoint.UNFINISHED_CHECKPOINT_SUFFIX}",
        ),
    ]

    @pytest.mark.unit
    @pytest.mark.parametrize(
        "chkpt_path, expected_marker_path", _chkpt_path_and_marker_path_pairs
    )
    def test_incomplete_checkpoints_marker_path(self, chkpt_path, expected_marker_path):
        ...
        """
        marker_path = NeMoModelCheckpoint.format_checkpoint_unfinished_marker_path(
            chkpt_path
        )
    ...
    def test_invalid_checkpoints_removed_from_topk(self, tmp_path):
        ...
        test_trainer = pl.Trainer(
            accelerator="cpu", enable_checkpointing=False, logger=False, max_epochs=7
        )
        exp_manager(
            test_trainer,
            {
        ...
        ckpt_filenames = {
            f.name for f in checkpoints_dir.rglob("*.ckpt") if f.is_file()
        }
        ...
        NeMoModelCheckpoint.set_checkpoint_unfinished_marker(
            checkpoints_dir / "epoch=6.ckpt"
        )
        (checkpoints_dir / "epoch=5.ckpt").unlink()

        test_trainer2 = pl.Trainer(
            accelerator="cpu", enable_checkpointing=False, logger=False, max_epochs=9
        )
        exp_manager(
            test_trainer2,
            {
