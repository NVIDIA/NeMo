...
class RefitTester:
    def _init_nemo_inference(self, model_info, tp, pp, load_model=True):
        ...
        pretrained_cfg = MegatronGPTModel.restore_from(
            restore_path=gpt_model_file, return_config=True, save_restore_connector=save_restore_connector,
        )
    ...
    def run(self, model_name, tp_size, pp_size, run_accuracy=False):
        ...
        print(f"Path: {0} and model: {1} will be tested".format(model_info["checkpoint"], model_name))
        ...
        output, _ = trt_llm_exporter.forward(
            input_texts=model_info["prompt_template"], **self.trt_llm_inference_params
        )
        ...
        if run_accuracy:
            ...
            ) = get_accuracy_with_lambada(trt_llm_exporter, None, None)
...
if __name__ == '__main__':
    ...
    refit_runner.run(args.model_name, args.tp_size, args.pp_size, args.run_accuracy)
