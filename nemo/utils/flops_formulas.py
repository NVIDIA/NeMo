...
def flux(config: FLOPSConfig):
    ...
    joint_layer_flops = base_factor * config.num_joint_layers * (
        10 * hs * hs +                                    # hidden size operations
        2 * hs * (config.model_channels + config.context_dim) * (1 + hs * 5) +  # channel and context joint attention
        2 * (config.model_channels + config.context_dim) * hs                    # final projection
    )

    # Single layer computations
    single_layer_flops = base_factor * config.num_single_layers * seq_len * hs * (
        3 +                          # linear Y
        1 +                          # Modulation
    ...
    other_flops = base_factor * (
        ...
        config.vec_in_dim * hs + hs * hs +                               # vector embedding
