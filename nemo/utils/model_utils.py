...
@rank_zero_only
def maybe_init_from_pretrained_checkpoint(model: 'ModelPT', cfg: OmegaConf, map_location: str = 'cpu'):
    """
    ...
    Args:
        model: A fully instantiated model.
    ...
    arg_matches = [1 if arg in cfg else 0 for arg in args]
    ...
    if 'init_from_nemo_model' in cfg and cfg.init_from_nemo_model is not None:
        with open_dict(cfg):
            ...
            restored_model = model.restore_from(model_path, map_location=map_location, strict=True)
