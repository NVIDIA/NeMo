...
def exp_manager(trainer: 'lightning.pytorch.Trainer', cfg: Optional[Union[DictConfig, Dict]] = None) -> Optional[Path]:
    ...
    log_file = log_dir / \
        f'nemo_log_globalrank-{global_rank}_localrank-{local_rank}.txt'
    ...
    if cfg.log_delta_step_timing:
        timing_callback = DeltaTimingCallback(
    ...
    if cfg.create_early_stopping_callback:
        early_stop_callback = EarlyStopping(
            **cfg.early_stopping_callback_params)
    ...
    if cfg.create_straggler_detection_callback:
        if HAVE_STRAGGLER_DET:
            ...
            raise ValueError(
                "`create_straggler_detection_callback` is True, but there is no Straggler Det. "
                "package installed."
            )
...
def _filter_out_unfinished_checkpoints(checkpoint_paths: Collection[Union[Path, str]]) -> Collection[Union[Path, str]]:
    ...
    for chkpt_path in checkpoint_paths:
        if NeMoModelCheckpoint.is_checkpoint_unfinished(chkpt_path):
            logging.warning(
                f'Checkpoint {chkpt_path} has the unfinished marker set - skipped while looking '
                'for the last one.'
            )
        else:
        ...
        if is_global_rank_zero() or not is_s3_url(dirpath):
            ...
            if is_s3_url(dirpath):
                ...
                if checkpoint_dir_exists:
                    ...
                    end_checkpoints = [
                        k for k in all_keys if k.endswith('end.ckpt')]
                    last_checkpoints = [
                        k for k in all_keys if k.endswith('last.ckpt')]
                else:
                    end_checkpoints = []
                    last_checkpoints = []
            else:  # default non-s3 implementation
                # Use <log_dir>/checkpoints/ unless `dirpath` is set
                checkpoint_dir = Path(dirpath) if dirpath else Path(
                ...
                dist_checkpoints = [d for d in list(
                    checkpoint_dir.glob("*")) if d.is_dir()]
                end_dist_checkpoints = [
                    d for d in dist_checkpoints if d.match("*end")]
                last_dist_checkpoints = [
                    d for d in dist_checkpoints if d.match("*last")]

                end_checkpoints = (
                    end_dist_checkpoints if end_dist_checkpoints else list(
                        checkpoint_dir.rglob("*end.ckpt"))
                )
                end_chkpt_cnt = len(end_checkpoints)
                end_checkpoints = _filter_out_unfinished_checkpoints(
                    end_checkpoints)
                ...
                last_checkpoints = (
                    last_dist_checkpoints if last_dist_checkpoints else list(
                        checkpoint_dir.rglob("*last.ckpt"))
                )
                last_chkpt_cnt = len(last_checkpoints)
                last_checkpoints = _filter_out_unfinished_checkpoints(
                    last_checkpoints)
            ...
            if not checkpoint_dir_exists or (not len(end_checkpoints) > 0 and not len(last_checkpoints) > 0):
                if resume_ignore_no_checkpoint:
                    warn = f"There were no checkpoints found in checkpoint_dir or no checkpoint "\
...
def get_log_dir(
    ...
    # Use user-defined exp_dir, project_name, exp_name, and versioning options
        ...
        if not version:
            if resume_if_exists:
                logging.warning(
                    "No version folders would be created under the log folder as "
                    "'resume_if_exists' is enabled."
                )
                version = None
    ...
    try:
        return (
            ...
            subprocess.check_output(
                ['git', 'rev-parse', 'HEAD'], stderr=subprocess.STDOUT).decode(),
        )
...
def configure_loggers(
    ...
    if create_neptune_logger:
        ...
        if "name" not in neptune_kwargs and "project" not in neptune_kwargs:
            raise ValueError(
                "name and project are required for neptune_logger")
...
def configure_checkpointing(
    ...
    if create_preemption_callback:
        ...
        if torch.cuda.is_available():
            ...
            logging.info(
                "Preemption is supported only on GPUs, disabling preemption")
    ...
    def _check_time_remaining(self, trainer: lightning.pytorch.Trainer) -> None:
        ...
        if trainer.should_stop:
            ...
            if checkpoint_callback:
                monitor_candidates = checkpoint_callback._monitor_candidates(
                    trainer)
                checkpoint_callback._save_last_checkpoint(
...
def clean_exp_ckpt(exp_log_dir: Union[str, Path], remove_ckpt: bool = True, remove_nemo: bool = False):
    ...
    if remove_nemo:
        ...
        nemo_files = glob.glob(os.path.join(
            exp_log_dir, "checkpoints", "*.nemo"))
