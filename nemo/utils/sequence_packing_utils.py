...
def create_hist(dataset: np.array, truncate_seq_len: int):
    ...
    for item_dict in dataset:
        # Minus 1 here account for the fact that transformer input and label have one less token than the full sequence
        # Input is missing the last token and label is missing the first token (for next token prediction).
