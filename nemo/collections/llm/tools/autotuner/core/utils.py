...
def extract_model_size_unified(
    source: Union[str, Dict[str, Any], object], source_type: str = "auto"
) -> Optional[float]:
    """
    ...
    # extract from model name or config name using pattern matching only
        ...
        size = extract_value_with_patterns(source, ExtractionPatterns.MODEL_SIZE_PATTERNS, float)
        if size:
            return size
        ...
        try:
            if hasattr(source, 'model'):
                ...
                if hasattr(source, '__class__'):
                    ...
                    size = extract_value_with_patterns(class_name, ExtractionPatterns.MODEL_SIZE_PATTERNS, float)
                    if size:
                        return size
    ...
    # live objects (nemo_run.config.Partial, etc.)
        ...
        try:
            if hasattr(source, 'trainer'):
                ...
                object_str = str(source)
                if object_str and object_str not in ['Config', '<Config>', 'None', '']:
                    precision = extract_value_with_patterns(object_str, ExtractionPatterns.PRECISION_PATTERNS, str)
                    if precision:
                        if 'bf16' in precision.lower():
                            return 'bf16'
                        elif 'fp16' in precision.lower():
...
def _extract_from_live_object(source: object, values: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if hasattr(source, 'data'):
            data = source.data
            data_mapping = {'micro_batch_size': 'mbs', 'global_batch_size': 'gbs', 'seq_length': 'seq_length'}
...
def _extract_data_values(data_obj: Union[str, object]) -> Dict[str, Any]:
    ...
    if (
        hasattr(data_obj, 'micro_batch_size')
        or hasattr(data_obj, 'global_batch_size')
        or hasattr(data_obj, 'seq_length')
    ):
        try:
            result = {}
...
def compare_configs_simplified(
    config1: Union[str, Dict[str, Any], object], config2: Union[str, Dict[str, Any], object]
) -> bool:
    """
...
def check_config_matches(base_config_path: str, generated_configs_dir: str) -> Tuple[bool, List[str]]:
    ...
    json_files = [
        f
        for f in os.listdir(generated_configs_dir)
