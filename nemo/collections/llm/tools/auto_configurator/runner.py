...
class AutoConfigurator:
    ...
    def __init__(
        ...
        ), f"model_type must be set to one of {list(SUPPORTED_MODELS.keys())}."
        ...
        assert (
            gpu_count > 0
        ), "num_nodes * gpus_per_node must be an int larger than zero."
        ...
        ), "max_steps_per_run must be an int and be at least 10 minutes."

        assert (
            context_parallel_sizes != "auto"
        ), "'auto' mode is not supported for context parallelism."
        assert (
            expert_parallel_sizes != "auto"
        ), "'auto' mode is not supported for expert parallelism."

        if mode == "finetune":
            assert (
                not calculate_model_size
            ), "model size estimation is not supported for 'finetune' mode."
            assert (
                tensor_parallel_sizes != "auto"
            ), "tensor parallelism must be specified for 'finetune' mode."
            assert (
                pipeline_parallel_sizes != "auto"
            ), "pipeline parallelism must be specified for 'finetune' mode."
            ...
            if max_model_parallel_size == "auto":
                ...
                self.max_model_parallel_size = (
                    max_mp if max_mp <= gpu_count else gpu_count
                )
...
def generate_configs(runner_config: AutoConfigurator = None) -> dict:
    ...
    for name, config in train_configs.items():
        ...
        # Set strategy params
        trainer.strategy.tensor_model_parallel_size = config.get(
            "tensor_model_parallel_size"
        )
        trainer.strategy.pipeline_model_parallel_size = config.get(
            "pipeline_model_parallel_size"
        )
        trainer.strategy.context_parallel_size = config.get("context_parallel_size")
        trainer.strategy.expert_model_parallel_size = config.get(
