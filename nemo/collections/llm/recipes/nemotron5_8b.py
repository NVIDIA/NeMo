...
def model() -> run.Config[pl.LightningModule]:
    """
    Factory function to create a Nemotron5 8B model configuration.

    Returns:
        run.Config[pl.LightningModule]: Configuration for the Nemotron5 8B model.

    Examples:
        CLI usage:
            $ nemo llm pretrain model=nemotron5_8b ...
...
def pretrain_recipe(
    ...
    """
    Create a pre-training recipe for Nemotron5 8B model.
    ...
    Examples:
        CLI usage:
            $ nemo llm pretrain --factory nemotron5_8b
            $ nemo llm pretrain --factory "nemotron5_8b(num_nodes=1, name='my_nemotron_pretrain')"
...
def pretrain_performance_optimizations(recipe: run.Partial) -> run.Partial:
    """
    Create a performance-optimized pre-training recipe for Nemotron5 8B model.
...
def nemo_resume() -> run.Config[nl.AutoResume]:
    """
    Configure automatic resumption from a NeMo checkpoint converted from Huggingface for 5 8B model.

    More info about the Huggingface model can be found at: https://huggingface.co/nvidia/nemotron5-8b.
    ...
    This function sets up the configuration to resume training from path nemo://nvidia/nemotron5-8b.
    This translates to the full path {NEMO_HOME}/models/nvidia/nemotron5-8b.
    ...
    Note:
        ...
        start from the pre-trained Nemotron5 8B model.
    """
    return run.Config(
        nl.AutoResume, restore_config=run.Config(nl.RestoreConfig, path="nemo://nvidia/nemotron5-8b")
    )
...
def finetune_recipe(
    ...
    """
    Create a fine-tuning recipe for Nemotron5 8B model.
    ...
    Examples:
        CLI usage:
            $ nemo llm finetune --factory nemotron5_8b

        Python API usage:
            >>> recipe = finetune_recipe(name="nemotron5_8b_finetune", num_nodes=2)
            >>> print(recipe)
    ...
    recipe = default_finetune_recipe(
        model(), "nvidia/nemotron5-8b", dir, name, num_nodes, num_gpus_per_node, packed_sequence
    )
