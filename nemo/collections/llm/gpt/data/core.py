...
class GPTSFTChatDataset(GPTSFTDataset):
    """Chat Dataset Subclass, applies chat template from tokenizer and exports loss masks."""

    def __init__(self, *args, **kwargs):
        self.tool_schemas = kwargs.pop("tool_schemas", None)
        if isinstance(self.tool_schemas, str):
            self.tool_schemas = json.loads(self.tool_schemas)

        super().__init__(*args, **kwargs)
        if not hasattr(self.tokenizer, "tokenizer") or not hasattr(self.tokenizer.tokenizer, "apply_chat_template"):
    ...
    def _process_example(self, example):
        ...
        # store metadata in dataset, in case user may have keys required in the prediction json files
        metadata = {k: v for k, v in example.items() if k not in ["messages"]}
        result["metadata"] = metadata
        if self.output_original_text:
            result["metadata"]["messages"] = example["messages"]
