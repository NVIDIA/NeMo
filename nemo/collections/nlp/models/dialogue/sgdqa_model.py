...
class SGDQAModel(NLPModel):
    ...
    def __init__(self, cfg: DictConfig, trainer: Trainer = None):
        ...
        self.encoder = SGDEncoder(
            hidden_size=self.bert_model.config.hidden_size,
            dropout=self._cfg.encoder.dropout,
        )
    ...
    def forward(self, input_ids, attention_mask, token_type_ids):
        ...
        encoded_utterance, token_embeddings = self.encoder(
    ...
    def validation_step(
        ...
        if (
            ...
            self.validation_step_outputs[dataloader_idx].append(
                {f"val_loss": loss, f"tensors": tensors}
            )
        else:
    ...
    def test_step(
        ...
        if (
            ...
            self.test_step_outputs[dataloader_idx].append(
                {f"test_loss": loss, f"tensors": tensors}
            )
        else:
    ...
    def eval_step_helper(self, batch: List[torch.Tensor]):
        ...
        if self.trainer.num_devices and self.trainer.world_size > 1:
            ...
            for ind in range(world_size):
                ...
                all_logit_req_slot_status.append(
                    torch.empty_like(logit_req_slot_status)
                )
                all_logit_cat_slot_status.append(
                    torch.empty_like(logit_cat_slot_status)
                )
                all_logit_cat_slot_value_status.append(
                    torch.empty_like(logit_cat_slot_value_status)
                )
                all_logit_noncat_slot_status.append(
                    torch.empty_like(logit_noncat_slot_status)
                )
            ...
            torch.distributed.all_gather(
                all_logit_req_slot_status, logit_req_slot_status
            )
            torch.distributed.all_gather(
                all_logit_cat_slot_status, logit_cat_slot_status
            )
            torch.distributed.all_gather(
                all_logit_cat_slot_value_status, logit_cat_slot_value_status
            )
            torch.distributed.all_gather(
                all_logit_noncat_slot_status, logit_noncat_slot_status
            )
        ...
        total_scores = torch.unsqueeze(start_scores, axis=2) + torch.unsqueeze(
            end_scores, axis=1
        )
        start_idx = torch.arange(max_num_tokens, device=total_scores.get_device()).view(
            1, -1, 1
        )
        end_idx = torch.arange(max_num_tokens, device=total_scores.get_device()).view(
            1, 1, -1
        )
    ...
    def multi_validation_epoch_end(self, outputs: List[dict], dataloader_idx: int = 0):
        ...
        metrics = self.multi_eval_epoch_end_helper(
            outputs=outputs, split=split, dataloader=dataloader
        )
    ...
    def multi_eval_epoch_end_helper(
        ...
        cat_slot_status_p = torch.cat(
            [x[f"tensors"]["cat_slot_status_p"] for x in outputs]
        )
        cat_slot_value_status = torch.cat(
            [x[f"tensors"]["cat_slot_value_status"] for x in outputs]
        )
        noncat_slot_status = torch.cat(
            [x[f"tensors"]["noncat_slot_status"] for x in outputs]
        )
        noncat_slot_status_p = torch.cat(
            [x[f"tensors"]["noncat_slot_status_p"] for x in outputs]
        )
        noncat_slot_p = torch.cat([x[f"tensors"]["noncat_slot_p"] for x in outputs])
        noncat_slot_start = torch.cat(
            [x[f"tensors"]["noncat_slot_start"] for x in outputs]
        )
        noncat_slot_end = torch.cat([x[f"tensors"]["noncat_slot_end"] for x in outputs])
        noncat_alignment_start = torch.cat(
            [x[f"tensors"]["noncat_alignment_start"] for x in outputs]
        )
        noncat_alignment_end = torch.cat(
            [x[f"tensors"]["noncat_alignment_end"] for x in outputs]
        )

        ids_to_service_names_dict = (
            self.dialogues_processor.schemas._services_id_to_vocab
        )
        example_id = get_str_example_id(
            dataloader.dataset, ids_to_service_names_dict, example_id_num
        )

        metrics = {}
        try:
            prediction_dir = (
                self.trainer.log_dir if self.trainer.log_dir is not None else ""
            )
        except:
            prediction_dir = ""
        ...
        if self.trainer.global_rank == 0:
            prediction_dir = os.path.join(
                prediction_dir,
                "predictions",
                "pred_res_{}_{}".format(split, self._cfg.dataset.task_name),
            )
            ...
            predictions = combine_predictions_in_example(
                predictions, service_id.shape[0]
            )
