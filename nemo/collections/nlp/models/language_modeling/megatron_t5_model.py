...
class MegatronT5Model(MegatronLMEncoderDecoderModel):
    ...
    def _validate_cfg(self):
        ...
        if (
            hasattr(self._cfg.data, "seq_length_dec")
            and self._cfg.data.get("dataset_type") == "t5"
        ):
            if (
                self._cfg.data.seq_length_dec
                < self._cfg.data.seq_length * self._cfg.data.masked_lm_prob
            ):
                raise ValueError(
    ...
    def _build_vocab(self):
        ...
        MegatronT5Model.add_special_tokens_to_tokenizer(
            ...
            add_sentinel_tokens_first=self._cfg.tokenizer.get(
    ...
    @classmethod
    def _add_sentinel_tokens(
        cls, tokenizer, num_sentinel_tokens, add_sentinel_tokens_in_reverse_order
    ):
        ...
        for i in range(num_sentinel_tokens):
            ...
            if len(tokenizer.text_to_ids(f"<extra_id_{i}>")) == 1:
                tokenizer.special_token_to_id[f"<extra_id_{i}>"] = (
                    tokenizer.text_to_ids(f"<extra_id_{i}>")[0]
                )
            else:
    ...
    def _add_base_special_tokens(cls, tokenizer, is_huggingface_converted_model):
        ...
        if not hasattr(tokenizer, "pad_token"):
            ...
            if is_huggingface_converted_model:
                if (
                    hasattr(tokenizer.tokenizer, "pad_id")
                    and tokenizer.tokenizer.pad_id() >= 0
                ):
        ...
        if not hasattr(tokenizer, "bos_token"):
            if (
                hasattr(tokenizer.tokenizer, "bos_id")
                and tokenizer.tokenizer.bos_id() > 0
            ):
        ...
        if not hasattr(tokenizer, "eos_token"):
            if (
                hasattr(tokenizer.tokenizer, "eos_id")
                and tokenizer.tokenizer.eos_id() > 0
            ):
    ...
    def add_special_tokens_to_tokenizer(
        ...
        if (
            tokenizer_cfg.library == "huggingface"
            or tokenizer_cfg.library == "megatron"
        ):
            additional_tokens = {
            ...
            if dataset_type == "ul2":
                ...
                for mask_type in mask_types:
                    additional_tokens["additional_special_tokens"].extend(
            ...
            if add_sentinel_tokens_first:
                if tokenizer_cfg.get("num_sentinel_tokens", 0) > 0:
                    cls._add_sentinel_tokens(
                        tokenizer,
                        tokenizer_cfg.num_sentinel_tokens,
                        add_sentinel_tokens_in_reverse_order,
                    )
                cls._add_base_special_tokens(
                    tokenizer, is_huggingface_converted_model=True
                )
            else:
                cls._add_base_special_tokens(
                    tokenizer, is_huggingface_converted_model=False
                )
                if tokenizer_cfg.get("num_sentinel_tokens", 0) > 0:
                    cls._add_sentinel_tokens(
                        tokenizer,
            ...
            if dataset_type == "ul2":
                for mask_type in ["r", "s", "x"]:
                    if len(tokenizer.text_to_ids(f"▁<extra_id_{mask_type}>")) == 1:
                        tokenizer.special_token_to_id[f"<extra_id_{mask_type}>"] = (
                            tokenizer.text_to_ids(f"<extra_id_{mask_type}>")[0]
                        )
                    else:
    ...
    def build_train_valid_test_datasets(self):
        ...
        eval_iters = (
            self.trainer.max_steps // self.trainer.val_check_interval + 1
        ...
        self._train_ds, self._validation_ds, self._test_ds = (
            build_train_valid_test_datasets(
                ...
                dataset_type=self._cfg.data.get(
                    "dataset_type", self.model_name.lower()
                ),
                ...
                respect_document_boundaries=self._cfg.data.get(
