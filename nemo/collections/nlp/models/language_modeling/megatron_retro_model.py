...
class MegatronRetroModel(MegatronGPTModel):
    ...
    def load_retro_config(self, cfg: DictConfig):
        assert (
            cfg.retro.get("retro_project_dir") is not None
        ...
        with open(retro_config_path) as f:
            ...
            cfg.tokenizer.model = (
                cfg.retro.get("retro_project_dir")
                + "/"
                + retro_preprocess_config.retro_gpt_tokenizer_model
            )
            ...
            with open_dict(cfg):
                cfg.retro_train_samples_with_neighbors = (
                    retro_preprocess_config.retro_gpt_train_samples
                )
                cfg.retro_valid_samples_with_neighbors = (
                    retro_preprocess_config.retro_gpt_valid_samples
                )
            cfg.data.retro_data.retro_block_size = (
                retro_preprocess_config.retro_block_size
            )
            cfg.data.retro_data.retro_chunk_length = (
                retro_preprocess_config.retro_gpt_chunk_length
            )
            cfg.data.retro_data.retro_split_preprocessing = (
                retro_preprocess_config.retro_gpt_split
            )
            cfg.data.retro_data.retro_neighbor_dirs = (
                retro_preprocess_config.retro_neighbor_dirs
            )
    ...
    def predict_step(
        self, batch: Any, batch_idx: int, dataloader_idx: Optional[int] = None, **extra
    ) -> Any:
        ...
        if inference_config is None:
            ...
            if compute_logprob:
                ...
                inference_config["retro_inference"] = inference_config[
                    "retro_inference"
                ]
    ...
    def get_forward_output_and_loss_func(self, validation_step=False):
        def fwd_output_and_loss_func(
            ...
            context_input_ids = context_input_ids.view(
                -1, context_input_ids.shape[-1]
            ).long()
            context_position_ids = context_position_ids.view(
                -1, context_position_ids.shape[-1]
            ).long()
    ...
    def build_retro_config(self) -> RetroConfig:
        ...
        retro_config.retro_chunk_length = self.cfg.data.retro_data.get(
            "retro_chunk_length"
        )
        retro_config.retro_encoder_num_layers = self.cfg.retro.get(
            "retro_encoder_num_layers", 2
        )
        retro_config.retro_encoder_hidden_dropout = self.cfg.retro.get(
            "retro_encoder_hidden_dropout", 0.1
        )
        retro_config.retro_encoder_attention_dropout = self.cfg.retro.get(
            "retro_encoder_attention_dropout", 0.1
        )
        retro_config.retro_num_neighbors = self.cfg.retro.get("retro_num_neighbors", 2)
        retro_config.retro_num_retrieved_chunks = self.cfg.retro.get(
            "retro_num_retrieved_chunks", 2
        )
        retro_config.retro_verify_neighbor_count = self.cfg.retro.get(
            "retro_verify_neighbor_count", True
        )
        retro_config.retro_retrieved_length = (
            retro_config.retro_num_retrieved_chunks * retro_config.retro_chunk_length
        )
        retro_config.retro_split_preprocessing = self.cfg.data.retro_data.get(
            "retro_split_preprocessing"
        )
        retro_config.retro_neighbor_dirs = self.cfg.data.retro_data.get(
            "retro_neighbor_dirs"
        )
