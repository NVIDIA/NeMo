...
class MegatronBertModel(MegatronBaseModel):
    ...
    def training_step(self, dataloader_iter):
        ...
        if (dataloader_iter._batch_idx + 1) % self.trainer.accumulate_grad_batches == 0:
            ...
            if len(loss_mean) > 2:
                self.log(
                    "reduced_lm_train_loss", loss_mean[1], prog_bar=True, batch_size=1
                )
                self.log(
    ...
    def build_LDDL_data(self, cfg):
        ...
        num_micro_batches = self.cfg.global_batch_size // (
            self.cfg.micro_batch_size * data_parallel_size
        )
        global_batch_size_on_this_data_parallel_rank = (
            num_micro_batches * self.cfg.micro_batch_size
        )
        samples_consumed_dploader = (
            self.compute_consumed_samples(0) // data_parallel_size
        )
    ...
    def build_transformer_config(self) -> TransformerConfig:
        ...
        assert (
            activation == "gelu"
        ), "Only gelu activation is support for BERT at the moment."
