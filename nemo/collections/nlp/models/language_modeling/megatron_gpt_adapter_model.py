        ...
        for _, module in self.named_modules():
            if isinstance(module, adapter_mixins.AdapterModuleMixin):
                for adapter_key in self.adapter_name_keys:
                    if (
                        model_utils.import_class_by_path(self.adapter_cfg._target_)
...
class MegatronNLPAdapter(ModelPT):
    ...
    def __init__(
        self, cfg: DictConfig, trainer: Trainer, adapter_name_keys: List[str], base_model: MegatronGPTModel,
    ):
        super().__init__(cfg, trainer)
...
class MegatronGPTAdapterLearningModel(MegatronGPTBaseAdapterModel):
    ...
    def __init__(
        self, adapter_tuning_cfg: DictConfig, trainer: Trainer, base_model: MegatronGPTModel,
    ):
        adapter_name_keys = [
            AdapterName.PRE_ATTN_ADAPTER,
            AdapterName.POST_ATTN_ADAPTER,
            ...
            if isinstance(module, adapter_mixins.AdapterModuleMixin):
                for adapter_key in self.adapter_name_keys:
                    if adapter_key == AdapterName.MLP_INFUSED:
                        ...
                        cfg = InfusedAdapterConfig(in_features=cfg.hidden_size // cfg.tensor_model_parallel_size)
                    else:
