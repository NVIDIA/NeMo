...
class MegatronGPTSFTModel(NLPAdapterModelMixin, MegatronGPTModel):
    ...
    def _build_dataset(self, data_cfg, is_train=True, is_val=False, is_test=False):
        ...
        for file_path, num_samples in zip(data_cfg.file_names, num_train_samples_per_dataset):
            if self.cfg.data.get('continue_pretrain', False):
                ...
                _train_ds, _validation_ds, _test_ds = build_train_valid_test_datasets(
                    cfg=self.cfg,
    ...
    def build_data_loader(self, dataset, data_cfg, consumed_samples=0):
        ...
        if self.cfg.data.get('continue_pretrain', False):
            collate_fn = self.continue_pretrain_collate_fn
        else:
    ...
    def continue_pretrain_collate_fn(self, batch):
        input_ids = torch.stack([item['tokens'][:-1] for item in batch])
        labels = torch.stack([item['tokens'][1:] for item in batch])
        contexts = torch.stack([torch.LongTensor([self.tokenizer.eos_id] * input_ids.shape[1]) for _ in batch]) # no context
        context_lengths = torch.LongTensor([0 for _ in batch]) # no context
        answers = input_ids # no answer
        loss_mask = torch.stack([torch.LongTensor([1.0] * input_ids.shape[1]) for _ in batch])
        metadata = ([{} for item in batch]) # no metadata

        attention_mask = torch.stack([item['attention_mask'][:,:-1,:-1] for item in batch])
        loss_mask = torch.stack([item['loss_mask'][:-1] for item in batch])
        position_ids = torch.stack([item['position_ids'][:-1] for item in batch])
