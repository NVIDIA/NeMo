...
class MegatronGPTSFTModel(NLPAdapterModelMixin, MegatronGPTModel):
    ...
    def _build_dataset(self, data_cfg, is_train=True):
        ...
        if not is_list_config:
            raise ValueError(
                f"SFT train/validation datasets must be provided as a list of individual JSONL files."
            )
    ...
    def fwd_bwd_step(self, dataloader_iter, forward_only, first_val_step=None):
        ...
        if (
            ...
            if self.cfg.optim.get("overlap_param_sync", False) and self.cfg.optim.get(
                ...
                param_sync_func = [
                    lambda x, model_index=model_index: self._optimizer.finish_param_sync(
                        model_index, x
                    )
