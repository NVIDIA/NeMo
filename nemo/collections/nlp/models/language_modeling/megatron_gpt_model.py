...
try:
    ...
    from megatron.core.utils import init_method_normal, scaled_init_method_normal, drain_embedding_wgrad_compute
...
class MegatronGPTModel(MegatronBaseModel, TextGeneration):
    ...
    def training_step(self, dataloader_iter):
        ...
        if self.cfg.get('pipeline_model_parallel_size', 1) > 1 and parallel_state.is_pipeline_last_stage(ignore_virtual=True):
            if self.cfg.get('defer_embedding_wgrad_compute',False) and self.mcore_gpt: #Silently ignore the optimization if MCORE is not used
                ...
                assert len(embedding_module.embedding_activation_buffer) == 0, "When you defer wgrads, this buffer should not hold stray activations"
        ...
        if self.cfg.get('pipeline_model_parallel_size', 1) > 1 and parallel_state.is_pipeline_last_stage(ignore_virtual=True):
            if self.cfg.get('defer_embedding_wgrad_compute',False) and self.mcore_gpt: #Silently ignore the optimization if MCORE is not used
                ...
                drain_embedding_wgrad_compute(embedding_module.config, embedding_activation_buffer, grad_output_buffer, weight)
