...
class MegatronT5SFTModel(NLPAdapterModelMixin, MegatronT5Model):
        ...
        if hasattr(self.cfg, "eval_languages"):
            ...
            if not hasattr(data_cfg, "metric"):
                ...
                if not hasattr(data_cfg.metric, "name"):
                    raise ValueError(
                        "Metric name is not provided in the metric config."
                    )
                ...
                if (
                    ...
                    and data_cfg.metric.name
                    in self._metrics_require_string2category_map
                ):
            ...
            if hasattr(data_cfg, "src_file_name") or hasattr(data_cfg, "file_names"):
                ...
                elif hasattr(data_cfg, "src_file_name") and isinstance(
                    data_cfg.src_file_name, ListConfig
                ):
                    metric = [
                        metric_class() for _ in range(len(data_cfg.src_file_name))
                    ]
                elif hasattr(data_cfg, "file_names") and isinstance(
                    data_cfg.file_names, ListConfig
                ):
                ...
                metric = [
                    metric_class()
                ]  # GLUE does need to specify average or num_classes.
    ...
    def cast_for_metric(
        ...
        # If labels are strings, we need to convert them to indices for some metrics.
            ...
            if label not in class_labels:
                raise ValueError(
                    f"Ground truth label {label} is not in the class labels list : {class_labels}"
                )
    ...
    def inference_step(self, dataloader_iter, mode: str):
        ...
        predicted_token_ids, _ = self.decode(
            ...
            bos_id=(
                ...
                if data_cfg.get("replace_bos_with_pad", False)
        ...
        if self.val_metric is not None or self.test_metric is not None:
            ...
            for _, (pred, label, category) in enumerate(
                zip(preds_text, labels_text, categories)
            ):
    ...
    def ids_to_text(cls, batch_ids, tokenizer):
        ...
        for ids in batch_ids:
            ...
            if hasattr(tokenizer, "special_token_to_id"):
                ids = [
                    id for id in ids if id not in tokenizer.special_token_to_id.values()
                ]
    ...
    def inference_epoch_end(self, outputs, mode, data_cfg):
        ...
        if mode == "validation":
            ...
            if averaged_metric is not None:
                self.log(
                    f"validation_{self.val_metric_name}", averaged_metric, batch_size=1
                )
        elif mode == "test":
    ...
    def write_predictions_to_file(self, outputs, output_file_path_prefix):
        with open(
            output_file_path_prefix + "_inputs_preds_labels.jsonl", "w"
        ) as f_json:
            assert (
                len(outputs["inputs"])
                == len(outputs["preds"])
    ...
    def setup_validation_data(self):
        self._validation_dl = self.setup_eval_data(
    ...
    def _build_train_dataset(self, data_cfg):
        ...
        if hasattr(data_cfg, "src_file_name") and hasattr(data_cfg, "tgt_file_name"):
            ...
            if (is_src_list_config and not is_tgt_list_config) or (
                is_tgt_list_config and not is_src_list_config
            ):
                raise ValueError(
            ...
            raise ValueError(
                "You must specify either (src_file_name and tgt_file_name) or file_names in data config"
            )
