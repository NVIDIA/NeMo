...
class MegatronBaseModel(NLPModel):
    ...
    def build_transformer_config(self) -> TransformerConfig:
        ...
        activation_func = utils_funcs.activation_to_func(activation)
        ...
        if "encoder" in cfg:
            # if both encoder and decoder exist, we assume they are identical.
            # The alternative behavior is not supported by MCore models at the moment.
            enc_dec_cfg = cfg['encoder']
        elif "decoder" in cfg:
            enc_dec_cfg = cfg['decoder']
        else:
            enc_dec_cfg = {}
        ...
        for field in fields(TransformerConfig):
            ...
            # then config and encoder/decoder config
            elif field.name in cfg:
                transformer_config_dict[field.name] = cfg[field.name]
            elif field.name in enc_dec_cfg:
                transformer_config_dict[field.name] = enc_dec_cfg[field.name]
            # then model parallel config
                ...
                logging.warning(
                    f"The model: {self.__class__} does not have field.name: {field.name} in its cfg. "
                    f"Add this key to cfg or config_mapping to make to make it configurable."
                )
