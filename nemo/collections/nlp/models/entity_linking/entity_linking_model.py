...
class EntityLinkingModel(NLPModel, Exportable):
    ...
    def query_index(self, query: str, top_n: int = 5) -> Dict:
        ...
        if self.index is None:
            self.log("Please use model.load_index(cfg) first, then try again")
            return
    ...
    def load_index(self, cfg: DictConfig):
        ...
        if not os.path.isfile(cfg.save_name):
            self.build_index(cfg)

        self.index_cfg = cfg
        self.index = faiss.read_index(cfg.save_name)
        self.idx2id = pkl.load(open(cfg.idx_to_id, "rb"))
        self.id2string = pkl.load(open(cfg.id_to_string, "rb"))

        if cfg.apply_pca:
            self.pca = pkl.load(open(cfg.pca.save_name, "rb"))
    ...
    def build_index(self, cfg: DictConfig):
        ...
        Args:
            cfg: Config file specifying index parameters
        """
        if os.path.isfile(cfg.save_name):
            self.log("Index file already exists, try loading the index instead")
            return

        # Get index dataset embeddings 
        if os.path.isfile(cfg.embeddings_save_name) and os.path.isfile(cfg.concept_save_name):
            self.log("Loading previously extracted index dataset embeddings.")
            embeddings = h5py.File(cfg.embedding_save_name, "r")
            concept_ids = pkl.load(open(cfg.concept_save_name, "rb"))

        else:
            self.log("Encoding index dataset, this may take a while...")
            self.setup_index_data(cfg)
        ...
        if not os.path.isfile(cfg.idx_to_id):
            self.log("Mapping concept_ids to their unique indices...")
        ...
        if cfg.apply_pca and not os.path.isfile(cfg.pca_embeddings_file):
            if not os.path.isfile(cfg.pca.save_name):
                self.log("Fitting PCA model for embeddings dimensionality reduction...")
                ...
                pkl.dump(pca, open(cfg.pca.save_name, "wb"))
            
            else:
                pca = pkl.load(open(cfg.pca.save_name, "rb"))

            self.log("Applying PCA transformation to entire index dataset...")
            embeddings = np.array(pca.transform(embeddings), dtype=np.float32)
            emb_file = h5py.File(cfg.pca_embeddings_file, "w")
            emb_file.create_dataset(cfg.dataset_name, data=embeddings)
            emb_file.close()

        elif cfg.apply_pca:
            embeddings = h5py.File(cfg.pca_embeddings_file, "r")
            embeddings = embeddings[config.index.dataset_name][:] 


        # Build faiss index from embeddings
        self.log(f"Training index with embedding dim size {cfg.dims} using {faiss.get_num_gpus} gpus")
        quantizer = faiss.IndexFlatL2(cfg.dims)
        index = faiss.IndexIVFFlat(quantizer, cfg.dims, cfg.nlist)
        index = faiss.index_cpu_to_all_gpus(index)
        index.train(embeddings)

        self.log("Adding dataset embeddings to index...")
        for i in tqdm(range(0, embeddings.shape[0], index_batch_size)):
            index.add(embeddings[i:i+index_batch_size])

        self.log("Saving index...")
        faiss.write_index(faiss.index_gpu_to_cpu(index), cfg.save_name)

        self.log("Index built and saved.")
    ...
    def _get_index_embeddings(self, cfg: DictConfig):
        ...
        with torch.no_grad():
            for batch in tqdm(self._index_dl):
                input_ids, input_type_ids, input_mask, batch_concept_ids = batch
        ...
        pkl.dump(concept_ids, open(cfg.concept_save_name, "wb"))
    ...
    def _get_query_embedding(self, query):
       model_input = self.tokenizer(query,
        ...
        query_emb = self.forward(**model_input)
