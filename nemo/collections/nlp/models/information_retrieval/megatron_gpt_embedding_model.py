...
def _gather_global_inbatch_representations(local_eos_tensor):
    ...
    global_eos_tensors = [
        torch.zeros_like(local_eos_tensor)
...
class MegatronGPTEmbeddingModel(MegatronGPTSFTModel):
    def __init__(self, cfg: DictConfig, trainer: Trainer):
        ...
        self.use_all_possible_negatives = self.cfg.get(
            "use_all_possible_negatives", True
        )
    ...
    def _build_dataset(self, data_cfg, is_train=True):
        ...
        if is_train:
            ...
            num_query_files = (
                len(data_cfg.query_file_names)
                if data_cfg.query_file_names is not None
                else 0
            )
            num_doc_files = (
                len(data_cfg.doc_file_names)
                if data_cfg.doc_file_names is not None
                else 0
            )
        ...
        if is_train:
            ...
            if packed_sequence:
                raise NotImplementedError(
                    "Packed sequence is not supported for MegatronGPTEmbeddingModel"
                )
    ...
    def training_step_fwd_bwd_step_call(self, dataloader_iter, forward_only):
        ...
        self.log(
            "avg_pos_cs", avg_pos_cs, prog_bar=True, rank_zero_only=True, batch_size=1
        )
        self.log(
            "avg_neg_cs", avg_neg_cs, prog_bar=True, rank_zero_only=True, batch_size=1
        )
    ...
    def local_validation_step(self, dataloader_iter):
        ...
        loss, non_loss_tensors = self.fwd_bwd_step(
    ...
    def loss_func(self, loss_mask, num_valid_tokens_in_ub, output_tensor):
        ...
        query_hs = eos_tensors[
            ::3, :
        ]  # every third tensor is a query (bs x hidden_size)
        pos_doc_hs = eos_tensors[
            1::3, :
        ]  # every third tensor is a positive doc (bs x hidden_size)
        neg_doc_hs = eos_tensors[
            2::3, :
        ]  # every third tensor is a negative doc (bs x hidden_size)
