...
class MegatronGPTRerankerModel(MegatronGPTEmbeddingModel):
    ...
    def training_step_fwd_bwd_step_call(self, dataloader_iter, forward_only):
        ...
        self.log(
            "logit_diff", logit_diff, prog_bar=True, rank_zero_only=True, batch_size=1
        )
        return loss_mean
    ...
    def inference_step_validation_call(
        ...
        loss, non_loss_tensors = self.local_validation_step(
            itertools.chain([dataloader_idx], [batch])
        )
        outputs = {
            "loss": loss,
            "metadata": metadata,  # [dict]
            "query_pos_doc_logit": non_loss_tensors[
                "query_pos_doc_logit"
    ...
    def inference_loss_func(self, loss_mask, num_valid_tokens_in_ub, eos_tensors):
        ...
        _blank = torch.zeros(
            1, device=query_pos_doc_hs.device, dtype=query_pos_doc_hs.dtype
        )[0]
        return {
            "loss": _blank,
    ...
    def loss_func(self, loss_mask, num_valid_tokens_in_ub, output_tensor):
        ...
        if not self.trainer.training:
            return self.inference_loss_func(
                loss_mask, num_valid_tokens_in_ub, eos_tensors
            )
        bs = eos_tensors.shape[0] // 2
        query_pos_doc_hs = eos_tensors[
            ::2, :
        ]  # every second tensor from idx 0 is a query w pos_doc (bs x 1)
        query_neg_doc_hs = eos_tensors[
            1::2, :
        ]  # every second tensor from idx 1 is a query w negative doc (bs x 1)

        if self.reward_model_loss:
            loss = -torch.nn.functional.logsigmoid(
                query_pos_doc_hs - query_neg_doc_hs
            ).mean()
        else:
    ...
    def gather_and_maybe_write_predictions(
        ...
        if self.global_rank == 0 and data_cfg.get("write_embeddings_to_file", False):
            ...
            self.write_embeddings_to_file(
                deduplicated_outputs, fldr_path, dataloader_idx
            )
