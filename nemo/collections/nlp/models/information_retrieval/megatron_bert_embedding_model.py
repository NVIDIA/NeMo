...
class MegatronBertEmbeddingModel(MegatronBertModel):
    ...
    def __init__(self, cfg: DictConfig, trainer: Trainer):
        ...
        assert self.backprop_type in [
            "local",
            "global",
        ], "Backprop type must be `local` or `global`"
    ...
    def build_train_valid_test_datasets(self, is_train=True):
        ...
        if is_train:
            ...
            if (
                self.cfg.data.data_test.query_file_names is None
                or self.cfg.data.data_test.doc_file_names is None
            ):
                return []
        ...
        if (
            ...
            if self.cfg.data.dataloader_type == "single":
                batch_sampler = MegatronPretrainingSampler(
                    ...
                    pad_samples_to_global_batch_size=not self.cfg.get(
                        "drop_last", False
                    ),
                )
    ...
    def training_step(self, dataloader_iter):
        ...
        if len(loss_mean) > 2:
            ...
            self.log(
                "reduced_sop_train_loss", loss_mean[2], prog_bar=True, batch_size=1
            )
    ...
    def validation_step(self, dataloader_iter):
        ...
        if prefix == "val":
            ...
            self.test_step_outputs[dataloader_idx].append(
                losses_reduced_per_micro_batch[0]
            )
        return loss
    ...
    def on_test_epoch_end(self):
        for dataloader_idx, output in enumerate(self.test_step_outputs):
            self.gather_and_maybe_write_predictions(
                output, self.cfg.data.data_test, "test", dataloader_idx
            )

    def gather_and_maybe_write_predictions(
        self, output, data_cfg, mode, dataloader_idx=0
    ):
