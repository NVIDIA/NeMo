...
from nemo.collections.nlp.data.token_classification.punctuation_capitalization_dataset import (
    ...
    PunctuationCapitalizationTrainDataConfig, load_label_ids,
...
from nemo.collections.nlp.models.token_classification.punctuation_capitalization_config import (
    is_legacy_model_config, legacy_model_config_to_new_model_config)
...
class PunctuationCapitalizationModel(NLPModel, Exportable):
    ...
    def eval_step(
        self, batch: Dict[str, torch.Tensor], mode: str, dataloader_idx: int
    ) -> Dict[str, None]:
        """
        ...
        self.metrics[mode]["punct_class_report"][dataloader_idx](
            punct_preds, punct_labels
        )
        self.metrics[mode]["capit_class_report"][dataloader_idx](
            capit_preds, capit_labels
        )
    ...
    def on_train_epoch_end(self) -> None:
        ...
        if shuffle:
            if isinstance(
                self.train_dataloader().dataset, BertPunctuationCapitalizationDataset
            ):
                self.train_dataloader().dataset.repack_batches_with_shuffle()

    def _multi_eval_epoch_end(
        self, mode: str, dataloader_idx: int
    ...
    def multi_validation_epoch_end(
        self, outputs: Any, dataloader_idx: int = 0
    ) -> Dict[str, Dict[str, torch.Tensor]]:
        """
    ...
    def update_config_after_restoring_from_checkpoint(self, **kwargs) -> None:
        ...
        if "common_dataset_parameters" in kwargs:
            ...
            self._cfg.common_dataset_parameters = OmegaConf.merge(
                ...
                OmegaConf.create(kwargs["common_dataset_parameters"]),
            )
        ...
        if "train_ds" in kwargs:
            if kwargs["train_ds"] is None:
                ...
                self._cfg.train_ds = OmegaConf.merge(
                    base, OmegaConf.create(kwargs["train_ds"])
                )
        if "validation_ds" in kwargs:
            if kwargs["validation_ds"] is None:
                ...
                self._cfg.validation_ds = OmegaConf.merge(
                    base, OmegaConf.create(kwargs["validation_ds"])
                )
        if "test_ds" in kwargs:
            if kwargs["test_ds"] is None:
                ...
                self._cfg.test_ds = OmegaConf.merge(
                    base, OmegaConf.create(kwargs["test_ds"])
                )
        if "optim" in kwargs:
            self._cfg.optim = kwargs["optim"]

    def setup_training_data(
        self, train_data_config: Optional[Union[Dict[str, Any], DictConfig]] = None
    ) -> None:
        """
        ...
        if train_data_config is not None:
            ...
            train_data_config = OmegaConf.merge(
                OmegaConf.structured(PunctuationCapitalizationTrainDataConfig),
                train_data_config,
            )
        ...
        self._train_dl = self._setup_dataloader_from_config(
            cfg=train_data_config, train=True
        )
        ...
        if not torch.distributed.is_initialized() or torch.distributed.get_rank() == 0:
            ...
            if label_vocab_dir is None:
                punct_label_ids_file, capit_label_ids_file = (
                    self._train_dl.dataset.save_labels_and_get_file_paths(
                        self._cfg.class_labels.punct_labels_file,
                        self._cfg.class_labels.capit_labels_file,
                    )
                )
            else:
                punct_label_ids_file = (
                    Path(label_vocab_dir).expanduser()
                    / self._cfg.class_labels.punct_labels_file
                )
            ...
            self.register_artifact(
                "class_labels.punct_labels_file", str(punct_label_ids_file)
            )
            self.register_artifact(
                "class_labels.capit_labels_file", str(capit_label_ids_file)
            )
    ...
    def _setup_metrics_dictionary(self) -> None:
        ...
        self.metrics = torch.nn.ModuleDict(
            {"val": eval_metrics, "test": copy.deepcopy(eval_metrics)}
        )

    def setup_validation_data(
        self, val_data_config: Optional[Union[Dict[str, Any], DictConfig]] = None
    ) -> None:
        """
        ...
        if val_data_config is not None:
            ...
            val_data_config = OmegaConf.merge(
                OmegaConf.structured(PunctuationCapitalizationEvalDataConfig),
                val_data_config,
            )
        ...
        self._validation_dl = self._setup_dataloader_from_config(
            cfg=val_data_config, train=False
        )
        ...
        self.metrics["val"]["punct_class_report"].append(
            ClassificationReport(**punct_kw)
        )
        self.metrics["val"]["capit_class_report"].append(
            ClassificationReport(**capit_kw)
        )

    def setup_test_data(
        self, test_data_config: Optional[Union[Dict[str, Any], DictConfig]] = None
    ) -> None:
        """
        ...
        self._test_dl = self._setup_dataloader_from_config(
            cfg=test_data_config, train=False
        )
        ...
        self.metrics["test"]["punct_class_report"].append(
            ClassificationReport(**punct_kw)
        )
        self.metrics["test"]["capit_class_report"].append(
            ClassificationReport(**capit_kw)
        )
    ...
    def _check_label_config_parameters(self) -> None:
        ...
        for (
            ...
            if already_set_label_ids is not None:
                config_label_ids = (
                    label_ids if label_vocab_file is None else file_label_ids
                )
                if config_label_ids is not None:
                    if label_vocab_file is None:
                        config_label_ids_source = f"Labels passed in config parameter `model.common_dataset_parameters.{label_ids_name}`"
                    else:
    ...
    def _extract_label_vocab_files_from_config(
        self,
        ...
        if self._is_model_being_restored():
            ...
            if self._cfg.common_dataset_parameters.label_vocab_dir is None:
                ...
                label_vocab_dir = Path(
                    self._cfg.common_dataset_parameters.label_vocab_dir
                ).expanduser()
                punct_label_vocab_file = (
                    label_vocab_dir / self._cfg.class_labels.punct_labels_file
                )
                capit_label_vocab_file = (
                    label_vocab_dir / self._cfg.class_labels.capit_labels_file
                )
    ...
    def _set_label_ids(self) -> None:
        ...
        """
        punct_label_vocab_file, capit_label_vocab_file = (
            self._extract_label_vocab_files_from_config()
        )
        if punct_label_vocab_file is not None:
            punct_labels_file = self.register_artifact(
                "class_labels.punct_labels_file", str(punct_label_vocab_file)
            )
            if punct_labels_file is None:
                ...
                self.punct_label_ids = OmegaConf.to_container(
                    self._cfg.common_dataset_parameters.punct_label_ids
                )
            else:
        ...
        if capit_label_vocab_file is not None:
            capit_labels_file = self.register_artifact(
                "class_labels.capit_labels_file", str(capit_label_vocab_file)
            )
            if capit_labels_file is None:
                ...
                self.capit_label_ids = OmegaConf.to_container(
                    self._cfg.common_dataset_parameters.capit_label_ids
                )
            else:
    ...
    def _setup_dataloader_from_config(
        self, cfg: DictConfig, train: bool
        ...
        if cfg.use_tarred_dataset:
            ...
            if train:
                ...
                if self._trainer is None:
                    ...
                    number_of_batches_is_multiple_of = (
                        self._trainer.num_nodes * self._trainer.num_devices
                    )
            if cfg.cache_dir is None:
                ...
                cache_dir = Path(cfg.cache_dir).joinpath(
                    "fsroot", *text_file.expanduser().resolve().parts[1:-1]
                )
        ...
        if cfg.shuffle and cfg.use_tarred_dataset:
            logging.warning(
                f"Shuffling in dataloader is not supported for tarred dataset."
            )
            shuffle = False
        else:
            shuffle = cfg.shuffle
    ...
    @staticmethod
    def _remove_margins(
        tensor: torch.Tensor, margin_size: int, keep_left: bool, keep_right: bool
    ) -> torch.Tensor:
        ...
        if not keep_right:
            tensor = tensor[
                : tensor.shape[0] - margin_size - 1
            ]  # remove right margin and SEP token
        return tensor
            ...
            for b_probs, logits in [(b_punct_probs, pl), (b_capit_probs, cl)]:
                p = torch.nn.functional.softmax(
                    self._remove_margins(
                        logits, margin, keep_left=first, keep_right=last
                    )[stm],
                    dim=-1,
                )
    ...
    @staticmethod
    def _update_accumulated_probabilities(
        acc_prob: np.ndarray, update: np.ndarray
    ) -> np.ndarray:
        """
        Args:
        ...
        """
        acc_prob = np.concatenate(
            [acc_prob * update[: acc_prob.shape[0]], update[acc_prob.shape[0] :]],
            axis=0,
        )
        return acc_prob
