...
class SmoothedCrossEntropy(torch.nn.Module):
    ...
    def __init__(self, label_smoothing: Optional[float] = 0.0, predict_last_k: Optional[int] = 0):
        super().__init__()
...
@experimental
class SmoothedCrossEntropyLoss(SmoothedCrossEntropy, Serialization, Typing):
    """
    ...
    def __init__(
        self, pad_id: Optional[int] = None, label_smoothing: Optional[float] = 0.0, predict_last_k: Optional[int] = 0
    ):
        super().__init__(label_smoothing=label_smoothing, predict_last_k=predict_last_k)
        self._pad_id = pad_id
    ...
    def forward(self, logits, labels, output_mask=None):
        if output_mask is not None:
            ...
            labels_mask = (labels != self._pad_id).to(logits.dtype)
        else:
        ...
        loss = super().forward(logits, labels, labels_mask)
        return loss
