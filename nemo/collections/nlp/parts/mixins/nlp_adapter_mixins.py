...
class NLPAdapterModelMixin:
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.use_mcore_gpt or self.use_mcore_t5:
            assert (
                HAVE_MEGATRON_CORE
            ), "You set `mcore_gpt` or `mcore_t5` as True but megatron core is not found."
    ...
    def _get_all_keys(
        ...
        k = [
            n
            for m in self._unwrap_model_list()
            for n, p in m.named_parameters(prefix="model")
        ]
        b = [
            n
            for m in self._unwrap_model_list()
            for n, p in m.named_buffers(prefix="model")
            if n.replace("model.module.", "model.", 1)
            in m.state_dict(prefix="model.").keys()
        ]
    ...
    def _check_and_add_peft_cfg(self, peft_cfg):
        ...
        for adapter_name, adapter_cfg in peft_cfg.get_config_dict().items():
            ...
            if (
                ...
                for layer in layers:
                    if layer.layer_number in (
                        layer_selection or list(range(1, self.cfg.num_layers + 1))
                    ):
    ...
    def add_adapter(self, peft_cfgs: Union[PEFTConfig, List[PEFTConfig]]):
        ...
        if self.cfg.optim.name == "distributed_fused_adam":
            raise ValueError(
                "distributed_fused_adam is not supported for PEFT. Please use fused_adam"
            )
        ...
        ptuning_only = len(peft_cfgs) == 1 and isinstance(
            peft_cfgs[0], PtuningPEFTConfig
        )
        self.ptuning_only_and_non_first_stage = (
            ptuning_only and not self.first_stage_of_pipeline()
        )
        ...
        for cfg in peft_cfgs:
            ...
            if (
                hasattr(cfg, "tunable_base_param_names")
                and cfg.tunable_base_param_names
            ):
                self.set_tunable_base_params(cfg)

    def _get_config_and_state_dict_from_nemo(
        self, filepath, map_location, sharded_state_dict=None
    ):
        cwd = os.getcwd()
        ...
        if self.use_peft:
            ...
            if not self.ptuning_only_and_non_first_stage:
                ...
                for _, module in [
                    elem
                    for m in self._unwrap_model_list()
                    for elem in m.named_modules(prefix="model")
                ]:
                    if (
                ...
                for name, param in [
                    elem
                    for m in self._unwrap_model_list()
                    for elem in m.named_parameters(prefix="model")
                ]:
    ...
    def load_adapters(
        ...
        if not peft_cfgs:
            ...
            peft_cfg_cls_lst = [
                PEFT_CONFIG_MAP[s] for s in conf.peft.peft_scheme.split(",")
            ]
        ...
        if not self.ptuning_only_and_non_first_stage:
            assert set(state_dict.keys()) == self.adapter_keys.union(
    ...
    def get_peft_state_dict(self):
        ...
        def filter_state_dict(state_dict):
            ...
            for k in self.adapter_keys.union(self.tunable_base_param_keys):
                ...
                peft_state_dict[new_k] = (
                    state_dict[new_k] if new_k in state_dict else state_dict[k]
                )
        ...
        if hasattr(self, "model") and isinstance(self.model, list):
            ...
            for i, m in enumerate(self.model):
                peft_state_dict[f"model_{i}"] = filter_state_dict(
                    m.state_dict(prefix="model.")
                )
        else:
            peft_state_dict = filter_state_dict(
                self._unwrap_model().state_dict(prefix="model.")
            )
    ...
    def sharded_state_dict(self, prefix: str = ""):
        use_mcore = (getattr(self, "mcore_gpt", False)) or (
            getattr(self, "mcore_t5", False)
        )
    ...
    def on_load_checkpoint(self, checkpoint) -> None:
        ...
        if self.use_peft and self.setup_complete:
            ...
            if cfg_peft and cfg_peft["peft_scheme"] == "qlora":
                ...
                qlora_load_model(
                    self.model.module if self.megatron_amp_O2 else self.model,
                    self.cfg,
    ...
    def merge_inference_cfg(cls, path: str, cfg: DictConfig) -> DictConfig:
        ...
        if hasattr(peft_cfg, "peft") and peft_cfg.peft.peft_scheme not in [
            None,
            "none",
        ]:
        ...
        with open_dict(cfg):
            ...
            cfg.inference.tokens_to_generate = peft_cfg.data.test_ds.get(
