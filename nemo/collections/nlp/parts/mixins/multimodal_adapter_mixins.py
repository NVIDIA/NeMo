...
class MultimodalAdapterModelMixin(NLPAdapterModelMixin):
    ...
    def add_adapter(self, peft_cfgs: Union[PEFTConfig, List[PEFTConfig]]):
        if self.cfg.get("virtual_pipeline_model_parallel_size", None):
            raise ValueError(
                "Virtual pipeline model parallel is not supported when using PEFT"
            )
    ...
    def load_adapters(
        ...
        if filepath.endswith(".nemo"):
            ...
            conf, state_dict = self._get_config_and_state_dict_from_nemo(
                filepath, map_location, sharded_state_dict
            )
                ...
                if name in [
                    ...
                    if (
                        ...
                        if autocast_dtype is not None:
                            module.adapter_layer[peft_name] = module.adapter_layer[
                                peft_name
                            ].to(autocast_dtype)
