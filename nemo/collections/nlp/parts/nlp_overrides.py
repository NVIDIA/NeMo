...
class NLPDDPStrategy(DDPStrategy):
    ...
    def configure_ddp(self):
        ...
        if (hasattr(self.model, "megatron_amp_O2") and self.model.megatron_amp_O2) or (
            hasattr(self.model, "with_distributed_adam")
            and self.model.with_distributed_adam
        ):
    ...
    def optimizer_sharded_state_dict(
        self, unsharded_optim_state=None, is_loading=False
    ):
        """
        ...
        if isinstance(optimizer, McoreDistributedOptimizer):
            ...
            return optimizer.sharded_state_dict(
                model_sharded_state_dict, unsharded_optim_state
            )
    ...
    def save_checkpoint(
        ...
        if self.use_distributed_checkpointing:
            ...
            include_optimizer = (
                ...
                else storage_options.get("include_optimizer", True)
            )
            ...
            self.checkpoint_io.save_checkpoint(
                checkpoint, ckpt_to_dir(filepath), storage_options=storage_options
            )

            if HAVE_MODELOPT and hasattr(
    ...
    def load_model_state_dict(
        ...
        # legacy state dict logic, does not use megatron core
            ...
            if model_key is not None:
                if isinstance(model_attr, Float16Module) or isinstance(
                    model_attr, MCoreFloat16Module
                ):
                    new_state_dict = {}
                    for key in checkpoint["state_dict"].keys():
                        new_key = key.replace(
                            f"{model_key}.", f"{model_key}.module.", 1
                        )
    ...
    def _check_param_groups_mismatch(
    ...
    def _fix_param_groups(
        ...
        model_has_expert_param = any(
            param.get("is_expert", False) for param in model_param_groups
        )
        checkpoint_has_expert_param = any(
            param.get("is_expert", False) for param in checkpoint_param_groups
        )
        ...
        if checkpoint_has_expert_param and not model_has_expert_param:
            ...
            expert_index = next(
                (
                    index
                    for index, entry in enumerate(checkpoint_param_groups)
                    if entry.get("is_expert", False)
                ),
                None,
            )
            if expert_index:
                # Temporary empty params so that loading doesn't fail
                model_param_groups.insert(
                    expert_index,
                    {"params": LocalNonpersistentObject([]), "is_expert": True},
                )
                if "optimizer" in sharded_state_dict["optimizer_states"][0]:
                    sharded_state_dict["optimizer_states"][0]["optimizer"][
                        "param_groups"
                    ] = model_param_groups
                else:
                    sharded_state_dict["optimizer_states"][0][
                        "param_groups"
                    ] = model_param_groups
            else:
                raise ValueError("Cannot find expert param in the checkpoint.")

        loaded_state_dict = self.checkpoint_io.load_checkpoint(
            checkpoint_path, sharded_state_dict=sharded_state_dict
        )
        if expert_index is not None:
            # Remove the temporary empty params added above
            if "optimizer" in loaded_state_dict["optimizer_states"][0]:
                loaded_state_dict["optimizer_states"][0]["optimizer"][
                    "param_groups"
                ].pop(expert_index)
            else:
                loaded_state_dict["optimizer_states"][0]["param_groups"].pop(
                    expert_index
                )
        return loaded_state_dict

    def load_checkpoint(
        self, checkpoint_path: Union[str, Path], load_optimizer_states: bool = True
    ) -> Dict[str, Any]:
        ...
        if self.use_distributed_checkpointing:
            ...
            if not fs.isdir(checkpoint_path):
                raise ValueError(
                    f"Distributed checkpoints should be a directory. Found: {checkpoint_path}."
                )
            ...
            if self._check_param_groups_mismatch(checkpoint_path, checkpoint):
                ...
                try:
                    checkpoint = self.checkpoint_io.load_checkpoint(
                        checkpoint_path, sharded_state_dict=checkpoint
                    )
    ...
    def _integrate_original_checkpoint_data(
        ...
        """
        original_checkpoint = (
            self.lightning_module.trainer._checkpoint_connector.dump_checkpoint()
        )
        ...
        if "optimizer" in checkpoint["optimizer_states"][0]:
            checkpoint["optimizer_states"][0]["optimizer"]["param_groups"] = (
                original_checkpoint["optimizer_states"][0]["optimizer"]["param_groups"]
            )
        else:
            checkpoint["optimizer_states"][0]["param_groups"] = original_checkpoint[
                "optimizer_states"
            ][0]["param_groups"]
    ...
    def use_distributed_checkpointing(self):
        ...
        has_dist_ckpt_io = HAVE_MEGATRON_CORE and isinstance(
            self.unwrapped_checkpoint_io, DistributedCheckpointIO
        )
        ...
        if has_sharded_state_dict and not has_dist_ckpt_io:
            ...
            self.checkpoint_io = DistributedCheckpointIO.from_config(
                self.lightning_module.cfg
            )
...
class NLPFSDPStrategy(FSDPStrategy):
    ...
    def __init__(
        ...
        **kwargs: Union[Any, Dict[str, Any]],
        ...
        assert sharding_strategy in list(
            fsdp_sharding_strategy.keys()
        ), "Not a supported sharding strategy."
        assert (
            sharding_strategy != "hybrid"
        ), "Hybrid sharding is currrently not supported."
    ...
    def _set_mixed_precision_recipe(
        ...
        set_buffer_dtype: Union[int, str],
    ) -> MixedPrecision:
        """
        ...
        if precision in ["16-true", "16-mixed", 16]:
            ...
            raise ValueError(
                f"Was unable to infer precision type, received {precision!r}."
            )
        # Over-write gradient reduction dtype to support bf16 computation with fp32 grad reduction
        if grad_reduce_dtype is not None:
            reduce_dtype = utils_funcs.torch_dtype_from_precision(
    ...
    def setup_environment(self) -> None:
        ...
        if not parallel_state.model_parallel_is_initialized():
            ...
            ), "FSDP does not support pipeline parallelism"
        ...
        self.kwargs["process_group"] = parallel_state.get_data_parallel_group(
            with_context_parallel=True
        )
    ...
    def load_optimizer_state_dict(self, checkpoint: Mapping[str, Any]) -> None:
        ...
        def _get_osd(opt_state):
            ...
            while True:
                ...
                assert isinstance(
                    temp_opt_state, dict
                ), "Fail to find optimizer state dict."
                ...
                if isinstance(list(temp_osd["state"].keys())[0], int):
                    ...
                    try:
                        with FSDP.summon_full_params(
                            self.model, writeback=True, rank0_only=False
                        ):
                        ...
                        temp_osd = FSDP.shard_full_optim_state_dict(
                            rekeyed_osd, self.model
                        )
...
class NLPSaveRestoreConnector(SaveRestoreConnector):
    ...
    def save_to(self, model, save_path: str):
        ...
        if (
            app_state.model_parallel_size is not None
            and app_state.model_parallel_size > 1
        ) or dist_ckpt:
            ...
            if dist_ckpt:
                ...
                dist_ckpt_dir = ckpt_to_dir(
                    os.path.join(dir_name, self.model_weights_ckpt)
                )
                ...
                checkpoint_io = DistributedCheckpointIO.from_config(
                    model.cfg, async_save=False
                )
                ...
                if app_state.data_parallel_rank == 0:
                    if app_state.pipeline_model_parallel_size == 1:
                        mp_model_weights = os.path.join(
                            dir_name,
                            f"mp_rank_{app_state.tensor_model_parallel_rank:02d}_"
                ...
                with tempfile.TemporaryDirectory() as tmpdir:
                    if dist_ckpt:
                        ...
                        for tp_rank in range(app_state.tensor_model_parallel_size):
                            ...
                            mp_model_weights = os.path.join(
                                dir_name,
                                f"mp_rank_{tp_rank:02d}_" + self.model_weights_ckpt,
                            )
                            shutil.move(
                                mp_model_weights,
                                os.path.join(
                        ...
                        for tp_rank, pp_rank in itertools.product(
                            ...
                            mp_model_weights = os.path.join(
                                dir_name,
                                f"tp_rank_{tp_rank:02d}_pp_rank_{pp_rank:03d}_"
                                + self.model_weights_ckpt,
                            )
                            shutil.move(
                                mp_model_weights,
    ...
    def modify_state_dict(self, conf, state_dict):
        ...
        if conf.get("megatron_legacy", False):
            ...
            for key in state_dict.keys():
                new_key = key.replace(
                    "bert_model.language_model", "bert_model.model.language_model"
                )
        ...
        if (
            ...
            for key_ in state_dict.keys():
                if (
                    key_
                    == "model.cond_stage_model.transformer.text_model.embeddings.position_ids"
                ):
                    continue
        ...
        if (
            conf.get("unet_config")
            and conf.get("unet_config").get("use_te_fp8") == False
        ):
        ...
        if state_dict is None:
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                if self.model_extracted_dir is not None and os.path.isdir(
                    ...
                    logging.info(
                        f"Restoration will occur within pre-extracted directory : "
                ...
                if HAVE_MODELOPT and hasattr(instance, "get_model_module_list"):
                    restore_sharded_modelopt_state(
                        instance.get_model_module_list(),
                        tmp_model_weights_dir,
                        prefix="model.",
                    )
...
class GradScaler(torch.cuda.amp.GradScaler):
    ...
    def _lazy_init_scale_growth_tracker(self, dev):
        ...
        if HAVE_AMP_C:
            self._hysteresis_tracker = torch.tensor(
                [self.hysteresis], dtype=torch.int32, device=dev
            )
        else:
...
class GlobalBatchDataFetcher(_DataFetcher):
    ...
    def __init__(
        self, prefetch_batches: int = 0, store_on_device: bool = False
    ) -> None:

        if not HAVE_APEX:
            logging.warning(
                "Apex was not found. Using model parallel or megatron models will error out."
            )
        if not HAVE_MEGATRON_CORE:
            logging.warning(
                "Megatron-core was not found. Using model parallel or megatron models will error out.."
            )

        super().__init__(
            prefetch_batches=prefetch_batches, store_on_device=store_on_device
        )
