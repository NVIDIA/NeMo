...
class DecEmb(torch.nn.Module, Exportable):
    ...
    def __init__(self, decoder_embedding, decoder, device, num_layers):
    ...
    def forward(self, input_ids, decoder_mask, encoder_mask, encoder_embeddings, decoder_mems):
        ...
        zeros = torch.zeros((decoder_mems.shape[0], self.num_layers - 1, dec_out.shape[1], decoder_mems.shape[-1])).to(self.device)
    ...
    def input_example(self, max_batch=1, max_dim=768, seq_len=6):
        ...
        decoder_mems = torch.zeros([max_batch, self.num_layers, seq_len, max_dim], dtype=torch.float32).to(self.device)
