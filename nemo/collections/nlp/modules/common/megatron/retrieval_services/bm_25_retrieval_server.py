...
def get_relevant_context(embed_text, dpr_group, context_retriever_group, topk_count=1, two_retriever=True):
    ...
    if two_retriever:
        ...
        values, indices = get_similar_idx_val(embed_text, dpr_group, topk_count,
            context_retriever_group, True, bm25)
    else:
        values, indices = get_similar_idx_val(embed_text, dpr_group, topk_count,
            context_retriever_group)
...
def add_retrieve_args(parser):
    ...
    group.add_argument('--car-manual', type=str, 
                        help='Path to the car manual')
    group.add_argument('--question-answer', type=str,
                        help='Path to the question-answer dataset')
    group.add_argument('--predict-ground-path', type=str,
                        help='Path to write the predictions')
    group.add_argument("--sliding-window-length", type=int, default=368,
                        help='Sliding window length for context')
    group.add_argument("--sliding-length", type=int, default=64,
                        help='Sliding length for context')
    group.add_argument("--few-shot-qa", type=int, default=24,
                        help='Few shot samples QA to use from the retriever')
    group.add_argument('--question-answer-splits', type=str, default='80, 20',
                       help='Comma-separated list of proportions for training,'
                       ' and test split.')
    group.add_argument("--max-answer-length", type=int, default=128,
                        help='max length of the generated answer.')
    group.add_argument("--top_k", type=int, default=0,
                        help='Top k sampling.')
    group.add_argument("--top_p", type=float, default=0.0,
                        help='Top p sampling.')
    group.add_argument("--beam_size", type=int, default=0,
                        help='beam size.')
    group.add_argument("--temperature", type=float, default=1.0,
                        help='Sampling temperature.')
    group.add_argument('--retriever-qa-type', type=str, default='random',
                       choices=['similar', 'random'],
                       help='Use highest or random retrieved documents')
    group.add_argument('--megatron-api-url', type=str,
                       default='http://10.14.74.235:5000/api',
                       help='url of the megatron api')
    group.add_argument('--use-megatron-api', action='store_true',
                       default=None, help='Use megatron api of 530B model')
    group.add_argument('--retriever-context-type', type=str, default='groundtruth',
                       choices=['groundtruth', 'knowledgebase', 'blendcontext'],
                       help='What is the sources of context to use')
    group.add_argument('--nctx', type=int, default=1,
                       help='number of ctx documents')
    group.add_argument('--use-context-title', action='store_true',
                       default=None, help='Use context title')
    group.add_argument('--header-context', type=str,
                       default='',
                       help='Additional contexts to use for the headings')
    group.add_argument('--mode', type=str,
                       default='single',
                       help='dpr model mode')
    group.add_argument('--prefix-question-with-prompt', action='store_true',
                       default=None, help='prefix question with prompt or not')
    group.add_argument('--prefix-idx', type=int, default=0,
                       help='prefix index to choose prompt')
    group.add_argument('--prefix-with-prompt', action='store_true',
                       default=None, help='prefix question with prompt or not')
    group.add_argument('--prefix-ending', action='store_true',
                       default=None, help='prefix at the end or not')
    group.add_argument('--add-title-to-retriever', action='store_true',
                       default=None, help='if we should add title to retriever')
    group.add_argument('--two-retriever', action='store_true',
                       default=None, help='if we should use two retriever')
    group.add_argument('--cqa', action='store_true',
                       default=None, help='if we should the format of cqa or not')
    group.add_argument('--output-dir', type=str,
                       default=None, help='output dir to write')
    return parser
        ...
        for idx, data_instance in enumerate(retriever_dataset):
            ...
            if len(embed_ids) > args.sliding_window_length:
                ...
                while True:
                    ...
                    retriever_embeddings = torch.cat((retriever_embeddings, \
                        embed_emb), dim=0) if idx > 0 else embed_emb
...
def test(query):
    ...
    context_retriever_group = get_retriever_embeddings(
        args, context_retriever_dataset, dpr_group, context_titles)

    question = query
    relevant_context_and_title = get_relevant_context(question,
                    bert_group, context_retriever_group, topk_count=10, two_retriever=args.two_retriever)
...
class BM25RetrievalServer(object):
    ...
    def __init__(
        ...
        self.context_retriever_group = get_retriever_embeddings(args, self.context_retriever_dataset, self.dpr_group, context_titles)
        api = Api(self.app)
        api.add_resource(
