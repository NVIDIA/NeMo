...
def get_iterator_k_split(batch: List[torch.Tensor], num_microbatches: int) -> Iterator:
    if isinstance(batch, dict):
        ...
        split_batch = [torch.tensor_split(item, num_microbatches, dim=0) if torch.is_tensor(item) else item for item in batch]
        microbatches = [[elem[i] if torch.is_tensor(elem) else elem for elem in split_batch] for i in range(num_microbatches)]
