...
class MegatronTokenLevelEncoderDecoderModule(
    MegatronModule, adapter_mixins.AdapterModuleMixin
):
    ...
    def __init__(
        ...
        if add_encoder:
            if pre_process:
                self.encoder_embedding = Embedding(
                    ...
                    position_embedding_type=encoder_cfg.get(
                        "position_embedding_type", "learned_absolute"
                    ),
                )
                self._encoder_embedding_key = "encoder_embedding"
            if (
                self.encoder_cfg.get("position_embedding_type", "learned_absolute")
                == "relative"
            ):
                ...
                self._encoder_relative_position_embedding_key = (
                ...
                self.encoder_relative_position_embedding = (
                ...
                self._encoder_relative_position_embedding_key = (
                    "encoder_alibi_position_embedding"
                )
            elif (
                self.encoder_cfg.get("position_embedding_type", "learned_absolute")
                == "kerple"
                ...
                self._encoder_relative_position_embedding_key = (
                    "encoder_kerple_position_embedding"
                )
            else:
            ...
            if encoder_cfg.get("use_flash_attention", False) and encoder_cfg.get(
                ...
                raise ValueError(
                    "flash-attention not supported with relative or kerple at this point"
                )
            ...
            encoder = get_encoder_model(
                ...
                num_attention_heads=encoder_cfg.num_attention_heads,
                apply_query_key_layer_scaling=encoder_cfg.get(
                    "apply_query_key_layer_scaling", True
                ),
                kv_channels=encoder_kv_channels,
                init_method=init_method_normal(
                    encoder_cfg.get("init_method_std", 0.02)
                ),
                ...
                precision=precision,
                fp32_residual_connection=encoder_cfg.get(
                    "fp32_residual_connection", False
                ),
                activations_checkpoint_method=encoder_cfg.get(
                    "activations_checkpoint_method", None
                ),
                activations_checkpoint_num_layers=encoder_cfg.get(
                    "activations_checkpoint_num_layers", 1
                ),
                activations_checkpoint_granularity=encoder_cfg.get(
                ...
                bias_dropout_add_fusion=encoder_cfg.get(
        ...
        if add_decoder:
            ...
            if pre_process:
                ...
                if hasattr(self, "encoder_embedding") and share_token_embeddings:
                    ...
                    self.decoder_embedding = Embedding(
                        ...
                        num_tokentypes=num_tokentypes,
                        embedding_dropout_prob=embedding_dropout,
                        position_embedding_type=decoder_cfg.get(
                            "position_embedding_type", "learned_absolute"
                        ),
                    )
            ...
            if (
                self.decoder_cfg.get("position_embedding_type", "learned_absolute")
                == "relative"
            ):
                ...
                self._decoder_relative_position_embedding_key = (
                ...
                if not self.decoder_cfg.relative_position_bias_self_attention_only:
                    ...
                    if (
                        ...
                        self.decoder_cross_attention_relative_position_embeddings_weight().data.fill_(
                            0
                        )
                        self.decoder_cross_attention_relative_position_embeddings_weight().shared = (
                            True
                        )
                ...
                self._decoder_relative_position_embedding_key = (
                    "decoder_alibi_position_embedding"
                )
            elif (
                self.decoder_cfg.get("position_embedding_type", "learned_absolute")
                == "kerple"
                ...
                self._decoder_relative_position_embedding_key = (
                    "decoder_kerple_position_embedding"
                )
            else:
            ...
            decoder = get_decoder_model(
                ...
                num_attention_heads=decoder_cfg.num_attention_heads,
                apply_query_key_layer_scaling=decoder_cfg.get(
                    "apply_query_key_layer_scaling", True
                ),
                kv_channels=decoder_kv_channels,
                init_method=init_method_normal(
                    decoder_cfg.get("init_method_std", 0.02)
                ),
                scaled_init_method=scaled_init_method_normal(
                ...
                precision=precision,
                fp32_residual_connection=decoder_cfg.get(
                    "fp32_residual_connection", False
                ),
                activations_checkpoint_method=decoder_cfg.get(
                    "activations_checkpoint_method", None
                ),
                activations_checkpoint_num_layers=decoder_cfg.get(
                    "activations_checkpoint_num_layers", 1
                ),
                activations_checkpoint_granularity=decoder_cfg.get(
                    "activations_checkpoint_granularity", None
                ),
                ...
                bias_dropout_add_fusion=decoder_cfg.get(
                    "bias_dropout_add_fusion", True
                ),
    ...
    def _validate_perceiver_config(self, cfg):
        if (
            ...
            raise ValueError(
                f"Perceivers with relative position embeddings are not supported"
            )
        ...
        # Only need to run encoder embedding and position ids if enc_input or enc_output is not provided.
            ...
            if self.pre_process and self.add_encoder:
                ...
                if self.is_adapter_available():
                    ...
                    ptuning_adapter = self.get_adapter_module(
        ...
        if output_enc_hidden_only:
            ...
            if self.pre_process and self.add_decoder:
                ...
                dec_input = self.decoder_embedding(
                    dec_input_ids, dec_position_ids, token_type_ids=token_type_ids
                )
            else:
            ...
            if (
                self.add_decoder
                and self.decoder_relative_position_embedding is not None
            ):
                decoder_self_attention_relative_position_bias = (
                    self.decoder_relative_position_embedding(
            ...
            if self.post_process and self.add_decoder:
                dec_output, enc_output = (
                    output  # [s, b, h], enc_output might be a dict if hiddens_module is used
                )
                ...
                if self.share_decoder_tokens_head_embeddings:
                    token_logits = self.tokens_head(
                        dec_output, self.word_embeddings_weight()
                    )
                else:
                ...
                if labels is not None:
                    ...
                    if self.fp16_cross_entropy:
                        ...
                        tokens_loss = vocab_parallel_cross_entropy(
                            token_logits, labels, label_smoothing
                        )
                    else:
                        tokens_loss = vocab_parallel_cross_entropy(
                            token_logits.float(), labels, label_smoothing
                        )
    ...
    def state_dict_for_save_checkpoint(
        ...
        state_dict_[self._decoder_embedding_key] = (
            self.decoder_embedding.state_dict_for_save_checkpoint(
                destination, prefix, keep_vars
            )
        )
        state_dict_[self._enc_dec_model_key] = (
            self.enc_dec_model.state_dict_for_save_checkpoint(
                destination, prefix, keep_vars
            )
        )
    ...
    def load_state_dict(self, state_dict, strict=True):
        """Customized load."""
        self.encoder_embedding.load_state_dict(
            state_dict[self._encoder_embedding_key], strict=strict
        )
        self.decoder_embedding.load_state_dict(
            state_dict[self._decoder_embedding_key], strict=strict
        )
        self.enc_dec_model.load_state_dict(
            state_dict[self._enc_dec_model_key], strict=strict
        )
...
class MegatronTokenLevelEncoderDecoderSpeechLLMModule(
    MegatronTokenLevelEncoderDecoderModule
):
    def __init__(self, *args, **kwargs):
        super(MegatronTokenLevelEncoderDecoderSpeechLLMModule, self).__init__(
            *args, **kwargs
        )
        ...
        if dec_input_ids.dim() <= 2:
            ...
            for i in range(dec_input_ids.size()[1]):
                if i == 0:
                    ...
                    current = self.speech_tokens_embeddings[i - 1](
                        dec_input_ids[:, i, :]
                    ).permute(1, 0, 2)
        ...
        if output_enc_hidden_only:
            ...
            if self.pre_process and self.add_decoder:
                ...
                if self.decoder_relative_position_embedding is None:
                    dec_input_ids_p = (
                        dec_input_ids[:, 0, :]
                        if dec_input_ids.dim() == 3
                        else dec_input_ids
                    )
                    dec_position_ids = build_position_ids(dec_input_ids_p)
                else:
                    dec_position_ids = None
                dec_input = self.get_decoder_embeddings(
                    dec_input_ids, dec_position_ids, token_type_ids
                )
                if not set_inference_key_value_memory and (
                    decoder_max_sequence_len or encoder_max_sequence_len
                ):
                    # In inference
            ...
            for _cross_attention_prior in cross_attention_prior:
                ...
                if _cross_attention_prior is not None:
                    ...
                    attn_prior_scaledown_start_step = (
                        self.attn_prior_scaledown_start_step
                    )
                    ...
                    if global_step >= attn_prior_end_step:
                        ...
                        global_step > attn_prior_scaledown_start_step
                        and global_step < attn_prior_end_step
                    ):
                        total_annealing_steps = (
                            attn_prior_end_step - attn_prior_scaledown_start_step
                        )
                        curr_annealing_step = (
                            global_step - attn_prior_scaledown_start_step
                        )
                        curr_cross_attention_prior = _cross_attention_prior + (
                            (1.0 - _cross_attention_prior)
                            * curr_annealing_step
                            / total_annealing_steps
                        )
                        _decoder_cross_attention_relative_position_bias = (
                            curr_cross_attention_prior.unsqueeze(1).repeat(
                        ...
                        _decoder_cross_attention_relative_position_bias = (
                            _cross_attention_prior.unsqueeze(1).repeat(
                                1, num_attention_heads, 1, 1
                            )
                        )
                        _decoder_cross_attention_relative_position_bias = torch.log(
                ...
                decoder_cross_attention_relative_position_bias.append(
                    _decoder_cross_attention_relative_position_bias
                )

            return_all_crossattention_probs = (
                return_all_crossattention_probs or self.logging_step
            )

            if single_encoder:
                decoder_cross_attention_relative_position_bias = (
                    decoder_cross_attention_relative_position_bias[0]
                )
            ...
            if self.post_process and self.add_decoder:
                ...
                if return_all_crossattention_probs:
                    ...
                    if (
                        text_limits is not None
                        and self.use_alignment_loss
                        and hasattr(self, "forward_sum_loss")
                    ):
                        attention_scores_filtered = [
                            attention_scores[lidx]
                            for lidx in self.alignment_decoder_layerids
                        ]
                        attention_scores_combined = torch.cat(
                            attention_scores_filtered, dim=1
                        )
                        ...
                        attention_logprobs = torch.mean(
                            attention_logprobs, dim=1, keepdim=True
                        )
                        ...
                        alignment_loss = self.forward_sum_loss(
                            attn_logprob=attention_logprobs,
                            in_lens=enc_len,
                            out_lens=dec_len,
                        )
                else:
                ...
                if labels is not None:
                    ...
                    if self.fp16_cross_entropy:
                        ...
                        if labels.dim() == 3:
                            raise NotImplementedError(
                                "fp16_cross_entropy is not support for labels of dimension 3"
                            )
                        tokens_loss = vocab_parallel_cross_entropy(
                            token_logits, labels, label_smoothing
                        )
                        ...
                        if labels.dim() == 2:
                            ...
                            logging.debug(
                                f"token_loss: {torch.all(torch.isfinite(tokens_loss))}"
                            )
                            if (
                                self.seq_pattern in ["parallel", "delay_parallel"]
                                and torch.count_nonzero(speech_mask) > 0
                                ...
                                for i in range(speech_layers):
                                    if (
                                        speech_logits_list[i].size()[0]
                                        != labels[i + 1, :, :].size()[0]
                                    ):
                                        raise Exception("TODO: add a permute")
                                    curr_codebook_loss = (
                                        vocab_parallel_cross_entropy(
                                            speech_logits_list[i].float(),
                                            labels[i + 1, :, :],
                                            label_smoothing,
                                        )
                                        * speech_mask.T
                                    ...
                                    logging.debug(
                                        f"token_loss_{i}: {torch.all(torch.isfinite(tokens_loss))}"
                                    )
                    ...
                    if self.hiddens_cfg is not None:
                        raise NotImplementedError(
                            "Not currently implemented for speechllm"
                        )
                    else:
                        return tokens_loss, [
                            token_logits,
                    ...
                    token_logits = token_logits.transpose(
                        0, 1
                    ).contiguous()  # (B, T, 30208)
                    speech_logits = torch.stack(
                        speech_logits_list, dim=-1
                    )  # T, B, 1024, 7
                    speech_logits = speech_logits.transpose(
                        0, 1
                    ).contiguous()  # (B, T, 1024, 7)

                    _si = self.speech_offset
                    _ei = _si + self.speech_codebook_size
                    first_layer_speech_logits = token_logits[:, :, _si:_ei].unsqueeze(
                        -1
                    )  # (b, s, 1023, 1)
    ...
    def state_dict(self):
        ...
        if hasattr(self, "speech_tokens_embeddings"):
            state_dict_["speech_tokens_embeddings"] = (
                self.speech_tokens_embeddings.state_dict()
            )
        return state_dict_
    ...
    def load_state_dict(self, state_dict, strict=True):
        ...
        if hasattr(self, "speech_tokens_heads"):
            self.speech_tokens_heads.load_state_dict(
                state_dict["speech_tokens_heads"], strict=strict
            )
        if hasattr(self, "speech_tokens_embeddings"):
            self.speech_tokens_embeddings.load_state_dict(
                state_dict["speech_tokens_embeddings"], strict=strict
            )
