...
class RetroModelTextGenerationStrategy(TextGenerationStrategy):
    def __init__(self, model, **args):
        ...
        self.service = ComboRetrievalService(
            ...
            service_ip=combo_cfg["service_ip"],
            service_port=combo_cfg["service_port"],
        )
        self.retrieved = []
        self.retrieved_text = []
    ...
    def tokenize_batch_with_context_and_completion(self, sentences, max_len, add_BOS):
        ...
        if add_BOS:
            context_tokens = [
                [
                    [tokenizer.bos_id] + tokenizer.text_to_ids(s[0]),
                    tokenizer.text_to_ids(s[1]),
                ]
                for s in sentences
            ]
        else:
            context_tokens = [
                [tokenizer.text_to_ids(s[0]), tokenizer.text_to_ids(s[1])]
                for s in sentences
            ]
        if self.pad_token_for_retrieval:
            padded = []
    ...
    def prepare_batch_at_step(
        ...
        if step == 0:
            ...
            positions2use = self.position_ids[:, context_length - 1].view(
                micro_batch_size, -1
            )
            # not using type2use. uncomment it if it is used
            # if type_ids is not None:
        ...
        retrieved = torch.tensor(
            np.array(self.retrieved), device=torch.cuda.current_device()
        )
        ...
        if retrieved.numel() == 0:
            ...
            retrieved = (
                torch.tensor(
                    self.service.get_knn(["a"], 0), device=torch.cuda.current_device()
                )
                .unsqueeze(0)
        ...
        if self.neighbors == 0:
            ...
            neighbors_array = torch.tensor(
                [1] * micro_batch_size, device=torch.cuda.current_device()
            )
        else:
            neighbors_array = torch.tensor(
                [self.neighbors] * micro_batch_size, device=torch.cuda.current_device()
            )
...
class RetroQAModelTextGenerationStrategy(RetroModelTextGenerationStrategy):
    def tokenize_batch(self, questions, max_len, add_BOS):
        ...
        context_tokens = [
            ...
            for n, q in zip(neighbor_tokens, questions)
        ]
        ...
        if self.pad_token_for_retrieval:
            ...
            for line in context_tokens:
                pad_len = (
                    self.chunk_size - len(line) % self.chunk_size
                ) % self.chunk_size
                if self.megatron_lm_compatible:
                    padded.append([tokenizer.eos_id] * pad_len + line)
                else:
...
class RetroFileQAModelTextGenerationStrategy(RetroQAModelTextGenerationStrategy):
    ...
    def tokenize_batch(self, questions, max_len, add_BOS):
        ...
        all_lookups = (
            np.array(chunks).reshape(1, self.neighbors + 1, -1).astype(np.int64)
        )
        reuse_neighbors = all_lookups[:, 1:]
        self.store.set("reuse_neighbors", pickle.dumps(reuse_neighbors))
        # combine question and context
        context_tokens = [
            ...
            for n, q in zip(first_context, questions)
        ]
