...
class MegatronGenerate(Resource):
    ...
    def chat_completion(self, data):
        ...
        logging.info("Log for Timing -- before lock {dm1}")
        with lock:  # Need to get lock to keep multiple threads from hitting code
            logging.info("Log for Timing -- inside lock {dm1}")
            MegatronGenerate.send_do_generate()  # Tell other ranks we're doing generate
            logging.info("Log for Timing -- after send_to_generate {dm1}")
            extra = {}
            if self.inference_strategy is not None:
                extra['strategy'] = self.inference_strategy
            ...
            logging.info("Log for Timing -- before generate {dm1}")
            output = generate(
                self.model,
                [conversation],
            ...
            logging.info("Log for Timing -- after generate {dm1}")
            for k in output:
                if isinstance(output[k], torch.Tensor):
                    output[k] = output[k].tolist()
        ...
        logging.info("Log for Timing -- returning generation {dm1}")
        return jsonify(
            {
    ...
    def post(self):
        ...
        if request.endpoint == "oai_completions":
            ...
            data = request.get_json()
            dm1 = data["messages"][-1]
            logging.info("Log for Timing -- entered post method {dm1}")
