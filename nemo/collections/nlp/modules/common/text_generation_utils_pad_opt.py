...
def right_pad(context_tokens_tensor, eos_id, pad_id):
    ...
    for idx, row in enumerate(context_tokens_tensor):
        ...
        # Place the truncated_row at the end of the corresponding row in the padded_tensor
        padded_tensor[idx, -len(truncated_row):] = truncated_row
            ...
            if parallel_state.is_pipeline_last_stage():
                ...
                if compute_logprob:
                    if output_logits is None:
                        ...
                        if all_probs:
                            full_logits = torch.unsqueeze(output[:,-1,:],1).contiguous()
                    else:
                        ...
                        if all_probs:
                            print('Kunal: ERROR! Shouldnt come here ideally')
                            full_logits = output
