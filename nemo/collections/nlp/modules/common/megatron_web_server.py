    ...
    with gr.Blocks(css=CSS) as demo:
        with gr.Row():
            with gr.Column(scale=2, width=200):
                ...
                token_to_gen = gr.Number(
                    label="Number of Tokens to generate", value=300, type=int
                )
                min_token_to_gen = gr.Number(
                    label="Min number of Tokens to generate", value=1, type=int
                )
                ...
                sampling_method = gr.Dropdown(
                    list(PRESETS.keys()),
                    label="Sampling Presets",
                    default="K50",
                    value="K50",
                )
                temperature = gr.Slider(
                    minimum=0.0, maximum=5.0, value=0.75, label="Temperature", step=0.1
                )
                top_p = gr.Slider(
                    minimum=0.0, maximum=1.0, step=0.02, value=0.95, label="Top P"
                )
                top_k = gr.Slider(
                    minimum=0, maximum=1024, step=2, value=50, label="Top K"
                )
                ...
                sampling_method.change(
                    ...
                    outputs=[temperature, top_p, top_k, repetition_penality],
                )
        ...
        with gr.Row():
            with gr.Column(scale=2, width=200):
                ...
                def set_sampling(x):
                    if x == "Custom":
                        values = [
                            gr.update(value=v, interactive=True)
                            for v in PRESETS[x].values()
                        ]
                        return values
                    else:
                        values = [
                            gr.update(value=v, interactive=False)
                            for v in PRESETS[x].values()
                        ]
                        return values
                ...
                prompt_presets.change(
                    set_prompt,
                    inputs=[prompt_presets],
                    outputs=[human_name, assistant_name],
                )
            ...
            with gr.Column(scale=1, min_width=900):
                    ...
                    *values,
                    ...
                    for i, meg in enumerate(session_state):
                        ...
                        prompt_text += (
                            turn
                            + name
                            + prompt_preset["END_OF_NAME"]
                            + meg
                            + prompt_preset["END_OF_TURN"]
                        )
                    prompt_text += (
                        prompt_preset["BOT_TURN_TOKEN"]
                        + assistant_name
                    ...
                    prompt_text = (
                        prompt_preset["SYSTEM_TURN_TOKEN"] + preamble + prompt_text
                    )
                ...
                msg.submit(
                    user, [msg, chatbot, session_state], [msg, chatbot], queue=False
                ).then(
                    bot,
                    [
                        chatbot,
...
class RetroDemoWebApp:
    def __init__(
        self, text_service_ip, text_service_port, combo_service_ip, combo_service_port
    ):
    ...
    def get_retro_generation(
        ...
        output_json = text_generation(
            data, self.text_service_ip, self.text_service_port
        )
    ...
    def run_demo(self, share, username, password, port):
        check_gradio_import()
        with gr.Blocks(
            css="table, th, td { border: 1px solid blue; table-layout: fixed; width: 100%; }"
        ) as demo:
            with gr.Row():
                with gr.Column(scale=2, width=200):
                    ...
                    add_BOS = gr.Checkbox(label="Add BOS token", value=False)
                    token_to_gen = gr.Number(
                        label="Number of Tokens to generate", value=30, type=int
                    )
                    min_token_to_gen = gr.Number(
                        label="Min number of Tokens to generate", value=1, type=int
                    )
                    ...
                    top_p = gr.Slider(
                        minimum=0.0, maximum=1.0, step=0.02, value=0.9, label="Top P"
                    )
                    top_k = gr.Slider(
                        minimum=0, maximum=10000, step=2, value=0, label="Top K"
                    )
                    repetition_penality = gr.Slider(
                        minimum=1.0,
                        maximum=5.0,
                    ...
                    weight = gr.Slider(
                        ...
                        maximum=1.0,
                        value=1.0,
                        label="Weight for the Static Retrieval DB",
                        step=0.02,
                    )
                    ...
                    add_EOS = gr.Checkbox(
                        label="Add EOS token to Retrieval Doc", value=False
                    )
                    with gr.Row():
                    ...
                    add_btn.click(
                        self.add_doc,
                        inputs=[add_retrival_doc, add_EOS],
                        outputs=[output_status],
                    )
                    reset_btn.click(
                        self.reset_index, inputs=[], outputs=[output_status]
                    )
