...
class TextGenerationStrategy:
    ...
    def forward_step(self, batch, tensor_shape):
        ...
        output_tensor = fwd_bwd_function(
            ...
            model=(
                [self.forward_model]
                if not isinstance(self.forward_model, list)
    ...
    def prepare_batch_at_step(
        ...
        if compute_attention_mask:
            attention_mask_repeat = torch.concat(
                [self.attention_mask for _ in range(micro_batch_size)]
            )
...
def neva_process_prompts(
    prompt, tokenizer, multimodal_cfg, num_media_latents, conv_template
):
    ...
    if multimodal_cfg["conv_template"] in ["nvgpt", "nv_steerlm", "nv_dpo"]:
        ...
        if list_data_dict[0]["conversations"][0]["value"].find("video") == -1:
            if (
                multimodal_cfg.get("media_type") is not None
                and multimodal_cfg.get("num_frames") is not None
            ):
        ...
        data_dict = preprocess_llama_2(
            sources, tokenizer, multimodal_cfg, is_mistral=True
        )
        ...
        raise ValueError(
            f"Conversation template `{conv_template}` is not supported in Neva now."
        )
...
class NevaModelTextGenerationStrategy(TextGenerationStrategy):
    def __init__(self, model):
        ...
        self.multimodal_cfg = dict(
            ...
            mm_mlp_adapter_type=getattr(
                self.cfg.mm_cfg, "mm_mlp_adapter_type", "linear"
            ),
            use_lita=getattr(self.cfg.mm_cfg, "use_lita", False),
        )
        if self.multimodal_cfg["crop_size"] is None:
            image_processor = CLIPImageProcessor.from_pretrained(
                self.cfg.mm_cfg.vision_encoder.from_pretrained,
        ...
        if self.multimodal_cfg["use_lita"]:
            if self.cfg.mm_cfg.get("lita"):
                lita = {
                    "lita_video_arch": getattr(
                        self.cfg.mm_cfg.lita, "lita_video_arch", "temporal_spatial_pool"
                    ),
                    "visual_token_format": getattr(
                        self.cfg.mm_cfg.lita, "visual_token_format", "v1"
                    ),
    ...
    def tokenize_batch(self, prompt, max_len, add_BOS):
        ...
        context_tokens, context_lengths = pad_batch(
            context_tokens, self.tokenizer.eos_id, max_len
        )
        context_tokens_tensor = torch.cuda.LongTensor(context_tokens)
        context_length_tensor = torch.cuda.LongTensor(context_lengths)
        return context_tokens_tensor, context_length_tensor
...
class PromptLearningModelTextGenerationStrategy(TextGenerationStrategy):
    ...
    def prepare_batch_at_step(
        ...
        tensor_shape = [
            tokens2use.shape[1],
            micro_batch_size,
            self.model.frozen_model.cfg.hidden_size,
        ]
        return batch, tensor_shape
...
class McoreRetroModelTextGenerationStrategy(TextGenerationStrategy):
    ...
    def tokenize_batch(self, sentences, max_len, add_BOS):
        ...
        context_tokens, context_lengths = pad_batch(
            batch=context_tokens, pad_id=tokenizer.eos_id, max_len=0
        )
    ...
    def tokenize_neighbors_batch(self, neighbors, retro_args):
        ...
        for i in range(len(neighbors)):
            ...
            for neighbour_tokens in valid_onesample_neighbours_tokens:
                if len(neighbour_tokens) >= r:
                    ...
                    padded_onesample_neighbour_tokens = neighbour_tokens + [
                        tokenizer.eos_id
                    ] * (r - len(neighbour_tokens))
                padded_valid_onesample_neighbours_tokens.append(
                    padded_onesample_neighbour_tokens
                )
            ...
            # append to batch
            padded_valid_neighbours_tokens.append(
                padded_valid_onesample_neighbours_tokens
            )
        ...
        padded_valid_neighbours_tokens_shape = torch.cuda.LongTensor(
            padded_valid_neighbours_tokens.shape
        )
    ...
    def init_batch(
        ...
        **extra,
        ...
        assert (
            bs == 1
        )  # similar to M-LM RETRO inference code, currently only support batch_size=1
        context_tokens = [
            context_tokens[0].tolist()
            + [self.model.tokenizer.eos_id] * context_tokens_length
        ]
        context_tokens = torch.cuda.LongTensor(context_tokens)
        self.model.model.config.retro_gpt_chunk_length = (
            context_tokens_length  # set RetroConfig of M-LM's RETRO model
        )
        ...
        self.forward_model.config.retro_retrieved_length = (
        ...
        for layer in layers:
            if not (
                isinstance(layer.cross_attention, IdentityOp)
            ):  # if this is encoder-decoder cross-attention layer
                # updating RetroDecoder (RetroDecoderCrossAttention, RetroDecoderBiasDropoutAdd)
                layer.cross_attention.retro_num_neighbors = (
                    inference_retro_num_neighbors
                )
                layer.cross_attention.retro_chunk_length = inference_retro_chunk_length
                layer.cross_attention.retro_retrieved_length = (
                    inference_retro_retrieved_length
                )
                ...
                if (
                    contain_encoder
                ):  # the first cross-attention decoder layer contain encoder
                    layer.cross_attention.encoder.layers[
                        0
                    ].cross_attention.retro_num_neighbors = (
                        inference_retro_num_neighbors
                    )
                    layer.cross_attention.encoder.layers[
                        0
                    ...
                    layer.cross_attention.encoder.layers[
                        0
                    ].cross_attention.retro_retrieved_length = (
                        inference_retro_retrieved_length
                    )
                    layer.cross_attention.encoder.layers[
                        0
                    ...
                    layer.cross_attention.encoder.layers[
                        0
                    ].pre_mlp_layernorm.retro_num_neighbors = (
                        inference_retro_num_neighbors
                    )
    ...
    def prepare_batch_at_step(
        ...
        **extra,
        ...
        if compute_attention_mask:
            attention_mask_repeat = torch.concat(
                [attention_mask2use for _ in range(micro_batch_size)]
            )
...
def model_inference_strategy_dispatcher(model, **args):
    ...
    elif isinstance(model, MegatronGPTModel) and not (
        isinstance(model, MegatronRetroModel)
    ):
        return GPTModelTextGenerationStrategy(model)
