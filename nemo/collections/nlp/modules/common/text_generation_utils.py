...
def megatron_gpt_generate(
    model, inputs, tokenizer, length_params, sampling_params, **strategy_args
):
...
def megatron_neva_generate(
    ...
    **strategy_args,
    ...
    for idx, prompt_dict in enumerate(prompt_dict_list):
        ...
        pattern = re.compile(
            rf"{DEFAULT_IM_START_TOKEN[model_type]}( ⁇ )+{DEFAULT_IM_END_TOKEN[model_type]}".replace(
                r"|", r"\|"
            )
        )
        ...
        if use_lita:
            pattern_lita = re.compile(
                rf"{DEFAULT_IM_START_TOKEN[model_type]}(.)+{DEFAULT_IM_END_TOKEN[model_type]}"
            )
            combined_pattern = re.compile(f"{pattern_lita.pattern}")
        else:
            combined_pattern = re.compile(f"{pattern.pattern}|{pattern_nvgpt.pattern}")
        clean_text = re.sub(
            combined_pattern, f"<{media_type_token}>", response["sentences"][0]
        )
        ...
        if torch.cuda.current_device() == 0:
            print(
                f"------------- PROMPT {idx} of {len(prompt_dict_list)} ------------ "
            )
            print(clean_text)
            print()
...
def repetition_penalty(logits, repetition_penalty, used_tokens):
    ...
    if used_tokens is not None and repetition_penalty != 1.0:
        ...
        logits = torch.scatter(
            logits, 1, used_tokens, logits_update / repetition_penalty
        )
    return logits
    ...
    **strategy_args,
    ...
    if torch.distributed.get_rank() == get_model_parallel_src_rank():
        ...
        if "neighbors" in strategy_args:
            # tokenize neighbors
            neighbors_tokens_tensor, neighbors_tokens_tensor_shape = (
                inference_strategy.tokenize_neighbors_batch(
            ...
            torch.distributed.broadcast(
                neighbors_tokens_tensor_shape, src, model_parallel_group
            )
            torch.distributed.broadcast(
                neighbors_tokens_tensor, src, model_parallel_group
            )
        else:
        ...
        if "neighbors" in strategy_args:
            ...
            model_parallel_group = parallel_state.get_model_parallel_group()
            src = get_model_parallel_src_rank()
            neighbors_tokens_tensor_shape = torch.empty(
                2, dtype=torch.float32, device=torch.cuda.current_device()
            )
            torch.distributed.broadcast(
                neighbors_tokens_tensor_shape, src, model_parallel_group
            )
    ...
    if (
        hasattr(model, "get_attention_mask_from_fusion")
        and model.get_attention_mask_from_fusion
    ):
    ...
    with torch.no_grad():
        ...
        if "neighbors_tokens" in extra:  # for Mcore retrieval RETRO model
            ...
            inference_strategy.init_batch(
                context_tokens, context_length, compute_attention_mask
            )
        ...
        while context_length < maxlen:
            ...
            if parallel_state.is_pipeline_last_stage(ignore_virtual=True):
                ...
                if output_logits is None:
                    ...
                    indices = torch.unsqueeze(new_tokens, 1).unsqueeze(2)
                    new_output_logits = torch.gather(
                        output_context, 2, indices
                    ).squeeze(2)
...
def sample_token_topk(
    logits, top_k=0, top_p=0.0, temperature=1.0, filter_value=-float("Inf")
):
    """
    Greedy sampling. Returns the token with the highest probability, and corresponding log_prob.
...
def sample_token_topk_beam_search(
    logits: torch.Tensor, beam_size: int = 1, dim: int = -1, log_softmax: bool = True
):
    """
...
def get_sampling_token_fn(
    sampling_method: str, sampling_kwargs: dict
) -> Tuple[Callable, dict]:
    """
    ...
    if sampling_method == "greedy-search":
        ...
        sampling_token_fn = partial(
            sample_token_topk, top_k=top_k, top_p=top_p, temperature=temperature
        )
