...
def set_cat_slot(
    ...
    cat_slot_values: Dict[str, List[str]],
) -> dict:
    """
    ...
    for slot_idx, slot in enumerate(cat_slot_values):
        ...
        if slot_status == STATUS_DONTCARE:
            ...
            value_idx = max(
                tmp, key=lambda k: tmp[k]["cat_slot_value_status"][0].item()
            )
...
def set_noncat_slot(
    ...
    for slot_idx, slot in enumerate(non_cat_slots):
        ...
        if slot_status == STATUS_DONTCARE:
            ...
            ch_start_idx = predictions_value[slot_idx][0]["noncat_alignment_start"][
                tok_start_idx
            ]
            ch_end_idx = predictions_value[slot_idx][0]["noncat_alignment_end"][
                tok_end_idx
            ]
...
def get_predicted_dialog(
    dialog: dict, all_predictions: dict, schemas: object, state_tracker: str
) -> dict:
    ...
    for turn_idx, turn in enumerate(dialog["turns"]):
            ...
            for frame in turn["frames"]:
                ...
                for action in frame["actions"]:
                    if action["slot"] and len(action["values"]) > 0:
                        sys_slots_agg[frame["service"]][action["slot"]] = action[
                            "values"
                        ][0]
        if turn["speaker"] == "USER":
            ...
            for frame in turn["frames"]:
                ...
                state["requested_slots"] = get_requested_slot(
                    predictions=predictions[1], slots=service_schema.slots
                )
...
def get_predicted_intent(predictions: dict, intents: List[str]) -> str:
    ...
    active_intent_id = max(
        predictions, key=lambda k: predictions[k][0]["intent_status"]
    )
...
def get_requested_slot(predictions: dict, slots: List[str]) -> List[str]:
    ...
    active_indices = [
        ...
        if predictions[k][0]["req_slot_status"] > REQ_SLOT_THRESHOLD
    ]
