...
class MegatronSequenceToSequenceDataset(MegatronDataset):
    ...
    def __init__(
        ...
        if not end_index:
            end_index = self._dataset_length(self.src_indexed_dataset) - 1
    ...
    def __getitem__(self, idx):
        ...
        enc_ids, _, enc_seq_length, dec_in_ids, dec_out_ids, dec_seq_length = self.build_tokens_types_from_ids(
            src_tokens, tgt_tokens
        )
        return (enc_ids, enc_seq_length, dec_in_ids, dec_out_ids, dec_seq_length)
    ...
    def build_tokens_types_from_ids(self, src_ids, tgt_ids):
        ...
        # [BOS]
        enc_ids.append(self.encoder_tokenizer.bos_id)
        ...
        # [EOS].
        enc_ids.append(self.encoder_tokenizer.eos_id)
        tokentypes_enc.append(0)

        # B.
        # Not enforcing sequence length checking for target sequences
        # if trg_ids is not None:
        dec_in_ids, dec_out_ids = [self.decoder_tokenizer.bos_id], []
        ...
        dec_out_ids.append(self.decoder_tokenizer.eos_id)

        return enc_ids, tokentypes_enc, len(enc_ids), dec_in_ids, dec_out_ids, len(dec_in_ids)

    def _collate_fn(self, batch):
        """Build collate function for data loader."""
        _, enc_seq_lengths, _, _, dec_seq_lengths = zip(*batch)
        ...
        enc_mask = (enc_ids != self.encoder_tokenizer.pad_id).long()
        dec_mask = (dec_in_ids != self.decoder_tokenizer.pad_id).long()
