...
class DialogueGPTDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        candidate_tokenized_sentences = [self.default_encode(candidate_sentence) for candidate_sentence in candidate_sentences]
        ...
        labels.data = torch.tensor([-100 if i < training_mask_end else labels.data[i] for i in range(len(labels.data))])
        ...
        return input_ids, attn_masks, labels, generate_input_ids, generate_attn_masks, candidate_input_ids, candidate_attn_masks, training_mask_end, utterance_length
