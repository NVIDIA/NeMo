...
class GLUEDataset(Dataset):
    ...
    def __init__(
        ...
        if task_name not in processors:
            raise ValueError(
                f"{task_name} not supported. Choose from {processors.keys()}"
            )
        ...
        if compute_features:
            ...
            if use_cache and os.path.exists(cached_features_file):
                ...
                token_params = {
                    ...
                    "sep_token_extra": (
                        tokenizer.eos_token
                        if "roberta" in tokenizer.name.lower()
                        else None
                    ),
                }
...
class TextToTextGLUEDataset(GLUEDataset):
    ...
    def collate_fn(self, batch):
        ...
        max_enc_query_length = (
            max([len(item) for item in enc_query]) if enc_query else 0
        )
        max_dec_input_length = (
            max([len(item) for item in dec_input]) if dec_input else 0
        )
        ...
        enc_query = [
            item + [self.tokenizer.pad_id] * (max_enc_query_length - len(item))
            for item in enc_query
        ]
        dec_input = [
            item + [self.tokenizer.pad_id] * (max_dec_input_length - len(item))
            for item in dec_input
        ]
        labels = [
            item + [self.tokenizer.pad_id] * (max_label_length - len(item))
            for item in labels
        ]
    ...
    def convert_examples_to_features(self):
        ...
        for ex_index, example in enumerate(self.examples):
            ...
            text_to_text_query = self.processor.get_t5_prompted_query(
