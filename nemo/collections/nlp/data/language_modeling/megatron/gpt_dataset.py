...
def build_dataset(
    ...
    def _build_dataset(current_data_prefix, current_num_samples):
        ...
        indexed_dataset = get_indexed_dataset_(
            current_data_prefix, data_impl, skip_warmup, delay_data_mmap
        )
...
def build_train_valid_test_datasets(
    ...
    if data_impl in ["mock"]:
        ...
        if data_prefix is not None and len(data_prefix) != 0:
            ...
            logging.warning(
                f"Requested data_impl={data_impl}, so ignoring data_prefix setting: {data_prefix}"
            )
    ...
    if isinstance(data_prefix, DictConfig):
        ...
        if cfg.data.splits_string is not None:
            logging.warning(
                cfg.data.splits_string
                + " ignored since data prefix is of type dictionary."
            )
...
def _build_train_valid_test_datasets(
    ...
    delay_data_mmap = cfg.data.get("delay_data_mmap", False)
    indexed_dataset = get_indexed_dataset_(
        data_prefix, data_impl, skip_warmup, delay_data_mmap
    )
...
def get_indexed_dataset_(data_prefix, data_impl, skip_warmup, delay_data_mmap=False):
    ...
    indexed_dataset = make_indexed_dataset(
        data_prefix, data_impl, skip_warmup, delay_data_mmap=delay_data_mmap
    )
    logging.info(
...
class GPTDataset(Dataset):
    def __init__(
        ...
        self.get_attention_mask_from_fusion = cfg.get(
        ...
        self.create_inputs = any(
            [self.reset_position_ids, self.reset_attention_mask, self.eod_mask_loss]
        )
        self.cached_inputs = False
        self.eos_id = tokenizer.eos_id
        self.no_seqlen_plus_one_input_tokens = cfg.data.get(
        ...
        self.exchange_indices_distributed = cfg.data.get(
        ...
        if doc_index_f == doc_index_l:
            ...
            sample_list.append(
                self.indexed_dataset.get(
                    self.doc_idx[doc_index_l], length=offset_l + self.add_extra_token
                )
            )
...
def _build_index_mappings(
        ...
        if (
            ...
            if num_epochs == 1:
                ...
                num_samples_per_epoch = (
                    tokens_per_epoch - add_extra_token
                ) // seq_length
            ...
            doc_idx = _build_doc_idx(
                documents, num_epochs, np_rng, separate_last_epoch, shuffle_documents
            )
    ...
    torch.distributed.all_reduce(
        counts, group=parallel_state.get_data_parallel_group(with_context_parallel=True)
    )
    torch.distributed.all_reduce(
    ...
    if not exchange_indices_distributed or (
        torch.distributed.get_rank() == 0 and using_cached_indices
    ):
        # Load mappings.
        start_time = time.time()
