...
class T5PromptLearningDataset(BasePromptLearningDataset):
    ...
    def load_data(self, dataset):
        ...
        for json_line in tqdm(dataset):
            ...
            if self.add_sentinel_to_input:
                input_ids = input_ids + self.tokenizer.text_to_ids(
            ...
            if len(input_ids) > self.max_seq_length:
                input_ids = self._truncate_input(
                    truncation_field, input_ids, taskname, doc, total_virtual_tokens
                )
            ...
            if answer_field in doc.keys():  # training and validation
                ...
                # a trick to align with the data format in t5 pretraining
                if self.add_sentinel_to_input:
                    answer_text_ids += self.tokenizer.text_to_ids(
                        T5Sentinel.FIRST.value
                    )
    ...
    def collate_fn(self, batch):
        ...
        enc_input, dec_input, labels, loss_mask, enc_mask, dec_mask = (
            self.pad_batch_and_build_loss_mask(enc_input, dec_input, dec_labels)
        )
    ...
    def pad_batch_and_build_loss_mask(self, enc_input, dec_input, dec_labels):
        ...
        if dec_input[0] and dec_labels[0]:
            max_dec_input_length = (
                max([len(item) for item in dec_input]) if dec_input[0] else 0
            )
            max_label_length = (
                max([len(item) for item in dec_labels]) if dec_labels[0] else 0
            )

            loss_mask = [
                ([1] * (len(item))) + ([0] * (max_label_length - len(item)))
                for item in dec_labels
            ]
            dec_input = [
        ...
        max_enc_query_length = (
            max([len(item) for item in enc_input]) if enc_input[0] else 0
        )
        enc_input = [
