...
class T5SpeechLMDataset(BasePromptLearningDataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.speech_offset = speech_offset if speech_offset is not None else 30000
    ...
    def load_data(self, dataset):
        ...
        print(f"---- self.base_data_dir {self.base_data_dir}")
        ...
        for json_line in tqdm(copy_dataset):
            ...
            print(f"_insert_text_in_template input_example {input_dict}")
            ...
            print(f"_insert_virtual_token_placeholders input_example {virtual_tokens}")

            # a trick to align with the data format in t5 pretraining
            ### input_ids = self.tokenizer.text_to_ids(virtual_tokens) + input_ids
            # new
            ...
            print(f"1111 input_ids {input_ids}")

            # Try to truncate input text to fit into the max sequence length
            if len(input_ids) > self.max_seq_length:
                input_ids = self._truncate_input(truncation_field, context_tokens, taskname, doc, total_virtual_tokens)
    ...
    def _get_tokens(self, doc, field, field_data):
        if doc[f"{field}_type"] == 'TEXT':
            ...
            print(f"field: {field} {len(field_tokens)} {field_tokens[0]}")
            ...
            print(f"field: {field} {len(field_tokens)} {type(field_tokens)} {field_tokens[0]}")
        else:
    ...
    def _insert_data_in_template(self, input_example, prompt_template_fields, doc, answer_field):
        ...
        # field_names = input_example.split(' ')
        # print(field_names)
        # print(prompt_template_fields)
        # print(list(out_dict.keys()))
        # out = []
        # for f in field_names:
        #     f = f.strip(' }{')
        #     if f in prompt_template_fields and f in out_dict:
        #         print(f"f = {f}")
        #         if len(out) == 0:
        #             print(f"111 Inside {f} {out_dict[f]}")
        #             out = out_dict[f]
        #         else:
        #             print(f"222 Inside {f} {out_dict[f]}")
        #             out = out + out_dict[f]
