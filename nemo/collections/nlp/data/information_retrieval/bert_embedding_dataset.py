...
class BertEmbeddingDataset(Dataset):
    ...
    def __init__(
        ...
        self.pad_token_id = (
            self.tokenizer.pad_id if self.tokenizer.pad_id else self.tokenizer.eos_id
        )
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            logging.error(
                f"Error while loading example {idx} from dataset {self.file_path}"
            )
            raise e
    ...
    def _process_example(self, example):
        ...
        if self.data_type == "train":
            ...
            if len(example["neg_doc"]) < self.num_hard_negatives:
                ...
                nd = nd + choices(
                    example["neg_doc"],
                    k=self.num_hard_negatives - len(example["neg_doc"]),
                )
            else:
            ...
            assert (
                len(nd) == self.num_hard_negatives
            ), "Error in sampling required number of hard negatives"
    ...
    def _collate_fn(self, batch):
        ...
        attention_mask = [
            self._create_attention_mask2(max_length, len) for len in lengths
        ]
        attention_mask = torch.stack(attention_mask)
        position_ids = [list(range(max_length)) for _ in batch]
        position_ids = torch.LongTensor(position_ids)
        input_ids = torch.LongTensor(
            self._collate_item(input_ids, max_length=max_length)
        )
        lengths = (
            torch.LongTensor(lengths) - 1
        )  # subtract 1 to account for the eos token
