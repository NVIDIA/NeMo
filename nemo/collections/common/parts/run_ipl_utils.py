...
def get_transcribed_names(manifest_filepaths: List[str]) -> List[List[str]]:
    """
    ...
    Example:
        ...
        [
            ["/path/to/transcribed_manifest_1.json"],
            ["/path/to/transcribed_manifest_2.json"]
        ]
    """
    # For manifest_filepath, modify the filenames by prepending 'transcribed_'
    transcribed_manifest_filepaths = []
    for file_path in manifest_filepaths:
        directory, filename = os.path.split(file_path)
        new_filename = f"transcribed_{filename}"
        new_path = os.path.join(directory, new_filename)
        transcribed_manifest_filepaths.append([new_path])

    return transcribed_manifest_filepaths
...
def update_training_sets(
    ...
    """
    updated_manifest_filepaths = get_transcribed_names(updated_manifest_filepaths)
