...
class PER:
    ...
    class Statistics(): 
        def __init__(self) -> None:
            self.punctuation_marks_statistics = pd.DataFrame(0, 
                                                            index = ["Correct", "Deletions", "Insertions", "Substitutions"], 
                                                            columns = PER.punctuation_marks)
            
            self.substitution_statistics = pd.DataFrame(0, 
                                                        index = PER.punctuation_marks, 
                                                        columns = PER.punctuation_marks)
            
            self.punctuation_marks_rates = pd.DataFrame(0, 
                                                        index = ["Correct", "Deletions", "Insertions", "Substitutions", "PER"], 
                                                        columns = PER.punctuation_marks)
            
            self.substitution_rates = pd.DataFrame(0, 
                                                   index = PER.punctuation_marks, 
                                                   columns = PER.punctuation_marks)
        ...
        def compute_rates(self) -> None: 
            ...
            for punctuation_mark in PER.punctuation_marks:         
                ...
                self.punctuation_marks_rates[punctuation_mark] = self.punctuation_marks_statistics[punctuation_mark].div(punctuation_mark_amounts_sum)
                self.substitution_rates[punctuation_mark] = self.substitution_statistics[punctuation_mark].div(punctuation_mark_amounts_sum)

                self.punctuation_marks_rates.loc["PER", punctuation_mark] = (self.punctuation_marks_rates.loc["Deletions", punctuation_mark] +
    ...
    class Sample:
        ...
        def compute_operations_amounts(self):
            ...
            if r_punct_amount + h_punct_amount == 0:
                ...
                for punctuation_mark in PER.punctuation_marks:
                    num_correct_of_punctution_mark = self.statistics.punctuation_marks_statistics.loc['Correct', punctuation_mark]
                    
                    num_substitutions_of_punctuation_mark = self.statistics.substitution_statistics.loc[punctuation_mark].sum()
                    num_substitutions_to_punctuation_mark = self.statistics.substitution_statistics.loc[:, punctuation_mark].sum()
                    
                    num_deletions_of_mark_per_sample = r_tokens.count(punctuation_mark) - (num_correct_of_punctution_mark + num_substitutions_of_punctuation_mark)
                    num_insertions_of_mark_per_sample = h_tokens.count(punctuation_mark) - (num_correct_of_punctution_mark + num_substitutions_to_punctuation_mark)                
                    
                    self.statistics.punctuation_marks_statistics.loc['Deletions', punctuation_mark] = num_deletions_of_mark_per_sample
                    self.statistics.punctuation_marks_statistics.loc['Insertions', punctuation_mark] = num_insertions_of_mark_per_sample
                    self.statistics.punctuation_marks_statistics.loc['Substitutions', punctuation_mark] = num_substitutions_of_punctuation_mark
                                
                return
    
    class Dataset:
        def __init__(self, input_manifest_path: str, 
                 reference_field: str = "text", hypothesis_field: str = "pred_text", 
        ...
        def read_manifest(self):
            with open(self.input_manifest_path, 'r') as manifest:
                ...
                for line in tqdm(lines, desc = "Reading manifest.."):
                    ...
                    sample = PER.Sample(reference = sample[self.reference_field],
        ...
        def write_manifest(self):
            with open(self.output_manifest_path, 'w') as manifest:
                for sample in tqdm(self.samples, desc = "Writing manifest"):
                    sample_dict = {self.reference_field : sample.reference,
        ...
        def print_rates(self):
            ...
            print(tabulate(self.statistics.punctuation_marks_rates, 
...
if __name__ == "__main__":
    ...
    parser.add_argument(
    "--manifest", required=True, type=str, help="Path .json manifest file with ASR predictions saved at `pred_text` field.",
    )
    parser.add_argument(
    "--punctuation_marks", required=True, type=str, help="String of punctuation marks for calculating rates \
    ...
    parser.add_argument(
    "--output_manifest_path", default=None, type=str, help="Path where output .json manifest file \
        with metrics will be saved",
    )
    parser.add_argument(
    "--punctuation_mask", default="[PUNCT]", type=str, help="Mask template for substituting punctuation\
    ...
    per = PER(punctuation_marks = args.punctuation_marks,
              punctuation_mask = args.punctuation_mask)
    
    per_dataset = per.Dataset(input_manifest_path = args.manifest, 
