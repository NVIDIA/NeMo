...
from ..utils.object_detection.rpn import AnchorGenerator, RegionProposalNetwork, RPNHead
...
class FasterRCNN(TrainableNM):
    ...
    def __init__(
        ...
        backbone = detection.backbone_utils.resnet_fpn_backbone('resnet50', pretrained_backbone)
        ...
        #    of the RPN
        self.fg_bg_sampler = det_utils.BalancedPositiveNegativeSampler(rpn_batch_size_per_image, rpn_positive_fraction)
        ...
        self.roi_heads.box_predictor = detection.faster_rcnn.FastRCNNPredictor(in_features, num_classes)
    ...
    def postprocess(self, predictions, image_shapes, original_image_sizes):
        # if self.training:
        #    return result
        result = predictions
    ...
    def compute_loss(self, objectness, pred_bbox_deltas, labels, regression_targets):
        ...
        box_loss = F.l1_loss(
            pred_bbox_deltas[sampled_pos_inds], regression_targets[sampled_pos_inds], reduction="sum",
        ) / (sampled_inds.numel())
    ...
    def forward(self, images, bounding_boxes, targets, num_objects):
        ...
        targets_tuple = [{"boxes": b, "labels": t} for b, t in zip(bboxes_unpadded, targets_unpadded)]
        ...
        proposals, anchors, objectness, pred_bbox_deltas = self.rpn(images, features, targets_tuple)

        # Empty!!! No detections in "training" mode.
        # print("Proposals for image 0: ", len(proposals[0]))

        # Calculate the regions.
        detections, class_logits, box_regression, labels, regression_targets = self.roi_heads(
        ...
        proposal_losses = {}
        if self.training:
            labels, matched_gt_boxes = self.rpn.assign_targets_to_anchors(anchors, targets_tuple)
