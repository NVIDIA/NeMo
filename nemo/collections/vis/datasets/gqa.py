    ...
    def __init__(
        ...
        if self._load_scene_graph and not self._split.startswith('test'):
            if self._split == 'train':
                ...
                self._scene_graph_loader = SceneGraphFeatureLoader(
                    scene_graph_dirs, vocab_object_file, vocab_attributes_file, vocab_relation_file, num_objects
                )
        ...
        logging.info(
            "Exemplary sample number {}\n  question_type: {}\n  image_ids: {}\n  question: {}\n  answer: {}".format(
                i, sample["types"], sample["imageId"], sample["question"], sample["answer"],
            )
        )
