...
class ConformerEncoderUNet(NeuralModule, StreamingEncoder, Exportable, AccessMixin):
    """
    The encoder for ASR model of Conformer U-Net.
    Based on this paper:
    'Conformer: Convolution-augmented Transformer for Speech Recognition' by Anmol Gulati et al.
    https://arxiv.org/abs/2005.08100

    Added U-Net-style skip connections, where the first encoder layer connects to the last layer, the second to the second-last, and so on — but without any time-domain subsampling.
    Like in this paper:
    ...
    def __init__(
        ...
        for i in range(n_layers):
            layer_idx = i + 1
            has_skip = use_unet_skip_connection and layer_idx > (n_layers // 2)
            ...
            self.layers.append(nn.ModuleList([nn.Linear(d_model * 2, d_model) if has_skip else None, layer]))
...
class ConformerEncoderUNetAdapter(ConformerEncoderUNet, adapter_mixins.AdapterModuleMixin):
...
class ConformerMultiLayerFeatureExtractor(NeuralModule, Exportable, AccessMixin):
    """
    A wrapper module that extracts features from multiple layers of a ConformerEncoderUNet,
    ...
    def forward(
        ...
        ### chunk of code adapted from ConformerEncoderUNet.forward_internal()
        total_registry = {}
        ...
        for layer_idx in self.layer_idx_list:
            try:
                ...
                raise RuntimeError(
                    f"Intermediate layer {layer_idx} was not captured! Check the layer index and the number of ConformerEncoderUNet layers."
                )
...
"""
if adapter_mixins.get_registered_adapter(ConformerEncoderUNet) is None:
    adapter_mixins.register_adapter(base_class=ConformerEncoderUNet, adapter_class=ConformerEncoderUNetAdapter)
