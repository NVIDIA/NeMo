...
class ArrayGeometry(object):
    ...
    def __init__(
        ...
        if not mic_positions.shape[1] == 3:
            raise ValueError(
                f"Expecting 3D positions, but received {mic_positions.shape[1]}-dim positions"
            )
    ...
    def new_rotated_array(self, yaw: float = 0, pitch: float = 0, roll: float = 0):
        ...
        """
        new_array = ArrayGeometry(
            mic_positions=self.positions,
            center=self.center,
            internal_cs=self.internal_cs,
        )
...
def convert_placement_to_range(
    ...
    for idx, key in enumerate(["x", "y", "height"]):
        ...
        if val is None:
            ...
            if len(val) != 2:
                raise ValueError(
                    f"Invalid value for placement for dim {idx}/{key}: {str(placement)}"
                )
            min_val, max_val = val
        ...
        if min_val > max_val or min(min_val, max_val) < 0:
            raise ValueError(
                f"Invalid range dim {idx}/{key}: min={min_val}, max={max_val}"
            )
...
class RIRCorpusGenerator(object):
    ...
    def check_cfg(self):
        ...
        for idx, key in enumerate(["width", "length", "height"]):
            ...
            if dim is None:
                # not provided
                raise ValueError(
                    f"Room {key} needs to be a scalar or a range, currently it is None"
                )
                ...
                # not a valid range
                raise ValueError(
                    f"Range must be specified with two positive increasing elements for {key}: {dim}"
                )
        ...
        if rt60 is None:
            # not provided
            raise ValueError(
                "RT60 needs to be a scalar or a range, currently it is None"
            )
            ...
            # not a valid range
            raise ValueError(
                f"RT60 range must be specified with two positive increasing elements: {rt60}"
            )
    ...
    def generate_room_params(self) -> dict:
        ...
        if room_cfg.rt60 is None:
            raise ValueError(
                "Room RT60 needs to be a scalar or a range, currently it is None"
            )
        ...
        for n in range(num_retries):
            ...
            for idx, key in enumerate(["width", "length", "height"]):
                ...
                if dim is None:
                    ...
                    assert (
                        0 < dim[0] <= dim[1]
                    ), f"Expecting two non-decreasing values for {key}, received {dim}"
                    # Reduce dimension if the previous attempt failed
                    room_dim[idx] = self.random.uniform(
                        low=dim[0], high=dim[1] - n * (dim[1] - dim[0]) / num_retries
                    )
                else:
        ...
        if room_absorption is None or room_max_order is None:
            raise RuntimeError(
                f"Evaluation of parameters failed for RT60 {rt60}s and room size {room_dim}."
            )
    ...
    def generate_array(self, room_dim: Iterable[float]) -> ArrayGeometry:
        ...
        if mic_cfg.positions == "random":
            ...
            for m in range(num_mics):
                position_m = [None] * 3
                for idx in range(3):
                    position_m[idx] = self.random.uniform(
                        low=placement_range[idx][0], high=placement_range[idx][1]
                    )
            ...
            center = np.zeros(3)
            placement_range = convert_placement_to_range(
                placement=mic_cfg.placement,
                room_dim=room_dim,
                object_radius=mic_array.radius,
            )

            for idx in range(len(center)):
                center[idx] = self.random.uniform(
                    low=placement_range[idx][0], high=placement_range[idx][1]
                )
            ...
            for key in ["yaw", "roll", "pitch"]:
                ...
                if angle is None:
                    raise ValueError(
                        f"Mic array {key} should be a scalar or a range, currently it is set to None."
                    )
                ...
                if np.isscalar(angle):
                    ...
                    assert (
                        angle[0] <= angle[1]
                    ), f"Expecting two non-decreasing values for {key}, received {angle}"
    ...
    def generate_source_position(self, room_dim: Iterable[float]) -> List[List[float]]:
        ...
        placement_range = convert_placement_to_range(
            placement=source_cfg.placement, room_dim=room_dim
        )
        source_position = []
    ...
    def generate(self):
        ...
        for subset, num_rooms in self.cfg.room.num.items():
            ...
            if not os.path.exists(output_dir_subset):
                ...
                and len(os.listdir(output_dir_subset)) > 0
            ):
                raise RuntimeError(
                    f"Output directory {output_dir_subset} is not empty."
                )
            ...
            for n_room in range(num_rooms):
                ...
                room_filepath = os.path.join(
                    output_dir_subset, f"{subset}_room_{n_room:06d}.h5"
                )
            ...
            if num_workers > 1:
                ...
                with multiprocessing.Pool(processes=num_workers) as pool:
                    metadata = list(
                        tqdm(
                            pool.imap(simulate_room_kwargs, examples),
                            total=len(examples),
                        )
                    )
            ...
            for data in metadata:
                data["room_filepath"] = os.path.relpath(
                    data["room_filepath"], start=output_dir
                )
...
def simulate_room(
    ...
    metadata = {
        ...
        "rir_rt60_measured": room_sim.measure_rt60().mean(
            axis=0
        ),  # average across mics for each source
        "anechoic_rt60_theory": room_anechoic.rt60_theory(),
        "anechoic_rt60_measured": room_anechoic.measure_rt60().mean(
            axis=0
        ),  # average across mics for each source
...
def save_rir_simulation(
    filepath: str, rir_dataset: Dict[str, List[np.array]], metadata: dict
):
...
def load_rir_simulation(
    filepath: str, source: int = 0, rir_key: str = "rir"
...
def convert_numpy_to_serializable(
    data: Union[dict, float, np.ndarray]
...
class RIRMixGenerator(object):
    ...
    def __init__(self, cfg: DictConfig):
        ...
        logging.info(
            "Initialized with %d subsets: %s", len(self.subsets), str(self.subsets)
        )
        ...
        for subset in self.subsets:
            ...
            for key in ["room", "target", "noise", "interference"]:
                try:
                    ...
                    logging.warning(
                        "\t\tManifest data not loaded. Exception: %s", str(e)
                    )
    ...
    def check_cfg(self):
        ...
        for key in ["azimuth", "elevation", "distance"]:
            ...
            if value is None or np.isscalar(value):
                ...
                # not a valid range
                raise ValueError(
                    f"Range must be specified with two positive increasing elements for {key}: {value}"
                )
        ...
        if not interference_cfg:
            ...
            interference_probability = interference_cfg.get(
        ...
        if not mix_cfg:
            raise ValueError(
                "Mix configuration not provided. Expecting configuration for each subset."
            )
    ...
    def generate_target(self, subset: str) -> dict:
        ...
        def select_target_source(room_metadata, room_indices):
            ...
            for room_index in room_indices:
                ...
                # Candidate sources
                sources = self.random.choice(
                    room_data["num_sources"], size=self.num_retries, replace=False
                )
                ...
                for source in sources:
                    ...
                    for constraint in ["azimuth", "elevation", "distance"]:
                        if self.cfg.target.get(constraint) is not None:
                            ...
                            if (
                                self.cfg.target[constraint][0]
                                <= source_value
                                <= self.cfg.target[constraint][1]
                            ):
        ...
        room_indices = self.random.choice(
            len(room_metadata), size=self.num_retries, replace=False
        )
        source, room_index = select_target_source(room_metadata, room_indices)

        if source is None:
            raise RuntimeError(
                f"Could not find a feasible source given target constraints {self.cfg.target}"
            )
        ...
        if "mic_array" in self.cfg:
            ...
            if mic_selection == "random":
                ...
                selected_mics = self.random.choice(
                    num_available_mics, size=num_mics, replace=False
                )
    ...
    def generate_interference(self, subset: str, target_cfg: dict) -> List[dict]:
        ...
        interference_probability = self.cfg.interference.get(
            "interference_probability", 0
        )
        ...
        for n in range(num_interferers):
            ...
            if source is None:
                logging.warning(
                    "Could not select a feasible interference source %d of %s",
                    n,
                    num_interferers,
                )
    ...
    def generate_mix(self, subset: str, target_cfg: dict) -> dict:
        ...
        mix_cfg["save"] = (
            OmegaConf.to_object(self.cfg.mix["save"]) if "save" in self.cfg.mix else {}
        )
    ...
    def generate(self):
        ...
        for subset in self.subsets:
            ...
            # Generate examples
            for n_example in tqdm(
                range(num_examples), total=num_examples, desc=f"Preparing {subset}"
            ):
                ...
                base_output_filepath = os.path.join(
                    output_dir_subset, f"{subset}_example_{n_example:09d}"
                )
                ...
                audio_metadata.update(
                    {
                        ...
                        "interference_dir": os.path.dirname(
                            self.cfg.interference[subset]
                        ),  # manifest_dir
                    }
                )
            ...
            if num_workers is not None and num_workers > 1:
                logging.info(f"Simulate using {num_workers} workers")
                examples_and_audio_metadata = zip(
                    examples, itertools.repeat(audio_metadata, len(examples))
                )
                with multiprocessing.Pool(processes=num_workers) as pool:
                    metadata = list(
                        tqdm(
                            pool.imap(
                                simulate_room_mix_helper, examples_and_audio_metadata
                            ),
                            total=len(examples),
                ...
                metadata = []
                for example in tqdm(
                    examples, total=len(examples), desc=f"Simulating {subset}"
                ):
                    metadata.append(
                        simulate_room_mix(**example, audio_metadata=audio_metadata)
                    )

            # Save manifest
            manifest_filepath = os.path.join(
                output_dir, f"{os.path.basename(output_dir)}_{subset}.json"
            )
            ...
            for data in tqdm(
                metadata,
                total=len(metadata),
                desc=f"Making filepaths relative {subset}",
            ):
                for key, val in data.items():
            ...
            plot_filepath = os.path.join(
                output_dir, f"{os.path.basename(output_dir)}_{subset}_info.png"
            )
...
def calculate_drr(
    rir: np.ndarray, sample_rate: float, n_direct: List[int], n_0_ms=2.5
) -> List[float]:
    ...
    for m in range(num_channels):
        ...
        pow_reverberant = (
            np.sum(np.abs(rir[0:dir_start, m]) ** 2)
            + np.sum(np.abs(rir[dir_end:, m]) ** 2)
        ) / len_rir
...
def simultaneously_active_rms(
    ...
    if len(x) != len(y):
        raise RuntimeError(
            f"Expecting signals of same length: len(x)={len(x)}, len(y)={len(y)}"
        )
...
def scaled_disturbance(
    ...
    if signal.shape != disturbance.shape:
        raise ValueError(
            f"Signal and disturbance shapes do not match: {signal.shape} != {disturbance.shape}"
        )
...
def prepare_source_signal(
    ...
    if ref_signal is None:
        ...
        samples_to_load = (
            int(min_duration * sample_rate) if min_duration is not None else 1
        )
        source_signals_metadata = {
            "audio_filepath": [],
            "duration": [],
            "offset": [],
            "text": [],
        ...
        while samples_to_load > 0:
            ...
            # concatenate
            audio_signal = (
                np.concatenate((audio_signal, segment_samples))
                if audio_signal is not None
                else segment_samples
            )
            # remaining samples
        ...
        for n in range(num_retries):
            ...
            while samples_to_load > 0:
                ...
                if (
                    max_offset := item["duration"]
                    - np.ceil(samples_to_load / sample_rate)
                ) > 0:
                ...
                source_signals_metadata["audio_filepath"].append(audio_filepath)
                source_signals_metadata["duration"].append(
                    len(segment_samples) / sample_rate
                )
            ...
            if signal_type == "diffuse" and num_mics > 1:
                try:
                    ...
                    audio_signal = (
                        audio_signal[: num_mics * total_len].reshape(num_mics, -1).T
                    )
                    ...
                    logging.info(
                        "Failed to generate approximate noise field: %s", str(e)
                    )
            ...
            try:
                ...
                simultaneously_active_rms(
                    ref_signal, audio_signal, sample_rate=sample_rate
                )
...
def simulate_room_mix(
    ...
    def load_rir(
        ...
        """
        rir, rir_sample_rate = load_rir_simulation(
            room_filepath, source=source, rir_key=rir_key
        )
    ...
    target_rir_early = get_early_rir(
        rir=target_rir, rir_anechoic=target_rir_anechoic, sample_rate=sample_rate
    )
    ...
    metadata.update(
        {
            ...
            "drr": calculate_drr(
                target_rir, sample_rate, n_direct=np.argmax(target_rir_anechoic, axis=0)
            ),
