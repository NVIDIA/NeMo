    ...
    def __init__(self, fs: int, **kwargs: Any):
        ...
        if not HAVE_TORCHAUDIO:
            raise ModuleNotFoundError(
                f"{self.__class__.__name__} metric needs `torchaudio`."
            )
        ...
        if fs != self.sample_rate:
            ...
            logging.warning(
                "Input signals will be resampled from fs=%d to %d Hz",
                fs,
                self.sample_rate,
            )
        self.fs = fs
        ...
        logging.debug(
            "Setup metric %s with input fs=%s", self.__class__.__name__, self.fs
        )
...
class SquimObjectiveMetric(Metric):
    ...
    def __init__(self, fs: int, metric: str, **kwargs: Any):
        ...
        if metric not in ["stoi", "pesq", "si_sdr"]:
            raise ValueError(
                f'Unsupported metric {metric}. Supported metrics are "stoi", "pesq", "si_sdr".'
            )

        self.metric = metric

        # Objective model
        self._squim_objective_metric_model = (
            torchaudio.pipelines.SQUIM_OBJECTIVE.get_model()
        )

        self.add_state("metric_sum", default=torch.tensor(0.0), dist_reduce_fx="sum")
        self.add_state("num_examples", default=torch.tensor(0), dist_reduce_fx="sum")
        logging.debug(
            "Setup %s with metric=%s, input fs=%s",
    ...
    def state_dict(self, *args, **kwargs):
        ...
        remove_keys = [
            key
            for key in state_dict.keys()
            if "_squim_objective_metric_resampler" in key
            or "_squim_objective_metric_model" in key
        ]
        for key in remove_keys:
            del state_dict[key]
        return state_dict
