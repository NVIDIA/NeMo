...
class MCoreNevaModel(MCoreLLaVAModel):
    ...
    def forward(
        ...
        if num_image_tiles is None:
            ...
            num_image_tiles = torch.tensor(
                num_image_tiles, dtype=torch.int, device=input_ids.device
            )
    ...
    def _preprocess_data(
        ...
        with torch.no_grad():
            ...
            max_seq_len = (
                max_seq_len - 1
            ) // shard_factor * shard_factor + shard_factor
            ...
            if packed_sequence and packed_seq_params.cu_seqlens_q[-1] != max_seq_len:
                ...
                last_seqlen_padded = (
                    max_seq_len - packed_seq_params.cu_seqlens_q_padded[-2]
                )
                assert (
                ...
                packed_seq_params.max_seqlen_q = max(
                    last_seqlen_padded, packed_seq_params.max_seqlen_q
                )
                packed_seq_params.max_seqlen_kv = max(
                    last_seqlen_padded, packed_seq_params.max_seqlen_kv
                )
            ...
            images_mask[
                torch.arange(max_seq_len, device=first_padding_idx.device).repeat(
                    batch_size, 1
                )
        ...
        truncate_labels = (
            has_labels and final_labels.shape[1] > self._language_max_sequence_length
        )
        if truncate_labels:
        ...
        if (
            packed_sequence
            and packed_seq_params.cu_seqlens_q_padded[-1]
    ...
    def _process_embedding_token_parallel(
        ...
        if self.context_parallel_lm > 1:
            ...
            if self.pre_process:
                batch["combined_embeddings"] = batch["combined_embeddings"].transpose(
                    0, 1
                )
