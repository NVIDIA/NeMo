    ...
    def forward(
        ...
        if use_inference_kv_cache:
            ...
            if torch.distributed.get_rank() in [0, 1]:
                print(f"!!! {self.vision_projection.encoder.linear_fc1.weight.float().sum()}")
...
def create_forward_hook(module_names):
    ...
    def forward_hook(module, inputs, output):
        ...
        log_filename = f"debug_fwd_log_{world_size}_rank_{rank}.jsonl"  # JSON Lines format
...
def create_backward_hook(module_names):
    ...
    def backward_hook(module, grad_input, grad_output):
        global_name = module_names.get(id(module), module.__class__.__name__)
        if "vision_projection" not in global_name:
            return
        ...
        rank = dist.get_rank() if (dist.is_available() and dist.is_initialized()) else 0
        world_size = dist.get_world_size() if (dist.is_available() and dist.is_initialized()) else 1
        log_filename = f"debug_bwd_log_{world_size}_rank_{rank}.jsonl"
        tensor_file = f"debug_bwd_log_{world_size}_rank_{rank}_{global_name.replace('.', '_')}.pt"
