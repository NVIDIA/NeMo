...
class LlamaImageTextSample(ImageTextSample):
    ...
    vision_mask: torch.Tensor = field(
...
class Llama3SampleEncoder(VQASampleEncoder):
    ...
    def process_image(self, image) -> Dict[str, torch.Tensor]:
        image_dict = self.image_processor.preprocess(
    ...
    def apply_prompt_template(self, input_text: VQASample, use_plain=False):
        ...
        logging.debug(
            f"apply_conversation_template context {input_text.context} answer {input_text.answers}"
        )
            ...
            messages.append(
                {
                    ...
                    "content": [
                        {
                            "type": "text",
                            "text": self.conversation_template_config.system,
                        }
                    ],
                }
            )

        if isinstance(input_text.context, list) and isinstance(
        ...
        elif isinstance(input_text.context, str) and isinstance(
            input_text.answers, str
        ):
    ...
    def tokenize(self, prompt: str) -> torch.Tensor:
        regex_pattern = (
            "("
            + "|".join(re.escape(token) for token in [self.image_token.token_str])
            + ")"
        )
    ...
    def encode(self, input_sample: VQASample, output_sample: LlamaImageTextSample):
        ...
        vision_mask = create_vision_mask_tensor(
            tokens=tokens, vision_token_id=self.image_token.token_id
        )
