...
def process_vision(processor, images, videos, fps):
    ...
    if videos is not None:
        ...
        if isinstance(fps, (int, float)):
            ...
            raise ValueError(
                f"The length of fps ({len(fps) if hasattr(fps, '__len__') else fps}) must be equal to the length of "
                f"video_grid_thw ({len(video_grid_thw)}) or fps should be a single number."
            )
...
class PreloadedSupervisedDataset(Dataset):
    ...
    def _process_vision(self, source, image_folder, video_folder):
        ...
        vision_tensors = process_vision(self.image_processor, image_inputs, video_inputs, video_sample_fps_list)
