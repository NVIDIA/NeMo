...
class Qwen25VLConfig3B(Qwen25VLConfig):
    ...
    vision_projection_config: TransformerConfig = field(
        default_factory=lambda: MultimodalProjectorConfig(input_size=5120, hidden_size=1280, ffn_hidden_size=5120)
    )
...
class HFQwen25VLImporter(io.ModelConnector["Qwen2_5_VLForConditionalGeneration", Qwen25VLModel]):
    ...
    def config(self) -> Qwen25VLConfig:
        ...
        merge_hidden_size = hf_config.vision_config.hidden_size * (hf_config.vision_config.spatial_merge_size**2)
        vision_projection_config = MultimodalProjectorConfig(
            input_size=merge_hidden_size,
            hidden_size=hf_config.vision_config.out_hidden_size,
