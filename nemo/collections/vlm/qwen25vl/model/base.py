...
from nemo.collections.vlm.qwen25vl.model.vision import Qwen25VisionModel
...
def qwen2vl_data_step(dataloader_iter) -> Dict[str, torch.Tensor]:
    ...
    required_keys.update(
        ("input_ids", "pixel_values", "image_grid_thw", "pixel_values_videos", "video_grid_thw", "second_per_grid_ts")
    )
...
@dataclass
class Qwen25VLConfig(TransformerConfig, io.IOMixin):
    """Qwen2.5VL Model Base Config"""

    language_transformer_config: Optional[Qwen2Config] = None
    vision_transformer_config: Optional[Qwen25VLVisionConfig] = None
    ...
    def __post_init__(self):
        ...
        if self.language_transformer_config is not None:
            ...
            # See Qwen2.5-VL 2B/7B/72B share the same mrope_section config, see below for details:
            # https://huggingface.co/Qwen/Qwen2.5-VL-72B-Instruct/blob/main/config.json
    ...
    def configure_model(self, tokenizer) -> "MCoreQwen25VLModel":
        # pylint: disable=C0115,C0116
...
class MCoreQwen25VLModel(MCoreLLaVAModel):
    """Qwen2.5VL Model Base Model Class"""
    ...
    def get_rope_index(
        ...
        """
        Refer to https://github.com/huggingface/transformers/blob/be37d34f44ff1bc928e59ffb8a30adecab8835a8/src/transformers/models/qwen2_5_vl/modeling_qwen2_5_vl.py
    ...
    def forward(
        ...
        if use_inference_kv_cache:
            ...
            window_index = self.vision_model.window_index
    ...
    def _preprocess_data(
        ...
        """
        MCoreQwen25VLModel uses its own version of _preprocess_data instead of MCoreLLaVAModel's (in
        ...
        This function won't shift labels as forward() and _preprocess_data() in MCoreQwen25VLModel
        ...
        if final_embedding is not None:
            ...
            # TODO: check and add self.context_parallel_lm to MCoreQwen25VLModel
...
class Qwen25VLModel(L.LightningModule, io.IOMixin, io.ConnectorMixin, fn.FNMixin):
    """Lightning Wrapper for Qwen2.5VL Model"""
