...
from megatron.core.transformer.attention import CrossAttention, Attention
...
class LlamaGPTModel(MCoreGPTModel):
    def __init__(        
        self,
        ...
        super().__init__(config, transformer_layer_spec, vocab_size, max_sequence_length, pre_process, post_process, fp16_lm_cross_entropy, parallel_output,
                         share_embeddings_and_output_weights, position_embedding_type, rotary_percent, rotary_base, seq_len_interpolation_factor)

        # For now we overwrite the self.decoder
        self.decoder = LlamaTransformerBlock(
            config=self.config,
...
class LlamaTransformerBlock(TransformerBlock):
    def __init__(self, *args, **kwargs):
        ...
        self.xlayers = [] # torch.nn.ModuleList()
        ...
        assert len(self.xlayers) == len(self.layers), 'This might indicate the PP is not set up correctly'
    ...
    def forward(
        ...
        xattn_caches: torch.Tensor # similar to TransformerBlock context, but for different levels
    ):
...
class LlamaCrossAttention(Attention):
    ...
    def forward(
        ...
        # Apply gate_attn (Added for Llama)
        # TODO check shape
        h = hidden_states + self.gate_attn.tanh() * core_attn_out

        # =================
        # Output. [sq, b, h]
        # =================

        output, bias = self.linear_proj(h)

        # Here we assume we do not have bias
        # Also assume no_ffn is False
        h = h + self.gate_ffwd.tanh() * output
