...
def model() -> run.Config[pl.LightningModule]:
    """
    Factory function to create a Llava 1.6 7B model configuration.

    Returns:
        run.Config[pl.LightningModule]: Configuration for the Llava 1.6 7B model.

    Examples:
        CLI usage:
            $ nemo llm pretrain model=llava16_7b ...

        Python API usage:
            >>> model_config = model()
            >>> print(model_config)
    """
    return run.Config(vlm.LlavaNextModel, config=run.Config(vlm.Llava16Config7B))
...
def finetune_recipe(
    ...
    Examples:
        ...
        Python API usage:
            >>> recipe = finetune_recipe(name="llava16_7b_finetune", num_nodes=1)
            >>> print(recipe)
    """
