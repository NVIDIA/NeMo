...
from megatron.energon import (Sample, Batch, DefaultTaskEncoder, generic_batch, 
                              InterleavedSample, SimilarityInterleavedSample, VQASample, CaptioningSample)
from megatron.energon.task_encoder.base import stateless
from nemo.collections.vlm.data.utils import greedy_knapsack, predict_seq_len, convert_to_packed
    ...
    def __post_init__(self):
        ...
        if self.hf_path:
            ...
            for attr in dir(self.hf_processor):
                if not callable(getattr(self.hf_processor, attr)) and not attr.startswith('_') and not hasattr(self, attr):
...
class TaskEncoder(DefaultTaskEncoder[
    ...
    def encode_vqa_sample(self, input_sample: VQASample) -> DataSample:
        ...
        conversation_prompt = self.config.tokenizer.apply_chat_template(messages, tokenize=False, add_generation_prompt=False)
        ...
        if self.config.pad_to_multiple_of:
            seqlen_padded = (seqlen + self.config.pad_to_multiple_of - 1) // self.config.pad_to_multiple_of * self.config.pad_to_multiple_of
        ...
        # Process image
        image = self.config.image_processor.preprocess(input_sample.image, return_tensors='pt', do_rescale=False)['pixel_values'][0]
