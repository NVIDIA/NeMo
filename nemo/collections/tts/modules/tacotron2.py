...
class Encoder(NeuralModule):
    ...
    def forward(self, *, token_embedding, token_len):
        for conv in self.convolutions:
            token_embedding = F.dropout(
                F.relu(conv(token_embedding)), 0.5, self.training
            )
...
class Decoder(NeuralModule):
    ...
    def input_types(self):
        ...
        if self.training:
            input_dict["decoder_inputs"] = NeuralType(
    ...
    def infer(self, *, memory, memory_lengths):
        ...
        while True:
            ...
            dec = (
                torch.le(torch.sigmoid(gate_output.data), self.gate_threshold)
                .to(torch.int32)
                .squeeze(1)
            )
...
class Postnet(NeuralModule):
    ...
    def forward(self, *, mel_spec):
        ...
        for i in range(len(self.convolutions) - 1):
            mel_spec_out = F.dropout(
                torch.tanh(self.convolutions[i](mel_spec_out)),
                self.p_dropout,
                self.training,
            )
        mel_spec_out = F.dropout(
            self.convolutions[-1](mel_spec_out), self.p_dropout, self.training
        )
