...
class TarredTTSDataset(_TarredAudioToTextDataset):
    ...
    def add_semantic_code(self, **kwargs):
        self.semantic_code_folder = kwargs.pop('semantic_code_folder', None)
        assert self.semantic_code_folder is not None, "Semantic code folder cannot be None."
        assert os.path.exists(self.semantic_code_folder) is not None, "Semantic code folder must exist."
    ...
    def general_collate_fn(self, batch):
        ...
        max_semantic_code_len = (
            max(semantic_code_lengths).item() if SemanticCode in self.sup_data_types_set else None
        )
        ...
        for i, sample_tuple in enumerate(batch):
            ...
            if SemanticCode in self.sup_data_types_set:
                semantic_codes.append(
                    general_padding(semantic_code, semantic_code_length.item(), max_semantic_code_len)
                )
        ...
        data_dict = {
            ...
            "semantic_code": torch.stack(semantic_codes) if SemanticCode in self.sup_data_types_set else None,
            "semantic_code_lens": torch.stack(semantic_code_lengths)
            if SemanticCode in self.sup_data_types_set
            else None,
        }
