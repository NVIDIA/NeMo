...
class MegatronT5OverrideModel(MegatronT5Model):
    def _build_tokenizer(self):
        if self._cfg.tokenizer.library == "sentencepiece":
            if hasattr(self._cfg.tokenizer, "sentencepiece_legacy"):
                ...
                legacy = (
                    True if self._cfg.tokenizer.library == "sentencepiece" else False
                )
            self.tokenizer = SentencePieceSpeechLLMTTSTokenizer(
                model_path=self.register_artifact(
                    "tokenizer.model", self._cfg.tokenizer.get("model", None)
                ),
                legacy=legacy,
            )

            if self._cfg.tokenizer.get("additional_special_tokens", None) is not None:
                tokens_list = OmegaConf.to_object(
                    self._cfg.tokenizer.additional_special_tokens
                )
    ...
    def model_provider_func(self, pre_process, post_process, add_encoder, add_decoder):
        ...
        if (
            ...
            and self.cfg.encoder.arch == "perceiver"
        ):
            raise ValueError(
                f"Perceivers with pipeline parallel > 1 is not supported yet."
            )
        ...
        model = MegatronTokenLevelEncoderDecoderSpeechLLMModule(
            ...
            share_decoder_tokens_head_embeddings=self.cfg.get(
...
class MegatronT5SpeechLMModel(MegatronBaseSpeechLM):
    ...
    def __init__(self, cfg: DictConfig, trainer: Trainer):
        ...
        attn_prior_scaledown_start_step = cfg.get(
        ...
        if self.context_conditioning == "decoder":
            ...
            ), "Decoder context duration must be fixed"
            self.decoder_context_len = int(
                self.codebook_fps * self.context_duration_min
            )
        ...
        if self.enc_output_to_layers is not None:
            ...
            self.enc_output_to_layers = [
                [l for l in encoder_layer]
                for encoder_layer in self.enc_output_to_layers
            ]
        ...
        self.frozen_model.enc_dec_model.attn_prior_scaledown_start_step = (
        ...
        self.frozen_model.enc_dec_model.num_cross_attention_heads = (
        ...
        if self.use_alignment_loss:
            ...
            self.frozen_model.enc_dec_model.forward_sum_loss = ForwardSumLoss(
                loss_scale=alignment_loss_scale
            )
            self.frozen_model.enc_dec_model.alignment_text_end_offset = cfg.get(
                "alignment_text_end_offset", 0
            )
            self.frozen_model.enc_dec_model.align_every_n_head = cfg.get(
                "align_every_n_head", 1
            )
            self.alignment_loss_start_step = cfg.get("alignment_loss_start_step", 0)
            self.alignment_loss_end_step = cfg.get(
                "alignment_loss_end_step", float("inf")
            )

        # Need to explicitly set this since it is already initialized
        self.frozen_model.enc_dec_model.tokens_head.parallel_output = (
        ...
        for _ in range(self.num_speech_codebooks - 1):
            ...
            init_method_std = (
                self.frozen_model.enc_dec_model.decoder_cfg.init_method_std
            )
        ...
        self.frozen_model.enc_dec_model.speech_tokens_heads = torch.nn.ModuleList(
            list_of_speech_heads
        )
        ...
        self.inference_cfg_interpolation_scale = cfg.get(
            "inference_cfg_interpolation_scale", 1.0
        )
        ...
        if not self.inference_apply_cfg_filter:
            ...
            self.inference_cfg_filter_interpolation_scale = cfg.get(
                "inference_cfg_filter_interpolation_scale", 1.0
            )
        ...
        if self.estimate_mos:
            ...
            self.non_matching_ref_audio_filepath = cfg.get(
                "non_matching_ref_audio_filepath", None
            )
    ...
    def forward(
        ...
        if isinstance(context_and_question_tokens, list):
            ...
            if cross_attention_prior is None:
                cross_attention_prior = [
                    None for _ in range(len(context_and_question_tokens))
                ]
            assert isinstance(cross_attention_prior, list)
            assert (
                len(context_and_question_tokens)
                == len(enc_mask)
                == len(position_ids)
                == len(cross_attention_prior)
            )
        else:
            multi_encoder = False
        ...
        if self.first_stage_of_pipeline() and inference_step == 0:
            ...
            for ei in range(len(context_and_question_tokens)):
                input_embeds = self.get_embeddings_and_combine(
                    [virtual_tokens, context_and_question_tokens[ei]],
                    taskname_ids,
                    inference,
                )
                # TODO: This check needs to be revisited with PP support.
                if hasattr(
                    self.frozen_model.enc_dec_model.encoder_embedding,
                    "position_embeddings",
                ):
            ...
            if inference_step != 0:
                enc_output = (
                    context_and_question_tokens
                    if multi_encoder
                    else context_and_question_tokens[0]
                )
        ...
        if not multi_encoder:
            ...
            _encoder_input = (
                encoder_input_list[0] if encoder_input_list is not None else None
            )
    ...
    def load_frozen_model(self, cfg, trainer):
        ...
        if cfg_language_model_path:
            t5_cfg = MegatronT5Model.restore_from(
                cfg_language_model_path, trainer=trainer, return_config=True
            )
        else:
        ...
        with open_dict(t5_cfg):
            ...
            t5_cfg.tokenizer.num_sentinel_tokens = cfg.get(
                "num_sentinel_tokens", 39184 - 29056
            )
        ...
        if cfg.get("train_from_scratch", False):
            ...
            num_params = sum(
                p.numel() for p in self.frozen_model.parameters() if p.requires_grad
            )
    ...
    def convert_tokens_to_range(
        self, tokens, apply_offset_correction=True, pattern=None
    ):
        # convert tokens to range [0, 1024]
        output_tokens = tokens.clone()
        if apply_offset_correction:
            output_tokens[0] = output_tokens[0] - self.speech_offset
        output_tokens = torch.clamp(
            output_tokens, min=0, max=self.speech_codebook_size - 1
        )
        if pattern is None:
        ...
        def fwd_output_and_loss_func(dataloader_iter, model):
            ...
            if alignment_loss is not None:
                self.logger.experiment.add_scalar(
                    "train_alignment_loss", alignment_loss, self.global_step
                )
                ...
                with torch.no_grad():
                    with torch.cuda.amp.autocast(enabled=False):
                        if torch.count_nonzero(speech_mask) == 0:
                            ...
                            self.logger.experiment.add_scalar(
                                "WER", score, self.global_step
                            )
                        else:
                            audio_len = (
                                self.decoder_context_len
                                + (labels[0][0][self.decoder_context_len :] != 0)
                                .sum()
                                .item()
                            )
                            labels_to_1024 = self.convert_tokens_to_range(
                                labels[0, :, 0:audio_len]
                            )
                            label_wav = self.decode_wav_from_codec_model(labels_to_1024)
                            dec_input_to_1024 = self.convert_tokens_to_range(
                                dec_input[0, :, 0:audio_len]
                            )
                            dec_input_wav = self.decode_wav_from_codec_model(
                                dec_input_to_1024
                            )
                            ...
                            if isinstance(context_and_question_tokens, list):
                                ...
                                context_end_step = context_and_question_tokens_lens[0][
                                    0
                                ].item()
                            ...
                            if context_end_step > 1:
                                ...
                                if is_speech_context:
                                    ...
                                    _context_token_list = [
                                        v.item() for v in _context_tokens[0, :]
                                    ]
                                    _context_text = (
                                        self.frozen_model.tokenizer.ids_to_text(
                                            [
                                                v
                                                for v in _context_token_list
                            ...
                            question_si = (
                                text_limits[0, 0].item() - virtual_tokens.shape[1]
                            )
                            question_ei = (
                                text_limits[0, 1].item() - virtual_tokens.shape[1]
                            )
                            ...
                            input_text = self.frozen_model.tokenizer.ids_to_text(
                                [
                                    v
                                    for v in input_token_list_all[
                                        question_si:question_ei
                                    ]
                                    if v < self.lm_vocab_size
                                ]
                            )
                            self.logger.experiment.add_text(
                                "Train Input Text", input_text, self.global_step
                            )
                            ...
                            if len(input_phoneme_tokens) > 0:
                                phoneme_text = self.phoneme_tokenizer.decode(
                                    input_phoneme_tokens
                                )
                                self.logger.experiment.add_text(
                                    "Train Input Phoneme Text",
                                    phoneme_text,
                                    self.global_step,
                                )
                            ...
                            attention_probs_list = out_logits[
                                2
                            ]  # list of (BS, 12, out_length, in_length)
                                    ...
                                    for _i in range(attention_probs.shape[1]):
                                        ...
                                        attention_sliced_list.append(
                                            attention_probs[
                                                0,
                                                _i,
                                                self.decoder_context_len : audio_len,
                                                text_si:text_ei,
                                            ]
                                        )
                            ...
                            first_layer_tokens = (
                                token_logits_example.argmax(dim=1) - self.speech_offset
                            )
                            other_layer_tokens = []
                            for _i in range(speech_logits_example.shape[2]):
                                other_layer_tokens.append(
                                    speech_logits_example[:, :, _i].argmax(dim=1)
                                )

                            all_layer_tokens = torch.stack(
                                [first_layer_tokens] + other_layer_tokens
                            )  # (8, t)
                            ...
                            predicted_wav = self.decode_wav_from_codec_model(
                                all_layer_tokens
                            )
                            self.logger.experiment.add_audio(
                                "train_tf_pred_wav",
            ...
            def loss_func(loss_args):
                ...
                if (
                    ...
                    logging.debug(
                        f"Adding alignment loss. cur:{curr_step} start:{self.alignment_loss_start_step}"
                    )
    ...
    def get_forward_output_only_func(self):
        ...
        def fwd_output_only_func(dataloader_iter, model):
            ...
            def id_func(output_tensor):
                return 0, {
                    "output_logits": output_tensor[0],
                    "token_and_speech_logits": output_tensor[1],
                }
    ...
    def training_step(self, dataloader_iter, batch_idx):
        ...
        if self.train_text_cfg_prob > 0.0:
            if self._rng.random() < self.train_text_cfg_prob:
                logging.info(
                    f"Text Classifier-Free Guidance is triggered for the {batch_idx}-th batch."
                )
                ...
                if isinstance(
                ...
                ):  # indicate self.encoder_type=multi_transformers.
                    ...
                    time_range = torch.arange(
                        question_tokens_unconditioned.size(2),
                        device=question_tokens_unconditioned.device,
                    ).unsqueeze(0)
                ...
                self.frozen_model.enc_dec_model.use_alignment_loss = (
        ...
        if self.train_audio_cfg_prob > 0.0:
            if self._rng.random() < self.train_audio_cfg_prob:
                logging.info(
                    f"Audio Classifier-Free Guidance is triggered for the {batch_idx}-th batch."
                )
        ...
        if tokens.dim() > 2:
            for i in range(tokens.size()[1]):  # for 8 channels
                if i == 0:
                    ...
                    out = self.embed_input(
                        tokens[:, i, :], taskname_ids, inference
                    )  # (B, T, D)
                else:
                    # Embed other layers using speech embeddings
                    cur = self.frozen_model.enc_dec_model.speech_tokens_embeddings[
                        i - 1
                    ](tokens[:, i, :])
        ...
        if batch_idx == 0 and self.is_rank_zero:
            ...
            for inf_key in self.predict_step_outputs[0]:
                if self.predict_step_outputs[0][inf_key] is not None:
                    self.logger.experiment.add_scalar(
                        f"Val_{inf_key}",
                        self.predict_step_outputs[0][inf_key],
                        self.global_step,
                    )
        ...
        if batch_idx == 0 and self.is_rank_zero:
            ...
            with torch.cuda.amp.autocast(enabled=False):
                if torch.count_nonzero(speech_mask) == 0:
                    ...
                    audio_len = (
                        self.decoder_context_len
                        + (labels[0][0][self.decoder_context_len :] != 0).sum().item()
                    )
                    labels_to_1024 = self.convert_tokens_to_range(
                        labels[0, :, 0:audio_len]
                    )
                    ...
                    self.logger.experiment.add_audio(
                        "val_label_wav", label_wav, self.global_step, self.sample_rate
                    )
                    self.logger.experiment.add_audio(
                        "val_dec_input_wav",
                        dec_input_wav,
                    ...
                    if context_end_step > 1:
                        ...
                        if is_speech_context:
                            ...
                            self.logger.experiment.add_text(
                                "val_context_text", _context_text, self.global_step
                            )
                    ...
                    self.logger.experiment.add_text(
                        "Val Input Text", input_text, self.global_step
                    )
                    ...
                    if len(input_phoneme_tokens) > 0:
                        ...
                        self.logger.experiment.add_text(
                            "Val Input Phoneme Text", phoneme_text, self.global_step
                        )
                    ...
                    attention_probs_list = output_logits[
                        2
                    ]  # list of (BS, 12, out_length, in_length)
                    if attention_probs_list is not None:
                        attention_sliced_list = []
                    ...
                    all_layer_tokens = torch.clip(
                        all_layer_tokens, 0, self.speech_codebook_size - 1
                    )
        ...
        for i in range(self.num_speech_codebooks - 1):
            if torch.count_nonzero(speech_mask) > 0:
                ...
                correct_predictions_i = (
                    correct_predictions_i * loss_mask * speech_mask
                )  # (bs,t)
                ...
                loss_i = (
                    torch.sum(loss_i * loss_mask * speech_mask) / total_predictions_i
                )
            else:
    ...
    def on_validation_epoch_end(self):
        ...
        if self.cfg.get("pipeline_model_parallel_size", 1) > 1:
            if parallel_state.is_pipeline_last_stage(ignore_virtual=False):
                ...
                averaged_loss_total_check = torch.stack(
                    [item["loss_total_check"] for item in outputs]
                ).mean()
                averaged_first_layer_accuracy = torch.stack(
                    [item["first_layer_accuracy"] for item in outputs]
                ).mean()
                ...
                logging.info(
                    f"Validation first_layer_accuracy: {averaged_first_layer_accuracy}"
                )
                logging.info(
                    f"Validation loss_total_check: {averaged_loss_total_check}"
                )

                for i in range(1, self.num_speech_codebooks):
                    averaged_speech_accuracy = torch.stack(
                        [item[f"speech_accuracy_{i}"] for item in outputs]
                    ).mean()
                    averaged_speech_loss = torch.stack(
                        [item[f"speech_loss_{i}"] for item in outputs]
                    ).mean()
                    self.log(
                    ...
                    self.log(
                        f"val_speech_loss_{i}",
                        averaged_speech_loss,
                        prog_bar=True,
                        rank_zero_only=True,
                        batch_size=1,
                    )
                    logging.info(
                        f"Validation speech_accuracy_{i}: {averaged_speech_accuracy}"
                    )
    ...
    def on_test_epoch_end(self):
        ...
        for key in average_metrics:
            ...
            self.logger.experiment.add_scalar(
                f"Inf Cumulative {key}", average_metrics[key], 0
            )
    ...
    def build_virtual_prompt_dataset(
        ...
        dataset = T5SpeechLMDataset(
            ...
            attention_prior_scaling_factor=self.cfg.data.get(
                "attention_prior_scaling_factor", 1.0
            ),
            ...
            use_beta_binomial_interpolator=self.cfg.get(
    ...
    def predict_step(
        ...
        with torch.no_grad():
            ...
            dec_input = torch.nn.functional.pad(
                dec_input, (0, max_inference_timesteps - dec_input.shape[2]), value=0
            )
            ...
            dec_input_mask = torch.nn.functional.pad(
                dec_input_mask,
                (0, max_inference_timesteps - dec_input_mask.shape[1]),
                value=1,
            )
            ...
            if self.inference_apply_text_cfg and self.inference_apply_audio_cfg:
                ...
                if isinstance(
                    ...
                    context_and_question_tokens = [
                        torch.cat(
                            (context_tokens, context_tokens_unconditioned), dim=0
                        ),
                        torch.cat(
                            (question_tokens, question_tokens_unconditioned), dim=0
                        ),
                    ]
                    enc_mask = [torch.cat((mask, mask), dim=0) for mask in enc_mask]
                    dec_input = torch.cat((dec_input, dec_input), dim=0)
                    position_ids = [
                        torch.cat((pos_ids, pos_ids), dim=0) for pos_ids in position_ids
                    ]
                else:
                    assert (
            ...
            for t in range(self.decoder_context_len + 1, dec_input.shape[2] - 1):
                ...
                if t == self.decoder_context_len + 1:
                    ...
                    output_logits, _, token_and_speech_logits = self.forward(
                        ...
                        dec_input_mask[
                            :, : t + 1
                        ],  # doesn't matter because of all ones.
                        position_ids,
                        taskname_ids,
                        labels=None,
                        speech_mask=speech_mask,
                    ...
                    token_and_speech_logits = output_tensor[0][
                        "token_and_speech_logits"
                    ]
                    ...
                    if self.frozen_model.enc_dec_model.return_all_crossattention_probs:
                        ...
                        attention_probs_mean = torch.stack(attention_probs).mean(
                            dim=0
                        )  # B, 12, 1, enc_timesteps
                ...
                if self.inference_apply_text_cfg or self.inference_apply_audio_cfg:
                    ...
                    token_logits = (
                        self.inference_cfg_interpolation_scale
                        * token_and_speech_logits[0][:batch_size]
                        + (1 - self.inference_cfg_interpolation_scale)
                        * token_and_speech_logits[0][batch_size:]
                    )
                    output_speech_logits = (
                        self.inference_cfg_interpolation_scale
                        * output_logits[:batch_size]
                        + (1 - self.inference_cfg_interpolation_scale)
                        * output_logits[batch_size:]
                    )
                else:
                ...
                # (B*8, 80) or (B, 80)
                output_logits_currtimestep_topk = torch.topk(
                    output_logits_currtimestep, top_k, dim=1
                )[0]

                # find indices which are not top k
                indices_to_remove = (
                    output_logits_currtimestep
                    < output_logits_currtimestep_topk[:, -1].unsqueeze(1)
                )
                # (B*8, 1024) or (B, 1024)

                if self.inference_apply_cfg_filter:
                    output_logits_currtimestep_rescored = (
                        output_logits_currtimestep_conditioned.clone()
                    )
                else:
                    output_logits_currtimestep_rescored = (
                        output_logits_currtimestep.clone()
                    )
                ...
                if (
                    ...
                    output_logits_currtimestep_rescored = (
                        self.inference_cfg_filter_interpolation_scale
                        * output_logits_currtimestep_rescored
                        + (1 - self.inference_cfg_filter_interpolation_scale)
                        * output_logits_currtimestep_unconditioned
                    )

                temperature = self.cfg.get(
                    "temperature", 0.85
                )  # Set temp 0.01 for greedy decoding
                output_logits_currtimestep_rescored = (
                    output_logits_currtimestep_rescored / temperature
                )
                ...
                for _b in range(token_preds.shape[0]):
                    if (
                        t > self.decoder_context_len + 10
                        and token_preds[_b] == self.tokenizer.eos_id
                    ):
                        if _b not in end_indices:
                            logging.info(
                                "End detected for item {}".format(_b)
                                + " at timestep {}".format(t)
                            )
                            end_indices[_b] = t
                ...
                if self.inference_apply_text_cfg or self.inference_apply_audio_cfg:
                    output_tokens_curr_timestep = torch.cat(
                        (output_tokens_curr_timestep, output_tokens_curr_timestep),
                        dim=0,
                    )
            ...
            # end of for loop
            output_tokens_combined = torch.stack(
                output_token_list
            )  # (T, B, 8) if speech else (T, B)
            if torch.count_nonzero(speech_mask) > 0:
                output_tokens_combined = output_tokens_combined.permute(
                    1, 2, 0
                )  # (B, 8, T)
            else:
            ...
            if "nemo_sv_model" not in self.additional_models:
                nemo_sv_model = nemo_asr.models.EncDecSpeakerLabelModel.from_pretrained(
                    model_name="titanet_large"
                )
                nemo_sv_model = nemo_sv_model.to(device)
                nemo_sv_model.encoder.disable_torch_distributed = (
                    True  # For multi-gpu training validation
                )
                nemo_sv_model.eval()
            ...
            if "asr_model" not in self.additional_models:
                ...
                asr_model.encoder.disable_torch_distributed = (
                    True  # For multi-gpu training validation
                )
                asr_model.eval()
            ...
            if "wavlm_sv_model" not in self.additional_models:
                wavlm_sv_extractor = Wav2Vec2FeatureExtractor.from_pretrained(
                    "microsoft/wavlm-base-plus-sv"
                )
                wavlm_sv_model = WavLMForXVector.from_pretrained(
                    "microsoft/wavlm-base-plus-sv"
                )
            ...
            if self.estimate_mos:
                ...
                ref_16khz_wav, _ = librosa.load(
                    self.non_matching_ref_audio_filepath, sr=16000
                )
            ...
            if len(attention_probs_all) != 0:
                attention_probs_all = torch.cat(
                    attention_probs_all, dim=2
                )  # B, 12, dec_timesteps, enc_timesteps
                attention_probs_all = attention_probs_all.mean(
                    dim=1
                )  # B, dec_timesteps, enc_timesteps
                ...
                for i in range(batch_size):
                    ...
                    if len(attention_probs_all) != 0:
                        attention_probs_example = attention_probs_all[i][
                            : end_index - (1 + self.decoder_context_len),
                            text_start_step:text_end_step,
                        ]  # T, enc_timesteps
                        ...
                        alignment_fp = os.path.join(
                            _exp_dir_path, f"attention_map_{step}.png"
                        )
            ...
            for i in range(batch_size):
                ...
                audio_len = (
                    self.decoder_context_len
                    + (labels[i][0][self.decoder_context_len :] != 0).sum().item()
                )

                if torch.count_nonzero(speech_mask) > 0:
                    dec_input_to_1024 = self.convert_tokens_to_range(
                        dec_input_raw[i, :, 0:audio_len]
                    )
                    dec_input_to_1024_answer = dec_input_to_1024[
                        :, self.decoder_context_len + 1 :
                    ]
                    dec_input_wav = self.decode_wav_from_codec_model(
                        dec_input_to_1024_answer
                    )
                    self.logger.experiment.add_audio(
                        "Inf Dec Input Wav", dec_input_wav, step, self.sample_rate
                    )

                    predicted_tokens = output_tokens_combined[
                        i
                    ]  # Should not contain context even if decoder context
                    if i in end_indices:
                        ...
                        if self.cfg.get("seq_pattern", "parallel") == "delay_parallel":
                            predicted_tokens = predicted_tokens[
                                :,
                                0 : end_indices[i]
                                - (1 + self.decoder_context_len)
                                + self.num_speech_codebooks,
                    ...
                    start_time = time.time()
                    predicted_tokens = self.convert_tokens_to_range(
                        predicted_tokens, apply_offset_correction=False
                    )
                    ...
                    total_audio_seconds = (
                        total_audio_seconds + predicted_wav.size(-1) / self.sample_rate
                    )

                    self.logger.experiment.add_audio(
                        "Inf Pred Wav", predicted_wav, step, self.sample_rate
                    )
                    self.logger.experiment.add_image(
                        "Inf Pred Tokens",
                    ...
                    audio_fp_pred = os.path.join(
                        _exp_dir_path, f"predicted_wav_{wav_num}.wav"
                    )
                    sf.write(
                        audio_fp_pred, predicted_wav.cpu().numpy(), self.sample_rate
                    )
                    audio_fp_gt = os.path.join(
                        _exp_dir_path, f"dec_input_wav_{wav_num}.wav"
                    )
                    ...
                    spk_embedding_pred = (
                        spk_embedding_pred.cpu().detach().numpy().flatten()
                    )
                    ...
                    if log_scalars:
                        self.logger.experiment.add_scalar(
                            f"Inf SV Cossim Individual Sample", similarity, step
                        )
                    ...
                    with torch.no_grad():
                        ...
                        wavlm_embeddings = torch.nn.functional.normalize(
                            wavlm_embeddings, dim=-1
                        ).cpu()

                    spk_embedding_pred_wavlm = (
                        wavlm_embeddings[0].cpu().detach().numpy().flatten()
                    )
                    spk_embedding_gt_wavlm = (
                        wavlm_embeddings[1].cpu().detach().numpy().flatten()
                    )
                    similarity_wavlm = np.dot(
                        spk_embedding_pred_wavlm, spk_embedding_gt_wavlm
                    ) / (
                        np.linalg.norm(spk_embedding_pred_wavlm)
                        * np.linalg.norm(spk_embedding_gt_wavlm)
                    )
                    ...
                    if isinstance(context_and_question_tokens, list):
                        ...
                        input_token_list = [
                            ...
                            for j in range(
                                context_and_question_tokens_lens[1][i].item()
                            )
                        ]
                        input_token_list = [
                            (ti, t)
                            for ti, t in enumerate(input_token_list)
                            if t != 0 and t < self.speech_offset
                        ]
                        ...
                        context_end_step = input_token_list[0][0]
                        context_tokens = context_and_question_tokens[i][
                            :, :context_end_step
                        ]
                    ...
                    if self.decoder_context_len > 0:
                        context_tokens = dec_input_to_1024[
                            :, : self.decoder_context_len + 1
                        ]
                        ...
                        if is_speech_context:
                            context_tokens = self.convert_tokens_to_range(
                                context_tokens, pattern=self.context_pattern
                            )
                            context_wav = self.decode_wav_from_codec_model(
                                context_tokens
                            )
                        else:
                            context_wav = None
                            _context_token_list = [
                                v.item() for v in context_tokens[0, :]
                            ]
                            _context_text = self.frozen_model.tokenizer.ids_to_text(
                                [
                                    v
                            ...
                            self.logger.experiment.add_text(
                                "Context Text", _context_text, self.global_step
                            )
                    ...
                    if context_wav is not None:
                        self.logger.experiment.add_audio(
                            "Context Wav", context_wav, step, self.sample_rate
                        )
                        context_wav_fp = os.path.join(
                            _exp_dir_path, f"context_wav_{wav_num}.wav"
                        )
                        sf.write(
                            context_wav_fp, context_wav.cpu().numpy(), self.sample_rate
                        )
                        # titanet
                        spk_embedding_context = nemo_sv_model.get_embedding(
                            context_wav_fp
                        )
                        spk_embedding_context = (
                            spk_embedding_context.cpu().detach().numpy().flatten()
                        )
                        # wavlm
                        ...
                        spk_embedding_context_wavlm = (
                            wavlm_embeddings[0].cpu().detach().numpy().flatten()
                        )

                    pred_similarity_context = np.dot(
                        spk_embedding_context, spk_embedding_pred
                    ) / (
                        np.linalg.norm(spk_embedding_context)
                        * np.linalg.norm(spk_embedding_pred)
                    )
                    gt_similarity_context = np.dot(
                        spk_embedding_context, spk_embedding_gt
                    ) / (
                        np.linalg.norm(spk_embedding_context)
                        * np.linalg.norm(spk_embedding_gt)
                    ...
                    pred_similarity_context_wavlm = np.dot(
                        spk_embedding_context_wavlm, spk_embedding_pred_wavlm
                    ) / (
                        np.linalg.norm(spk_embedding_context_wavlm)
                        * np.linalg.norm(spk_embedding_pred_wavlm)
                    )
                    gt_similarity_context_wavlm = np.dot(
                        spk_embedding_context_wavlm, spk_embedding_gt_wavlm
                    ) / (
                        np.linalg.norm(spk_embedding_context_wavlm)
                        * np.linalg.norm(spk_embedding_gt_wavlm)
                    ...
                    if log_scalars:
                        self.logger.experiment.add_scalar(
                            f"Inf SV Cossim Context Pred", pred_similarity_context, step
                        )
                        self.logger.experiment.add_scalar(
                            f"Inf SV Cossim Context GT", gt_similarity_context, step
                        )
                    pred_context_similarity_list.append(pred_similarity_context)
                    gt_context_similarity_list.append(gt_similarity_context)
                    pred_context_similarity_list_wavlm.append(
                        pred_similarity_context_wavlm
                    )
                    ...
                    self.logger.experiment.add_text(
                        "Inf Task Question", task_question, step
                    )
                    ...
                    task_question_phoneme_tokens = [
                        v[1] - self.lm_vocab_size
                        for v in input_token_list
                        if v[1] >= self.lm_vocab_size
                    ]
                    if len(task_question_phoneme_tokens) > 0:
                        ...
                        self.logger.experiment.add_text(
                            "Inf Task Question Phoneme Text", phoneme_text, step
                        )

                    # store predicted_tokens for each layer to compute token error rate
                    for layer_idx in range(self.num_speech_codebooks):
                        ter_dict[layer_idx]["hypothesis"].append(
                            predicted_tokens[layer_idx].cpu().numpy().tolist()
                        )
                        ter_dict[layer_idx]["gt"].append(
                            dec_input_to_1024_answer[layer_idx].cpu().numpy().tolist()
                        )
                    ...
                    if self.estimate_mos:
                        ...
                        if context_wav is not None:
                            squim_mos_score_context = squim_mos_model(
                                context_wav.to(device).unsqueeze(0)
                            ).item()
            ...
            if log_scalars:
                for layer_idx in range(self.num_speech_codebooks):
                    wer = word_error_rate(
                        ter_dict[layer_idx]["hypothesis"],
                        ter_dict[layer_idx]["gt"],
                        use_cer=True,
                    )
                    self.logger.experiment.add_scalar(
                        f"Inf TER Layer {layer_idx}", wer, 0
                    )

            greedy_transcripts = []
            if len(audio_to_pred) > 0:
                greedy_transcripts.extend(
                    asr_model.transcribe([i["audio"] for i in audio_to_pred])[0]
                )
            if len(audio_to_pred_zh) > 0:
                greedy_transcripts.extend(
                    asr_model_zh.transcribe([i["audio"] for i in audio_to_pred_zh])[0]
                )
            ...
            for i in range(0, len(greedy_transcripts) - 1, 2):
                ...
                step = (
                    batch_idx * test_dataloader_batch_size
                    + all_audio_to_pred[i]["step"]
                )
                question_text = question_texts[i // 2]

                # No need to process text since both are ASR outputs
                cer_sample = word_error_rate(
                    [greedy_transcripts[i]], [greedy_transcripts[i + 1]], use_cer=True
                )
                wer_sample = word_error_rate(
                    [greedy_transcripts[i]], [greedy_transcripts[i + 1]], use_cer=False
                )

                # Processing text since one is ASR output and the other is the GT text
                cer_gt = word_error_rate(
                    [self.process_text(greedy_transcripts[i])],
                    [self.process_text(question_text)],
                    use_cer=True,
                )
                ...
                self.logger.experiment.add_text(
                    "Inf Predicted Text", greedy_transcripts[i], step
                )
                self.logger.experiment.add_text(
                    "Inf GT Text", greedy_transcripts[i + 1], step
                )
                self.logger.experiment.add_text(
                    "Inf Question Text", question_text, step
                )
                if log_scalars:
                    self.logger.experiment.add_scalar(
                        f"Inf CER Transcript", cer_sample, step
                    )
                    self.logger.experiment.add_scalar(
                        f"Inf WER Transcript", wer_sample, step
                    )
                    self.logger.experiment.add_scalar(
                        f"Inf CER GT Transcript", cer_gt, step
                    )
                ...
                if question_type[all_audio_to_pred[i]["step"]] == "Phoneme TTS":
                    if log_scalars:
                        self.logger.experiment.add_scalar(
                            f"Inf CER Phoneme Task", cer_sample, step
                        )
                        self.logger.experiment.add_scalar(
                            f"Inf WER Phoneme Task", wer_sample, step
                        )
                        self.logger.experiment.add_scalar(
                            f"Inf CER GT Phoneme Task", cer_gt, step
                        )
                    ...
                    question_type[all_audio_to_pred[i]["step"]] == "Text to speech this"
                ):
                    if log_scalars:
                        self.logger.experiment.add_scalar(
                            f"Inf CER TTS Task", cer_sample, step
                        )
                        self.logger.experiment.add_scalar(
                            f"Inf WER TTS Task", wer_sample, step
                        )
                        self.logger.experiment.add_scalar(
                            f"Inf CER GT TTS Task", cer_gt, step
                        )
            ...
            pred_context_similarity_avg_wavlm = np.mean(
                pred_context_similarity_list_wavlm
            )
            ...
            if log_scalars:
                self.logger.experiment.add_scalar(
                    f"Inf SV Avg Cossim", similarity_avg, batch_idx
                )
            self.predict_step_outputs.append(
                {
                    ...
                    "squim_mos_pred": (
                        np.mean(squim_mos_list_pred)
                        if len(squim_mos_list_pred) > 0
                        else None
                    ),
                    "squim_mos_context": (
                        np.mean(squim_mos_list_context)
                        if len(squim_mos_list_context) > 0
                        else None
                    ),
                    "squim_mos_gt": (
                        np.mean(squim_mos_list_gt)
                        if len(squim_mos_list_gt) > 0
                        else None
                    ),
                    "cer_transcript": np.mean(cer_batch),
                    "wer_transcript": np.mean(wer_batch),
                    "cer_phoneme": (
                        np.mean(cer_phoneme) if len(cer_phoneme) > 0 else None
                    ),
                    "wer_phoneme": (
                        np.mean(wer_phoneme) if len(wer_phoneme) > 0 else None
                    ),
                    ...
                    "cer_phoneme_gt": (
                        np.mean(cer_phoneme_gt) if len(cer_phoneme_gt) > 0 else None
                    ),
                    "wer_phoneme_gt": (
                        np.mean(wer_phoneme_gt) if len(wer_phoneme_gt) > 0 else None
                    ),
    ...
    def on_predict_epoch_end(self, outputs: List[Any]) -> None:
        ...
        all_labels = list(
            itertools.chain(*[item["labels_text"] for item in outputs[0]])
        )
