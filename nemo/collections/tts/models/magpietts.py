...
def setup_tokenizers(
    all_tokenizers_config, use_text_conditioning_tokenizer, mode="train"
):
    ...
    aggregated_tokenizer = AggregatedTTSTokenizer(
        tokenizers, tokenizer_names
    )  # TTS Transcript tokenizer
...
class MagpieTTS_Model(ModelPT):
    ...
    def __init__(self, cfg: DictConfig, trainer: "Trainer" = None):
        ...
        self.use_text_conditioning_encoder = cfg.get(
        ...
        self.context_audio_bos_id = (
            cfg.num_audio_tokens_per_codebook - 2
        )  # For backward compatibility
        self.context_audio_eos_id = (
            cfg.num_audio_tokens_per_codebook - 1
        )  # For backward compatibility
        ...
        for _ in range(cfg.num_audio_codebooks):
            audio_embeddings.append(
                nn.Embedding(cfg.num_audio_tokens_per_codebook, cfg.embedding_dim)
            )
        ...
        self.final_proj = nn.Linear(
            cfg.decoder.d_model,
            cfg.num_audio_codebooks * cfg.num_audio_tokens_per_codebook,
        )

        codec_model = AudioCodecModel.restore_from(
            cfg.get("codecmodel_path"), strict=False
        )
        ...
        if self.model_type == "single_encoder_sv_tts":
            ...
            self.speaker_projection_layer = nn.Linear(
                cfg.speaker_emb_dim, cfg.embedding_dim
            )
            ...
            self.transcript_decoder_layers = cfg.get(
                "transcript_decoder_layers", [3, 4, 5, 6, 7, 8]
            )
            ...
            for layer in self.transcript_decoder_layers:
                multi_encoder_mapping[layer] = (
                    0  # 0 means text goes to this layer, 1 means context goes to this layer
                )
            for layer in self.context_decoder_layers:
                multi_encoder_mapping[layer] = 1
            self.multi_encoder_mapping = multi_encoder_mapping
            self.context_encoder = transformer_2501.Transformer(
                **dict(cfg.context_encoder)
            )
            ...
            assert (
                cfg.alignment_loss_scale == 0.0
            ), "Alignment loss is not supported for decoder pretrain synthesizer"
        else:
            raise ValueError(f"Unsupported model type {self.model_type}")

        if self.use_text_conditioning_encoder:
            self.context_text_embedding = nn.Embedding(
                self.text_conditioning_tokenizer.vocab_size, cfg.embedding_dim
            )
    ...
    def audio_to_codes(self, audio, audio_len, audio_type="target"):
        ...
        if audio_type == "target":
            ...
            raise ValueError(
                f"Received audio_type of {audio_type}. Must be `target` or `context`"
            )

        self._codec_model.eval()
        with torch.no_grad():
            codes, codes_len = self._codec_model.encode(
                audio=audio, audio_len=audio_len
            )
            # Add a timestep to begining and end of codes tensor
            bos_tensor = torch.full(
                (codes.size(0), codes.size(1), 1),
                audio_bos_id,
                dtype=codes.dtype,
                device=codes.device,
            )
    ...
    def codes_to_audio(self, codes, codes_len):
        ...
        with torch.no_grad():
            ...
            codes[codes == self.audio_bos_id] = (
                0  # zero is the padding token in the audio codebook
            )
            codes[codes == self.audio_eos_id] = 0
            # self.additional_models['codec'] = self.additional_models['codec'].to(codes.device)
            audio, audio_len = self._codec_model.decode(
                tokens=codes, tokens_len=codes_len
            )
            # audio: (B, T)
            # audio_len: (B,)
    ...
    def forward(
        ...
        all_code_logits = self.final_proj(
            decoder_out["output"]
    ...
    def sample_codes_from_logits(self, all_code_logits_t, temperature=0.7, topk=80):
        ...
        for idx in range(self.cfg.num_audio_codebooks):
            si = idx * self.cfg.num_audio_tokens_per_codebook
            ei = si + self.cfg.num_audio_tokens_per_codebook
            codebook_logits = all_code_logits_t[
                :, si:ei
            ]  # (B, num_tokens_per_codebook)
            codebook_logits_topk = torch.topk(codebook_logits, topk, dim=-1)[
                0
            ]  # (B, topk)
            ...
            codebook_probs = torch.softmax(
                codebook_logits / temperature, dim=-1
            )  # (B, num_tokens_per_codebook)
    ...
    def log_attention_probs(
        ...
        with torch.no_grad():
            attention_prob_matrix = torch.cat(
                attention_prob_matrix, dim=1
            )  # (B, C, audio_timesteps, text_timesteps)
            attention_prob_matrix_mean = attention_prob_matrix.mean(
                dim=1
            ...
            for idx in range(min(3, attention_prob_matrix_mean.size(0))):
                item_attn_matrix = attention_prob_matrix_mean[idx][
                    dec_context_size : dec_context_size + audio_codes_lens[idx],
                    : text_lens[idx],
                ]
    ...
    def log_train_val_example(
        ...
        pred_audio, pred_audio_lens = self.codes_to_audio(
            pred_audio_codes, audio_codes_lens_target
        )
        target_audio, target_audio_lens = self.codes_to_audio(
            target_audio_codes, audio_codes_lens_target
        )
        ...
        if context_audio_codes is not None and context_audio_codes.shape[2] > 3:
            ...
            context_audio, context_audio_lens = self.codes_to_audio(
    ...
    def compute_alignment_loss(
        self, attention_scores, text_lens, audio_lens, dec_context_size=0
    ):
        # attention scores: List of (B, C, audio_timesteps, text_timesteps)
        attention_scores_combined = torch.cat(
    ...
    def prepare_context_tensors(self, batch):
        ...
        if self.model_type != "decoder_pretrain_synthesizer":
            ...
            text_encoder_out = self.encoder(
                text_embedded, text_mask, cond=None, cond_mask=None
            )[
                "output"
            ]  # (B, T, E)
        ...
        if self.model_type == "single_encoder_sv_tts":
            ...
            speaker_embeddings = self.get_speaker_embeddings(
                target_audio_16khz, target_audio_lens_16khz
            )
            speaker_embeddings_projected = self.speaker_projection_layer(
                speaker_embeddings
            )
            ...
            if self.use_text_conditioning_encoder:
                ...
                context_text_embedded = self.context_text_embedding(
                    context_text_tokens
                )  # (B, L, E)
                ...
                if context_audio_embedded.size(1) < context_text_embedded.size(1):
                    ...
                    context_audio_embedded = torch.cat(
                        [context_audio_embedded, padding], dim=1
                    )
                    ...
                    context_text_embedded = torch.cat(
                        [context_text_embedded, padding], dim=1
                    )  # (B, T, E)
                has_text_context = (
                    batch["has_text_context"].unsqueeze(-1).unsqueeze(-1).float()
                )  # (B, 1, 1)
                context_input_embedded = (
                    has_text_context * context_text_embedded
                    + (1 - has_text_context) * context_audio_embedded
                )
                context_input_lens = (
    ...
    def prepare_dummy_cond_for_cfg(
        self, cond, cond_mask, additional_decoder_input, additional_dec_mask
    ):
    ...
    def process_batch(self, batch, mode="train"):
        ...
        if use_cfg and torch.rand(1).item() < self.cfg.cfg_unconditional_prob:
            ...
            if (
                ...
                max_codebook_val = self.cfg.get(
                    "dec_random_input_max", self.cfg.num_audio_tokens_per_codebook
                )
                ...
                random_audio_tokens = torch.randint(
                    0,
                    max_codebook_val,
                    audio_codes_input.size(),
                    device=audio_codes_input.device,
                )
                random_audio_tokens = random_audio_tokens * audio_codes_mask.unsqueeze(
                    1
                )
                dec_dropout_mask = (
                    torch.rand(
                        (1, 1, audio_codes_input.size(2)),
                        device=audio_codes_input.device,
                    )
                ...
                audio_codes_input = (
                    audio_codes_input * dec_dropout_mask
                    + random_audio_tokens * (~dec_dropout_mask)
                )
        ...
        if context_tensors["additional_decoder_input"] is not None:
            dec_input_embedded = torch.cat(
                [additional_decoder_input, audio_codes_embedded], dim=1
            )
            dec_input_mask = torch.cat(
                [additional_decoder_mask, audio_codes_mask], dim=1
            )
        else:
        ...
        logits = logits[
            :, dec_context_size:, :
        ]  # Remove the context audio embeddings from the logits
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if batch_idx == 0 and self.global_rank == 0:
            ...
            if (
                ...
                and len(
                    attn_info[self.transcript_decoder_layers[0]][
    ...
    def infer_batch(
        ...
        with torch.no_grad():
            ...
            audio_codes_bos = torch.full(
                (text.size(0), self.cfg.num_audio_codebooks, 1),
                self.audio_bos_id,
                device=text.device,
            ).long()
            ...
            for idx in range(max_decoder_steps):
                ...
                if context_tensors["additional_decoder_input"] is not None:
                    ...
                    _audio_codes_mask = torch.cat(
                        [context_tensors["addtional_decoder_mask"], audio_codes_mask],
                        dim=1,
                    )
                else:
                ...
                if use_cfg:
                    ...
                    if isinstance(context_tensors["cond"], list):
                        cfg_cond = [
                            ...
                            for cond_item, dummy_cond_item in zip(
                                context_tensors["cond"], dummy_cond
                            )
                        ]
                        cfg_cond_mask = [
                        ...
                        cfg_cond = torch.cat(
                            [context_tensors["cond"], dummy_cond], dim=0
                        )
                        cfg_cond_mask = torch.cat(
                            [context_tensors["cond_mask"], dummy_cond_mask], dim=0
                        )
                    cfg_audio_codes_embedded = torch.cat(
                        [_audio_codes_embedded, _audio_codes_embedded], dim=0
                    )
                    cfg_audio_codes_mask = torch.cat(
                        [_audio_codes_mask, _audio_codes_mask], dim=0
                    )
                    if dummy_additional_decoder_input is not None:
                        cfg_audio_codes_embedded[
                            batch_size:, : dummy_additional_decoder_input.size(1)
                        ] = dummy_additional_decoder_input
                    ...
                    all_code_logits = (
                        1 - cfg_scale
                    ) * uncond_logits + cfg_scale * cond_logits
                else:
                ...
                all_code_logits_t = all_code_logits[
                    :, -1, :
                ...
                for item_idx in range(all_codes_next_argmax.size(0)):
                    if item_idx not in end_indices:
                        ...
                        if (pred_token == self.audio_eos_id) or (
                            pred_token_multinomial == self.audio_eos_id
                        ):
                            print(
                                "End detected for item {} at timestep {}".format(
                                    item_idx, idx
                                )
                            )
            ...
            predicted_codes_lens = torch.tensor(
                predicted_lens, device=text.device
            ).long()

            predicted_audio, predicted_audio_lens = self.codes_to_audio(
        ...
        with torch.no_grad():
            ...
            for idx in range(predicted_audio.size(0)):
                ...
                audio_path = os.path.join(
                    audio_dir, f"predicted_audioRank{self.global_rank}_{item_idx}.wav"
                )
    ...
    def on_validation_epoch_end(self):
        ...
        self.log(
            "val_alignment_loss", val_alignment_loss, prog_bar=True, sync_dist=True
        )
    ...
    def _setup_train_dataloader(self, cfg):
        ...
        if cfg.dataloader_params.num_workers == 0:
            ...
            dataset.text_tokenizer, dataset.text_conditioning_tokenizer = (
                self._setup_tokenizers(self.cfg)
            )
    ...
    def _setup_test_dataloader(self, cfg):
        ...
        if cfg.dataloader_params.num_workers == 0:
            ...
            # For num workers > 0 tokenizer will be assigned in worker_init_fn (since it is not picklable)
            dataset.text_tokenizer, dataset.text_conditioning_tokenizer = (
                self._setup_tokenizers(self.cfg, mode="test")
            )
...
class MagpieTTS_ModelInference(MagpieTTS_Model):
    ...
    def transcribe_with_whisper(self, audio_filepath, language):
        ...
        forced_decoder_ids = (
            self.whisper_processor.get_decoder_prompt_ids(language=language)
            if language
            else None
        )
    ...
    def process_text(self, input_text):
        ...
        single_space_text = single_space_text.translate(
    ...
    def test_step(self, batch, batch_idx):
        with torch.no_grad():
            ...
            for idx in range(predicted_audio.size(0)):
                ...
                predicted_codes_torch = predicted_codes_torch[
                    :, : predicted_codes_lens[idx]
                ]
                torch.save(
                    predicted_codes_torch,
                    os.path.join(
                        audio_dir,
                        f"predicted_audioRank{self.global_rank}_{item_idx}_codes.pt",
                    ),
                )
                ...
                if not batch_invalid:
                    with torch.no_grad():
                            ...
                            if self.cfg.get("pref_set_language", "en") == "en":
                                ...
                                pred_transcripts = [
                                    self.transcribe_with_whisper(
                                        audio_path, self.cfg.pref_set_language
                                    )
                        ...
                        pred_speaker_embeddings = (
                            self.get_speaker_embeddings_from_filepaths(
                                predicted_audio_paths
                            )
                        )
                        gt_speaker_embeddings = (
                            self.get_speaker_embeddings_from_filepaths(
                                batch["audio_filepaths"]
                            )
                        )
            ...
            for idx in range(predicted_audio.size(0)):
                if not batch_invalid:
                    ...
                    cer_gt = word_error_rate(
                        [pred_transcript], [gt_transcript], use_cer=True
                    )
                    wer_gt = word_error_rate(
                        [pred_transcript], [gt_transcript], use_cer=False
                    )
                    ...
                    spk_similarity = np.dot(spk_embedding_pred, spk_embedding_gt) / (
                        np.linalg.norm(spk_embedding_pred)
                        * np.linalg.norm(spk_embedding_gt)
                    )
                else:
                ...
                with open(
                    os.path.join(
                        audio_dir,
                        f"predicted_audioRank{self.global_rank}_{item_idx}_metrics.json",
                    ),
                    "w",
                ) as f:
...
class MagpieTTS_ModelDPO(MagpieTTS_Model):
    ...
    def preference_loss(
        ...
        if loss_type == "ipo":
            ...
            gt_rewards_delta = gt_reward_scale * (
                chosen_gt_rewards - rejected_gt_rewards
            )
            ...
            losses = torch.exp(logalpha_hat_chosen) * (
    ...
    def process_batch_dpo(self, batch_chosen_rejected):
        ...
        with torch.no_grad():
            reference_model_output_chosen = self._reference_model.process_batch(
                batch_chosen
            )
            reference_model_output_rejected = self._reference_model.process_batch(
                batch_rejected
            )
        ...
        for codebook_idx in range(self.cfg.num_audio_codebooks):
            ...
            ref_codebook_logits_chosen = reference_model_output_chosen["logits"][
                :, :, si:ei
            ]
            ref_codebook_logits_rejected = reference_model_output_rejected["logits"][
                :, :, si:ei
            ]

            codebook_labels_chosen = model_output_chosen["audio_codes_target"][
                :, codebook_idx
            ]
            codebook_labels_rejected = model_output_rejected["audio_codes_target"][
                :, codebook_idx
            ]

            codebook_log_probs_chosen = self._get_batch_logps(
                codebook_logits_chosen,
                codebook_labels_chosen,
                model_output_chosen["loss_mask"],
            )
            codebook_log_probs_rejected = self._get_batch_logps(
                codebook_logits_rejected,
                codebook_labels_rejected,
                model_output_rejected["loss_mask"],
            )
            with torch.no_grad():
                ref_codebook_log_probs_chosen = self._get_batch_logps(
                    ref_codebook_logits_chosen,
                    codebook_labels_chosen,
                    reference_model_output_chosen["loss_mask"],
                )
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log(
            "train_pref_loss", dpo_outputs["pref_loss"], prog_bar=True, sync_dist=True
        )
        self.log(
            "train_sft_loss", dpo_outputs["sft_loss"], prog_bar=True, sync_dist=True
        )
