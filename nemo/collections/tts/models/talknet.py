...
class TalkNetDursModel(ModelPT):
    ...
    @staticmethod
    def _log(durs_true, true_len, durs_pred):
        ...
        acc = ((durs_true == durs_pred) * mask).sum().float() / mask.sum()
        acc = acc * 100
    ...
    def training_step(self, batch, batch_idx):
        ...
        log = self._log(durs, text_len, durs_pred)
        loss, acc = log['loss'], log['acc']
...
class TalkNetSpectModel(SpectrogramGenerator):
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss = self._log(true_mel, text_len, pred_mel)['loss']
    ...
    def validation_step(self, batch, batch_idx):
        ...
        true_mel, _ = self.preprocessor(audio, audio_len)
        pred_mel = self(text=text, text_len=text_len)
        log = self._log(true_mel, text_len, pred_mel)
