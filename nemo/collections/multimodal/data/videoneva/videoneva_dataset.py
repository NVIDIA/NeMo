...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, data_path: str, tokenizer, multimodal_cfg: dict):
        ...
        self.image_processor = multimodal_cfg["image_processor"]
        self.video_processor = multimodal_cfg["video_processor"]

        self.image_loader = TarOrFolderVisualLoader(self.image_folder)
        self.video_loader = TarOrFolderVisualLoader(self.video_folder)
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in sources[0] and 'video' not in sources:
            ...
            for video_file in self.list_data_dict[i]['video']:
                video = self.video_loader.open_video(video_file)
                if video is None:
                    logging.warning(f"Image {video_file} could not be found!")
                if isinstance(self.video_processor, LanguageBindVideoProcessor):
                    video = self.image_processor.preprocess(video, return_tensors='pt')['pixel_values'][0]
                else:
                    video = self.video_processor(video)
                videos.append(video)
            videos_tensor = torch.tensor([])
            if videos:
        ...
        if self.multimodal_cfg['is_multimodal']:
            if "images" in sources[0]:
                if isinstance(self.image_processor, CLIPImageProcessor):
                    crop_size = [self.image_processor.crop_size['height'], self.image_processor.crop_size['width']]
                else:
            ...
            if "videos" in sources[0]:
                data_dict['video'] = videos_tensors
        return data_dict
...
class NevaDataset(LazySupervisedDataset):
    ...
    def __init__(self, data_path: str, tokenizer, multimodal_cfg: dict):
        ...
        if data_path.endswith(".json"):
            ...
            video_folder = multimodal_cfg['video_folder']
