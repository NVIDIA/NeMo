...
class TarOrFolderVideoLoader:
    ...
    Attributes:
        ...
        data_config (dict): Dataset configuration parameters

    Methods:
        __init__(self, video_folder, data_config): Initializes the loader with the specified video folder.
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, data_path: str, tokenizer, multimodal_cfg: dict, data_cfg: dict):
        ...
        self.video_loader = TarOrFolderVideoLoader(self.video_folder)
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in sources[0]:
            ...
            images = []
            for video_file in self.list_data_dict[i]['image']:
                video = self.video_loader.open_video(image_file)
                image_frames = self.video_loader.flatten_frames(video)
                if image is None:
