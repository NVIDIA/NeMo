    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in sources[0]:
            ...
            if videos:
                ...
                cur_token_len = (media_tensors[0][0].shape[1] // 14) * (
                    media_tensors[0][0].shape[2] // 14
                )  # FIXME: 14 is hardcoded patch size
                sources = preprocess_multimodal(
                    copy.deepcopy(sources),
                    self.multimodal_cfg,
        ...
        if self.multimodal_cfg['is_multimodal']:
            ...
            media_tensors = torch.cat((media_tensors, zero_padding.unsqueeze(0)), dim=1)
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if media is None:
            ...
            media = rearrange(media, "b n T c h w -> b (n T) 1 c h w")
