...
def get_video_to_text_bpe_dataset_from_config(
    ...
    if is_concat:
        if "concat_sampling" in config and config["concat_sampling"] is None:
            logging.warning(
                f"Concat dataset requires `concat_sampling` but it was not provided. Config: {config}"
            )
            return None
    ...
    if config.get("is_tarred", False):
        ...
        if is_concat:
            raise NotImplementedError(
                "get_concat_tarred_dataset method not implemented"
            )
        else:
...
def get_tarred_dataset(
    ...
    for dataset_idx, (tarred_audio_filepath, manifest_filepath) in enumerate(
        ...
        if tokenizer is None:
            raise Exception(
                "video_to_text.TarredVideoToCharDataset class not Implemented"
            )
        else:
