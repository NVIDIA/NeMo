...
class GeneralizedAttention(nn.Module):
    ...
    def __init__(
        ...
        self.value_conv = nn.Conv2d(
            in_channels=in_channels, out_channels=self.v_dim * num_heads, kernel_size=1, bias=False
        )
        ...
        self.proj_conv = nn.Conv2d(
            in_channels=self.v_dim * num_heads, out_channels=in_channels, kernel_size=1, bias=True
        )
                ...
                for ix in range(max_len):
                    local_constraint_map[
                        ...
                        max((iy - self.spatial_range) // self.kv_stride, 0) : min(
                            (iy + self.spatial_range + 1) // self.kv_stride + 1, max_len
                        ),
                        max((ix - self.spatial_range) // self.kv_stride, 0) : min(
                            (ix + self.spatial_range + 1) // self.kv_stride + 1, max_len
                        ),
                    ] = 0

            self.local_constraint_map = nn.Parameter(
                torch.from_numpy(local_constraint_map).byte(), requires_grad=False
            )
    ...
    def get_position_embedding(self, h, w, h_kv, w_kv, q_stride, kv_stride, device, dtype, feat_dim, wave_length=1000):
        ...
        feat_range = torch.arange(0, feat_dim / 4).to(device=device, dtype=dtype)
    ...
    def forward(self, x_input):
        ...
        if self.attention_type[1] or self.attention_type[3]:
            ...
            position_feat_x = (
                self.appr_geom_fc_x(position_embed_x)
            ...
            position_feat_y = (
                self.appr_geom_fc_y(position_embed_y)
            ...
            if self.attention_type[1] or self.attention_type[3]:
                if self.attention_type[1] and self.attention_type[3]:
                    ...
                    energy_x = torch.matmul(
                        proj_query_reshape.permute(0, 1, 3, 2, 4), position_feat_x.permute(0, 1, 2, 4, 3)
                    )
        ...
        if self.spatial_range >= 0:
            cur_local_constraint_map = (
                self.local_constraint_map[:h, :w, :h_kv, :w_kv].contiguous().view(1, 1, h * w, h_kv * w_kv)
            )
        ...
        out = (
            torch.matmul(attention, proj_value_reshape)
            .permute(0, 1, 3, 2)
            .contiguous()
