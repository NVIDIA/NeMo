...
class ContinousDDPMPrecond(PrecondModel):
    def __init__(
        ...
        self.scheduler = GaussianDiffusionContinuousTimes(
            noise_schedule=noise_schedule, timesteps=timesteps
        )
        self.pred_objective = pred_objective
        assert (
            noise_cond_aug == False
        ), "noise cond aug currently not supported for DDPM"
    ...
    def get_xt(self, x0, t=None, epsilon=None):
        ...
        x_noisy, log_snr, alpha, sigma = self.scheduler.q_sample(
            x_start=x0,
            t=t,
            noise=epsilon,
        )
    ...
    def forward(self, x, time, text_embed, text_mask, **model_kwargs):
        ...
        return self.unet(
            x=x, time=time, text_embed=text_embed, text_mask=text_mask, **model_kwargs
        )

    def compute_loss(
        self, x0, text_embed, text_mask, time=None, noise=None, **model_kwargs
    ):
...
class EDMPrecond(PrecondModel):
    ...
    def forward(self, x, time, text_embed, text_mask, **model_kwargs):
        ...
        if self.noise_cond_aug:
            ...
            assert (
                "x_low_res" in model_kwargs
            ), "x_low_res does not exist when attemping to apply noise augmentation"
            ...
            if self.inference:
                ...
                x_low_res_noisy, time_low_res = self.get_xt(
                    x0=x_low_res, t=time_low_res, epsilon=None
                )
            else:
                x_low_res_noisy, time_low_res = self.get_xt(
                    x0=x_low_res, t=None, epsilon=None
                )
        ...
        F_x = self.unet(
            batch_mul(c_in, x), c_noise, text_embed, text_mask, **model_kwargs
        )
    ...
    def sample_time(self, batch_size, device=None):
        return (
            torch.randn(batch_size, device=device, generator=self.rng) * self.p_std
            + self.p_mean
        ).exp()
    ...
    def compute_loss(
        ...
        target = x0
        return self.loss_fn(
            batch_mul(weight, target),
            batch_mul(weight, pred),
        )
