...
class Imagen(torch.nn.Module, Serialization):
    def __init__(self, cfg, model_parallel_config):
        ...
        if self.unet_type == "base":
            ...
            unet = EfficientUNetModel(
                **cfg.unet,
                text_embed_dim=cfg.conditioning.embed_dim,
                noise_cond_aug=self.noise_cond_aug,
            )
        ...
        if self.channels_last:
            assert (
                OPT_GROUP_NORM
            ), "Training in channels last format requires optmized group norm implementation."
        ...
        if self.preconditioning_type == "DDPM":
            ...
            self.model = ContinousDDPMPrecond(
                unet=unet, **cfg.preconditioning, noise_cond_aug=self.noise_cond_aug
            )
            self.sampler = DDPMSampler(
                unet_type=self.unet_type, denoiser=self.model.scheduler
            )
            ...
            raise NotImplemented(
                f"{self.preconditioning_type} preconditioning is not implemented."
            )
    ...
    def setup_rng(self):
        ...
        logging.info(
            f"RNG seed set as {self.rng_seed} for rank {parallel_state.get_data_parallel_rank()}"
        )
    ...
    def get_text_encoder(self, encoder_path=None):
        ...
        return T5Encoder(
            max_seq_len=self.conditioning.token_length, encoder_path=encoder_path
        )

    def forward(self, x_start, text_embed, text_mask, x_lowres=None):
        if self.unet_type == "base":
            assert (
                x_lowres[0].item() == DUMMY_TENSOR.item()
            ), "Base model should have no low-resolution conditioning"
            x_lowres = None
        else:
            assert x_lowres[0].dim() not in [
                0,
                1,
            ], "SR model should have low-resolution conditioning"
...
class MegatronImagen(MegatronBaseModel):
    ...
    def on_fit_start(self) -> None:
        if self.online_encoding:
            ...
            self.text_encoder = self.model.get_text_encoder(
                encoder_path=self.text_encoder_path
