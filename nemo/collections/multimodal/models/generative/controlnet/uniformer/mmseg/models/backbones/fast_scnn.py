...
from nemo.collections.multimodal.models.generative.controlnet.uniformer.mmseg.models.decode_heads.psp_head import PPM
...
class LearningToDownsample(nn.Module):
    ...
    def __init__(
        ...
        self.dsconv1 = DepthwiseSeparableConvModule(
            dw_channels1, dw_channels2, kernel_size=3, stride=2, padding=1, norm_cfg=self.norm_cfg
        )
...
class FeatureFusionModule(nn.Module):
    ...
    def __init__(
        ...
        self.dwconv = ConvModule(
            lower_in_channels, out_channels, 1, conv_cfg=self.conv_cfg, norm_cfg=self.norm_cfg, act_cfg=self.act_cfg
        )
        self.conv_lower_res = ConvModule(
            out_channels, out_channels, 1, conv_cfg=self.conv_cfg, norm_cfg=self.norm_cfg, act_cfg=None
        )
        self.conv_higher_res = ConvModule(
            higher_in_channels, out_channels, 1, conv_cfg=self.conv_cfg, norm_cfg=self.norm_cfg, act_cfg=None
        )
