...
from nemo.collections.multimodal.models.stable_diffusion.ldm.autoencoder import VQModelInterface, IdentityFirstStage, \
    AutoencoderKL
from nemo.collections.multimodal.models.stable_diffusion.ldm.ddpm import random_dropout
from nemo.collections.multimodal.models.stable_diffusion.ldm_config import DDPMDiffusionModelConfig, \
...
from nemo.collections.multimodal.modules.stable_diffusion.distributions.distributions import normal_kl, \
...
class DDPM(DiffusionModel):
    ...
    def __init__(self, cfg: DDPMDiffusionModelConfig, trainer=None):
        ...
        self.use_fp16 = (trainer is not None) and (trainer.precision == 16)
        ...
        if self.use_fp16:
            with open_dict(cfg.unet_config):
                cfg.unet_config.use_fp16 = True
...
class LatentDiffusion(DDPM):
    ...
    def __init__(self, cfg: LatentDiffusionModelConfig, trainer=None):
        ...
        if self.use_fp16:
            with open_dict(cfg.cond_stage_config):
                cfg.cond_stage_config.update({"use_fp16": True})
        else:
            with open_dict(cfg.cond_stage_config):
                cfg.cond_stage_config.update({"use_fp16": False})
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False,
        ...
        if self.model.conditioning_key is not None:
            ...
            if cond_key != self.first_stage_key:
                if cond_key in ['captions', 'coordinates_bbox', 'txt']:
                    xc = batch[cond_key]
            ...
            if self.text_embedding_dropout_rate > 0:
                assert (self.text_embedding_dropout_rate < 1.)
    ...
    def configure_gradient_clipping(self, optimizer, optimizer_idx, gradient_clip_val, gradient_clip_algorithm):
        ...
        if not hasattr(self, "grad_clip_logged"):
            logging.info("Gradient clipping: val, %f; algo, %s",
                         gradient_clip_val, gradient_clip_algorithm)
            self.grad_clip_logged = True

        parameters = amp.master_params(optimizer)
        if gradient_clip_algorithm == GradClipAlgorithmType.VALUE:
            torch.nn.utils.clip_grad_value_(parameters, clip_value=gradient_clip_val)
        elif gradient_clip_algorithm == GradClipAlgorithmType.NORM:
            clip_grad_norm_(parameters, gradient_clip_val)
