...
class PLMSSampler(AbstractBaseSampler):
    ...
    def make_schedule(self, ddim_num_steps, ddim_discretize="uniform", ddim_eta=0., verbose=False):
        ...
        super().make_schedule(ddim_num_steps, ddim_discretize="uniform", ddim_eta=0., verbose=False)
    ...
    def p_sampling_fn(self, x, c, t, index, repeat_noise=False, use_original_steps=False, quantize_denoised=False,
        ...
        if len(old_eps) == 0:
            ...
            x_prev, pred_x0 = self._get_x_prev_and_pred_x0(use_original_steps, b, index, device, x, e_t,
                                                           quantize_denoised, repeat_noise, temperature, noise_dropout)
            e_t_next = self._get_model_output(x_prev, t_next, unconditional_conditioning, unconditional_guidance_scale,
                                              score_corrector, c, corrector_kwargs)
        ...
        x_prev, pred_x0 = self._get_x_prev_and_pred_x0(use_original_steps, b, index, device, x, e_t_prime,
                                                       quantize_denoised, repeat_noise, temperature, noise_dropout)
