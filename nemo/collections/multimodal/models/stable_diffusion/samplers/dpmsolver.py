...
from nemo.collections.multimodal.modules.stable_diffusion.diffusionmodules.util import interpolate_fn, expand_dims
...
class NoiseScheduleVP:
    ...
    def marginal_log_mean_coeff(self, t):
        ...
        if self.schedule == 'discrete':
            ...
            def log_alpha_fn(s):
                return torch.log(torch.cos((s + self.cosine_s) / (1. + self.cosine_s) * math.pi / 2.))
    ...
    def inverse_lambda(self, lamb):
        ...
        if self.schedule == 'linear':
            ...
            def t_fn(log_alpha_t):
                return torch.arccos(torch.exp(log_alpha_t + self.cosine_log_alpha_0)) * 2. * (1. + self.cosine_s) / math.pi - self.cosine_s
            t = t_fn(log_alpha)
            return t
