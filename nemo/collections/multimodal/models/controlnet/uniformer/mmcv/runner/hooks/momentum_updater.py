...
class CyclicMomentumUpdaterHook(MomentumUpdaterHook):
    ...
    def __init__(self, by_epoch=False, target_ratio=(0.85 / 0.95, 1), cyclic_times=1, step_ratio_up=0.4, **kwargs):
        if isinstance(target_ratio, float):
            ...
            raise ValueError('target_ratio should be either float ' f'or tuple, got {type(target_ratio)}')
    ...
    def before_run(self, runner):
        ...
        self.momentum_phases.append(
            [iter_up_phase, max_iter_per_phase, max_iter_per_phase, self.target_ratio[0], self.target_ratio[1]]
        )
        ...
        for (start_iter, end_iter, max_iter_per_phase, start_ratio, end_ratio) in self.momentum_phases:
            ...
            if start_iter <= curr_iter < end_iter:
                ...
                return annealing_cos(
                    base_momentum * start_ratio, base_momentum * end_ratio, progress / (end_iter - start_iter)
                )
...
class OneCycleMomentumUpdaterHook(MomentumUpdaterHook):
    ...
    def __init__(
        self, base_momentum=0.85, max_momentum=0.95, pct_start=0.3, anneal_strategy='cos', three_phase=False, **kwargs
    ):
        ...
        if pct_start < 0 or pct_start > 1 or not isinstance(pct_start, float):
            raise ValueError('Expected float between 0 and 1 pct_start, but ' f'got {pct_start}')
        self.pct_start = pct_start
        # validate anneal_strategy
        if anneal_strategy not in ['cos', 'linear']:
            raise ValueError('anneal_strategy must by one of "cos" or ' f'"linear", instead got {anneal_strategy}')
    ...
    def before_run(self, runner):
        ...
        if self.three_phase:
            self.momentum_phases.append(
                {
                    'end_iter': float(self.pct_start * runner.max_iters) - 1,
            ...
            self.momentum_phases.append(
                {
                    'end_iter': float(2 * self.pct_start * runner.max_iters) - 2,
                    'start_momentum': 'base_momentum',
                    'end_momentum': 'max_momentum',
                }
            )
            self.momentum_phases.append(
                {'end_iter': runner.max_iters - 1, 'start_momentum': 'max_momentum', 'end_momentum': 'max_momentum'}
            )
        else:
            self.momentum_phases.append(
                {
            ...
            self.momentum_phases.append(
                {'end_iter': runner.max_iters - 1, 'start_momentum': 'base_momentum', 'end_momentum': 'max_momentum'}
            )
        ...
        for i, phase in enumerate(self.momentum_phases):
            ...
            if curr_iter <= end_iter or i == len(self.momentum_phases) - 1:
                ...
                momentum = self.anneal_func(
                    param_group[phase['start_momentum']], param_group[phase['end_momentum']], pct
                )
                break
            start_iter = end_iter
        return momentum
