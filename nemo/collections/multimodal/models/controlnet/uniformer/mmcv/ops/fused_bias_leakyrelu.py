...
class FusedBiasLeakyReLUFunctionBackward(Function):
    ...
    def forward(ctx, grad_output, out, negative_slope, scale):
        ...
        grad_input = ext_module.fused_bias_leakyrelu(
            grad_output, empty, out, act=3, grad=1, alpha=negative_slope, scale=scale
        )
    ...
    def backward(ctx, gradgrad_input, gradgrad_bias):
        ...
        gradgrad_out = ext_module.fused_bias_leakyrelu(
            gradgrad_input, gradgrad_bias.to(out.dtype), out, act=3, grad=1, alpha=ctx.negative_slope, scale=ctx.scale
        )
