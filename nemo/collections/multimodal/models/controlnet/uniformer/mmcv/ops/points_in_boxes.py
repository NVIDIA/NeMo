...
ext_module = ext_loader.load_ext(
    '_ext', ['points_in_boxes_part_forward', 'points_in_boxes_cpu_forward', 'points_in_boxes_all_forward']
)
...
def points_in_boxes_part(points, boxes):
    ...
    assert points.shape[0] == boxes.shape[0], (
        'Points and boxes should have the same batch size, ' f'but got {points.shape[0]} and {boxes.shape[0]}'
    )
    assert boxes.shape[2] == 7, 'boxes dimension should be 7, ' f'but got unexpected shape {boxes.shape[2]}'
    assert points.shape[2] == 3, 'points dimension should be 3, ' f'but got unexpected shape {points.shape[2]}'
...
def points_in_boxes_cpu(points, boxes):
    ...
    for b in range(batch_size):
        ext_module.points_in_boxes_cpu_forward(
            boxes[b].float().contiguous(), points[b].float().contiguous(), point_indices[b]
        )
...
def points_in_boxes_all(points, boxes):
    ...
    assert boxes.shape[0] == points.shape[0], (
        'Points and boxes should have the same batch size, ' f'but got {boxes.shape[0]} and {boxes.shape[0]}'
    )
    assert boxes.shape[2] == 7, 'boxes dimension should be 7, ' f'but got unexpected shape {boxes.shape[2]}'
    assert points.shape[2] == 3, 'points dimension should be 3, ' f'but got unexpected shape {points.shape[2]}'
    batch_size, num_points, _ = points.shape
