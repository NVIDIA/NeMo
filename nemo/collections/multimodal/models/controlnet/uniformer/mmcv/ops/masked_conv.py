...
ext_module = ext_loader.load_ext('_ext', ['masked_im2col_forward', 'masked_col2im_forward'])


class MaskedConv2dFunction(Function):
    @staticmethod
    def symbolic(g, features, mask, weight, bias, padding, stride):
        return g.op('mmcv::MMCVMaskedConv2d', features, mask, weight, bias, padding_i=padding, stride_i=stride)
    ...
    def forward(ctx, features, mask, weight, bias, padding=0, stride=1):
        ...
        if mask_inds.numel() > 0:
            ...
            ext_module.masked_col2im_forward(
                masked_output, mask_h_idx, mask_w_idx, output, height=out_h, width=out_w, channels=out_channel
            )
        return output
