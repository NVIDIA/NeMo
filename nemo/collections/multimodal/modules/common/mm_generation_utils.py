...
from nemo.collections.multimodal.modules.common.mm_generation_strategy import model_inference_strategy_dispatcher
...
def send_generate_info(
    ...
    end_strings,
    additional_context_tokens_tensor,
    additional_context_tokens_mask_tensor,
):
    """
    ...
    input_info = [
        ...
        repetition_penalty,
        min_tokens_to_generate,
        additional_context_tokens_tensor.size(2),   # num_additional_codebooks
    ]
    ...
    torch.distributed.broadcast(additional_context_tokens_tensor, src, model_parallel_group)
    torch.distributed.broadcast(additional_context_tokens_mask_tensor, src, model_parallel_group)
...
def receive_generate_info():
    ...
    num_additional_codebooks = int(input_info_tensor[11].item())

    context_length_tensor = torch.empty(batch_size, dtype=torch.int64, device=torch.cuda.current_device())
    context_tokens_tensor = torch.empty(batch_size, seq_len, dtype=torch.int64, device=torch.cuda.current_device())
    additional_context_tokens_tensor = torch.empty(
        batch_size, 
        seq_len, 
        num_additional_codebooks, 
        dtype=torch.int64, 
        device=torch.cuda.current_device(),
    )
    additional_context_tokens_mask_tensor = torch.empty(
        batch_size,
        seq_len,
        num_additional_codebooks,
        dtype=torch.float32,
...
def synced_generate(
    ...
    additional_context_tokens_tensor=None,
    additional_context_tokens_mask_tensor=None,
):
    context_length = context_length_tensor.min().item()
    tokenizer = model.tokenizer
    if isinstance(tokenizer, TabularTokenizer):
        raise NotImplementedError("TabularTokenizer not implemented for MultiModal")
        ...
        batch_token_iterator = sample_sequence_batch(
            ...
            additional_context_tokens_tensor=additional_context_tokens_tensor,
            additional_context_tokens_mask_tensor=additional_context_tokens_mask_tensor,
        )
    ...
    **strategy_args,
    ...
    if torch.distributed.get_rank() == get_model_parallel_src_rank():
        if isinstance(inputs, tuple):
            context_tokens_tensor, context_length_tensor, additional_context_tokens_tensor, additional_context_tokens_mask_tensor = inputs
        else:
...
def sample_sequence_batch(
    ...
    with torch.no_grad():
        ...
        additional_tokens = additional_context_tokens_tensor
        additional_tokens_mask = additional_context_tokens_mask_tensor
