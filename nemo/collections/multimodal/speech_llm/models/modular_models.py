...
class ModularAudioGPTModel(SpeechLLMAdapterMixin, MegatronGPTSFTModel):
    ...
    def _create_attention_mask(self, encoder_input: torch.Tensor):
        ...
        attention_mask = torch.tril(
            torch.ones((batch_size, max_len, max_len), device=encoder_input.device)
        ).view(batch_size, 1, max_len, max_len)
    ...
    def _concat_features(self, embs1, emb1_lens, embs2, emb2_lens):
        ...
        for emb1, emb1_len, emb2, emb2_len in zip(embs1, emb1_lens, embs2, emb2_lens):
            ...
            padded_new_emb = torch.zeros(
                emb1.shape[0] + emb2.shape[0], emb1.shape[-1], device=emb1.device
            )
    ...
    def _concat_multi_features(
        ...
        for i in range(batch_size):
            ...
            input_len_list = [
                start_idx_list_i[j + 1] - start_idx_list_i[j]
                for j in range(len(start_idx_list_i) - 1)
            ]
            ...
            encoder_length_i = (
                encoded_len[i].sum() + input_length[i]
            )  # total length of audio and text features
        ...
        encoder_input = torch.stack(
            [
                torch.nn.functional.pad(f, (0, 0, 0, max_length - f.size(0)))
                for f in encoder_input_list
            ]
        )
    ...
    def inject_perception_input(
        ...
        if isinstance(encoded, torch.Tensor):
            # single audio
            encoder_input, encoder_length = self._concat_features(
        ...
        if (
            not hasattr(lm_embedding, "transpose_batch_sequence")
            or lm_embedding.transpose_batch_sequence
        ):
    ...
    def _shift_labels_by_emb_len(
        self, labels, label_lens, emb_lens, max_len, pad_token=0
    ):
    ...
    def _get_text_embeddings(self, text_tokens, position_ids):
        ...
        text_embeddings = lm_embedding.word_embeddings(
            text_tokens
    ...
    def prepare_llm_input(self, audio_batch):
        ...
        encoder_input, attention_mask, encoder_length, _, encoder_max_length = (
        ...
        labels = self._shift_labels_by_emb_len(
            labels, input_length, encoded_len, encoder_max_length, pad_token=0
        )
    ...
    def _gpt_forward(
        ...
        if self.megatron_amp_O2:
            encoder_input = encoder_input.type(
                self.model.module.embedding.word_embeddings.weight.dtype
            )
        if self.mcore_gpt:
            output = self.model(
                input_ids=input_ids,
    ...
    def forward(
        ...
        if "audio_signal" in audio_batch:
            encoder_input, attention_mask, labels, loss_mask, _ = (
                self.prepare_llm_input(audio_batch)
            )
    ...
    def fwd_bwd_step(self, dataloader_iter, forward_only, first_val_step=None):
        ...
        for batch in (audio_batch, text_batch):
            ...
            if "tokens" in batch and "text_input_ids" in batch:
                seq_length = max(
                    batch["tokens"].shape[1], batch["text_input_ids"].shape[1]
                )
    ...
    def get_forward_output_only_func(self):
        def fwd_output_only_func(dataloader_iter, model):
            ...
            if self.megatron_amp_O2:
                input_embeddings = input_embeddings.type(
                    self.model.module.embedding.word_embeddings.weight.dtype
                )
            output_tensor = model(
                input_ids=None,
    ...
    def get_forward_output_and_loss_func(self, validation_step=False, tuning=False):
        def fwd_output_and_loss_func(
            ...
            if not self.mcore_gpt:
                batch["checkpoint_activations_all_layers"] = (
                    checkpoint_activations_all_layers
                )
            ...
            def loss_func(multimodal_output):
                ...
                for key, (output, loss_mask) in multimodal_output.items():
                    cur_loss = self.loss_func(
                        loss_mask.contiguous(), loss_mask.sum(), output.contiguous()
                    )
                ...
                elif validation_step and not self.cfg.data.get(
                    "validation_drop_last", True
                ):
    ...
    def build_data_loader(
        self, dataset, data_cfg, consumed_samples=0, is_predict=False, is_eval=False
    ):
        return build_speechllm_dataloader(
            dataset, data_cfg, consumed_samples, is_predict=is_predict, is_eval=is_eval
        )
        ...
        with open_dict(gpt_cfg):
            ...
            if not use_multi_encoder:
                ...
                for key in gpt_cfg.perception.encoders:
                    model_key = gpt_cfg.perception.encoders[key].get(
                        "model_key", "encoder"
                    )
                    ...
                    if "preprocessor" in audio_cfg[key]:
                        gpt_cfg.perception.encoders[key]["preprocessor"] = audio_cfg[
                            key
                        ].preprocessor
            ...
            if not use_multi_encoder:
                ...
                if (
                    ...
                    and gpt_cfg.perception.multi_layer_feat.aggregator.get(
                        "mode", "cat"
                    )
                    == "cat"
                ):
                    input_dim = encoder_dim * len(
                        gpt_cfg.perception.multi_layer_feat.layer_idx_list
                    )
            else:
                input_dim = 0
                if speaker_cfg is not None:
                ...
                for enc_cfg in gpt_cfg.perception.encoders.values():
                    encoder_dim = get_nested_dict_value(
                        enc_cfg.model, enc_cfg.get("model_dim_key", "d_model")
                    )
                    if (
                        enc_cfg.get("use_multi_layer_feat", False)
                        and enc_cfg.multi_layer_feat.aggregator.get("mode", "cat")
                        == "cat"
                    ):
                        input_dim += encoder_dim * len(
                            enc_cfg.multi_layer_feat.layer_idx_list
                        )
                    else:
                        input_dim += encoder_dim
    ...
    @classmethod
    def _modify_config(
        cls, gpt_cfg, cfg, audio_cfg, add_cfg_to_tree=False, speaker_cfg=None
    ):
        """
        ...
        with open_dict(gpt_cfg):
            ...
            if isinstance(audio_cfg, (ListConfig, list)):
                ...
                if not all([sr == sample_rate[0] for sr in sample_rate]):
                    raise ValueError(
                        "All audio encoders must have the same sample rate."
                    )
    ...
    def get_pretraind_audio_model(cls, encoder_cfg: DictConfig) -> ModelPT:
        ...
        if pretrained_model.endswith(".nemo"):
            logging.info(
                f"Loading pretrained audio model from local file: {pretrained_model}"
            )
            ...
            audio_model = encoder_cls.from_pretrained(
        ...
        if "speaker_model" in cfg.model.perception:
            ...
            if speaker_cfg.get("pretrained_model", None) is not None:
                if speaker_cfg.pretrained_model.endswith(".nemo"):
                    logging.info(
                        f"Loading pretrained speaker model from local file: {speaker_cfg.pretrained_model}"
                    )
                    speaker_model = model_cls.restore_from(
                        speaker_cfg.pretrained_model, map_location="cpu"
                    )
                else:
                    logging.info(
                        f"Loading pretrained speaker model from NGC: {speaker_cfg.pretrained_model}"
                    )
                    speaker_model = model_cls.from_pretrained(
                        speaker_cfg.pretrained_model, map_location="cpu"
                    )
    ...
    def get_audio_encoder_models_and_configs(cls, cfg):
        if "encoders" in cfg.model.perception:
            ...
            if pretrained_audio_model.endswith(".nemo"):
                logging.info(
                    f"Loading pretrained audio model from local file: {pretrained_audio_model}"
                )
                audio_model = model_class.restore_from(
                    pretrained_audio_model, map_location="cpu"
                )
            else:
                logging.info(
                    f"Loading pretrained audio model from NGC: {pretrained_audio_model}"
                )
                audio_model = model_class.from_pretrained(
                    pretrained_audio_model, map_location="cpu"
                )
    ...
    def load_pretrained_audio_weights(
        ...
        model,
        audio_model,
        speaker_model: Optional[EncDecSpeakerLabelModel] = None,
    ):
        ...
        if not use_multi_encoder:
            if cfg.model.perception.get("use_multi_layer_feat", False):
                model.perception.encoder.encoder.load_state_dict(
                    audio_model.encoder.state_dict(), strict=True
                )
            else:
            ...
            logging.info(
                f"Loaded pretrained audio model weights from {cfg.model.pretrained_audio_model}"
            )
            ...
            if speaker_model is not None:
                model.perception.speaker_model.load_state_dict(
                    speaker_model.state_dict(), strict=True
                )
    ...
    def restore_from_pretrained_models(
        ...
        if (
            ...
            raise RuntimeError(
                "PEFT training needs at least one pretrained audio model present."
            )

        if not cfg.model.restore_from_path:
            raise RuntimeError("PEFT training needs a trained base model present.")

        base_model_cfg = MegatronGPTSFTModel.merge_cfg_with(
        ...
        if "peft" in cfg.model:
            ...
            if cfg.model.peft.restore_from_path is not None:
                ...
                model.load_adapters(
                    cfg.model.peft.restore_from_path,
                ...
                raise ValueError(
                    f"PEFT scheme not not found in PEFT_CONFIG_MAP: {cfg.model.peft.peft_scheme}"
                )
        else:
            logging.info(
        ...
        model = cls.load_pretrained_audio_weights(
            cfg, model, audio_model, speaker_model
        )
    ...
    @classmethod
    def load_audio_encoder_for_inference(
        cls, cfg: DictConfig, model_cfg: DictConfig, model: ModelPT
    ) -> ModelPT:
        """
        ...
        """
        if (
            model_cfg.freeze_audio_encoder
            and model_cfg.get("pretrained_audio_model", None) is not None
        ):
            with open_dict(cfg):
    ...
    def merge_inference_cfg(
        ...
        if pretrained_model_cfg:
            ...
            cfg.model.peft.restore_from_path
            or cfg.model.peft.restore_from_ckpt.checkpoint_dir
        ):
            if (
                cfg.model.peft.restore_from_path
                and cfg.model.peft.restore_from_path.endswith(".nemo")
            ):
                ...
                cfg.model.peft.restore_from_hparams_path
            ):  # not a .nemo model we expect a hparams.yaml file
                model_cfg = OmegaConf.to_container(
                    OmegaConf.load(cfg.model.peft.restore_from_hparams_path).cfg
                )
        ...
        if hasattr(model_cfg, "peft") and model_cfg.peft.peft_scheme not in [
            None,
            "none",
        ]:
        ...
        with open_dict(model_cfg):
            ...
            if (
                "context_key" not in model_cfg.data.train_ds
                or "answer_key" not in model_cfg.data.train_ds
            ):
        ...
        with open_dict(cfg):
            if model_cfg.data.test_ds is not None:
                ...
                cfg.inference.tokens_to_generate = model_cfg.data.test_ds.get(
    ...
    def load_adapters_for_inference(
        ...
        if cfg.model.peft.restore_from_path:
            if "\\" in cfg.model.peft.restore_from_path:
                cfg.model.peft.restore_from_path = (
                    cfg.model.peft.restore_from_path.replace("\\", "")
                )
            if "peft" in model_cfg and "peft_scheme" in model_cfg.peft:
                ...
                torch_state_dict = torch.load(
                    cfg.model.peft.restore_from_path, weights_only=False
                )["state_dict"]
            ...
            if not os.path.isdir(checkpoint_path):
                ...
                if "peft" in model_cfg:
                    ...
                    model.load_adapters(
                        checkpoint_path,
                        peft_cfgs=peft_cfg_cls(model_cfg),
                        map_location="cpu",
                    )
                else:
                    model.load_state_dict(
                        torch.load(checkpoint_path, weights_only=False), strict=False
                    )
            else:
            ...
            peft_cfg_cls = PEFT_CONFIG_MAP[model_cfg.peft.peft_scheme]
            model.load_adapters(
                cfg.model.restore_from_path, peft_cfg_cls(model_cfg), map_location="cpu"
            )
        return model
    ...
    def _build_vocab(self):
        ...
        if self._cfg.get("override_vocab_size", None) is not None:
            ...
            self.padded_vocab_size = self._vocab_size_with_padding(
                ...
                make_vocab_size_divisible_by=self._cfg.get(
                    "make_vocab_size_divisible_by", 128
                ),
                tensor_model_parallel_size=self._cfg.get(
        ...
        if self.setup_complete and self.trainer.state.fn == "fit":
            ...
            if self.cfg.freeze_audio_encoder:
                state_dict = {
                    ...
                    if not k.startswith("perception.encoder.")
                }
    ...
    def load_state_dict(self, state_dict, strict: bool = True):
        ...
        if self.use_peft and self.setup_complete:
            ...
            if not self.ptuning_only_and_non_first_stage:
                if set(state_dict.keys()) != self.adapter_keys.union(
    ...
    def inference_step(self, dataloader_iter, mode):
        ...
        if "tokens" in batch:
            ...
            metadata = batch.get("metadata", [{}] * len(batch["tokens"]))
            batch.pop("tokens")
        loss = super(MegatronGPTSFTModel, self).validation_step(
            itertools.chain([batch]), dataloader_idx
        )
        ...
        if self.get_inference_config() is None:
            logging.warning(
                f"inference_config is not set. Use default: {default_inference_config}"
            )
        ...
        if audio_batch:
            inputs_text = [
                self.tokenizer.ids_to_text(c.tolist()) for c in audio_batch["contexts"]
            ]
            labels_text = [
                self.tokenizer.ids_to_text(a.tolist()) for a in audio_batch["answers"]
            ]
            preds_text = [
            ...
            inputs_text = [
                ...
                for c in text_batch["text_context_ids"]
            ]
            labels_text = [
                self.tokenizer.ids_to_text(a.tolist())
                for a in text_batch["text_answer_ids"]
            ]
            preds_text = [
                self.tokenizer.ids_to_text(
                    t[l.item() :][: data_cfg.get("tokens_to_generate")]
        ...
        if data_cfg.get("end_string", None):
            ...
            end_string_re = self.tokenizer.ids_to_text(
                self.tokenizer.text_to_ids(data_cfg.end_string)
            )
            preds_text_cleaned = []
            labels_text_cleaned = []
        ...
        if data_cfg.get("remove_text_pc", False):
            preds_text = [
                remove_punctuations(p.lower(), data_cfg.get("punctuations", None))
                for p in preds_text
            ]
            labels_text = [
                remove_punctuations(l.lower(), data_cfg.get("punctuations", None))
    ...
    def predict_step(
        ...
        if "context_lengths" in batch:
            batch["context_lengths"] = (
                batch["context_lengths"].cuda() + response["audio_feat_lens"]
            )
    ...
    def inference_epoch_end(self, outputs, mode, data_cfg):
        ...
        for dataloader_idx, output in enumerate(outputs):
            ...
            if metric_name != "loss":
                ...
                metric_fn = (
                    self.val_metric[0] if mode == "validation" else self.test_metric[0]
                )
                if metric_label_key in deduplicated_outputs["metadata"][0]:
                    labels = [
                        m[metric_label_key] for m in deduplicated_outputs["metadata"]
                    ]
                else:
                    ...
                    metric_result = torch.Tensor(
                        [
                            sacrebleu.corpus_bleu(
                                deduplicated_outputs["preds"], [labels]
                            ).score
                        ]
                    ).to(self.device)
                else:
            ...
            if self.global_rank == 0 and data_cfg.get(
        ...
        if mode == "validation":
            ...
            if averaged_metric is not None:
                self.log(
                    f"validation_{self.val_metric_name}",
                    averaged_metric,
                    sync_dist=True,
                    batch_size=1,
                )
    ...
    def setup_eval_dataloader(self, datasets, data_cfg):
        ...
        if not isinstance(datasets, list):
            return self.build_data_loader(
                dataset=datasets, data_cfg=data_cfg, consumed_samples=0, is_eval=True
            )
        for dataset in datasets:
            eval_dl = self.build_data_loader(
                dataset=dataset, data_cfg=data_cfg, consumed_samples=0, is_eval=True
            )
    ...
    def setup_predict_dataloader(self, data_cfg):
        ...
        if not isinstance(datasets, list):
            return self.build_data_loader(
                dataset=datasets, data_cfg=data_cfg, consumed_samples=0, is_predict=True
            )
        for dataset in datasets:
            eval_dl = self.build_data_loader(
                dataset=dataset, data_cfg=data_cfg, consumed_samples=0, is_predict=True
            )
            dataloaders.append(eval_dl)
        return dataloaders
    ...
    def configure_sharded_model(self):
        ...
        if self.use_fsdp:
            ...
            frozen_submodule_names, frozen_submodules = find_frozen_submodules(
                self.model
            )
    ...
    def oomptimizer_schema(self, schema: str = "audio") -> dict:
        ...
        if schema == "audio":
            ...
            raise RuntimeError(
                f"Unknown schema type for oomptimizer of class {type(self)}: '{schema}'"
            )
...
class CrossAttendModularAudioGPTModel(ModularAudioGPTModel):
    ...
    def prepare_llm_input(self, audio_batch):
        ...
        if "audio_ratio" in audio_batch:
            ...
            encoder_input = encoder_input * audio_ratio + input_embeds * (
                1 - audio_ratio
            )
        ...
        return (
            ...
            labels,
            loss_mask,
            (encoded, encoded_len, extra_outputs),
        )
    ...
    def state_dict(self, destination=None, prefix=None, keep_vars=False):
        if self.setup_complete:
            return_state_dict = super().state_dict(
                destination=destination, prefix=prefix, keep_vars=keep_vars
            )
            state_dict = self.perception_cross_attn.state_dict(
                prefix="perception_cross_attn."
            )
            return_state_dict.update(state_dict)
            return return_state_dict
        else:
    ...
    def configure_sharded_model(self):
        ...
        if self.use_fsdp:
            self.perception_cross_attn = self.trainer.strategy._setup_model(
                self.perception_cross_attn
            )
            self.perception_cross_attn = self.perception_cross_attn.cuda(
