...
class ModularizedAudioT5Model(MegatronT5LoraModel):
    ...
    def __init__(self, cfg: DictConfig, trainer: Trainer):
        ...
        if hasattr(self.cfg.data, "test_ds") and hasattr(
            self.cfg.data.test_ds, "metric"
        ):
            self.test_metric_label_key = self.cfg.data.test_ds.metric.get(
                "label_key", "labels"
            )
        if hasattr(self.cfg.data, "validation_ds") and hasattr(
            self.cfg.data.validation_ds, "metric"
        ):
    ...
    def load_frozen_model(self, cfg, trainer):
        ...
        t5_cfg_base = MegatronT5Model.restore_from(
    ...
    def init_model(self, cfg: DictConfig, trainer: Trainer):
        ...
        if self.megatron_amp_O2:
            self.word_embeddings = (
                self.frozen_model.enc_dec_model.module.encoder_embedding
            )
        else:
    ...
    def setup_optimizer_param_groups(self):
        ...
        if self.cfg.get("freeze_llm", True):
            ...
            if self.cfg.get("freeze_encoder", False):
                for (
                    param
                ) in self.frozen_model.enc_dec_model.enc_dec_model.encoder.parameters():
                    param.requires_grad = False
                known_groups.append("enc_dec_model.encoder.")
            if self.cfg.get("freeze_decoder", False):
                for (
                    param
                ) in self.frozen_model.enc_dec_model.enc_dec_model.decoder.parameters():
                    param.requires_grad = False
            ...
            if self.cfg.get("freeze_word_emb", False):
                ...
                for pname in names:
                    for param in getattr(
                        self.frozen_model.enc_dec_model, pname
                    ).parameters():
                        param.requires_grad = False
    ...
    def inject_perception_input(self, encoded, encoded_len, input_ids, input_length):
        def _concat_embs(embs1, emb1_lens, embs2, emb2_lens):
            ...
            for emb1, emb1_len, emb2, emb2_len in zip(
                ...
                if (
                    self.cfg.get("ignore_dummy_audio", False) and emb1_len <= 1
        ...
        if self.cfg.audio_prompt_first:
            ...
            encoder_input, encoder_length = _concat_embs(
                input_embeds, input_length, encoded, encoded_len
            )
        ...
        attention_mask = torch.tril(
            torch.ones((b, max_len, max_len), device=encoder_input.device)
        ).view(b, 1, max_len, max_len)
    ...
    def prepare_llm_input(self, audio_batch):
        ...
        encoder_input, attention_mask, encoder_length, _, encoder_max_length = (
            self.inject_perception_input(encoded, encoded_len, input_ids, input_length)
        )
        # generate encoder_mask from encoder_length
        enc_mask = (
            torch.arange(encoder_input.shape[1], device=encoder_input.device)[None, :]
            < encoder_length[:, None]
        )
    ...
    def forward(
        ...
        if "audio_signal" in audio_batch:
            encoder_input, attention_mask, enc_mask = self.prepare_llm_input(
                audio_batch
            )
            ...
            dec_input = torch.cat(
                [torch.full([b, 1], self.bos_id, device=labels.device), labels[:, :-1]],
                dim=-1,
            )
            ...
            def loss_func(multimodal_output):
                ...
                for key, (output, loss_mask) in multimodal_output.items():
                    cur_loss = self.loss_func(
                        loss_mask.contiguous(), output.contiguous()
                    )
    ...
    def build_data_loader(self, dataset, data_cfg, consumed_samples=0, is_eval=False):
        return build_speechllm_dataloader(
            dataset, data_cfg, consumed_samples, is_eval=is_eval
        )
    ...
    def _modify_config(cls, gpt_cfg, cfg, audio_cfg, add_cfg_to_tree=False):
        ...
        with open_dict(gpt_cfg):
            ...
            gpt_cfg.freeze_modality_adapter = cfg.model.get(
            ...
            gpt_cfg.tensor_model_parallel_size = cfg.model.get(
                ...
                (
                    gpt_cfg.tensor_model_parallel_size
                    if hasattr(gpt_cfg, "tensor_model_parallel_size")
                    else 1
                ),
            )
            ...
            if hasattr(gpt_cfg, "label_smoothing"):
                gpt_cfg.label_smoothing = cfg.model.get(
                    "label_smoothing", gpt_cfg.label_smoothing
                )
            ...
            gpt_cfg.pretrained_audio_model = cfg.model.get(
                "pretrained_audio_model", None
            )
    ...
    def load_audio_model(cls, pretrained_audio_model):
        try:
            if pretrained_audio_model.endswith(".nemo"):
                ...
                audio_model = ASRModel.restore_from(
                ...
                audio_model = ASRModel.from_pretrained(
                    pretrained_audio_model, map_location="cpu"
                )
        except:
            logging.info(
                f"Fail in loading it with ASRModel. Try again with SpeechEncDecSelfSupervisedModel."
            )
            if pretrained_audio_model.endswith(".nemo"):
                logging.info(
                    f"Loading pretrained audio model from local file: {pretrained_audio_model}"
    ...
    def restore_from_pretrained_models(
        ...
        model_cfg = cls._modify_config(
            base_model_cfg, cfg, audio_model.cfg, add_cfg_to_tree=False
        )
        ...
        if cfg.model.get("load_audio_encoder", True):
            model.perception.encoder.load_state_dict(
                audio_model.encoder.state_dict(),
                strict="adapter" not in cfg.model.perception,
            )
            logging.info(
                f"Loaded pretrained audio model from {cfg.model.pretrained_audio_model}"
            )
        else:
            logging.info(
                f"Not load pretrained audio model from {cfg.model.pretrained_audio_model}"
            )
        ...
        if stage != "validate":
            if hasattr(self.cfg.data, "test_ds"):
                ...
                self._test_ds = self._build_dataset(
                    self.cfg.data.test_ds, is_train=False
                )
    ...
    def inference_step(self, dataloader_iter, mode, dataloader_idx=0):
        ...
        if "tokens" in batch:
            ...
            metadata = batch.get("metadata", [{}] * len(batch["tokens"]))
            batch.pop("tokens")
        loss = self._validation_step_internal(
    ...
    def inference_epoch_end(self, outputs, mode, data_cfg):
        ...
        if mode == "validation":
            ...
            if averaged_metric is not None:
                self.log(
                    f"test_{self.test_metric_name}", averaged_metric, sync_dist=True
                )
    ...
    def fwd_bwd_step(self, dataloader_iter, batch_idx, forward_only):
        ...
        for batch in (audio_batch, text_batch):
            ...
            if "tokens" in batch and "text_input_ids" in batch:
                ...
                dec_seq_length = max(
                    batch["answers"].shape[1], batch["text_answer_ids"].shape[1]
                )
            ...
            data_iter = get_iterator_k_split(
                batch, get_num_microbatches(), enforce_divisible_batch=False
            )
    ...
    def loss_func(self, loss_mask, output_tensor):
        ...
        loss = (
            torch.sum(losses.view(-1) * loss_mask) / loss_mask.sum()
        )  # sequence level nll
        return loss
    ...
    def setup_mcore_distributed_parallel(self):
        ...
        if self.with_distributed_adam and self.use_mcore_dist_optim:
            raise ValueError(
                "T5 does not support both distributed adam and mcore distributed data parallel."
            )
...
class DecoderTextPromptModularizedAudioT5Model(ModularizedAudioT5Model):
    ...
    def forward(
        ...
        dec_mask = (dec_input != self.tokenizer.eos_id) * (
            dec_input != self.tokenizer.pad_id
        ).long().contiguous()
        output = self.frozen_model.enc_dec_model(
            enc_input_ids=None,
            enc_attn_mask=enc_mask,
