...
class RandomBlockMaskingAugmentation(NeuralModule):
    """
    ...
    def __init__(
        self, mask_prob: float = 0.5, block_size: int = 48,
    ):
        super().__init__()
    ...
    def output_types(self):
        ...
        """
        return {
            "maksed_feats": NeuralType(('B', 'D', 'T'), SpectrogramType()),
            "masks": NeuralType(('B', 'D', 'T'), SpectrogramType()),
        }

    @typecheck()
    def forward(
        self, input_feats: torch.Tensor, input_lengths: torch.Tensor, mask_value: Optional[torch.Tensor] = None
    ):
        """
        Args:
            ...
            mask_value (Union[float, torch.Tensor]): value to replace masked time steps with
        Returns:
            masked_feats (Tensor): masked features, shape=(batch, features, time)
            masks (Tensor): the generated masks, shape=(batch, features, time)
        """

        batch_size, feat_dim, _ = input_feats.shape
        if mask_value is None:
            mask_value = torch.zeros(feat_dim, device=input_feats.device)
