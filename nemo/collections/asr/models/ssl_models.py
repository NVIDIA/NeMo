...
class SpeechEncDecSelfSupervisedModel(ModelPT, ASRModuleMixin, AccessMixin):
    ...
    def __init__(self, cfg: DictConfig, trainer: Trainer = None):
        ...
        if "loss_list" in self._cfg:
            ...
            for decoder_loss_name, decoder_loss_cfg in self._cfg.loss_list.items():
                ...
                new_decoder_loss = {
                    "decoder": SpeechEncDecSelfSupervisedModel.from_config_dict(
                        decoder_loss_cfg.decoder
                    ),
                    "loss": SpeechEncDecSelfSupervisedModel.from_config_dict(
                ...
                self.output_from_layer[decoder_loss_name] = decoder_loss_cfg.get(
                    "output_from_layer", None
                )
                self.targets_from_loss[decoder_loss_name] = decoder_loss_cfg.get(
                    "targets_from_loss", None
                )
                self.start_step[decoder_loss_name] = decoder_loss_cfg.get(
                    "start_step", 0
                )
                self.transpose_encoded[decoder_loss_name] = decoder_loss_cfg.get(
        ...
        self.dropout_features = (
            torch.nn.Dropout(self._cfg.dropout_features)
            if "dropout_features" in self._cfg
            else None
        )

        # dropout for targets (applied before quantization)
        self.dropout_features_q = (
            torch.nn.Dropout(self._cfg.dropout_features_q)
            if "dropout_features_q" in self._cfg
            else None
        )
    ...
    def setup_validation_data(self, val_data_config: Optional[Union[DictConfig, Dict]]):
        ...
        if (
            ...
            and isinstance(
                self._validation_dl.dataset, torch.utils.data.IterableDataset
            )
        ):
            # We also need to check if limit_train_batches is already set.
            # If it's an int, we assume that the user has set it to something sane, i.e. <= # training batches,
            ...
            if isinstance(self._trainer.limit_val_batches, float):
                self._trainer.limit_val_batches = int(
                    ...
                    * ceil(
                        (len(self._validation_dl.dataset) / self.world_size)
                        / val_data_config["batch_size"]
                    )
                )
    ...
    def forward(
        ...
        spec_masks = torch.logical_and(
            masked_spectrograms < 1e-5, masked_spectrograms > -1e-5
        ).float()
    ...
    def decoder_loss_step(
        ...
        if self.decoder_losses is None:
            if (
                hasattr(self.decoder_ssl, "needs_labels")
                and self.decoder_ssl.needs_labels
            ):
                outputs = self.decoder_ssl(
            ...
            for dec_loss_name, dec_loss in self.decoder_losses.items():
                ...
                if self.output_from_layer[dec_loss_name] is None:
                    ...
                    dec_input = registry[self.output_from_layer[dec_loss_name]][
                        "encoder"
                    ][-1]
                ...
                if self.targets_from_loss[dec_loss_name] is not None:
                    ...
                    target_lengths = self.decoder_losses[target_loss][
                        "loss"
                    ].target_lengths
                ...
                loss_value = (
                    loss_value + current_loss_value * self.loss_alphas[dec_loss_name]
                )
    ...
    def training_step(self, batch, batch_nb):
        ...
        if self.decoder_losses is not None:
            for dec_loss_name, dec_loss in self.decoder_losses.items():
                self.decoder_losses_active[dec_loss_name] = (
    ...
    def validation_pass(self, batch, batch_idx, dataloader_idx=0):
        ...
        loss_value, _ = self.decoder_loss_step(
            spectrograms, spec_masks, encoded, encoded_len, targets, target_lengths
        )
...
class EncDecMaskedTokenPredModel(SpeechEncDecSelfSupervisedModel):
    ...
    def __init__(self, cfg: DictConfig, trainer: Trainer = None):
        ...
        if self.cfg.get("mask_position", "pre_conv") == "post_conv":
            ...
            self.cfg.masking.block_size = (
                self.cfg.masking.block_size // self.cfg.encoder.subsampling_factor
            )
        ...
        if self.cfg.get("mask_position", "pre_conv") == "post_conv":
            ...
            self.pre_encoder = ConvFeatureMaksingWrapper(
                self.encoder.pre_encode, self.mask_processor
            )
    ...
    def forward(
        ...
        if self.pre_encoder is not None:
            ...
            encoded, encoded_len = self.encoder(
                audio_signal=masked_signal, length=processed_signal_length
            )
    ...
    def inference_pass(
        self, batch, batch_idx=0, dataloader_idx=0, mode="val", apply_mask=False
    ):
        input_signal, input_signal_length = batch[0], batch[1]
        if isinstance(batch, DALIOutputs) and batch.has_processed_signal:
            log_probs, encoded_len, masks, tokens = self.forward(
    ...
    def test_step(self, batch, batch_idx=0, dataloader_idx=0):
        metrics = self.inference_pass(
            batch, batch_idx, dataloader_idx, mode="test", apply_mask=True
        )
        if (
            type(self.trainer.val_dataloaders) == list
            and len(self.trainer.val_dataloaders) > 1
    ...
    def multi_validation_epoch_end(self, outputs: list, dataloader_idx: int = 0):
        ...
        for i, x in enumerate(outputs):
            if not isinstance(x, dict):
                logging.warning(
                    f"Batch {i} output in validation dataloader {dataloader_idx} is not a dictionary: {x}"
                )
            if "val_loss" in x:
...
class EncDecDenoiseMaskedTokenPredModel(EncDecMaskedTokenPredModel):
    ...
    def input_types(self) -> Optional[Dict[str, NeuralType]]:
        ...
        return {
            ...
            "processed_noise_signal": NeuralType(
                ("B", "D", "T"), SpectrogramType(), optional=True
            ),
            "processed_noise_length": NeuralType(
                tuple("B"), LengthsType(), optional=True
            ),
            "noisy_input_signal": NeuralType(
                ("B", "T"), input_signal_eltype, optional=True
            ),
            "noisy_input_signal_length": NeuralType(
                tuple("B"), LengthsType(), optional=True
            ),
            "processed_noisy_signal": NeuralType(
                ("B", "D", "T"), SpectrogramType(), optional=True
            ),
            "processed_noisy_signal_length": NeuralType(
                tuple("B"), LengthsType(), optional=True
            ),
            "apply_mask": NeuralType(optional=True),
        }
    ...
    def forward(
        ...
        has_noisy_input_signal = (
            noisy_input_signal is not None and noisy_input_signal_length is not None
        )
        has_processed_noisy_input_signal = (
            processed_noisy_input_signal is not None
            and processed_noisy_input_signal_length is not None
        )
        ...
        if not has_processed_noisy_input_signal:
            processed_noisy_input_signal, processed_noisy_input_signal_length = (
                self.preprocessor(
        ...
        if self.pre_encoder is not None:
            ...
            encoded, encoded_len = self.encoder(
                ...
                length=processed_noisy_input_signal_length,
            )
            masks = self.pre_encoder.get_current_mask()
        else:
            _, tokens = self.quantizer(input_signal=processed_signal)
