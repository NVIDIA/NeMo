...
class ClusterEmbedding:
    ...
    def run_clustering_diarizer(self, manifest_filepath, out_dir):
        ...
        if isEmbReady:    
            ...
            if self.run_clus_from_loaded_emb:
                self.run_multiscale_clustering(manifest_filepath, emb_scale_seq_dict, out_dir, run_clustering=True)
                metric, speaker_mapping_dict = self.score 
            else:
                self.run_multiscale_clustering(manifest_filepath, emb_scale_seq_dict, out_dir, run_clustering=False)
            emb_sess_avg_dict, emb_scale_seq_dict, base_clus_label_dict = self.load_embeddings_from_pickle(out_dir, 
                                                                                                  speaker_mapping_dict, 
                                                                                                  session_scale_mapping_dict)
            session_scale_mapping_dict = self.get_scale_map(self.embs_and_timestamps)
            ...
            # self.score = self.sd_model.diarize()
    ...
    def get_embs_and_timestamps(self, ):
    ...
    def run_multiscale_clustering(self, manifest_filepath, emb_scale_seq_dict, out_dir, run_clustering=False):
        # import ipdb; ipdb.set_trace() 
        self.cfg_base.diarizer.out_dir = out_dir
        ...
        self.sd_model._out_dir = out_dir
