...
class SortformerEncLabelModel(ModelPT, ExportableEncDecModel, SpkDiarizationMixin):
    ...
    def __init__(self, cfg: DictConfig, trainer: Trainer = None):
        ...
        self.preprocessor = SortformerEncLabelModel.from_config_dict(
            self._cfg.preprocessor
        )

        if hasattr(self._cfg, "spec_augment") and self._cfg.spec_augment is not None:
            self.spec_augmentation = SortformerEncLabelModel.from_config_dict(
                self._cfg.spec_augment
            )
        else:
            self.spec_augmentation = None

        self.encoder = SortformerEncLabelModel.from_config_dict(self._cfg.encoder).to(
            self.device
        )
        self.sortformer_modules = SortformerEncLabelModel.from_config_dict(
            self._cfg.sortformer_modules
        ).to(self.device)
        self.transformer_encoder = SortformerEncLabelModel.from_config_dict(
            self._cfg.transformer_encoder
        ).to(self.device)
        if self._cfg.encoder.d_model != self._cfg.model_defaults.tf_d_model:
            self.sortformer_modules.encoder_proj = (
                self.sortformer_modules.encoder_proj.to(self.device)
            )
        else:
    ...
    def frontend_encoder(
        self, processed_signal, processed_signal_length, bypass_pre_encode: bool = False
    ):
        """
    ...
    def _diarize_output_processing(
        ...
        for sample_idx, uniq_id in enumerate(uniq_ids):
            ...
            diar_output_lines = generate_diarization_output_lines(
                ...
                model_spk_num=len(speaker_timestamps),
            )
    ...
    def _setup_diarize_dataloader(self, config: Dict) -> "torch.utils.data.DataLoader":
        ...
        temporary_datalayer = self.__setup_dataloader_from_config(
    ...
    def oom_safe_feature_extraction(self, input_signal, input_signal_length):
        ...
        div_batch_count = min(
            int(max_batch_sec * org_batch_size // self.max_batch_dur + 1),
            org_batch_size,
        )
    ...
    def process_signal(self, audio_signal, audio_signal_length):
        ...
        Returns:
            ...
            processed_signal_length (torch.Tensor): The length of each processed signal.
                Shape: (batch_size,)
        """
        audio_signal, audio_signal_length = audio_signal.to(
            self.device
        ), audio_signal_length.to(self.device)
    ...
    def forward_for_export(
        self, chunk, chunk_lengths, spkcache, spkcache_lengths, fifo, fifo_lengths
    ):
        """
    ...
    def forward_streaming(
        ...
        if (
            sig_length < max_n_frames
        ):  # need padding to have the same feature length for all GPUs
        ...
        if self.training:
            ...
            if rand_num < self.sortformer_modules.causal_attn_rate:
                self.encoder.att_context_size = [
                    -1,
                    self.sortformer_modules.causal_attn_rc,
                ]
        ...
        num_chunks = math.ceil(
            ...
            / (
                ...
                * self.sortformer_modules.subsampling_factor
            )
        )
    ...
    def forward_streaming_step(
        ...
        if self.async_streaming:
            (
                ...
                [
                    streaming_state.spkcache_lengths,
                    streaming_state.fifo_lengths,
                    chunk_pre_encode_lengths,
                ],
            )
        else:
        ...
        if self.async_streaming:
            streaming_state, chunk_preds = (
                self.sortformer_modules.streaming_update_async(
                    streaming_state=streaming_state,
    ...
    def _get_aux_train_evaluations(self, preds, targets, target_lens) -> dict:
        ...
        targets_ats = get_ats_targets(
            targets.clone(), preds, speaker_permutations=self.speaker_permutations
        )
    ...
    def training_step(self, batch: list, batch_idx: int) -> dict:
        ...
        self.log_dict(
            train_metrics, sync_dist=True, on_step=True, on_epoch=False, logger=True
        )
    ...
    def _get_aux_validation_evaluations(self, preds, targets, target_lens) -> dict:
        ...
        val_ats_loss = self.loss(
            probs=preds, labels=targets_ats, target_lens=target_lens
        )
        val_pil_loss = self.loss(
            probs=preds, labels=targets_pil, target_lens=target_lens
        )
    ...
    def multi_validation_epoch_end(self, outputs: list, dataloader_idx: int = 0):
        if not outputs:
            logging.warning(
                f"`outputs` is None; empty outputs for dataloader={dataloader_idx}"
            )
            return None
    ...
    def _get_aux_test_batch_evaluations(
        self, batch_idx: int, preds, targets, target_lens
    ):
        """
        Compute auxiliary validation evaluations including losses and metrics.
        ...
        logging.info(
            f"batch {batch_idx}: f1_acc={f1_acc}, precision={precision}, recall={recall}"
        )
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(tqdm(self._test_dl)):
                ...
                self._get_aux_test_batch_evaluations(
                    batch_idx, preds, targets, target_lens
                )
                preds = preds.detach().to("cpu")
                if preds.shape[0] == 1:  # batch size = 1
                    self.preds_total_list.append(preds)
                else:
                    self.preds_total_list.extend(
                        torch.split(preds, [1] * preds.shape[0])
                    )
                torch.cuda.empty_cache()

        logging.info(
            f"Batch F1Acc. MEAN: {torch.mean(torch.tensor(self.batch_f1_accs_list))}"
        )
        logging.info(
            f"Batch Precision MEAN: {torch.mean(torch.tensor(self.batch_precision_list))}"
        )
        logging.info(
            f"Batch Recall MEAN: {torch.mean(torch.tensor(self.batch_recall_list))}"
        )
        logging.info(
            f"Batch ATS F1Acc. MEAN: {torch.mean(torch.tensor(self.batch_f1_accs_ats_list))}"
        )
