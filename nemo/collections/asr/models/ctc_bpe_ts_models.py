...
class TSEncDecCTCModelBPE(EncDecCTCModelBPE):
    ...
    def input_types(self) -> Optional[Dict[str, NeuralType]]:
        ...
        d["target_signal"] = NeuralType(('B', 'T'), AudioSignal())
        return d

    @property
    def output_types(self) -> Optional[Dict[str, NeuralType]]:
        d = super().output_types
        d["target_signal"] = NeuralType(('B', 'D', 'T'), AcousticEncodedRepresentation())
        d["target_signal_estimate"] = NeuralType(('B', 'D', 'T'), AcousticEncodedRepresentation())
        d["target_signal_length"] = NeuralType(tuple('B'), LengthsType())
        return d
    ...
    def forward(
        ...
        mask, mask_len, pre_encoded_audio, pre_encoded_audio_lengths = self.speaker_beam(
            audio_signal=processed_signal, length=processed_signal_length, emb=speaker_embedding,
        )
        ...
        if target_signal is not None:
            return (
                ...
                greedy_predictions,
                target_signal,
                target_signal_estimate,
                target_signal_length,
            )
        else:
            return log_probs, encoded_len, greedy_predictions, None, None, None
    ...
    def training_step(self, batch, batch_nb):
        ...
        tensorboard_logs.update(
            dict(
                [
                    (f"learning_rate_group{i}", param_group['lr'])
        ...
        for dec_loss_name, dec_loss in self.decoder_losses.items():
            ...
            if (
                ...
                and (
                    self.trainer.global_step < self.start_step[dec_loss_name]
                    or (
                        self.trainer.global_step >= self.stop_step[dec_loss_name]
                        and self.stop_step[dec_loss_name] >= 0
                    )
                )
            ):
                continue
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        self._wer.reset()
        tensorboard_logs.update(
            {'val_wer_num': wer_num, 'val_wer_denom': wer_denom, 'val_wer': wer,}
        )
    ...
    def transcribe(
        ...
        try:
            ...
            for test_batch in tqdm(temporary_datalayer, desc="Transcribing"):
                ...
                (
                    ...
                    target_signal=target_signal.to(device) if 'reconstruction' in self.decoder_losses else None,
                )
        ...
        if logprobs:
            return hypotheses, target_signal, target_signal_estimate
        return hypotheses
