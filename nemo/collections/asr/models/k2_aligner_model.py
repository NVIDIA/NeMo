...
class AlignerWrapperModel(ASRModel):
    ...
    def __init__(self, model: ASRModel, cfg: DictConfig):
        ...
        if self.alignment_type == "forced":
            ...
            raise NotImplementedError(
                f"alignment_type=`{self.alignment_type}` is not supported at the moment."
            )
    ...
    def _init_ctc_alignment_specific(self, cfg: DictConfig):
        ...
        if self.alignment_type == "forced":
            ...
            if decoder_module_cfg is not None:
                self.graph_decoder._decoder.intersect_pruned = decoder_module_cfg.get(
                    "intersect_pruned"
                )
                self.graph_decoder._decoder.intersect_conf = decoder_module_cfg.get(
                    "intersect_conf"
                )
            return
    ...
    def _init_model_specific(self, cfg: DictConfig):
        ...
        if isinstance(self._model, EncDecCTCModel):
            ...
            if prob_suppress_value > 1 or prob_suppress_value <= 0:
                raise ValueError(
                    f"Suppression value has to be in (0,1]: {prob_suppress_value}"
                )
            if (
                prob_suppress_index < -(self.blank_id + 1)
                or prob_suppress_index > self.blank_id
            ):
                raise ValueError(
        ...
        if isinstance(self._model, EncDecRNNTModel):
            ...
            self.log_softmax = (
                ...
                else not self._model.joint.log_softmax
            )
    ...
    def _rnnt_joint_pruned(
        ...
        """
        encoder_outputs = self._model.joint.enc(
            encoder_outputs.transpose(1, 2)
        )  # (B, T, H)
        decoder_outputs = self._model.joint.pred(
        ...
        res = self._model.joint.joint_net(
            encoder_outputs_pruned + decoder_outputs_pruned
        )
    ...
    def _apply_prob_suppress(self, log_probs: torch.Tensor) -> torch.Tensor:
        ...
        y = torch.cat(
            [
                exp_probs[:, :, : self.prob_suppress_index],
                exp_probs[:, :, self.prob_suppress_index + 1 :],
            ],
            2,
        ).sum(-1)
        b1 = torch.full(
            (exp_probs.shape[0], exp_probs.shape[1], 1),
            self.prob_suppress_value,
            device=log_probs.device,
        )
        b2 = (
            ((1 - self.prob_suppress_value * x) / y)
            .unsqueeze(2)
            .repeat(1, 1, exp_probs.shape[-1] - 1)
        )
        return (
            exp_probs
            * torch.cat(
                [
                    b2[:, :, : self.prob_suppress_index],
                    b1,
                    b2[:, :, self.prob_suppress_index :],
                ],
                2,
    ...
    def _prepare_ctc_argmax_predictions(
        ...
        if hasattr(self._model, "transcribe_decoder"):
            predictions, _, probs = self.transcribe_decoder.forward(
    ...
    def _predict_impl_rnnt_argmax(
        ...
        for s_id, hypothesis in zip(sample_id, hypotheses):
            ...
            token_len = [
                j - i
                for i, j in zip(
                    token_begin, token_begin[1:] + [len(hypothesis.alignments)]
                )
            ]
            ...
            if self.word_output:
                words = [
                    w
                    for w in self._model.decoding.decode_tokens_to_str(pred_ids).split(
                        " "
                    )
                    if w != ""
                ]
                words, word_begin, word_len, word_prob = (
                    self._process_tokens_to_words(
                        tokens, token_begin, token_len, token_prob, words
                    )
                    if hasattr(self._model, "tokenizer")
                    else self._process_char_with_space_to_words(
                        tokens, token_begin, token_len, token_prob, words
                    )
                )
                results.append(
                ...
                results.append(
                    (
                        ...
                        [
                            FrameCtmUnit(t, b, l, p)
                            for t, b, l, p in zip(
                                tokens, token_begin, token_len, token_prob
                            )
                        ],
                    )
                )
    ...
    def _process_tokens_to_words(
        ...
        for word in words:
            ...
            word_prob.append(
                sum(token_prob[k] * token_len_nonzero[k] for k in range(i, j))
                / denominator
            )
            i = j
    ...
    def _process_char_with_space_to_words(
        ...
        if len(space_idx) == 0:
            ...
            word_prob = [
                sum(t_p * t_l for t_p, t_l in zip(token_prob, token_len_nonzero))
                / denominator
            ]
        else:
            space_word = "[SEP]"
            ...
            word_prob = [
                sum(token_prob[k] * token_len_nonzero[k] for k in range(space_idx[0]))
                / denominator
            ]
            ...
            for word, i, j in zip(words[1:], space_idx, space_idx[1:] + [len(tokens)]):
                ...
                word_prob.append(
                    sum(token_prob[k] * token_len_nonzero[k] for k in range(i + 1, j))
                    / denominator
                )
    ...
    def _results_to_ctmUnits(
        ...
        if self.model_type == "rnnt":
            ...
            token_end = torch.cat(
                (token_begin[1:], torch.tensor([len(pred) - len(non_blank_idx)]))
            )
        else:
        ...
        token_prob = [
            ...
            for i, j in zip(
                non_blank_idx.tolist(), non_blank_idx[1:].tolist() + [len(pred)]
            )
        ]
        if self.word_output:
            ...
            return s_id, [
                FrameCtmUnit(t, b, l, p)
                for t, b, l, p in zip(words, word_begin, word_len, word_prob)
            ]
        return s_id, [
            FrameCtmUnit(t, b, l, p)
            for t, b, l, p in zip(tokens, token_begin, token_len, token_prob)
        ]
    ...
    def _predict_impl_ctc(
        ...
        if self.alignment_type == "argmax":
            predictions, probs = self._prepare_ctc_argmax_predictions(
            ...
            predictions, probs = self.graph_decoder.align(
                log_probs, encoded_len, transcript, transcript_len
            )
        else:
            raise NotImplementedError()
    ...
    def _predict_impl_rnnt(
        ...
        """
        if self.alignment_type == "argmax":
            return self._predict_impl_rnnt_argmax(
                encoded, encoded_len, transcript, transcript_len, sample_id
            )
        elif self.alignment_type == "forced":
            decoded = self._model.decoder(
                targets=transcript, target_length=transcript_len
            )[0]
            log_probs = (
                self._rnnt_joint_pruned(encoded, encoded_len, decoded, transcript_len)
                if self.predictor_window_size > 0
                and self.predictor_window_size < transcript_len.max()
                else self._model.joint(encoder_outputs=encoded, decoder_outputs=decoded)
            )
            apply_log_softmax = (
                True
                if self.log_softmax is None and encoded.is_cuda
                else self.log_softmax
            )
    ...
    def transcribe(
        ...
        try:
            ...
            for test_batch in tqdm(
                temporary_datalayer, desc="Aligning", disable=not verbose
            ):
