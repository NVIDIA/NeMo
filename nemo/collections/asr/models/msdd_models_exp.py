...
class EncDecDiarLabelModel(ModelPT, ExportableEncDecModel, ClusterEmbeddingTest):
    ...
    * msdd_model
    ...
    def __init__(self, cfg: DictConfig, trainer: Trainer = None):
        ...
        ClusterEmbeddingTest.__init__(self, 
    ...
    def interpolate_embs(self, emb_fix, ms_seg_timestamps, base_seq_len):
        ...
        session_scale_mapping_dict, session_scale_dist_mat = self.get_scale_dist_mat(ms_seg_timestamps[:, :base_seq_len, :])
...
class NeuralDiarizer:
    ...
    def get_range_clus_avg_emb(self, test_batch, _test_data_collection, split_count):
        ...
        for k in range(_signals.shape[0]):
            ...
            for diar_win_idx in range(split_count):
                ...
                emb_vectors_split = torch.zeros_like(emb_vectors)
                for spk_idx in range(len(test_data_collection.target_spks)):
                    stt, end = diar_win_idx * self.diar_window_length, min((diar_win_idx+1) * self.diar_window_length, clus_label_tensor.shape[0])
                    seq_len = end - stt
                    if stt < clus_label_tensor.shape[0]:
                        target_clus_label_tensor = clus_label_tensor[stt:end]
                        emb_seq, seg_length = signals[stt:end, : , :], min(self.diar_window_length, clus_label_tensor.shape[0] - diar_win_idx*self.diar_window_length)
                        target_clus_label_bool = (target_clus_label_tensor == test_data_collection.target_spks[spk_idx])
                        # There are cases where there is no corresponding speaker in split range, so any(target_clus_label_bool) could be False.
                        if any(target_clus_label_bool) == True:
                            emb_vectors_split[:, :, spk_idx] = torch.mean(emb_seq[target_clus_label_bool], dim=0)
                        ...
                        if seq_len < self.diar_window_length:
                         ...
                         emb_seq = torch.zeros(self.diar_window_length, emb_seq.shape[1], emb_seq.shape[2]).to(self.device)
                         seq_len = 0
                sess_emb_vectors.append(emb_vectors_split)
                sess_emb_seq.append(emb_seq)
                sess_sig_lengths.append(seq_len)
    ...
    def run_overlap_aware_eval(self, preds_list, targets_list, signal_lengths_list):
        ...
        logging.info(f"     0.25, True [Threshold: does not matter]  [infer_overlap={infer_overlap}]   [use_clus={use_clus}]")
        score_A_ucr = diar_eval(self.msdd_model, preds_list, threshold=0.5, infer_overlap=infer_overlap, collar=0.25, ignore_overlap=True, use_clus=use_clus, use_dec=use_dec)

        use_clus, infer_overlap, use_dec = False, False, True
        logging.info(f"     0.25, True [Threshold: does not matter]  [infer_overlap={infer_overlap}]   [use_dec={use_dec}]")
        score_A_dec = diar_eval(self.msdd_model, preds_list, threshold=0.5,  infer_overlap=infer_overlap, collar=0.25, ignore_overlap=True, use_clus=use_clus, use_dec=use_dec)
        ...
        for threshold in thres_range:
            ...
            print(" ======================================================================================================\n")
            logging.info(f" 0.0 False [Threshold: {threshold:.4f}]  [infer_overlap={infer_overlap}]   [use_clus={use_clus}]")
            ...
            logging.info(f" 0.0 False [Threshold: {threshold:.4f}]  [infer_overlap={infer_overlap}]   [use_clus={use_clus}]")
            score_A_bk = diar_eval(self.msdd_model, preds_list, threshold=threshold,  infer_overlap=False,         collar=0.25, ignore_overlap=True, use_clus=use_clus, use_dec=use_dec)
            ...
            logging.info(f" 0.0 False [Threshold: {threshold:.4f}]  [infer_overlap={infer_overlap}]   [use_dec={use_dec}]")
