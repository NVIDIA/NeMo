...
class BatchedFrameASRCTC_tgt_spk:
    ...
    def read_audio_file(
        ...
        self.frame_bufferer.query_buffer_placeholder = np.zeros(
            (self.batch_size, max_query_len + self.frame_bufferer.feature_buffer_len)
        )
    ...
    def _get_batch_preds(self):
        ...
        for idx, pred in enumerate(predictions):
            ...
            if not has_signal_ended:
                self.all_preds[global_index_key].append(
                    pred[self.query_pred_len[global_index_key] : encoded_len[idx]].cpu().numpy()
                )
    ...
    def transcribe(
        ...
        for idx, preds in enumerate(self.all_preds):
            ...
            for a_idx, pred in enumerate(preds):
                decoded = pred.tolist()
                self.unmerged[idx] += decoded[
                    max(0, len(decoded) - 1 - delay) : len(decoded) - 1 - delay + tokens_per_chunk
                ]
        output = []
        for idx in range(self.batch_size):
...
class BatchedAudioBufferer_tgt_spk:
    def __init__(self, asr_model, frame_len=1.6, total_buffer=4.0, batch_size=4, pad_to_buffer_len=True):
    ...
    def get_buffers_batch(self):
        ...
        while len(batch_frames) > 0:
            ...
            for i, frame_buffer in enumerate(frame_buffers):
                ...
                self.query_buffer_placeholder[
                    i, batch_frames[i][0].shape[0] : batch_frames[i][0].shape[0] + frame_buffer.shape[1]
                ] = frame_buffer[0, :]
