...
def write_overlap_segments(outfile, AUDIO_RTTM_MAP, uniq_id, overlap_range_list, include_uniq_id, deci=3):
    """
...
def get_duration_from_time_tup(time_tup):
    """
    Get duration value from time tuple and round the number..
    """
    return float("{:.3f}".format(time_tup[1] - time_tup[0]))

def _write_rttm2manifest(AUDIO_RTTM_MAP: str, manifest_file: str, include_uniq_id: bool = False, deci: int = 3) -> str:
    """
    ...
    with open(manifest_file, 'w') as outfile:
        for key in AUDIO_RTTM_MAP:
            ...
            if AUDIO_RTTM_MAP[key].get('duration', None):
                manifest_duration = AUDIO_RTTM_MAP[key]['duration']
                offset = AUDIO_RTTM_MAP[key].get('offset', 0)
            else:
                sound = sf.SoundFile(audio_path)
                manifest_duration = sound.frames / sound.samplerate
                offset = 0.0

            max_end_time = offset + manifest_duration
            lines = f.readlines()
            time_tup = (-1, -1)
            segment_range_list = []
            for line in lines:
                if time_tup[0] > (max_end_time):
                    continue
                ...
                if start == 0 and dur == 0:  # No speech segments
                    ...
                    if time_tup[0] >= 0 and start > time_tup[1]:
                        if time_tup[0] < max_end_time and dur_mod > 0:
                            dur_mod = get_duration_from_time_tup(time_tup)
                            segment_range_list.append([time_tup[0], time_tup[0]+dur_mod])
                        elif not AUDIO_RTTM_MAP[key]['duration'] and not AUDIO_RTTM_MAP[key]['offset']:
                            logging.warning(
                                "RTTM label has been truncated since start is greater than duration of audio file"
                            )
                        if time_tup[0] < max_end_time < time_tup[1]:
                            time_tup = (start, max_end_time)
                        else:
            ...
            dur_mod = get_duration_from_time_tup(time_tup)
            if time_tup[0] < max_end_time and dur_mod > 0 and min(max_end_time-time_tup[0], dur_mod) > 0:
                dur_mod = round(min(max_end_time-time_tup[0], dur_mod), 2)
                segment_range_list.append([time_tup[0], time_tup[0]+dur_mod])
            elif not AUDIO_RTTM_MAP[key]['duration'] and not AUDIO_RTTM_MAP[key]['offset']:
            ...
            overlap_range_list = getSubRangeList(
                source_range_list=segment_range_list, target_range=[offset, offset + duration]
            )
