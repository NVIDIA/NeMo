...
def get_buffered_pred_feat_tgt_spk_ctc_batchview_sample(
    ...
    frame_len: float,  # pylint: disable=unused-argument
    ...
    if filepaths is None and manifest is None:
        raise ValueError("Either filepaths or manifest should not be None")
    if filepaths:
        for audio_file in tqdm(filepaths, desc="Sample:"):
            asr.reset()
            asr.read_audio_file(audio_file, delay, model_stride_in_secs)
            hyp = asr.transcribe(tokens_per_chunk, delay)
            hyps.append(hyp)
    else:
        print('Batchview: sample')
        with open(manifest, "r", encoding='utf_8') as mfst_f:
            for json_line in tqdm(mfst_f, desc="Sample:"):
                asr.reset()
                row = json.loads(json_line.strip())
                if 'text' in row:
...
def get_buffered_pred_feat_tgt_spk_ctc_batchview_dataset(
    ...
    frame_len: int,  # pylint: disable=unused-argument
    tokens_per_chunk: int,
    delay: int,
    model_stride_in_secs: int,
    batch_size: int,
    ...
    if manifest is None:
        raise ValueError("manifest should not be None")
    ...
    separater_duration = asr.asr_model.cfg.test_ds.separater_duration
    separater_unvoice_ratio = asr.asr_model.cfg.test_ds.separater_unvoice_ratio
    print('Batchview: dataset')
    with open(manifest, "r", encoding='utf_8') as mfst_f:
        print("Parsing manifest files...")
        for json_line in mfst_f:
            row = json.loads(json_line.strip())
            audio_file = get_full_path(audio_file=row['audio_filepath'], manifest_file=manifest)
            offset = row['offset']
            duration = row['duration']
...
def setup_model_ts(cfg: DictConfig, map_location: torch.device) -> Tuple[ASRModel, str]:
