...
class WERBPE_TS(WER):
    ...
    def __init__(
        ...
        if ctc_decode is not None:
            logging.warning(
                f"`ctc_decode` was set to {ctc_decode}. Note that this is ignored."
            )
    ...
    def ctc_decoder_predictions_tensor_with_ts(
        ...
        for ind in range(prediction_cpu_tensor.shape[self.decoding.batch_dim_index]):
            ...
            for pdx, p in enumerate(prediction):
                if (
                    p != previous or previous == self.decoding.blank_id
                ) and p != self.decoding.blank_id:
            ...
            word_ts, word_seq = self.get_ts_from_decoded_prediction(
                decoded_prediction, hypothesis, char_ts
            )
    ...
    def get_ts_from_decoded_prediction(
        ...
        for idx, ch in enumerate(decoded_char_list):
            ...
            if (
                idx == stt_idx
                or space == decoded_char_list[idx - 1]
                or (space in ch and len(ch) > 1)
            ) and (ch != space):
                _stt = char_ts[idx]
                stt_ch_idx = idx
                word_open_flag = True
            ...
            close_cond = (
                idx == end_idx or ch in ["<unk>"] or space in decoded_char_list[idx + 1]
            )
            if (word_open_flag and ch != space) and close_cond:
                ...
                stitched_word = "".join(
                    decoded_char_list[stt_ch_idx : end_ch_idx + 1]
                ).replace(space, "")
                word_seq.append(stitched_word)

        assert len(word_ts) == len(
            hypothesis.split()
        ), "Text hypothesis does not match word timestamps."
...
class WER_TS(WER):
    ...
    def ctc_decoder_predictions_tensor_with_ts(
        ...
        for ind in range(prediction_cpu_tensor.shape[self.decoding.batch_dim_index]):
            ...
            text, timestamp_list = self.decode_tokens_to_str_with_ts(
                decoded_prediction, decoded_timing_list
            )
...
def get_wer_feat_logit(
    audio_file_path, asr, frame_len, tokens_per_chunk, delay, model_stride_in_secs
):
    """
...
class FrameBatchASRLogits(FrameBatchASR):
    ...
    def read_audio_file_and_return(
        self, audio_filepath: str, delay: float, model_stride_in_secs: float
    ):
    ...
    def transcribe_with_ts(
        ...
        for idx, pred in enumerate(self.all_preds):
            ...
            _stt, _end = (
                len(decoded) - 1 - delay,
                len(decoded) - 1 - delay + tokens_per_chunk,
            )
...
class ASRDecoderTimeStamps:
    ...
    def set_asr_model(self):
        ...
        if "quartznet" in self.ASR_model_name.lower():
            ...
            self.decoder_delay_in_sec = if_none_get_default(
                self.params["decoder_delay_in_sec"], 0.04
            )
            self.word_ts_anchor_offset = if_none_get_default(
                self.params["word_ts_anchor_offset"], 0.12
            )
            ...
            self.decoder_delay_in_sec = if_none_get_default(
                self.params["decoder_delay_in_sec"], 0.08
            )
            self.word_ts_anchor_offset = if_none_get_default(
                self.params["word_ts_anchor_offset"], 0.12
            )
            ...
            self.encdec_class = EncDecCTCModelBPE
            self.decoder_delay_in_sec = if_none_get_default(
                self.params["decoder_delay_in_sec"], 0.16
            )
            self.word_ts_anchor_offset = if_none_get_default(
                self.params["word_ts_anchor_offset"], 0.2
            )
            self.asr_batch_size = if_none_get_default(self.params["asr_batch_size"], 4)
            self.model_stride_in_secs = 0.08
            ...
            raise ValueError(
                f"Cannot find the ASR model class for: {self.params['self.ASR_model_name']}"
            )

        if self.ASR_model_name.endswith(".nemo"):
            asr_model = self.encdec_class.restore_from(restore_path=self.ASR_model_name)
        else:
            asr_model = self.encdec_class.from_pretrained(
                model_name=self.ASR_model_name, strict=False
            )
    ...
    def load_LM_for_CTC_decoder(
        ...
        logging.info(
            f"Loading language model : {self.ctc_decoder_params['pretrained_language_model']}"
        )
        ...
        if "EncDecCTCModelBPE" in str(type(asr_model)):
            ...
            raise ValueError(
                f"Cannot find a vocabulary or tokenizer for: {self.params['self.ASR_model_name']}"
            )
    ...
    def run_ASR_QuartzNet_CTC(
        self, asr_model: Type[EncDecCTCModel]
    ) -> Tuple[Dict, Dict]:
        """
        ...
        with torch.amp.autocast(asr_model.device.type):
            ...
            for idx, logit_np in enumerate(transcript_logits_list):
                ...
                if self.beam_search_decoder:
                    ...
                    trans, char_ts_in_feature_frame_idx = self.clean_trans_and_TS(
                        text[0], char_ts[0]
                    )
                    ...
                    word_ts = self.get_word_ts_from_spaces(
                        char_ts_in_feature_frame_idx,
                        spaces_in_sec,
                        end_stamp=logit_np.shape[0],
                    )
    ...
    def _get_spaces(
        self, trans: str, char_ts: List[str], time_stride: float
        ...
        stt_idx = 0
        assert (len(trans) > 0) and (
            len(char_ts) > 0
        ), "Transcript and char_ts length should not be 0."
        assert len(trans) == len(
            char_ts
        ), "Transcript and timestamp lengths do not match."
        ...
        for k, s in enumerate(trans):
            if s == blank:
                spaces_in_sec.append(
                    [
                        round(char_ts[k] * time_stride, 2),
                        round((char_ts[k + 1] - 1) * time_stride, 2),
                    ]
                )
    ...
    def run_ASR_CitriNet_CTC(
        self, asr_model: Type[EncDecCTCModelBPE]
    ) -> Tuple[Dict, Dict]:
        """
    ...
    def set_buffered_infer_params(
        ...
        preprocessor = nemo_asr.models.EncDecCTCModelBPE.from_config_dict(
            cfg.preprocessor
        )
        ...
        onset_delay = (
            math.ceil(
                ((self.total_buffer_in_secs - self.chunk_len_in_sec) / 2)
                / self.model_stride_in_secs
            )
            + 1
        )
            ...
            for idx, audio_file_path in enumerate(self.audio_file_list):
                ...
                if self.beam_search_decoder:
                    ...
                    greedy_predictions = torch.from_numpy(
                        np.array(greedy_predictions_list)
                    ).unsqueeze(0)
                    text, char_ts, word_ts = (
                        werbpe_ts.ctc_decoder_predictions_tensor_with_ts(
                            self.model_stride_in_secs,
    ...
    def run_pyctcdecode(
        ...
        """
        beams = self.beam_search_decoder.decode_beams(
            logprob, beam_width=self.ctc_decoder_params["beam_width"]
        )
        word_ts_beam, words_beam = [], []
        for idx, (word, _) in enumerate(beams[0][2]):
            ts = self.get_word_ts_from_wordframes(
                idx, beams[0][2], self.model_stride_in_secs, onset_delay_in_sec
            )
    ...
    def get_word_ts_from_wordframes(
        idx,
        word_frames: List[List[float]],
        frame_duration: float,
        onset_delay: float,
        word_block_delay: float = 2.25,
    ):
        """
