...
class CtcK2Mixin(ABC):
    ...
    def _prepare_log_probs_and_targets(
        ...
        return (
            ...
            supervisions,
            (
                torch.where(targets < self.blank, targets + 1, targets)
    ...
    def _prepare_emissions_graphs(
        self, log_probs: torch.Tensor, supervisions: torch.Tensor
    ) -> "k2.DenseFsaVec":
    ...
    def _maybe_normalize_gradients(
    ...
    def _extract_labels_and_probabilities(
        ...
        for i in range(shortest_path_fsas.shape[0]):
            ...
            non_eps_mask = (shortest_path_fsa.labels != -1) & (
                shortest_path_fsa.labels < self.num_classes
            )
            if return_ilabels:
            ...
            probs.append(
                get_arc_weights(shortest_path_fsa)[non_eps_mask]
                .exp()
                .to(device=shortest_path_fsas.device)
            )
...
class RnntK2Mixin(CtcK2Mixin):
    ...
    def _prepare_log_probs_and_targets(
        ...
        if (
            ...
            and self.predictor_window_size < target_lengths.max()
        ):
            ...
            shift_mask = raw_step_indices[0].unsqueeze(0).repeat(
                len(unique), 1
            ) == unique.unsqueeze(-1)
            step_indices = (
                raw_step_indices[0],
                (
                    torch.arange(ranges_begin.size(1))
                    .unsqueeze(0)
                    .repeat(ranges_begin.size(0), 1)
                    * window_size_with_blank
            ...
            pad_shift_mask = pad_indices_row.unsqueeze(0).repeat(
                len(pad_unique), 1
            ) == pad_unique.unsqueeze(-1)
            pad_indices = (
                pad_indices_row,
                T * window_size_with_blank
                + max_count
                - torch.cumsum(pad_shift_mask, 1)[pad_shift_mask],
            )
            self.__step_indices = (
        ...
        non_pad_mask_true = make_non_pad_mask_3d(
            input_lengths, target_lengths + 1, T, U
        ).flatten(1)
        ...
        rearranged_indices[non_pad_mask_fake] = rearranged_indices_buffer[
            non_pad_mask_true
        ]
        rearranged_indices[~non_pad_mask_fake] = rearranged_indices_buffer[
            ~non_pad_mask_true
        ...
        return super()._prepare_log_probs_and_targets(
        ...
        if self.__step_indices is None or self.__supervisions_add is None:
            log_probs_eps = torch.cat(
                (
                    log_probs,
                    torch.zeros(
                        (log_probs.size(0), log_probs.size(1), 1),
            ...
            mask = torch.zeros(
                (
                    log_probs.size(0),
                    log_probs.size(1)
                    + int(len(self.__step_indices[0]) / log_probs.size(0)),
                ),
                dtype=torch.bool,
            )
            mask[self.__step_indices] = True
            log_probs_eps = torch.zeros(
                (mask.size(0), mask.size(1), log_probs.size(2) + 1),
                device=log_probs.device,
            )
            log_probs_eps[mask] = torch.tensor(
                [torch.finfo(torch.float32).min] * log_probs.size(2) + [0],
                device=log_probs.device,
            )
            ...
            input_lengths = (
                supervisions[:, -1]
                + self.__supervisions_add[supervisions[:, 0].to(dtype=torch.long)]
            )
