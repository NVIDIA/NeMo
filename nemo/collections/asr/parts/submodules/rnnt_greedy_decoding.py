...
class GreedyBatchedTDTInfer(_GreedyRNNTInfer):
    ...
    def _greedy_decode_blank_as_pad_loop_labels(
        ...
        if partial_hypotheses is not None:
            raise NotImplementedError("`partial_hypotheses` support is not implemented")
        if self.preserve_frame_confidence:
            raise NotImplementedError("`preserve_frame_confidence` is not implemented")
        ...
        while (current_batch_size := active_indices.shape[0]) > 0:
            ...
            time_indices_found = time_indices.clone().detach()  # preserve time indices for hyps, of batch_size size
            logits = (
            ...
            scores, labels = logits[:, :-duration_lengths].max(-1)
            jump_scores, jump_durations_indices = logits[:, -duration_lengths:].max(-1)
            ...
            durations[blank_mask] = torch.maximum(durations[blank_mask], torch.ones_like(durations[blank_mask]))
            ...
            time_indices[active_indices] += durations  # add duration immediately
            advance_mask = torch.logical_and(blank_mask, (time_indices[active_indices] < out_len[active_indices]))
            while advance_mask.any():
                advance_indices = active_indices[advance_mask]
                time_indices_found[advance_indices] = time_indices[advance_indices]  # preserve time indices for hyps
                logits = (
                    self.joint.joint_after_projection(
                ...
                more_scores, more_labels = logits[:, :-duration_lengths].max(-1)
                jump_scores, jump_durations_indices = logits[:, -duration_lengths:].max(-1)
                durations = all_durations[jump_durations_indices]
                ...
                blank_mask = labels == self._blank_index
                more_blank_mask = more_labels == self._blank_index
                durations[more_blank_mask] = torch.maximum(
                    durations[more_blank_mask], torch.ones_like(durations[more_blank_mask])
                )
                time_indices[advance_indices] += durations
                advance_mask = torch.logical_and(blank_mask, (time_indices[active_indices] < out_len[active_indices]))
            ...
            batched_hyps.add_results_(
                active_indices, labels, time_indices_found[active_indices].clone(), scores,
            )
