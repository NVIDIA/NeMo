...
class ModifiedAESBatchedRNNTComputer(ConfidenceMethodMixin):
    ...
    def batched_modified_adaptive_expansion_search_torch(
        ...
        expansion_beam_indices = (
            torch.arange(self.beam_size, device=device)
            .unsqueeze(0)
            .unsqueeze(-1)
            .repeat(batch_size, 1, self.beam_size + self.maes_expansion_beta)
        )
        ...
        while active_mask.any():
            ...
            while to_update.any() and expansion_steps < self.maes_num_steps:
                if self.ngram_lm_batch is None:
                    ...
                    total_logps, idx = total_logps.view(batch_size, -1).topk(
                    ...
                    labels, total_logps, beam_idx = self.get_topk_lm(
                        batch_size, batched_hyps, batch_indices, expansion_beam_indices, lm_scores, to_update, logps
                    )
                ...
                decoder_output, state, *_ = self.decoder.predict(
                    labels.view(-1, 1), prev_state, add_sos=False, batch_size=batch_size * self.beam_size
                )
                decoder_output = self.joint.project_prednet(decoder_output)
                ...
                if self.ngram_lm_batch is not None:
                    batch_lm_states_candidates = torch.index_select(
                        batch_lm_states_candidates, dim=0, index=beam_state_idx
                    )
                    ...
                    batch_lm_states = torch.gather(
                        batch_lm_states_candidates, dim=1, index=labels_w_blank_replaced.unsqueeze(-1)
                    ).flatten()
    ...
    def get_topk_lm(
        self, batch_size, batched_hyps, batch_indices, expansion_beam_indices, lm_scores, to_update, log_probs
    ):
        if self.pruning_mode is PruningMode.LATE:
            ...
            # pruning with gamma
            total_logps = batched_hyps.scores.unsqueeze(-1) + label_logps
            total_logps[total_logps <= total_logps.max(dim=-1, keepdim=True).values - self.maes_expansion_gamma] = (
                float('-inf')
            )
