...
def _vad_feature_segment_collate_fn(
    batch, window_length_in_sec, shift_length_in_sec, frame_unit_in_sec
):
    ...
    for feat_i, feat_i_len, tokens_i, _ in batch:
        ...
        if has_audio:
            slices = max(
                1, torch.div(feat_i_len - slice_length, shift, rounding_mode="trunc")
            )
