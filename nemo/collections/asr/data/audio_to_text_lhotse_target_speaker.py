...
class LhotseSpeechToTextTgtSpkBpeDataset(torch.utils.data.Dataset):
    """
    This dataset is based on BPE datasets from audio_to_text_lhotse.py.

    It reads cut object and return audio, text and speaker target tensor.
    Output follows format of query_cut - separater_audio - target-cut
    """

    @property
    def output_types(self) -> Optional[Dict[str, NeuralType]]:
        """Return the output types for this dataset."""
        return {
    ...
    def __init__(self, cfg, tokenizer):
        """Initialize the target speaker ASR dataset."""
        super().__init__()
        ...
        self.boundary_segments = self.cfg.get('boundary_segments', False)
        if self.query_noise_augment:
            ...
            with open(self.query_noise_path, 'r', encoding='utf-8') as f:
    ...
    def __getitem__(self, cuts) -> Tuple[torch.Tensor, ...]:
        ...
        # order matters:
        # rir_augment and codec_augment (output monocut) should be applied before mix_noise (output mixedcut)
