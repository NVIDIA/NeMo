...
def all_to_all_spatial_p2temporal_p(input_):
    ...
    split_tensors = torch.split(
        input_exchanged, split_size_or_sections=input_exchanged.shape[0] // world_size, dim=0
    )
...
class AdaLN_STDiTv3(MegatronModule):
    ...
    def __init__(self, config: TransformerConfig, n_adaln_chunks=6):
        ...
        self.scale_shift_table = nn.Parameter(torch.randn(self.n_adaln_chunks, config.hidden_size) / config.hidden_size**0.5)
    ...
    def forward(self, timestep_emb, batch):
        ...
        return (rearrange(self.scale_shift_table[None] + timestep_emb.reshape(batch, self.n_adaln_chunks, -1), "B C D -> C B D")
...
class STDiTV3LayerWithAdaLN(TransformerLayer):
    ...
    def forward(
        ...
        # adaln
        pre_spatial_attn_ada_norm_output = self.spatial_adaln_stdit.modulated_layernorm(hidden_states, shift=shift_sa, scale=scale_sa)
        ...
        if self.config.dynamic_sequence_parallel:
            spatial_attention_output, temporal_dim, spatial_dim = self.dynamic_sequence_parallel_comm(
                spatial_attention_output,
                T=temporal_dim,
                S=spatial_dim,
                temporal_p2spatial_p=True
            )
        ...
        hidden_states, pre_mlp_ada_norm_output = self.spatial_adaln_stdit.add_bias_modulated_layernorm(hidden_states, 
                                                    cross_attention_output,
                                                    cross_attention_output_bias,
                                                    shift=shift_mlp, 
                                                    scale=scale_mlp)
        ...
        # residual + drop_out_path
        if self.drop_path_spatial is None:
            hidden_states = self.spatial_adaln_stdit.scale_add_bias(hidden_states, mlp_output, mlp_output_bias, gate_mlp)
        else:
        ...
        # adaln 
        pre_temporal_attn_ada_norm_output = self.temporal_adaln_stdit.modulated_layernorm(hidden_states, shift=shift_sa, scale=scale_sa)

        # ************************** temoral self attention ***************************
        
        # rearrange format 
        pre_temporal_attn_ada_norm_output = rearrange(pre_temporal_attn_ada_norm_output, "(T S) B D -> T (B S) D", T=self.config.stdit_dim_T, S=self.config.stdit_dim_S) 
        ...
        # rearrange format 
        temporal_attention_output = rearrange(temporal_attention_output + temporal_attention_output_bias, "T (B S) D -> (T S) B D", T=self.config.stdit_dim_T, S=self.config.stdit_dim_S)
        ...
        hidden_states, pre_mlp_ada_norm_output = self.temporal_adaln_stdit.add_bias_modulated_layernorm(hidden_states, 
                                                                        cross_attention_output,
                                                                        cross_attention_output_bias,
                                                                        shift=shift_mlp, 
                                                                        scale=scale_mlp)
        ...
        # residual + drop_out_path
        if self.drop_path_temporal is None:
            hidden_states = self.temporal_adaln_stdit.scale_add_bias(hidden_states, mlp_output, mlp_output_bias, gate_mlp)
        else:
