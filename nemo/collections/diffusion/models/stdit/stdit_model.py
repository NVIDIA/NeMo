...
class STDiTModel(VisionModule):
    ...
    def __init__(
        ...
        position_embedding_type: Literal["learned_absolute", "rope"] = "learned_absolute",      # Todo : need to change in open-sora stditv3
        ...
        **kwargs,
        ...
        if self.pre_process:
            ...
            self.x_embedder = PatchEmbed3D(
                (self.patch_temporal, self.patch_spatial, self.patch_spatial), 
                in_channels, 
    ...
    def forward(
        ...
        **kwargs,
        ...
        # timestep part
        # =============
        timestep_emb, t = self.encode_timesteps(timesteps, fps, Batch, torch.bfloat16)          # timestep_emb shape [Batch, Dim]
    ...
    def unpatchify(self, x, Sequence_t, Sequence_h, Sequence_w, Origin_t, Origin_h, Origin_w):
        ...
        x = rearrange(
            ...
            S_t=Sequence_t, S_h=Sequence_h, S_w=Sequence_w,
            P_t=self.patch_temporal, P_h=self.patch_spatial, P_w=self.patch_spatial,
    ...
    def gen_packed_seq_params(self, batch_size, q_fiexed_seqlen, context_mask, qkv_format):
        ...
        cu_seqlens_kv_list = torch.cumsum(torch.cat((torch.tensor([0]).cuda(), context_mask.sum(dim=1))), dim=0).tolist()
