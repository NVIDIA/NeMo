...
def convert_weights_single(diffuser_dict, transformer_config):
    mcore_dict={}
    key_mapping = {
    ...
    for key in key_mapping.keys():
        ...
        if key in diffuser_dict:
            mcore_dict[new_key] = diffuser_dict[key]
    to_q, to_k, to_v = diffuser_dict['attn.to_q.weight'],diffuser_dict['attn.to_k.weight'],diffuser_dict['attn.to_v.weight']
    mcore_dict['self_attention.linear_qkv.weight'] = _import_qkv(transformer_config, to_q, to_k, to_v)
    to_q_b, to_k_b, to_v_b = diffuser_dict['attn.to_q.bias'],diffuser_dict['attn.to_k.bias'],diffuser_dict['attn.to_v.bias']
    mcore_dict['self_attention.linear_qkv.bias'] = _import_qkv_bias(transformer_config, to_q_b, to_k_b, to_v_b)
    return mcore_dict


def convert_weights_double(diffuser_dict, transformer_config):
    mcore_dict={}
    key_mapping = {
    ...
    for key in key_mapping.keys():
        ...
        if key in diffuser_dict:
            mcore_dict[new_key] = diffuser_dict[key]
    to_q, to_k, to_v = diffuser_dict['attn.to_q.weight'], diffuser_dict['attn.to_k.weight'], diffuser_dict[
        'attn.to_v.weight']
    mcore_dict['self_attention.linear_qkv.weight'] = _import_qkv(transformer_config, to_q, to_k, to_v)
    to_q_b, to_k_b, to_v_b = diffuser_dict['attn.to_q.bias'], diffuser_dict['attn.to_k.bias'], diffuser_dict[
        'attn.to_v.bias']
    mcore_dict['self_attention.linear_qkv.bias'] = _import_qkv_bias(transformer_config, to_q_b, to_k_b, to_v_b)
    add_to_q, add_to_k, add_to_v = diffuser_dict['attn.add_q_proj.weight'], diffuser_dict['attn.add_k_proj.weight'], \
    diffuser_dict['attn.add_v_proj.weight']
    mcore_dict['self_attention.added_linear_qkv.weight'] = _import_qkv(transformer_config, add_to_q, add_to_k, add_to_v)
    add_to_q_b, add_to_k_b, add_to_v_b = diffuser_dict['attn.add_q_proj.bias'], diffuser_dict['attn.add_k_proj.bias'], \
    diffuser_dict['attn.add_v_proj.bias']
    mcore_dict['self_attention.added_linear_qkv.bias'] = _import_qkv_bias(transformer_config, add_to_q_b, add_to_k_b,
                                                                          add_to_v_b)
    return mcore_dict
