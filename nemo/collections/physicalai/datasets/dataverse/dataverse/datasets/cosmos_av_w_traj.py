...
class CosmosAV(AlpamayoV2):
    ...
    def egomotion_alpamayo_parser_initial_frame(self, video_idx, timestamps):
        ...
        ed_config = egomotion_decoder.EgoMotionDecoderConfig(
            decode_strategy=f"at_0_frame", prediction_start_offset_range=[0.0, 0.0]
        )
    ...
    def _read_raw_video(
        ...
        for data_field in data_fields:
            if data_field == DataField.IMAGE_RGB:
                ...
                if len(timestamps) == 0:
                    raise ValueError(f"view_idxs do not contain the base camera for trajectory computation.")
