...
class CosmosAV(AlpamayoV2):
    def __init__(
        ...
        super().__init__(tar_dirs, probe_tar,
                        rectify,
                        camkeys,
                        tar_cache_path)
    ...
    def _buiild_t5_dirs(self):
        ...
        if t5_index_path.exists():
            ...
            for file in tqdm.tqdm(glob.glob(self.uuid_dirs + "*.parquet",
                                            recursive=True)):
                cache_file_path = os.path.join(file.replace(".parquet", "").replace(self.uuid_dirs, self.t5_dirs),
    ...
    def _read_raw_video(
        ...
        for data_field in data_fields:
            if data_field == DataField.IMAGE_RGB:
                ...
                traj, rig_info = self.egomotion_alpamayo_parser_initial_frame(
