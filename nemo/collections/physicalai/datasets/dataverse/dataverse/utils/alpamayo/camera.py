...
def get_3D_points_from_dist(
    ...
    Args:
        ...
        dist (torch.tensor): distance along the ray, [..., 1] or [..., N_samples, 1] if sampling muliple points along rays
...
def get_3D_points_from_depth(
    ...
    Args:
        ...
        depth (torch.tensor): z depth from camera center, [..., 1] or [..., N_samples, 1] if sampling muliple points along rays
...
def dist_to_pointcloud(dist: torch.tensor, intr: torch.tensor, extr: torch.tensor):
    """Convert dist along ray to pointcloud.
    Args:
        dist (torch.tensor): [1,H,W]/[B,1,H,W]
    ...
    dist = dist.view(*center.shape[:-1], 1)  # [HW, 1]/[B,HW,1]
    pc_dist = get_3D_points_from_dist(center, ray, dist)  # HW,3/B,HW,3
    return pc_dist
