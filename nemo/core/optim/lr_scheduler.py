...
from nemo.core.config import (SchedulerParams, get_scheduler_config,
...
class WarmupAnnealHoldPolicy(_LRScheduler):
    ...
    def get_lr(self):
        ...
        # Reset learning rate
        if (
            len(self.optimizer.param_groups) > 0
            and "reset_lr" in self.optimizer.param_groups[0].keys()
        ):
...
def prepare_lr_scheduler(
    ...
    if max_steps_from_cfg is not None:
        if max_steps_from_cfg == -1:
            logging.warning(
                "`max_steps` is set to -1 in the scheduler config, scheduler will not be instantiated"
            )
            return None
        ...
        if train_dataloader.batch_size is not None:
            ...
            and train_dataloader.sampler is not None
        ):
            if (
                ...
                raise ValueError(
                    f"Could not find batch_size from sampler: {train_dataloader.sampler}"
                )
        else:
            raise ValueError(
