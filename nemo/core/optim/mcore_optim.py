...
class McoreDistributedOptimizer(torch.optim.Optimizer):
    ...
    def sharded_state_dict(self, model_sharded_state_dict, optimizer_state_dict=None, is_loading=False, dist_ckpt_parallel_save=False):
        sharding_type = (
            'fully_sharded_bucket_space' if dist_ckpt_parallel_save else 'dp_zero_gather_scatter'
        )
