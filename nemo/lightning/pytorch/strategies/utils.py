...
def setup_parallel_ranks(strategy: pl.strategies.Strategy):
    ...
    _strategy_lib.init_parallel_ranks(
        env.world_size(), env.global_rank(), env.local_rank(), parallelism
    )
...
def setup_data_sampler(trainer: pl.Trainer):
    ...
    if datamodule is not None:
        if (
            hasattr(trainer.strategy, "data_sampler")
            and trainer.strategy.data_sampler is not None
        ):
    ...
    if trainer.strategy.data_sampler is not None:
        trainer.strategy.data_sampler.setup(
            trainer.strategy.cluster_environment.global_rank()
        )
...
def fix_progress_bar(
    trainer: pl.Trainer, replace_progress_bar: bool = True, progress_interval: int = 1
) -> None:
    """
    ...
    """
    callbacks: List[pl.Callback] = cast(
        List[pl.Callback], getattr(trainer, "callbacks")
    )
...
def mcore_to_pyt_sharded_state_dict(
    ...
    def _mcore_to_pyt_sharded_tensor(
        tens: List[torch.Tensor], sh_tens: List[ShardedTensor]
    ) -> TorchShardedTensor:
        ...
        for ten, sh_ten in zip(tens, sh_tens):
            ...
            sh_ten.axis_fragmentations = sh_ten.axis_fragmentations[
    ...
    def _convert(checkpoint, sharded_state_dict, k, device_mesh=None):
        ...
        if isinstance(sharded_state_dict[k], Dict):
            for kk in sharded_state_dict[k]:
                _convert(
                    checkpoint[k], sharded_state_dict[k], kk, device_mesh=device_mesh
                )
            ...
            if dtensor:
                checkpoint[k] = _mcore_to_pyt_dtensor(
                    checkpoint[k], sharded_state_dict[k], device_mesh=device_mesh
                )
            else:
                checkpoint[k] = _mcore_to_pyt_sharded_tensor(
                    checkpoint[k], sharded_state_dict[k]
                )
...
def pyt_to_mcore_state_dict(
        ...
        for i, placement in enumerate(dten.placements):
            if isinstance(placement, Shard):
                ...
                rank_offsets.append(
                    (
                        ax + prepend_axis_num,
                        dten.device_mesh.get_local_rank(i),
                        axis_fragm[ax],
                    )
                )
    ...
    def _torch_to_mcore_sharded_tensor(
        ...
        assert all(
            [
                ls.metadata.placement == local_shards[0].metadata.placement
        ...
        axis = list(range(len(global_shape)))
        axis_fragm = [
            global_shape[i] // local_shards[0].metadata.shard_sizes[i] for i in axis
        ]
        rank_offsets = []
        ...
        for i in range(len(local_shards)):
            ...
            for j in range(len(axis)):
                ...
                rank_offsets.append(
                    (axis[j] + prepend_axis_num, axis_rank_offset, axis_fragm[j])
                )
    ...
    def _convert(
        ...
        if isinstance(v, Dict):
            ...
            state_dict[k] = _torch_to_mcore_sharded_object(
                sh_key, v, prepend_offsets, prefix
            )
    ...
    for k, v in state_dict.items():
        prepend_offsets = []
        sh_key = k
        allow_shape_mismatch = k.endswith(
            ".word_embeddings.weight"
        )  # vocab size can be different
        if k.startswith("module.decoder.layers."):
            sh_key = k.split(".")
