...
def fsdp2_strategy_parallelize(
    ...
    if device_mesh.size() > 1:
        assert device_mesh.ndim == 1, "Hybrid-sharding not supported"

        assert HAS_FULLY_SHARD is not None, "Expected to have fully_shard"
        # Find transformer layers and apply parallelisms
        parallelize_helper(model, device_mesh, mp_policy)
        ...
        model = fully_shard(
            model, mesh=device_mesh, mp_policy=mp_policy, reshard_after_forward=True, offload_policy=offload_policy
        )
