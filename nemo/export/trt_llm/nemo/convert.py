...
def split_and_save_weight(tp_rank, saved_dir, split_factor, key, vals, storage_type, act_range, config):
    ...
    elif "mlp.dense_h_to_4h.weight" in key or "mlp.dense_h_to_4h.bias" in key or "mlp.linear_fc1.weight" in key:
    ...
    # Ammo modification
        ...
        vals = [
            val.reshape(hidden_dim, num_kv_heads // tp_size, q_num + 2, size_per_head).transpose(0, 2, 1, 3)
            for val in vals
        ]

        # Combine all the Qs, Ks and Vs together for each val
        q_splits, k_splits, v_splits = zip(*[np.split(val, [q_num, q_num + 1], axis=1) for val in vals])
        ...
        qkv_split_concat = np.concatenate([q_splits_concat, k_splits_concat, v_splits_concat], axis=1)
