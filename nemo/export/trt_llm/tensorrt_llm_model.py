...
from tensorrt_llm.functional import RaggedTensor, Tensor, expand_mask, gather_last_token_logits, shape
...
class LMHeadModelBuilder(ModelBuilder, GenerationMixin):
    ...
    def prepare_inputs(
        ...
        use_ib_gpt_attention_plugin = default_net().plugin_config.inflight_batching_gpt_attention_plugin
        ...
        if self._use_prompt_tuning:
            prompt_embedding_table = Tensor(
                ...
                dim_range=OrderedDict(
                    [("prompt_embedding_table_size", [p_embedding_range]), ("hidden_size", [self._hidden_size]),]
                ),
            )
            if remove_input_padding:
                tasks = Tensor(
                    ...
                    dtype=trt.int32,
                    shape=[1, -1],
                    dim_range=OrderedDict([("batch_size_fake", [1]), ("input_len_task", [num_tokens_range]),]),
                )
            else:
                tasks = Tensor(
                    name="tasks",
                    dtype=trt.int32,
                    shape=[-1, 1],
                    dim_range=OrderedDict([("batch_size_beam_width", [bb_range]), ("broadcast_dim", [1]),]),
                )
            prompt_vocab_size = Tensor(
                name="prompt_vocab_size", dtype=trt.int32, shape=[1], dim_range=OrderedDict([("size", [1])]),
            )
        ...
        if use_ib_gpt_attention_plugin:
            ...
            inflight_batching_args = InflightBatchingParam(
                ...
                host_beam_widths=Tensor(
                    name="beam_widths", dtype=trt.int32, shape=[-1], dim_range=OrderedDict(batch_size_bw=[bs_range]),
                ),
                # [nbReq, 2]
