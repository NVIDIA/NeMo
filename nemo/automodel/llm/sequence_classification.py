...
class AutoModelForSequenceClassificationConfig:
    """
...
def forward_with_loss_no_cp(model, batch, labels, config: AutoModelForSequenceClassificationConfig, loss_mask=None):
...
def forward_step(
    ...
    model: torch.nn.Module,
    config: AutoModelForSequenceClassificationConfig,
):
    timers = state.timers
    ...
    with straggler_timer(bdata=True):
        tokens, labels, attention_mask = get_batch_from_iterator(data_iterator).values()
    ...
    with straggler_timer:
        output_tensor = forward_with_loss_no_cp(model=model, batch=batch, labels=labels, config=config, loss_mask=None)
