PYTHONPATH=./ CUDA_VISIBLE_DEVICES=0 HYDRA_FULL_ERROR=1 NEMO_ENABLE_COLORING=1 python examples/multimodal/speech_llm/modular_audio_gpt_train.py \
    ...
    --config-name=modular_audio_t5_tts_config \
    name=debug \
    ...
    exp_manager.exp_dir=/workspace/Experiments/test/ \
    trainer.devices=1 \
    trainer.max_steps=170000 \
...
model.perception.modality_adapter.n_layers=1 \
    ...
    model.data.train_ds.batch_duration=80 \
    ...
    model.data.train_ds.max_seq_length=384 \
    ...
    ++model.data.validation_ds.max_seq_length=384 \
    ...
    ++model.data.validation_ds.output_dir=/workspace/Results/test \
