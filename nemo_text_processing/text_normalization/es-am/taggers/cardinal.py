...
def get_one_to_one_thousand(cardinal):
	fst = ("1" @ cardinal).project("output") 
	for i in range(2, 1000):
		fst |= (str(i) @ cardinal).project("output")
	return fst.optimize()

class FilterPunctuation:
    def __init__(self):
		# Depending on region, cardinals are either delineated with a space/period (EU and South America)
		# or a comma (Central America, U.S.). For decimals, each system uses the other's cardinal separator
		# so can't just format blindly and maintain separation in paths.
        ...
        self.fst = cardinal_string.optimize()
...
class CardinalFst(GraphFst):
    def __init__(self, deterministic: bool = True):
        ...
        filter = FilterPunctuation().fst
        self.graph = filter @ self.graph
