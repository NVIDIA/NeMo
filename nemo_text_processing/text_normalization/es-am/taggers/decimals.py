...
def get_quantity(decimal: 'pynini.FstLike', cardinal_graph: 'pynini.FstLike') -> 'pynini.FstLike':
    ...
    """
    numbers = cardinal_graph
    
	# Need to limit to 10^6 (maximum limit for these cardinal strings)
    numbers = pynini.closure(NEMO_DIGIT, 1, 6) @ numbers
...
class DecimalFst(GraphFst):
    ...
    def __init__(self, cardinal: GraphFst, deterministic: bool = True):
        ...
        graph_integer = cardinal.graph
        graph_integer @= pynini.cdrewrite(reverse_apocope, "", "[EOS]", NEMO_SIGMA) # Masculine integer strings do not need apocope for final digit
