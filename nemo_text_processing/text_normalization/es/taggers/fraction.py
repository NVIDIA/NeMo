...
from nemo_text_processing.text_normalization.en.graph_utils import NEMO_CHAR, NEMO_DIGIT, NEMO_SIGMA, NEMO_SPACE, GraphFst
...
class FractionFst(GraphFst):
    ...
    def __init__(self, ordinal, deterministic: bool = True): # Change so we're using cardinal from ordinal
        ...
        graph_fractions_ordinals += pynutil.insert("\" morphosyntactic_features: \"ordinal\"") # We note the root for processing later
        ...
        graph_fractions_cardinals @= pynini.cdrewrite(block_higher_powers_of_ten, pynini.accep("[BOS]"), pynini.accep("[EOS]"), NEMO_SIGMA)
        ...
        graph_fractions_cardinals += pynutil.insert("\" morphosyntactic_features: \"add_root\"") # We note the root for later processing NEMO_CHAR.plus #  blocking these entries to reduce erroneous possibilities in debugging
        
        if deterministic:
          graph_fractions_cardinals = pynini.closure(NEMO_DIGIT, 1, 2) @ graph_fractions_cardinals# Past one hundred the conventional scheme can be hard to read. For determinism we lock here
          graph_fractions_cardinals = pynutil.add_weight(graph_fractions_cardinals, -0.01) # To take precedence over default schema of x / x
          graph_fractions_ordinals = pynutil.add_weight(graph_fractions_ordinals, -0.01) # To take precedence over default schema of x / x
        
        graph_denominator = graph_fractions_ordinals | graph_fractions_cardinals | (cardinal_graph + pynutil.insert("\"")) # Last form is simply recording the cardinal
