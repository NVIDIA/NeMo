...
class OrdinalFst(GraphFst):
    ...
    Finite state transducer for classifying ordinal
        e.g. nittionde -> ordinal { integer: "90" }
    ...
    def __init__(self, cardinal: GraphFst):
        ...
        graph = pynini.closure(NEMO_CHAR) + pynini.union(
            graph_digit, graph_teens, pynini.cross("ljonte", "ljon"),
            pynini.cross("tionde", "tio"), pynini.cross("drade", "dra"),
            pynini.cross("gonde", "go"), pynini.cross("ljarte", "ljard"),
            pynini.cross("sende", "sen")
        )
