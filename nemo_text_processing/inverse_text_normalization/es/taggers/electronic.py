...
class ElectronicFst(GraphFst):
    ...
    def __init__(self):
        ...
        alpha_num = (
            NEMO_ALPHA
            | pynini.string_file(get_abs_path("es/data/numbers/digit.tsv"))
            | pynini.string_file(get_abs_path("es/data/numbers/zero.tsv"))
        )
        username = (
            pynutil.insert("username: \"")
            + pynini.closure(
                alpha_num + delete_extra_space + pynini.closure(pynini.cross("punto", '.') + delete_extra_space, 0, 1)
            )
            + alpha_num
            + pynutil.insert("\"")
        )
        single_alphanum = pynini.closure(alpha_num + delete_extra_space) + alpha_num
        server = single_alphanum | pynini.string_file(get_abs_path("es/data/electronic/server_name.tsv")).invert()
        domain = single_alphanum | pynini.string_file(get_abs_path("es/data/electronic/domain.tsv")).invert()
        domain_graph = (
